...
**Tech Stack:** Node.js ESM scripts, Cargo, pnpm, existing `run-cloud-router-application.test.mjs` product tooling tests.
...
**Files:**
- Modify: `scripts/run-cloud-router-application.test.mjs`
- Modify: `package.json`
- Create: `scripts/run-cloud-router-rust-tests.mjs`
- Create: `scripts/stop-cloud-router-test-processes.mjs`
...
Run: `node scripts/run-cloud-router-application.test.mjs`
...
Run: `node scripts/run-cloud-router-rust-tests.mjs admin-api --dry-run`
...
Run: `node scripts/stop-cloud-router-test-processes.mjs --dry-run`
...
```bash
node scripts/run-cloud-router-rust-tests.mjs quick --dry-run
node scripts/run-cloud-router-rust-tests.mjs admin-api --dry-run
node scripts/stop-cloud-router-test-processes.mjs --dry-run
```
...
**Files:**
- Modify: `scripts/run-cloud-router-rust-tests.mjs`
- Modify: `scripts/run-cloud-router-application.test.mjs`
- Modify: `package.json`
- Create: `scripts/measure-cloud-router-test-targets.mjs`
...
Added `scripts/measure-cloud-router-test-targets.mjs` and `pnpm test:rust:measure` for curated slow targets:

```text
sdkwork-cloudrouter-admin-gateway:database_config_router
sdkwork-cloudrouter-standalone-gateway:database_config_router
sdkwork-cloudrouter-edge-runtime:database_config_router
sdkwork-cloudrouter-edge-runtime:provider_passthrough_route
sdkwork-cloudrouter-edge-runtime:edge_server
sdkwork-cloudrouter-router-service:openai_compatible_http_relay
sdkwork-cloudrouter-installer:installer_cli
```
...
```text
sdkwork-cloudrouter-edge-runtime edge_server: cold 142.2s, hot 0.7s
sdkwork-cloudrouter-router-service sqlite_pricing_catalog_loader: cold 97.9s, hot 0.3s
sdkwork-cloudrouter-edge-runtime provider_adapter_invocation: 4.0s
```
...
Made `stop-cloud-router-test-processes.mjs` tolerate already-exited repo-local processes during stop operations. This avoids cleanup failures caused by race conditions while stopping stale test binaries.

### Updated Verification Notes

- `node scripts/run-cloud-router-application.test.mjs`: PASS.
- `node scripts/run-cloud-router-rust-tests.mjs quick`: PASS.
- `node scripts/run-cloud-router-rust-tests.mjs quick` hot cache: 3.9s.
- `node scripts/measure-cloud-router-test-targets.mjs --dry-run --target sdkwork-cloudrouter-admin-gateway:database_config_router --target sdkwork-cloudrouter-standalone-gateway:database_config_router`: PASS.
- `cargo check -p sdkwork-cloudrouter-http`: PASS.
