...
**Goal:** Implement an appbase-owned OAuth system and remove Cloud Router `open_platform_*` technical debt without compatibility aliases.

**Architecture:** Appbase owns OAuth database tables, app-api runtime route contracts, provider ingress route contracts, and backend-api management route contracts. Cloud Router consumes appbase backend SDK resources from an independent `/admin/oauth` admin module and deletes product-local open-platform ownership.
...
### Task 3: Cloud Router Open Platform Removal Guard

**Files:**
- Modify or create focused Cloud Router tests under existing test locations after locating current open-platform contract sources.
- Delete Cloud Router-owned `open_platform_*` schema/API/store/admin packages when located.

- [ ] **Step 1: Write failing removal tests**

Assert no schema registry source, generated OpenAPI, Rust route manifest, generated Cloud Router backend SDK, sidebar route, or admin package exposes `open_platform`, `openPlatform`, or `/admin/open-platform`.

- [ ] **Step 2: Run red removal test**

Run the narrow Cloud Router test for the guard. Expected: FAIL while old open-platform artifacts exist.
...
### Task 4: Cloud Router `/admin/oauth` Module

**Files:**
- Create: `apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudrouter-pc-admin-oauth/`
- Modify: Cloud Router PC admin module registry/sidebar/i18n files after locating current registry pattern.
...
Build dense backend-admin pages and service boundary using `getSdkworkAppbaseBackendSdkClient` and `@sdkwork/iam-backend-sdk` only. No raw HTTP, no Cloud Router backend SDK for appbase-owned OAuth.
...
- [ ] **Step 3: Run Cloud Router focused tests**
