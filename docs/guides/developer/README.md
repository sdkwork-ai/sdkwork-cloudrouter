...
- DeepSeek Harness desktop (`dsh://`) does not accept the `v1/import` deep-link contract yet, so its quick-import entry is manual-only: the console shows the install banner and a YAML document with the `llm-pi-ai.providers.cloudrouter` OpenAI-compatible route for `~/.dsh/settings.yaml` plus the `CLOUDROUTER_API_KEY` entry for `~/.dsh/.credentials.yaml` (the harness resolves models itself via "Fetch available models" in Settings → Models).
