> Migrated from `docs/superpowers/plans/2026-05-17-agent-platform.md` on 2026-06-24.
...
**Goal:** Build the first SDKWork agent platform slice: standard contracts in appbase plus Cloud Router run/metering contracts.

**Architecture:** `sdkwork-appbase` provides framework-neutral agent contracts. Cloud Router implements product-facing Rust API contracts and SQL-backed persistence for run creation, step metadata, and metering linkage. UI/admin work follows after generated SDK contracts are stable.

**Tech Stack:** TypeScript/Vitest in `sdkwork-appbase`; Rust/Axum/SQLx in Cloud Router; schema registry generated PostgreSQL DDL.
...
### Task 2: Cloud Router Agent Run Contract

**Files:**
- Modify: `services/sdkwork-cloudrouter-router-service/src/ports/app_generation_agent_run_store.rs`
- Modify: `services/sdkwork-cloudrouter-router-service/src/api/app_generation_agent.rs`
- Modify: `services/sdkwork-cloudrouter-router-service/tests/app_generation_history_api.rs`
- Modify: `services/sdkwork-cloudrouter-router-service/src/infrastructure/sql/sqlite/app_generation_agent_run_store.rs`
- Modify: `services/sdkwork-cloudrouter-router-service/src/infrastructure/sql/postgres/app_generation_agent_run_store.rs`
...
- [ ] Run focused Cloud Router Rust tests.
