...
1. **单一事实来源**：每个业务域只保留一个 system-of-record 表，禁止 `plus_*` / `studio_catalog_*` 与 v4.1 表并存或双写。
2. **分类统一**：所有垂直分类写入 `c_category`，用字符串 `category_type` 区分域（如 `app_store`、`skill_market`、`skills_collection`），不再使用 `plus_category.type` 整数和 `group_name`。
...
| `iam_` / `commerce_` / `ops_` | Appbase 标准表 | 沿用现有 appbase 表，不做 cloud-router 替代表|
...
| 图标/截图/安装包| `appstore_app.resource_list`、`install_config` 为 JSON |
| 下载量评分 | **Option A**：`appstore_app.download_count`、`rating_avg`、`rating_count` |
...
## 6. 门禁与验证
- **Schema Registry**：`docs/schema-registry/sdkwork-cloudrouter.tables.yaml` 不得再注册 v4.1 已退役的 `appstore_app`、`plus_category`、`studio_catalog_*`、`platform_*`、`content_course*` 等表。
- **Schema Guardian**：`tools/schema_guardian.py` 和 `V41_PLATFORM_LEGACY_ALIASES` 会扫描 registry、frontend-field-contracts 片段、编译快照、OpenAPI 和 API manifest，禁止引用退役别名（如 `platform_app_template` 、`appstore_app_template`）。
