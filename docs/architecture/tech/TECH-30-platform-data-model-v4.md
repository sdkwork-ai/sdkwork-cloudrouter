> Migrated from `docs/30-platform-data-model-v4.md` on 2026-06-24.
> Owner: SDKWork maintainers

> 版本：v4.1  
> 日期�?026-06-20  
> 状态：**现行标准**（greenfield，无生产存量数据�? 
> 替代：[17-AppCenter-platform_app-compatible-design.md](./17-AppCenter-platform_app-compatible-design.md)、[18-SkillsHub-AgentSkills-PlusCategory-compatible-design.md](./18-SkillsHub-AgentSkills-PlusCategory-compatible-design.md)

## 1. 设计原则

1. **单一事实来源**：每个业务域只保留一�?system-of-record 表，禁止 `plus_*` / `studio_catalog_*` �?v4.1 表并存或双写�?2. **分类统一**：所有垂直分类写�?`c_category`，用字符�?`category_type` 区分域（�?`app_store`、`skill_market`、`skills_collection`），不再使用 `plus_category.type` 整数�?`group_name`�?3. **应用平台 JSON 优先**：应用媒体、安装包、发布说明等结构化数据保存在 `appstore_app` JSON 列（`resource_list`、`install_config` 等），不�?App Store 单独维护 `platform_asset` / `platform_artifact` / `platform_action`�?4. **技能媒体独�?*：Skills Hub �?asset/artifact/action 使用 `ai_skill_*` 表；技能主数据使用 `ai_agent_skill*`�?5. **内容域统一前缀**：论坛等内容使用 `content_*`（如 `content_forum_post`、`content_reaction`）；课程�?`sdkwork-course` 拥有，使�?`course_*` 前缀�?
## 2. 表分�?
| 前缀 | 职责 | 代表�?|
| --- | --- | --- |
...
| 图标/截图/安装�?| `appstore_app.resource_list`、`install_config` �?JSON |
| 下载�?评分 | **Option A**：`appstore_app.download_count`、`rating_avg`、`rating_count` |
| 开发�?| `iam_user` + `appstore_app` 归属字段 |
| 应用模板 | `appstore_app_template`、`appstore_app_template_version`、`appstore_app_template_usage` |

Admin/App API 契约表名：`appstore_app`、`c_category`（OpenAPI `x-table` �?frontend-field-contracts `data_sources` 均已对齐）�?
