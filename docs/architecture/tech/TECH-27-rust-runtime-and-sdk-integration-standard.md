...
This document defines the target runtime and SDK boundary. It is not a release
approval or evidence that every described control is implemented. The active
[production-readiness revalidation](../../engineering/reviews/REVIEW-20260714-production-readiness-revalidation.md)
records open egress, streaming, schema, concurrency, API, and verification
blockers. In particular, `/readyz` is not a generic migration/drift or
feature-schema proof, and settlement configuration names do not establish a
bounded throughput, backlog, or recovery policy.
