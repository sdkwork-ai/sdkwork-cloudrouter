...
- 用户、VIP、account、优惠券、算力积分充值（Token Bank）、订单、支付、退款、发票由 T1 能力仓库（`sdkwork-account`、`sdkwork-order`、`sdkwork-payment` 等）和 `sdkwork-iam` 等组合模块提供；Cloud Router 通过生成 SDK 消费，不在本地重复定义资金事实表。
