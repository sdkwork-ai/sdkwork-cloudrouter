...
> Version: 2.0  
> Date: 2026-06-24  
> Status: **active** — cloud-router owns only generated gateway schema

## 1. Principle

`sdkwork-cloudrouter` is the AI gateway and product shell. It **does not** compose sibling database modules or duplicate platform SoR tables. External capabilities are consumed through dependency SDKs and runtime services only.

| Capability | System of record | Cloud-router DB lifecycle |
| --- | --- | --- |
...
| Commerce usage settlement projections | `sdkwork-cloudrouter` | Generated in `schema.sql` (`commerce_usage_*`, `analytics_*`) |
...
| AI gateway / routing / ops | `sdkwork-cloudrouter` | Generated in `schema.sql` |
...
Cloud-router database lifecycle is limited to:

1. Generated cloud-router schema (`generated/schema/postgres/schema.sql`)
...
Assembly registry (`docs/schema-registry/sdkwork-cloudrouter.tables.yaml`) declares:

- Local `table_fragments` for cloud-router-owned tables only
- **No** `registry_dependencies` — sibling domains are not merged into effective registry

Effective snapshot: `generated/schema/registry/sdkwork-cloudrouter.tables.effective.yaml` (~93 tables)

## 4. Retired / external in cloud-router
...
`0002_cloudrouter_legacy_projection.sql` is retired (empty stub).
...
- Admin commerce UI uses `@sdkwork/cloudrouter-backend-sdk` commerce facade (no local commerce SoR tables)
