...
`sdkwork-cloudrouter` 对外暴露三类稳定接口面：
...
| 账务 | `/app/v3/api/billing/**`；支付/退款通知由订单网关承接（`/app/v3/api/orders/payments|refunds/webhooks/**`），本网关不挂载回调路径 |
...
- 为 cloud-router 新增本地专用 App/Backend path，绕过 Java app-api/backend-api 契约。
