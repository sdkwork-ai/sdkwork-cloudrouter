...
All notable changes to `sdkwork-cloudrouter` release records will be documented here.
...
- Scoped Linux desktop `.deb` packages to desktop behavior: they install the shared TOML template under `/usr/share/sdkwork/router/config/cloudrouter.toml.example` and do not create `/etc/sdkwork/router` runtime files or a systemd service.
...
- Added a macOS launchd service runner so `.pkg` service packages run `cloudrouterctl ensure` and `cloudrouterctl refresh-catalog --force` before replacing the runner with the gateway process.
- Standardized public host-service release asset names on `server` while keeping internal package IDs and deployment mode as `service`; for example `linux-x64-service` now builds `cloudrouter-linux-x64-server-0.3.0.deb`.
...
- `node --check scripts/plan-cloud-router-install-packages.mjs`
- `node --check scripts/build-cloud-router-native-installer.mjs`
- `node --check scripts/run-cloud-router-application.test.mjs`
- `node scripts/plan-cloud-router-install-packages.mjs --check --json`
- `node scripts/build-cloud-router-native-installer.mjs --package-id linux-x64-service --check --dry-run --json`
- `node scripts/build-cloud-router-install-package.mjs --all --check --dry-run --json`
- `node scripts/build-cloud-router-native-installer.mjs --all --check --dry-run --json`
- `pnpm app-store:seed:check`
- `cargo test -p sdkwork-cloudrouter-config`
- `node scripts/run-cloud-router-application.test.mjs --test-name-pattern "installation documentation covers release|install package planner covers|install package manifests distinguish|install package builder emits service and container|native installer builder emits apt-installable|native installer builder CLI validates"`
...
- Updated Linux `.deb` service installation so the package creates `/etc/sdkwork/router/cloudrouter.toml`, `/etc/sdkwork/router/cloudrouter.env`, and `/etc/sdkwork/database/database.secret`, then enables but does not start the service until PostgreSQL is configured.
- Simplified English and Chinese installation and initialization guides around `apt install`, protected PostgreSQL TOML/password configuration, `systemctl start cloudrouter`, and health checks.
- Standardized the external runtime surface on `cloudrouter`, including `cloudrouter-*` release assets, the `cloudrouter` gateway process, the `cloudrouterctl` installer/admin CLI, `cloudrouter.service`, and `/opt/sdkwork/router`, `/etc/sdkwork/router`, `/etc/sdkwork/router/cloudrouter.env`, `/var/lib/sdkwork/router`, and `/var/log/sdkwork/router` runtime paths.
- Removed release checkout dependency on Git LFS by committing curated runtime skill seed JSON files directly to Git while leaving large CloudHub mirror snapshots as optional LFS data.

### Verification

- `cargo fmt --all`
- `cargo test -p sdkwork-cloudrouter-standalone-gateway --test app_session_route -- --nocapture`
- `cargo test -p sdkwork-cloudrouter-router-service --test app_auth_api -- --nocapture`
- `cargo test -p sdkwork-cloudrouter-installer --test installer_cli installer_cli_reports_invalid_env_catalog_root_as_machine_readable_config_error -- --nocapture`
...
- `cargo test -p sdkwork-cloudrouter-router-service --test database_installer sqlite_installer_repairs`
...
- Git LFS is no longer a release package build requirement; it is only required when refreshing large CloudHub mirror snapshots.
