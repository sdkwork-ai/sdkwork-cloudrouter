...
This iteration covers the relay/control-plane core, the narrow Chat PostgreSQL
persistence authority, and the admin Analytics contract. Chat UI,
high-volume pagination, and production-like PostgreSQL behavior remain
separate release evidence boundaries.
...
| P0 partially closed | Financial ingestion has an atomic SQL batch but lacks bounded durable recovery. | Existing text/decimal bounds remain, Adapter usage rejects counts above `64`, and persistence adapters prevalidate and persist all lines in one transaction. Settlement notification and retry wrappers preserve the batch boundary; a primary batch failure fails closed instead of enqueueing partial single-line retries. Usage now rejects `pricing_snapshot` above 16 KiB before parsing, retry adapters reject encoded envelopes above 32 KiB before decoding, and oversized poison records retain only bounded byte-count/SHA-256 evidence. The desktop-only in-memory queue validates the same envelope contract and rejects a 1025th entry; non-desktop startup requires Redis. Durable batch outbox, Redis-wide capacity/retention, and failure-injection evidence remain open. | Human-reviewed finance/privacy contract, allowlisted snapshot projection, canonical-payload conflict policy, durable atomic batch outbox, Redis capacity/backpressure/retention, and PostgreSQL contention/recovery evidence. SQLite evidence is required only for a separately approved client-local owner. |
...
| Closed in worktree | Admin Analytics accepted ambiguous windows, exposed lossy numerics, and could accumulate tenant-wide distributions. | The route now requires bounded UTC windows, rejects aliases and invalid pairs with `400`, and limits rankings to `3..=50`. PostgreSQL uses one read-only repeatable-read transaction, UTC/latest-30 trend buckets, numeric sort keys, and at most 150 ranked user IDs for distribution SQL. Int64/decimal JSON and generated SDK types are strings; consumers reject numeric substitutes; checked half-up percentage tests pass. Focused Rust, Vitest, API materialization, standard-extension, SDK guardian, and all three SDK build checks pass. | Repeat the focused suites from a clean candidate and add production-like PostgreSQL query-plan, latency, load, cancellation, and process-RSS evidence before treating the read model as production-proven. |
...
remains absent. The new pricing-snapshot byte-bound unit test passed `1/1`, the
retry-queue module tests passed `6/6`, the PostgreSQL settlement SQL-contract
suite passed `9/9`, and the router service compiled warning-free. Focused
warning-denying checks also passed for the router service and Analytics
repository after removing their remaining dead code. The API operation-pattern
check, route-collision check, and
...
library compiled in an earlier focused check; it has not been repeated with
