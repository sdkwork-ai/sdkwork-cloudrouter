# Migrated Legacy Documentation

Legacy documentation trees were ingested into Canon shards and archived here for stable history.

## Archived Paths

- `docs/00-设计文档索引.md`
- `docs/01-PRD-sdkwork-cloudrouter.md`
- `docs/02-技术架构设计.md`
- `docs/03-技术选型.md`
- `docs/04-模块规划.md`
- `docs/05-数据库设计.md`
- `docs/06-API-Gateway与接口标准设计.md`
- `docs/07-性能设计.md`
- `docs/08-安全设计.md`
- `docs/09-部署架构设计.md`
- `docs/10-API路径一致性与自由切换架构.md`
- `docs/11-数据契约与核心表设计.md`
- `docs/12-前端功能模块与数据库表结构映射.md`
- `docs/13-页面级数据结构覆盖与SchemaRegistry落地设计.md`
- `docs/14-数据结构细节复核与补强记录.md`
- `docs/15-new-api-sub2api价格体系对比与CloudRouter定价设计.md`
- `docs/16-前端代码契约复核与数据设计覆盖检查.md`
- `docs/17-AppCenter-PlusApp-compatible-design.md`
- `docs/18-SkillsHub-AgentSkills-PlusCategory-compatible-design.md`
- `docs/19-Finance-Trade-Java-compatible-design.md`
- `docs/20-schema-guardian-quality-gate.md`
- `docs/21-schema-compiler-postgres-ddl.md`
- `docs/22-domain-type-generator.md`
- `docs/23-schema-manifest.md`
- `docs/24-openapi-schema-components.md`
- `docs/25-frontend-contract-guardian.md`
- `docs/26-java-legacy-contract-audit.md`
- `docs/27-rust-runtime-and-sdk-integration-standard.md`
- `docs/28-architecture-standard-guardian.md`
- `docs/29-rust-backend-module-standard.md`
- `docs/30-flyway-schema-contract-audit.md`
- `docs/30-platform-data-model-v4.md`
- `docs/31-cloudrouter-openapi-generator.md`
- `docs/31-product-composition-model.md`
- `docs/32-sdkwork-models-standard.md`
- `docs/33-sdkwork-models-install-flow.md`
- `docs/34-login-qrcode-system.md`

Do not add new Canon content under this directory.
