...
| Config example | `/opt/sdkwork/router/config/config.toml.example` | generated | examples only; runtime config is mounted |
...
| --- | --- | --- |
| `/etc/sdkwork/router/config.toml` | read-only config | runtime TOML; overridable with `SDKWORK_CLOUDROUTER_CONFIG_FILE` |
...
1. **Runtime TOML** — `docker/config/config.toml` is mounted read-only at
   `/etc/sdkwork/router/config.toml` (mirror of the generated
   `config/config.toml.example`). Edit non-sensitive settings there
