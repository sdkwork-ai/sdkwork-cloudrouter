...
```yaml
错误设计:
  ❌ 所有61张表定义都集中在sdkwork-cloudrouter.tables.yaml
  ❌ Cloud Router重复定义了其他模块的表（iam_*, commerce_*, integration_*）
  ❌ 违反"单一所有权原则" - 各模块应该负责自己的表
  ❌ 违反"高内聚低耦合" - 所有表耦合在一个文件
  ❌ 违反"积木架构" - 各模块应该独立管理自己的schema
  
根本原因:
  ❌ 历史包袱 - 原来的设计就是所有表都在cloud-router
...
```yaml
正确设计（积木架构）:
  ✅ Cloud Router只定义自己的16张本地表
  ✅ 各模块在自己的schema registry中定义自己的表
  ✅ Cloud Router通过registry_dependencies引用其他模块
...
|--------|-------------------|-------------------|----------|
| **Cloud Router本地表** | 61张全部定义 | 16张本地定义 | ✅ 减少73%冗余定义 |
| **其他模块表** | 在cloud-router重复定义 | 仅引用不定义 | ✅ 避免重复建设 |
| **模块独立性** | 耦合在一个文件 | 各模块独立管理 | ✅ 高内聚低耦合 |
| **DDL归属** | 全部在cloud-router | 各模块管理自己的DDL | ✅ 单一所有权 |
| **migrations归属** | 全部在cloud-router | 各模块管理自己的migrations | ✅ 解耦部署 |
...
### 2.1 Cloud Router Schema Registry（仅16张本地表）

```yaml
文件: docs/schema-registry/sdkwork-cloudrouter.tables.yaml
...
### 3.1 Cloud Router本地DDL

```
E:\sdkwork-space\sdkwork-cloudrouter\database\migrations\
...
```
1. router-service写入ai_usage（cloud-router本地）
2. 发布UsageRecorded事件
...
**关键点**：
- ✅ Cloud Router不写入commerce_account_ledger_entry表
- ✅ 通过事件解耦，异步处理
...
✅ **Schema Registry主文件**
- sdkwork-cloudrouter.tables.yaml（仅16张本地表）
...
Schema Registry文件:
  - Cloud Router: 1个主文件 + 3个表定义文件
  - 各模块: 各自的schema registry文件

DDL脚本:
  - Cloud Router: 3个DDL文件（ai-routing/ai-metering/ai-pricing）
  - 各模块: 各自的DDL文件
```
...
| 方面 | 错误设计 | 正确设计 | 效果 |
|------|---------|---------|------|
| **表定义归属** | 所有表在cloud-router | 各模块定义自己的表 | ✅ 单一所有权 |
| **DDL归属** | 全部在cloud-router | 各模块管理自己的DDL | ✅ 解耦部署 |
| **模块耦合度** | 高耦合（沥青路面） | 低耦合（积木组合） | ✅ 高内聚低耦合 |
| **架构模式** | 违反积木架构 | 符合积木架构 | ✅ 行业最佳实践 |

### 9.2 最终结论

**Cloud Router表命名与模块划分优化方案已完美纠正！**
