# SDKWork Cloud Router Runtime Topology
...
start only the local adaptive web dev ingress and its private renderers; every
gateway-attached base URL binds to the locally started `sdkwork-api-cloud-gateway`
development process (`SDKWORK_LOCAL_PLATFORM_API_GATEWAY_HTTP_URL`), and they
never start a local API, gateway, or database process owned by this repository.
Product-prefixed `cloudrouter:*`, platform-first `desktop:*`, and tool-first
`tauri:*` scripts are retired.
