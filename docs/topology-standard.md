...
for test, `https://api-staging.sdkwork.com` for staging, `https://api.sdkwork.com`
for production, per `APP_RUNTIME_TOPOLOGY_NAMING.md` section 9). The command
fails before client startup when the gateway URL is absent; it never falls
back to loopback API defaults.
