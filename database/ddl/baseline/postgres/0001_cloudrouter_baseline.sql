...
-- ============================================================
-- Storage admin center tables (object_provider / object_bucket /
-- storage_*) — owned by cloud-router-platform router-service admin
-- storage store. Restored from the legacy storage surface contract.
-- ============================================================
CREATE TABLE IF NOT EXISTS object_provider (
    ...
    deleted_by BIGINT,
    metadata JSONB NOT NULL DEFAULT '{}'::jsonb,
    supplier_code VARCHAR(96) NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_object_provider_tenant_code ON object_provider (tenant_id, organization_id, supplier_code);
...
CREATE UNIQUE INDEX IF NOT EXISTS uk_object_bucket_scope_name ON object_bucket (tenant_id, organization_id, logical_scope, bucket_name);
...
CREATE INDEX IF NOT EXISTS idx_object_bucket_tenant_status ON object_bucket (tenant_id, organization_id, status, id);
...
CREATE TABLE IF NOT EXISTS storage_default_bucket_policy (
    ...
    bucket_logical_scope VARCHAR(64),
...
CREATE INDEX IF NOT EXISTS idx_storage_default_bucket_policy_tenant_status ON storage_default_bucket_policy (tenant_id, organization_id, status, id);
...
CREATE TABLE IF NOT EXISTS storage_quota_policy (
    ...
    quota_limit_bytes BIGINT NOT NULL DEFAULT 0,
    single_file_limit_bytes BIGINT,
    enforcement VARCHAR(32) NOT NULL DEFAULT 'active',
    idempotency_key VARCHAR(128),
    request_id VARCHAR(128)
);
...
CREATE INDEX IF NOT EXISTS idx_storage_quota_policy_scope ON storage_quota_policy (tenant_id, organization_id, scope_type, scope_id, id);
...
CREATE INDEX IF NOT EXISTS idx_storage_usage_counter_tenant ON storage_usage_counter (tenant_id, organization_id, id);
...
CREATE TABLE IF NOT EXISTS storage_usage_ledger (
    ...
    scope_type VARCHAR(64) NOT NULL,
    scope_id VARCHAR(128) NOT NULL,
    usage_event_type VARCHAR(64) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_storage_usage_ledger_scope ON storage_usage_ledger (tenant_id, organization_id, scope_type, scope_id, id);
...
CREATE INDEX IF NOT EXISTS idx_storage_reconciliation_run_tenant_status ON storage_reconciliation_run (tenant_id, organization_id, status, id);
...
CREATE INDEX IF NOT EXISTS idx_storage_gc_job_tenant_status ON storage_gc_job (tenant_id, organization_id, status, id);
