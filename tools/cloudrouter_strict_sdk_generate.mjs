...
export async function runStrictGenerateCommand(options) {
  ...
  if (language !== 'typescript') {
    throw new Error(`Strict CloudRouter SDK generation only supports TypeScript, got: ${language}`);
  }
...
function runProjectRuntimeStandardizer(outputPath, config) {
  ...
  if (
    workspace.sdkFamilyName !== 'cloudrouter-open-sdk'
  ...
  const details = [result.stdout, result.stderr]
    ...
    .join('\n');
  throw new Error(
    `CloudRouter SDK runtime standardization failed for ${workspace.typescriptDirectory}${details ? `:\n${details}` : ''}`,
  );
}
  ...
  return `${JSON.stringify(
    {
      language: 'typescript',
      sdkType,
      name: metadata.name || sdk.name || (sdkType === 'app' ? 'cloudrouter-app-sdk' : 'cloudrouter-backend-sdk'),
...
function inferPackageNameFromSpec(spec, sdkType) {
  ...
  if (sdkType === 'ai') {
    return '@sdkwork/cloudrouter-open-sdk';
  }
...
function buildStrictWarnings(originalFiles, strictFiles) {
  ...
  return strictFiles
    ...
    .map((file) => ({
      code: 'CLOUDROUTER_STRICT_TYPESCRIPT_CONTRACT',
      message: `Applied CloudRouter strict TypeScript contract normalization to ${file.path}`,
    }));
}
