...
class CloudRouterOpenApiPrecisionAuditResult:
    ok: bool
    messages: list[str]
...
class CloudRouterOpenApiPrecisionAudit:
    ...
    def run(self) -> CloudRouterOpenApiPrecisionAuditResult:
        messages: list[str] = []
        try:
        ...
        return CloudRouterOpenApiPrecisionAuditResult(ok=not messages, messages=messages)

    def _surface_operations(self, manifest: dict[str, Any], surface: str) -> list[dict[str, Any]]:
        generator = CloudRouterOpenApiGenerator(root=self.root, manifest_path=self.manifest_path)
        manifest_operations = [
            operation
            for operation in manifest.get("operations", [])
            if (
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit CloudRouter OpenAPI response precision.")
    ...
    result = CloudRouterOpenApiPrecisionAudit(
        root=args.root,
    ...
    if result.ok:
        print("CloudRouter OpenAPI precision audit passed")
        return 0
