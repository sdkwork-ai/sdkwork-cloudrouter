...
class CloudRouterSkillGuardianResult:
    ok: bool
    messages: list[str]


class CloudRouterSkillGuardian:
    """Validate project-local skills that enforce CloudRouter SDK integration."""
    ...
    def run(self) -> CloudRouterSkillGuardianResult:
        messages: list[str] = []
        ...
        return CloudRouterSkillGuardianResult(ok=not messages, messages=messages)
    ...
    def _check_skill_seed_bundle(self) -> list[str]:
        ...
        for artifact in artifacts:
            ...
            if isinstance(target_skill, dict) and self._is_cloudhub_community_skill(target_skill):
                if not isinstance(artifact_ref, str) or not artifact_ref.startswith("cloudhub://skills/"):
                    messages.append(f"skill seed artifact {artifact_uuid} must use cloudhub artifactRef")
                if artifact.get("runtime") != "metadata":
                    messages.append(f"skill seed artifact {artifact_uuid} must use metadata runtime for CloudHub")
            else:
        ...
        for skill in skills:
            ...
            elif self._is_cloudhub_community_skill(skill):
                if skill.get("builtin") is not False or skill.get("isBuiltin") is not False:
                    messages.append(f"skill seed skill {skill_key} must be non-builtin CloudHub community metadata")
                if skill.get("runtime") != "metadata":
                    messages.append(f"skill seed skill {skill_key} must use metadata runtime for CloudHub community data")
                source = skill.get("source")
                if not isinstance(source, dict) or source.get("vendor") != "cloudhub":
                    messages.append(f"skill seed skill {skill_key} must preserve cloudhub source metadata")
            else:
                messages.append(f"skill seed skill {skill_key} must be SDKWork official or CloudHub community seed data")
        ...
        messages.extend(self._check_cloudhub_local_mirror(skills_root, skills))
        return messages

    def _check_cloudhub_local_mirror(self, skills_root: Path, skills: list[Any]) -> list[str]:
        cloudhub_skills = [
            skill
            for skill in skills
            if isinstance(skill, dict) and self._is_cloudhub_community_skill(skill)
        ]
        if not cloudhub_skills:
            return []

        messages: list[str] = []
        raw_index_path = skills_root / "cloudhub" / "raw" / "index.json"
        normalized_manifest_path = skills_root / "cloudhub" / "manifest.json"
        ...
        if raw_index.get("mirrorMode") != "full-cursor-mirror":
            messages.append("skill seed CloudHub raw mirror must be full-cursor-mirror")
        if raw_index.get("source", {}).get("listApi") != "https://cloudhub.ai/api/v1/skills":
            messages.append("skill seed CloudHub raw mirror must use the public CloudHub list API")

        items = raw_index.get("items")
        if not isinstance(items, list) or len(items) < len(cloudhub_skills):
            messages.append("skill seed CloudHub raw mirror must include all seeded community skills")
        ...
        for skill in cloudhub_skills:
            source = skill.get("source") if isinstance(skill.get("source"), dict) else {}
            slug = source.get("slug")
            if slug not in mirrored_slugs:
                messages.append(f"skill seed CloudHub mirror is missing raw list slug for {skill.get('skillKey')}")
            if slug not in seeded_manifest_slugs:
                messages.append(f"skill seed CloudHub normalized manifest is missing seeded slug for {skill.get('skillKey')}")
        return messages
    ...
    def _is_cloudhub_community_skill(self, skill: dict[str, Any]) -> bool:
        return skill.get("sourceType") == "COMMUNITY" and skill.get("provider") == "CloudHub"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check sdkwork-cloudrouter project-local SDK skills.")
    parser.add_argument("--root", type=Path, default=Path.cwd(), help="sdkwork-cloudrouter root directory")
    args = parser.parse_args()

    result = CloudRouterSkillGuardian(root=args.root).run()
    if result.ok:
        print("CloudRouter project skills passed")
        return 0
