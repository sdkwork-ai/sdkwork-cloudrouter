...
class CloudRouterOpenApiContractAuditResult:
    ok: bool
    messages: list[str]


class CloudRouterOpenApiContractAudit:
    ...
    def run(self) -> CloudRouterOpenApiContractAuditResult:
        messages: list[str] = []
        ...
        return CloudRouterOpenApiContractAuditResult(ok=not messages, messages=messages)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit Cloud Router OpenAPI operation contract strength.")
    ...
    result = CloudRouterOpenApiContractAudit(root=args.root, openapi_dir=args.openapi_dir).run()
    if result.ok:
        print("Cloud Router OpenAPI contract audit passed")
        return 0
