...
class CloudRouterGatewayOpenApiCheckResult:
    ok: bool
    messages: list[str]
...
class CloudRouterGatewayOpenApiGenerator:
    """Generate the Cloud Router gateway OpenAPI document for /v1 and vendor APIs."""

    OUTPUT = Path("apps") / "sdkwork-cloudrouter-pc" / "public" / "openapi.json"
    ...
    def _default_schema_description(self, schema_name: str) -> str:
        ...
        if schema_name.startswith("OpenAi"):
            return f"OpenAI-compatible {label} schema exposed by Cloud Router."
        if schema_name.startswith("Google"):
            return f"Google Gemini {label} schema exposed by Cloud Router vendor routing."
        if schema_name.startswith("Anthropic"):
            return f"Anthropic Claude {label} schema exposed by Cloud Router vendor routing."
        if schema_name.startswith("Volcengine"):
            return f"Volcengine Ark {label} schema exposed by Cloud Router vendor routing."
        if schema_name.startswith("Suno"):
            return f"Suno-compatible {label} schema exposed by Cloud Router vendor routing."
        if schema_name.startswith("Midjourney"):
            return f"Midjourney-compatible {label} schema exposed by Cloud Router vendor routing."
        if schema_name.startswith("Kling"):
            return f"Kling-compatible {label} schema exposed by Cloud Router vendor routing."
        if schema_name.startswith("NanoBanana"):
            return f"Nano Banana compatible {label} schema exposed by Cloud Router vendor routing."
        if schema_name.startswith("Vidu"):
            return f"Vidu {label} schema exposed by Cloud Router vendor routing."
        if schema_name.startswith("Provider"):
            return f"Reusable provider {label} schema shared by Cloud Router vendor modules."
        return f"{label.capitalize()} schema exposed by Cloud Router."
    ...
    def _public_request_properties(
        ...
        *,
        ...
        if operation_id == "createCompletion":
            properties.update({
                "model": self._string_schema("Model id or Cloud Router catalog key routed to a provider account."),
                "prompt": {
                    "oneOf": [
                        {"type": "string"},
        ...
        if operation_id == "createModeration":
            properties.update({
                "model": self._string_schema("Moderation model id or Cloud Router catalog key."),
        ...
        if tag == "Videos":
            properties.update({
                "model": self._string_schema("Video model id or Cloud Router catalog key."),
        ...
        if tag == "Audio":
            properties.update({
                "model": self._string_schema("Audio model id or Cloud Router catalog key."),
        ...
        if tag == "Realtime":
            properties.update({
                "model": self._string_schema("Realtime model id or Cloud Router catalog key."),
    ...
    def _openai_response_resource_schemas(self) -> dict[str, Any]:
        return {
            "OpenAiResponseInputTokenCountRequest": {
                ...
                "required": ["model", "input"],
                "properties": {
                    "model": self._string_schema("Model id or Cloud Router catalog key used for token counting."),
                    "input": {
                        "oneOf": [
                            {"type": "string"},
            ...
            "OpenAiResponseCompactRequest": {
                ...
                "additionalProperties": {"$ref": "#/components/schemas/ProviderJsonValue"},
                "properties": {
                    "model": self._string_schema("Model id or Cloud Router catalog key used for compaction."),
    ...
    def _openai_realtime_resource_schemas(self) -> dict[str, Any]:
        return {
            ...
            "OpenAiRealtimeTranscriptionSessionCreateRequest": {
                ...
                "additionalProperties": {"$ref": "#/components/schemas/ProviderJsonValue"},
                "properties": {
                    "model": self._string_schema("Realtime transcription model id or Cloud Router catalog key."),
                    "input_audio_format": self._string_schema("Input audio format for transcription."),
                    "input_audio_transcription": self._json_value_schema("Realtime transcription configuration."),
            ...
            "OpenAiRealtimeTranslationSessionCreateRequest": {
                ...
                "additionalProperties": {"$ref": "#/components/schemas/ProviderJsonValue"},
                "properties": {
                    "model": self._string_schema("Realtime translation model id or Cloud Router catalog key."),
                    "source_language": self._string_schema("Source language for realtime translation."),
                    "target_language": self._string_schema("Target language for realtime translation."),
                    "metadata": self._metadata_schema("Developer-defined realtime metadata."),
                },
    ...
    def check(self) -> CloudRouterGatewayOpenApiCheckResult:
        if not self.output_path.exists():
            return CloudRouterGatewayOpenApiCheckResult(
                ok=False,
                messages=[f"Cloud Router gateway OpenAPI spec is missing: {self.output_path}"],
            )
        ...
        try:
            ...
            return CloudRouterGatewayOpenApiCheckResult(
                ok=False,
                messages=[f"Cloud Router gateway OpenAPI spec is invalid JSON: {exc}"],
            )
        audit_messages = self._vendor_schema_quality_messages(actual_spec)
        if audit_messages:
            return CloudRouterGatewayOpenApiCheckResult(ok=False, messages=audit_messages)
        ...
        if actual != expected:
            return CloudRouterGatewayOpenApiCheckResult(
                ok=False,
                messages=[f"Cloud Router gateway OpenAPI spec is stale: {self.output_path}"],
            )
        return CloudRouterGatewayOpenApiCheckResult(ok=True, messages=[])
    ...
    def _tags(self) -> list[dict[str, str]]:
        return [
            ...
            {"name": "Models", "description": "List and inspect Cloud Router model catalog entries exposed through /v1."},
            ...
            {"name": "Images/vidu", "description": "Vidu image generation APIs exposed through Cloud Router vendor routing."},
            {"name": "Videos/vidu", "description": "Vidu video generation APIs exposed through Cloud Router vendor routing."},
            {"name": "Images/midjourney", "description": "Midjourney-compatible image APIs exposed through Cloud Router vendor routing."},
            ...
            {"name": "Files/google", "description": "Google Gemini file APIs exposed through Cloud Router vendor routing."},
            {"name": "Files/anthropic", "description": "Anthropic file APIs exposed through Cloud Router vendor routing."},
    ...
    def _paths(self) -> dict[str, Any]:
        paths: dict[str, Any] = {}
        paths["/v1/models"] = {"get": self._operation("Models", "listModels", "List models", "Lists Cloud Router models available to the caller.", None, "OpenAiModelList")}
        paths["/v1/models/{model}"] = {
            "get": self._operation("Models", "retrieveModel", "Retrieve model", "Retrieves one model from the Cloud Router catalog.", None, "OpenAiModel", parameters=[self._path_param("model", "Model identifier or catalog key.")]),
        }
    ...
    def _responses(
        ...
        return {
            success_status: {"description": "Successful response from Cloud Router or the selected upstream provider.", "content": success_content},
    ...
    def _components(self) -> dict[str, Any]:
        return {
            "securitySchemes": {
                "bearerAuth": {"type": "http", "scheme": "bearer", "bearerFormat": "Cloud Router API key"}
            },
            "schemas": {
                "JsonObject": {"type": "object", "additionalProperties": True, "description": "Provider-specific JSON payload accepted by Cloud Router."},
                "DeleteResult": {
                    "type": "object",
                ...
                "OpenAiModel": {
                    ...
                    "properties": {
                        "id": {"type": "string", "description": "Model identifier or Cloud Router catalog key."},
                ...
                "OpenAiChatCompletionRequest": {
                    ...
                    "properties": {
                        "model": {"type": "string", "description": "Model id or Cloud Router catalog key routed to a provider account."},
                ...
                "OpenAiEmbeddingsRequest": {
                    ...
                    "required": ["model", "input"],
                    "properties": {
                        "model": {"type": "string", "description": "Embedding model id or Cloud Router catalog key routed to a provider account."},
                        "input": {
                            "oneOf": [
                                {"type": "string"},
                                {"type": "array", "items": {"type": "string"}},
                ...
                "OpenAiImageGenerationRequest": {"type": "object", "additionalProperties": True, "required": ["model", "prompt"], "properties": {"model": {"type": "string", "description": "Image model id or Cloud Router catalog key."}, "prompt": {"type": "string", "description": "Text prompt describing the image to generate."}, "n": {"type": "integer", "minimum": 1, "maximum": 16, "description": "Number of images to generate when supported."}, "size": {"type": "string", "description": "Requested image size."}, "quality": {"type": "string", "description": "Requested image quality when supported."}, "response_format": {"type": "string", "description": "Desired response format, such as url or b64_json."}}},
                "OpenAiImageEditRequest": {"type": "object", "additionalProperties": True, "required": ["model", "prompt"], "properties": {"model": {"type": "string", "description": "Image edit model id or Cloud Router catalog key."}, "prompt": {"type": "string", "description": "Text prompt describing the edit."}, "image": {"$ref": "#/components/schemas/OpenAiImageReferenceInputList"}, "mask": {"$ref": "#/components/schemas/OpenAiImageReferenceInput"}}},
                "OpenAiImageEditMultipartRequest": {"type": "object", "additionalProperties": True, "required": ["model", "prompt", "image"], "properties": {"model": {"type": "string", "description": "Image edit model id or Cloud Router catalog key."}, "prompt": {"type": "string", "description": "Text prompt describing the edit."}, "image": {"$ref": "#/components/schemas/OpenAiBinaryFilePart"}, "mask": {"$ref": "#/components/schemas/OpenAiBinaryFilePart"}}},
                "OpenAiImageVariationRequest": {"type": "object", "additionalProperties": True, "required": ["model", "image"], "properties": {"model": {"type": "string", "description": "Image variation model id or Cloud Router catalog key."}, "image": {"$ref": "#/components/schemas/OpenAiImageReferenceInput"}, "size": {"type": "string", "description": "Requested image size."}}},
                "OpenAiImageVariationMultipartRequest": {"type": "object", "additionalProperties": True, "required": ["model", "image"], "properties": {"model": {"type": "string", "description": "Image variation model id or Cloud Router catalog key."}, "image": {"$ref": "#/components/schemas/OpenAiBinaryFilePart"}, "size": {"type": "string", "description": "Requested image size."}}},
                "OpenAiAudioTranscriptionRequest": {"type": "object", "additionalProperties": True, "required": ["model", "file"], "properties": {"model": {"type": "string", "description": "Transcription model id or Cloud Router catalog key."}, "file": {"$ref": "#/components/schemas/OpenAiFileReferenceInput"}, "language": {"type": "string", "description": "Optional source language hint."}, "prompt": {"type": "string", "description": "Optional text prompt to guide transcription."}, "response_format": {"type": "string", "description": "Desired transcription response format."}}},
                "OpenAiAudioTranscriptionMultipartRequest": {"type": "object", "additionalProperties": True, "required": ["model", "file"], "properties": {"model": {"type": "string", "description": "Transcription model id or Cloud Router catalog key."}, "file": {"$ref": "#/components/schemas/OpenAiBinaryFilePart"}, "language": {"type": "string", "description": "Optional source language hint."}, "prompt": {"type": "string", "description": "Optional text prompt to guide transcription."}, "response_format": {"type": "string", "description": "Desired transcription response format."}}},
                "OpenAiAudioTranslationRequest": {"type": "object", "additionalProperties": True, "required": ["model", "file"], "properties": {"model": {"type": "string", "description": "Translation model id or Cloud Router catalog key."}, "file": {"$ref": "#/components/schemas/OpenAiFileReferenceInput"}, "prompt": {"type": "string", "description": "Optional text prompt to guide translation."}, "response_format": {"type": "string", "description": "Desired translation response format."}}},
                "OpenAiAudioTranslationMultipartRequest": {"type": "object", "additionalProperties": True, "required": ["model", "file"], "properties": {"model": {"type": "string", "description": "Translation model id or Cloud Router catalog key."}, "file": {"$ref": "#/components/schemas/OpenAiBinaryFilePart"}, "prompt": {"type": "string", "description": "Optional text prompt to guide translation."}, "response_format": {"type": "string", "description": "Desired translation response format."}}},
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Cloud Router gateway OpenAPI spec.")
    ...
    generator = CloudRouterGatewayOpenApiGenerator(root=args.root, output_path=args.output)
    if args.check:
        result = generator.check()
        if result.ok:
            print("Cloud Router gateway OpenAPI spec is current")
            return 0
    ...
    print(f"Wrote Cloud Router gateway OpenAPI spec to {output}")
    return 0
