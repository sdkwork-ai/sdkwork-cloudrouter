...
BACKEND_DOMAIN_CALL_PATTERN = re.compile(
    r"getCloudRouterBackendSdkClient\(\)\.([a-zA-Z0-9_.]+)\("
)
...
def _load_openapi_index(root: Path) -> dict[str, tuple[str, str]]:
    spec_path = root / "generated" / "openapi" / "cloudrouter-backend-openapi.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Materialize missing cloudrouter frontend operation contracts.")
