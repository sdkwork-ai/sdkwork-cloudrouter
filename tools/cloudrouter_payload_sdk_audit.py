...
class CloudRouterPayloadSdkAuditResult:
    ok: bool
    messages: list[str]


class CloudRouterPayloadSdkAudit:
    ...
    SDK_DIRECTORIES = {
        "app": "cloudrouter-app-sdk/cloudrouter-app-sdk-typescript",
        "backend": "cloudrouter-backend-sdk/cloudrouter-backend-sdk-typescript",
    }
    SDK_FAMILIES = {
        "app": "cloudrouter-app-sdk",
        "backend": "cloudrouter-backend-sdk",
    }
    ...
    def __init__(
        ...
        self._dependency_resolver = CloudRouterOpenApiGenerator(
            root=self.root,
            manifest_path=self.manifest_path,
        )

    def run(self) -> CloudRouterPayloadSdkAuditResult:
        messages: list[str] = []
        try:
        ...
        return CloudRouterPayloadSdkAuditResult(ok=not messages, messages=messages)
    ...
    def _load_spec(self, surface: str, messages: list[str]) -> dict[str, Any] | None:
        ...
        if not path.exists():
            messages.append(f"cloudrouter {surface} OpenAPI spec is missing: {path}")
            return None
        payload = self._load_json(path)
        if not isinstance(payload, dict):
            messages.append(f"cloudrouter {surface} OpenAPI spec root must be an object: {path}")
            return None
        return payload
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit explicit CloudRouter payload schemas in OpenAPI and generated SDKs.")
    ...
    result = CloudRouterPayloadSdkAudit(
        root=args.root,
    ...
    if result.ok:
        print("CloudRouter payload SDK audit passed")
        return 0
