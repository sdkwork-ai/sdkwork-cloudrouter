...
# cloudrouter-openapi-strong-types-marker
import argparse
import copy
import json
import re
...
class CloudRouterOpenApiCheckResult:
    ok: bool
    messages: list[str]


class CloudRouterOpenApiGenerator:
    """Generate app/backend OpenAPI specs from the CloudRouter API contract manifest."""
    ...
    API_AUTHORITY_OUTPUTS = {
        "app": Path("apis/app-api/cloudrouter/cloudrouter-app-api.openapi.json"),
        "backend": Path("apis/backend-api/cloudrouter/cloudrouter-backend-api.openapi.json"),
    }
    MODELS_CATALOG_OUTPUTS = {
        "app": "cloudrouter-models-catalog-app-openapi.json",
        "backend": "cloudrouter-models-catalog-backend-openapi.json",
    }
    ...
    TITLES = {
        "app": "SDKWork Cloud Router App API",
        "backend": "SDKWork Cloud Router Backend API",
    }
    SERVERS = {
    ...
    def _apply_contract_overrides(
        ...
        if overrides.get("owner") != "sdkwork-cloudrouter" or overrides.get("surface") != "backend-api":
    ...
    def _surface_sdk_manifest(self, surface: str) -> dict[str, Any]:
        family = f"cloudrouter-{surface}-sdk"
    ...
    def check(self) -> CloudRouterOpenApiCheckResult:
        messages: list[str] = []
        try:
            for surface in self.SURFACES:
                output = self.output_path(surface)
                ...
                if not output.exists():
                    messages.append(f"cloudrouter {surface} OpenAPI spec is missing: {output}")
                    continue
                actual = self._normalized_openapi_text(surface, from_disk=True)
                if actual != expected:
                    messages.append(f"cloudrouter {surface} OpenAPI spec is stale: {output}")
                api_authority_output = self.api_authority_output_path(surface)
                api_authority_expected = self._normalized_openapi_text(surface, from_disk=False)
                if not api_authority_output.exists():
                    messages.append(f"cloudrouter {surface} API authority OpenAPI spec is missing: {api_authority_output}")
                    continue
                ...
                if api_authority_actual != api_authority_expected:
                    messages.append(
                        f"cloudrouter {surface} API authority OpenAPI spec is stale: {api_authority_output}"
                    )
                models_catalog_output = self.models_catalog_output_path(surface)
                models_catalog_expected = self.render_models_catalog_json(surface)
                if not models_catalog_output.exists():
                    messages.append(
                        f"cloudrouter models catalog {surface} OpenAPI spec is missing: {models_catalog_output}"
                    )
                    continue
                models_catalog_actual = models_catalog_output.read_text(encoding="utf-8")
                if models_catalog_actual != models_catalog_expected:
                    messages.append(
                        f"cloudrouter models catalog {surface} OpenAPI spec is stale: {models_catalog_output}"
                    )
        except (OSError, ValueError, json.JSONDecodeError) as exc:
            messages.append(str(exc))
        return CloudRouterOpenApiCheckResult(ok=not messages, messages=messages)
    ...
    def _tags(self, operations: list[dict[str, Any]]) -> list[dict[str, str]]:
        ...
        return [
            {
                "name": tag,
                "description": f"{self._tag_label(tag)} operations exposed by Cloud Router.",
            }
            for tag in tags
        ]
    ...
    def _components(
        ...
        schemas = {
            "JsonValue": {
                "description": "JSON value accepted by flexible Cloud Router metadata and extension maps.",
                "oneOf": [
    ...
    def _default_schema_node_description(self, *, schema_name: str, location: str, node: dict[str, Any]) -> str:
        if location == f"#/components/schemas/{schema_name}":
            return f"{self._schema_label(schema_name).capitalize()} schema exposed by Cloud Router."
        property_name = self._property_name_from_location(location)
        if property_name:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate sdkwork-cloudrouter app/backend OpenAPI specs.")
    ...
    generator = CloudRouterOpenApiGenerator(root=args.root, manifest_path=args.manifest, output_dir=args.output_dir)
    if args.check:
        result = generator.check()
        if result.ok:
            print("CloudRouter OpenAPI specs are current")
            return 0
