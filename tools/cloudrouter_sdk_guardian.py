...
class CloudRouterSdkGuardianResult:
    ok: bool
    messages: list[str]
...
class CloudRouterSdkGuardian:
    ...
    EXPECTED = (
        ExpectedSdk(
            family_directory="cloudrouter-app-sdk",
            typescript_directory="cloudrouter-app-sdk-typescript",
            package_name="@sdkwork/cloudrouter-app-sdk",
            sdk_type="app",
            client_name="SdkworkAppClient",
        ),
        ExpectedSdk(
            family_directory="cloudrouter-backend-sdk",
            typescript_directory="cloudrouter-backend-sdk-typescript",
            package_name="@sdkwork/cloudrouter-backend-sdk",
            sdk_type="backend",
            client_name="SdkworkBackendClient",
        ),
        ExpectedSdk(
            family_directory="cloudrouter-open-sdk",
            typescript_directory="cloudrouter-open-sdk-typescript",
    ...
    def run(self) -> CloudRouterSdkGuardianResult:
        messages: list[str] = []
        ...
        return CloudRouterSdkGuardianResult(ok=not messages, messages=messages)
    ...
    def _check_family_openapi_sync(self, expected: ExpectedSdk, family: Path) -> list[str]:
        ...
        if expected.family_directory == "cloudrouter-open-sdk":
    ...
    def _check_sdk_family(self, expected: ExpectedSdk, family: Path) -> list[str]:
        ...
        if assembly is not None:
            ...
            elif expected.family_directory == "cloudrouter-open-sdk":
    ...
    def _check_portal_boundary(self) -> list[str]:
        ...
        commons_root = portal_root / "packages" / "sdkwork-cloudroutes-pc-commons"
        ...
        boundary_relative = "apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudroutes-pc-commons/src/sdk-clients.ts"
        ...
        runtime_relative = "apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudroutes-pc-commons/src/runtime.ts"
        ...
        index_relative = "apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudroutes-pc-commons/src/index.ts"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check sdkwork-cloudrouter generated SDK packages.")
    parser.add_argument("--root", type=Path, default=Path.cwd(), help="sdkwork-cloudrouter root directory")
    args = parser.parse_args()

    result = CloudRouterSdkGuardian(root=args.root).run()
    if result.ok:
        print("CloudRouter generated SDKs passed")
        return 0
