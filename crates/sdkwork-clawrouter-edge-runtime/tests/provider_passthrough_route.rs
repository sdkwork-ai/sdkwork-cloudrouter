...
) -> Result<Router, sdkwork_clawrouter_edge_runtime::GatewayRouterError> {
...
async fn seeded_gateway_router_with_provider_configs_and_query_string_api_key_policy(
    ...
    sdkwork_clawrouter_edge_runtime::runtime::router_with_database_api_key_provider_configs_usage_settlement_worker_config_startup_install_mode_and_query_string_api_key_policy(
        catalog.database_config().unwrap(),
        Some(catalog.api_key_security_config().unwrap()),
        provider_relay_config,
        provider_secret_map_config,
...
async fn seeded_gateway_router_with_provider_adapter_config(
    ...
    sdkwork_clawrouter_edge_runtime::runtime::router_with_database_api_key_provider_configs_adapter_config_and_startup_install_mode(
        catalog.database_config().unwrap(),
        Some(catalog.api_key_security_config().unwrap()),
        None,
        provider_secret_map_config,
...
fn openai_compatible_passthrough_path_manifest_excludes_provider_control_plane() {
    let paths = sdkwork_clawrouter_edge_runtime::openai_compatible_passthrough_paths();
    let method_paths = sdkwork_clawrouter_edge_runtime::openai_method_passthrough_paths();
...
async fn openai_passthrough_rejects_methods_absent_from_the_public_contract() {
    let router = sdkwork_clawrouter_edge_runtime::gateway_passthrough_router();
...
fn gateway_openapi_v1_paths_are_product_routes_or_openai_passthrough_routes() {
    ...
    let openapi_paths = spec["paths"]
        ...
        .map(String::as_str)
        .collect::<BTreeSet<_>>();
    let passthrough_paths = sdkwork_clawrouter_edge_runtime::openai_compatible_passthrough_paths()
        .iter()
        .chain(sdkwork_clawrouter_edge_runtime::openai_method_passthrough_paths().iter())
        .chain(sdkwork_clawrouter_edge_runtime::stored_chat_completion_passthrough_paths().iter())
        .copied()
        .collect::<BTreeSet<_>>();
...
fn gateway_openapi_vendor_paths_are_runtime_provider_routes_without_public_manifest() {
    ...
    let runtime_providers =
        sdkwork_clawrouter_edge_runtime::provider_native_passthrough_providers()
            .iter()
            .copied()
            .collect::<BTreeSet<_>>();
...
async fn gateway_mounts_provider_native_passthrough_boundaries_without_404() {
    let router = sdkwork_clawrouter_edge_runtime::router();
...
async fn gateway_rejects_local_provider_native_passthrough_target_in_production() {
    ...
    .unwrap();
    let router = sdkwork_clawrouter_edge_runtime::router_with_provider_passthrough_config(config);
...
async fn gateway_forwards_configured_vendor_prefixed_vidu_passthrough_request() {
    ...
    .unwrap();
    let router =
        sdkwork_clawrouter_edge_runtime::router_with_provider_passthrough_config_for_development(
            config,
        );
