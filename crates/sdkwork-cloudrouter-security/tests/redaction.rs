...
fn sensitive_header_values_are_redacted() {
    assert_eq!(
        "[REDACTED]",
        sdkwork_cloudrouter_security::redact_header_value("authorization", "Bearer abc")
    );
    assert_eq!(
        "public",
        sdkwork_cloudrouter_security::redact_header_value("x-request-id", "public")
    );
}
...
fn redact_url_keeps_only_scheme_and_host() {
    assert_eq!(
        ...
        sdkwork_cloudrouter_security::redact_url("https://api.openai.com/v1/chat/completions")
    );
    assert_eq!(
        "https://api.openai.com",
        sdkwork_cloudrouter_security::redact_url("https://api.openai.com/v1/chat?key=sk-secret")
    );
}
...
fn redact_url_strips_userinfo() {
    assert_eq!(
        "https://api.openai.com",
        sdkwork_cloudrouter_security::redact_url("https://user:pass@api.openai.com/v1")
    );
    assert_eq!(
        "https://api.openai.com",
        sdkwork_cloudrouter_security::redact_url("https://token@api.openai.com/v1?key=secret")
    );
}
...
fn redact_url_preserves_port() {
    assert_eq!(
        ...
        sdkwork_cloudrouter_security::redact_url("https://api.openai.com:8443/v1/chat")
    );
}
...
fn redact_error_message_redacts_embedded_urls() {
    ...
    let redacted = sdkwork_cloudrouter_security::redact_error_message(message);
