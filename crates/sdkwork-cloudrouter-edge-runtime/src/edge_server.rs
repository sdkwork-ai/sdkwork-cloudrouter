...
const GENERATED_SDK_ARCHIVES: &[GeneratedSdkArchiveSpec] = &[
    GeneratedSdkArchiveSpec {
        ...
        language: "typescript",
        version: "0.1.0",
        file_name: "sdkwork-cloudrouter-app-sdk-typescript-0.1.0.zip",
    },
    GeneratedSdkArchiveSpec {
        package_name: "@sdkwork/cloudrouter-backend-sdk",
        language: "typescript",
        version: "0.1.0",
        file_name: "sdkwork-cloudrouter-backend-sdk-typescript-0.1.0.zip",
    },
];
...
const DEFAULT_SDK_README_PACKAGE_NAME: &str = "@sdkwork/cloudrouter-app-sdk";
const DEFAULT_SDK_README_DESCRIPTION: &str = "SDKWork Cloud Router app API SDK";
...
impl EdgeServerConfig {
    ...
    fn target_for_path(&self, path: &str) -> Option<&str> {
        if path == sdkwork_cloudrouter_http::OPENAPI_SCHEMA_TABS_PATH {
            return Some(&self.gateway_base_url);
        }
        if path == "/openapi.json" {
            return Some(&self.gateway_base_url);
        }
        if path == sdkwork_cloudrouter_http::PAYMENT_AGGREGATE_OPENAPI_PATH {
            return Some(&self.gateway_base_url);
        }
        if path == sdkwork_cloudrouter_http::PAAS_OPENAPI_PATH {
            return Some(&self.gateway_base_url);
        }
        if path == sdkwork_cloudrouter_http::CLOUD_SERVICES_OPENAPI_PATH {
            return Some(&self.gateway_base_url);
        }
...
fn surface_for_path(path: &str) -> Option<EdgeApiSurface> {
    if path == sdkwork_cloudrouter_http::OPENAPI_SCHEMA_TABS_PATH
        || path == "/openapi.json"
        || path == sdkwork_cloudrouter_http::PAYMENT_AGGREGATE_OPENAPI_PATH
        || path == sdkwork_cloudrouter_http::PAAS_OPENAPI_PATH
        || path == sdkwork_cloudrouter_http::CLOUD_SERVICES_OPENAPI_PATH
        || path == "/v1"
        || path.starts_with("/v1/")
    {
...
async fn edge_health() -> impl IntoResponse {
    Json(json!({
        "status": "ok",
        "service": "sdkwork-cloudrouter-edge-server",
    }))
}
    ...
    let Some(root) = &state.config.portal_tool_api_sdk_archive_root else {
        return json_response(
            ...
            json!({
                ...
                "error": "SDK archive generation is not available in the Rust edge server. Configure SDKWORK_CLOUDROUTER_TOOL_API_SDK_ARCHIVE_ROOT to serve prebuilt SDK ZIP archives produced by the release pipeline.",
            }),
        );
    };
...
fn build_shell_snippet(method: &str, url: &str, body: Option<&serde_json::Value>) -> String {
    let mut lines = vec![
        ...
        "  -H \"Authorization: Bearer $CLOUDROUTER_API_KEY\"".to_owned(),
    ];
    if let Some(body) = body {
...
fn build_generic_http_snippet(method: &str, url: &str, body: Option<&serde_json::Value>) -> String {
    let mut lines = vec![
        ...
        "Authorization: Bearer <CLOUDROUTER_API_KEY>".to_owned(),
    ];
    if let Some(body) = body {
        lines.push("Content-Type: application/json".to_owned());
        lines.push(String::new());
...
fn quick_start_snippet(request: &SdkReadmeRequest, package_name: &str) -> String {
    match request.language.as_str() {
        ...
        _ => format!(
            "Initialize {name} with base URL {base_url} and your CLOUDROUTER_API_KEY.",
            name = request.name,
            base_url = request.base_url,
        ),
    }
...
fn normalize_portal_csp_origin(value: &str) -> Result<String, String> {
    ...
    {
        return Err(
            "SDKWORK_CLOUDROUTER_EDGE_CSP_CONNECT_SRC entries must be HTTP/HTTPS origins without directives"
                .to_owned(),
        );
    }
    ...
    if uri.path() != "/" || uri.query().is_some() {
        return Err(
            "SDKWORK_CLOUDROUTER_EDGE_CSP_CONNECT_SRC entries must be origins without path or query"
                .to_owned(),
        );
    }
    Ok(trimmed.to_owned())
