...
pub(crate) fn router_with_database_status_and_passthrough_placeholder(
    config: Option<&sdkwork_cloudrouter_config::DatabaseConfig>,
    include_passthrough_placeholder: bool,
    readiness_check: Option<sdkwork_cloudrouter_http::ReadinessCheckFn>,
    deployment_mode: Option<sdkwork_cloudrouter_config::DeploymentMode>,
) -> axum::Router {
    let router = match deployment_mode {
        Some(deployment_mode) => {
            sdkwork_cloudrouter_http::service_router_with_database_config_readiness_check_and_deployment_mode(
                SERVICE_NAME,
                config,
                readiness_check,
                deployment_mode,
        ...
        None => sdkwork_cloudrouter_http::service_router_with_database_config_and_readiness_check(
            SERVICE_NAME,
            config,
            readiness_check,
        ),
