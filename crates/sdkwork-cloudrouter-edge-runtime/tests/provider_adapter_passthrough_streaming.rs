...
use sdkwork_cloudrouter_config::{ProviderAdapterConfig, ProviderRelayConfig};
...
async fn provider_adapter_streaming_passthrough_returns_headers_before_adapter_eof() {
    ...
    .unwrap();
    let router =
        sdkwork_cloudrouter_edge_runtime::router_with_provider_passthrough_and_adapter_config_for_development(
            relay_config,
            Some(adapter_config),
        );
