...
use sdkwork_cloudrouter_router_service::domain::{
    ...
    PriceSide, PricingPlan, ProviderRetryPolicy, QuotaPolicy, RoutingCapability,
...
async fn invocation_router_dispatches_openai_model_call_through_pipeline() {
    ...
    let router =
        sdkwork_cloudrouter_edge_runtime::invocation_router_with_catalog_api_key_hasher_dispatcher_and_secret_resolver(
...
async fn invocation_router_provider_native_adapter_uses_standard_chain_and_records_usage() {
    ...
    let usage_recorder = Arc::new(RecordingUsageRecorder::default());
    let router =
        sdkwork_cloudrouter_edge_runtime::invocation_router_with_full_pipeline_and_provider_adapter_config(
            Arc::new(catalog_with_hashed_api_key(&key_hash)),
            hasher,
            Arc::new(sdkwork_cloudrouter_edge_runtime::InvocationHttpDispatcher::for_development()),
            sdkwork_cloudrouter_edge_runtime::InvocationRouterOptions {
                ...
                ..sdkwork_cloudrouter_edge_runtime::InvocationRouterOptions::default()
            },
        );
...
async fn invocation_router_fallback_preserves_explicit_provider_native_routes() {
    ...
    let invocation_router = sdkwork_cloudrouter_edge_runtime::invocation_router_with_full_pipeline(
        Arc::new(catalog_with_hashed_api_key(&key_hash)),
        hasher,
        dispatcher.clone(),
        Some(secret_resolver()),
