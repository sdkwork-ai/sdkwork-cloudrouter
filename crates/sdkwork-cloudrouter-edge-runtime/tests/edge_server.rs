        ...
        "/v1/sdk-generator/jobs/job-123/download?format=zip" => {
            ...
            *response.status_mut() = StatusCode::OK;
            ...
            response.headers_mut().insert(
                ...
                HeaderValue::from_static("attachment; filename=\"generated-cloudrouter-sdk.zip\""),
            );
            response
        }
...
fn temp_portal_dist_dir(test_name: &str) -> PathBuf {
    let suffix = SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    std::env::temp_dir().join(format!("sdkwork-cloudrouter-{test_name}-{suffix}"))
}
...
fn write_portal_dist_fixture(root: &Path) {
    ...
    std::fs::write(
        ...
        r#"<!doctype html>
<html lang="en">
  <head><title>Cloud Router</title></head>
  <body>
    <div id="root"></div>
...
async fn edge_server_exposes_own_health_check_without_upstream_probe() {
    let router = sdkwork_cloudrouter_edge_runtime::edge_server_router(
        sdkwork_cloudrouter_edge_runtime::EdgeServerConfig::try_new(
            "http://127.0.0.1:1",
            "http://127.0.0.1:1",
            "http://127.0.0.1:1",
            "http://127.0.0.1:1",
    ...
    assert_eq!("sdkwork-cloudrouter-edge-server", json["service"]);
}
...
async fn edge_server_can_dispatch_to_in_process_upstreams() {
    ...
    let router = sdkwork_cloudrouter_edge_runtime::edge_server_router_with_in_process_upstreams(
        sdkwork_cloudrouter_edge_runtime::EdgeServerConfig::try_new(
            "http://127.0.0.1:1",
            "http://127.0.0.1:1",
            "http://127.0.0.1:1",
...
async fn edge_server_can_serve_portal_dist_without_node_server() {
    ...
    let config = sdkwork_cloudrouter_edge_runtime::EdgeServerConfig::try_new(
        &gateway.base_url,
        &admin.base_url,
        &app.base_url,
        "http://127.0.0.1:3901",
    ...
    let router = sdkwork_cloudrouter_edge_runtime::edge_server_router(config);
    ...
    assert!(runtime_env.contains("window.__CLOUDROUTER_ENV__ = Object.freeze("));
    assert!(runtime_env.contains(r#""VITE_API_BASE_URL":"https://tenant-api.example.com/api""#));
    assert!(runtime_env
        .contains(r#""VITE_CLOUDROUTER_OPEN_API_BASE_URL":"https://tenant-api.example.com/api""#));
    assert!(runtime_env.contains(r#""VITE_CLOUDROUTER_APP_API_BASE_URL":"/app/v3/api""#));
    assert!(runtime_env.contains(r#""VITE_CLOUDROUTER_BACKEND_API_BASE_URL":"/backend/v3/api""#));
...
async fn edge_server_handles_enabled_portal_tool_api_inside_rust_edge() {
    ...
    let code_snippet_body = json!({
        ...
        "openAPISpec": {
            "openapi": "3.1.0",
            "info": { "title": "SDKWork Cloud Router", "version": "1.0.0" },
            "paths": { "/v1/models": { "get": {} } }
        }
    });
    ...
    assert!(code.contains("CLOUDROUTER_API_KEY"));
    ...
    assert!(readme.contains("@sdkwork/cloudrouter-app-sdk"));
    ...
    assert!(default_readme.contains("@sdkwork/cloudrouter-app-sdk"));
...
async fn edge_server_serves_prebuilt_sdk_archive_from_configured_root() {
    ...
    std::fs::write(
        archive_root.join("sdkwork-cloudrouter-app-sdk-typescript-0.1.0.zip"),
        archive_bytes,
    )
    .unwrap();
    ...
    assert!(response.headers()[header::CONTENT_DISPOSITION]
        .to_str()
        .unwrap()
        .contains("sdkwork-cloudrouter-app-sdk-typescript-0.1.0.zip"));
    let body = to_bytes(response.into_body(), usize::MAX).await.unwrap();
...
async fn edge_server_generates_sdk_with_default_current_origin_generator_url() {
    ...
    let body = json!({
        "spec": {
            "openapi": "3.1.0",
            "info": { "title": "SDKWork Cloud Router App API", "version": "1.0.0" },
    ...
    assert!(response.headers()[header::CONTENT_DISPOSITION]
        .to_str()
        .unwrap()
        .contains("generated-cloudrouter-sdk.zip"));
    ...
    assert!(upload_body.contains("SDKWork Cloud Router App API"));
...
fn edge_server_config_rejects_non_origin_forward_targets() {
    assert!(sdkwork_cloudrouter_edge_runtime::EdgeServerConfig::try_new(
