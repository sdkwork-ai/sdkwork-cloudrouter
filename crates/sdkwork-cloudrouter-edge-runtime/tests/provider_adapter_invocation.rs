...
use sdkwork_cloudrouter_provider_adapter_http::AdapterInvokeResult;
use sdkwork_cloudrouter_provider_adapter_registry::ProviderAdapterRouteConfig;
...
async fn gateway_adapter_transport_posts_stable_envelope_to_internal_adapter() {
    ...
    let transport =
        sdkwork_cloudrouter_edge_runtime::provider_adapter_transport::ProviderAdapterHttpTransport::for_development(
            "test-token",
        );
