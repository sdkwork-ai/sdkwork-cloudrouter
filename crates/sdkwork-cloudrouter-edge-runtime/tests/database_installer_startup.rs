...
async fn gateway_env_startup_rejects_zero_config_server_placeholder_postgres() {
    ...
    let router_result = sdkwork_cloudrouter_edge_runtime::runtime::router_from_env().await;
...
async fn gateway_env_startup_rejects_static_server_snowflake_node_id_before_database_bootstrap() {
    ...
    std::env::set_var("SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE", "server");
    std::env::set_var("SDKWORK_CLOUDROUTER_CONFIG_FILE", &config_path);
    std::env::set_var("SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID", "17");
    ...
    let error = router_result
        ...
        .to_string();
    assert!(error.contains("SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID"));
