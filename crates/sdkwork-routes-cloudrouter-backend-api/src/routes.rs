...
use sdkwork_cloudrouter_router_service::infrastructure::sql::postgres::{
    ...
    PostgresAdminServiceNodeStore, PostgresAdminStorageStore, PostgresAdminTransactionCenterStore,
...
type AdminUpstreamRuntimeVerifier = Arc<dyn AdminUpstreamAccountVerifier + Send + Sync>;
...
type AdminChainPolicyRuntimeStore = Arc<dyn AdminChainPolicyStore + Send + Sync>;
...
pub fn build_sdkwork_cloudrouter_router_backend_api_router() -> Router {
    router()
}

pub async fn build_sdkwork_cloudrouter_router_backend_api_router_from_env(
) -> Result<Router, ProductCatalogRouterError> {
    router_from_env().await
}

#[derive(Clone)]
struct AdminAccessChecker(PgPool);

#[derive(Clone)]
struct AdminSubjectBoundaryConfig {
    subject_boundary: sdkwork_cloudrouter_http::AppSubjectBoundaryConfig,
...
struct AdminRouterRuntime<'a> {
    ...
    official_pricing_read_store: Option<OfficialPricingRuntimeReadStore>,
    ...
    match deployment_mode {
        Some(deployment_mode) => {
            sdkwork_cloudrouter_http::service_router_with_filtered_contract_routes_database_config_readiness_check_and_deployment_mode(
                SERVICE_NAME,
                sdkwork_cloudrouter_http::ApiSurface::Backend,
                config,
    ...
    C: UpstreamAccountRouteCatalog + Send + Sync + 'static,
{
    ...
    let subject_boundary_config = match (trusted_subject_config.clone(), app_session_config.clone())
    {
        (Some(trusted_subject_config), Some(app_session_config)) => {
            Some(sdkwork_cloudrouter_http::AppSubjectBoundaryConfig::new(
                trusted_subject_config,
                app_session_config,
            ))
        }
    ...
    if let Some(installer) = database_installer {
        let system_router =
            sdkwork_cloudrouter_router_service::api::admin_system_router_with_installer(installer);
        router = merge_admin_router_with_subject_boundary(
            router,
            &admin_subject_boundary_config,
            system_router,
    ...
    if let Some(manager) = cache_manager.clone() {
        let cache_router =
            sdkwork_cloudrouter_router_service::api::admin_cache_router_with_manager(manager);
        router = merge_admin_router_with_subject_boundary(
            router,
            &admin_subject_boundary_config,
            cache_router,
    ...
    if let Some(admin_subject_boundary_config) = admin_subject_boundary_config {
        if let (Some(store), Some(api_key_hasher)) = (api_key_command_store, api_key_hasher.clone())
        {
            router = router.merge(layer_with_admin_subject_boundary(
                ...
                sdkwork_cloudrouter_router_service::api::admin_user_api_key_command_router_with_store(
                    store,
                    api_key_hasher,
        ...
        if let Some(store) = announcement_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_announcement_router_with_store(
                    store,
                    Arc::new(OsApiKeySecretGenerator),
                ),
            ));
        ...
        if let Some(store) = auth_settings_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_auth_settings_router_with_store(
                    store,
                    Arc::new(OsApiKeySecretGenerator),
                ),
            ));
        ...
        if let Some(store) = site_settings_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_site_settings_router_with_store(
                    store,
                    Arc::new(OsApiKeySecretGenerator),
                ),
            ));
        ...
        if let Some(store) = runtime_region_settings_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_runtime_region_settings_router_with_store(
                    store,
                    Arc::new(OsApiKeySecretGenerator),
                ),
            ));
        ...
        if let Some(store) = upstream_store {
            let verifier = upstream_verifier
        ...
        if let Some(store) = ip_rate_limit_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_ip_rate_limit_router_with_store(
                    store,
                    Arc::new(OsApiKeySecretGenerator),
                ),
            ));
        ...
        if let Some(store) = firewall_rule_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_firewall_rule_router_with_store(
                    store,
                    Arc::new(OsApiKeySecretGenerator),
                ),
            ));
        }
        if let Some(store) = chain_policy_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_chain_policy_router_with_store(
                    store,
                    Arc::new(OsApiKeySecretGenerator),
                ),
            ));
        ...
        if let Some(store) = model_rate_limit_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_model_rate_limit_router_with_store(
                    store,
                    Arc::new(OsApiKeySecretGenerator),
                ),
            ));
        ...
        if let Some(store) = finance_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_finance_router_with_store(store),
            ));
        }
        if let Some(store) = marketing_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_marketing_router_with_store(
                    store,
                    Arc::new(OsApiKeySecretGenerator),
                ),
            ));
        ...
        if let Some(read_store) = official_pricing_read_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_official_pricing_router_with_read_store(
                    read_store,
                ),
            ));
        }
        ...
        if let Some(store) = mcp_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_mcp_router_with_store(store),
            ));
        }
        ...
        if let Some(store) = storage_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_storage_router_with_store(store),
            ));
        }
        ...
        if let Some(store) = dashboard_read_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_dashboard_router_with_read_store(
                    store,
                ),
            ));
        }
        if let Some(store) = analytics_read_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_analytics_router_with_read_store(
                    store,
                ),
            ));
        }
        ...
        if let Some(store) = record_store {
            router = router.merge(layer_with_admin_subject_boundary(
                admin_subject_boundary_config.clone(),
                sdkwork_cloudrouter_router_service::api::admin_record_router_with_store(store),
            ));
        }
    }
    router
...
fn ai_routing_cache_invalidating_upstream_store(
    store: AdminUpstreamRuntimeStore,
    cache_manager: Option<RuntimeCacheManager>,
) -> AdminUpstreamRuntimeStore {
    match cache_manager {
        Some(manager) => Arc::new(AiRoutingCacheInvalidatingAdminUpstreamStore::new(
            store, manager,
        )),
        None => store,
    }
...
pub async fn router_with_postgres_shared_runtime(
    ...
    let chain_policy_store: AdminChainPolicyRuntimeStore =
        Arc::new(PostgresAdminChainPolicyStore::new(pool.clone()));
    ...
    let official_pricing_read_store: OfficialPricingRuntimeReadStore =
...
pub async fn router_with_database_config(
    ...
    router_with_database_api_key_trusted_subject_app_session(
        config,
        Some(api_key_config),
...
pub async fn router_with_database_and_api_key_config_and_startup_install_mode(
    ...
    router_with_database_api_key_trusted_subject_app_session_and_startup_install_mode(
        config,
        api_key_config,
        trusted_subject_config,
        app_session_config,
...
async fn router_with_database_api_key_trusted_subject_app_session(
    config: DatabaseConfig,
    api_key_config: Option<ApiKeySecurityConfig>,
    trusted_subject_config: Option<TrustedSubjectConfig>,
    app_session_config: Option<AppSessionConfig>,
...
async fn router_with_database_api_key_trusted_subject_app_session_and_startup_install_mode(
    config: DatabaseConfig,
    api_key_config: Option<ApiKeySecurityConfig>,
    trusted_subject_config: Option<TrustedSubjectConfig>,
    app_session_config: Option<AppSessionConfig>,
...
fn cache_manager_from_env_or_toml(
    ...
    let redis_config = RedisConfig::from_env_or_runtime_toml_with_default_enabled(runtime_toml, true)
        ...
        .ok_or_else(|| {
            ProductCatalogRouterError::Config(format!(
                "Redis cache is required for {} deployment mode. Configure SDKWORK_CLOUDROUTER_REDIS_URL or structured Redis host/port/database fields.",
...
fn deployment_mode_from_env_or_toml(
    ...
    Ok(sdkwork_cloudrouter_config::DeploymentRuntime::resolve(runtime_toml)?.mode)
}
...
fn configured_models_catalog_root(runtime_toml: Option<&RuntimeTomlConfig>) -> Option<String> {
    sdkwork_cloudrouter_config::runtime::config_value(
        sdkwork_cloudrouter_router_service::infrastructure::sql::installer::ENV_MODELS_CATALOG_ROOT,
...
fn build_api_key_secret_storage_config(
    config: &ApiKeySecurityConfig,
) -> Result<ApiKeySecretStorageConfig, ProductCatalogRouterError> {
    let codec = RingAeadApiKeySecretCodec::new(config.pepper_secret())
        .map_err(|error| ProductCatalogRouterError::Config(error.to_string()))?;
    Ok(ApiKeySecretStorageConfig::new(
...
fn layer_with_admin_subject_boundary(config: AdminSubjectBoundaryConfig, router: Router) -> Router {
    if sdkwork_cloudrouter_http::cloud_web_framework_enabled_from_env() {
...
async fn admin_web_framework_access_boundary(
    ...
    admin_forbidden_response(
        &request,
        "cloudrouter.admin.access permission is required".to_owned(),
    )
}

fn has_web_framework_admin_access(context: &WebRequestContext) -> bool {
    context.principal().is_some() && context.has_permission("cloudrouter.admin.access")
}
...
async fn admin_request_subject_boundary(
    ...
    if attach_subject {
        sdkwork_cloudrouter_http::attach_trusted_request_subject(&mut request, subject);
    }
...
fn resolve_admin_request_subject(
    ...
    sdkwork_cloudrouter_http::verified_app_request_subject(
        request.headers_mut(),
        &method,
        &path_and_query,
        subject_boundary,
...
impl AdminAccessChecker {
    async fn has_admin_access(&self, subject: TrustedRequestSubject) -> Result<bool, sqlx::Error> {
        has_postgres_admin_access(&self.0, subject).await
    }
}
...
mod tests {
    use super::{
        admin_forbidden_response, has_web_framework_admin_access,
