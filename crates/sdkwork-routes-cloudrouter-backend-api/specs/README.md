# sdkwork-routes-cloudrouter-backend-api Specs

Local component contract for the Cloud Router **backend-api** Rust route crate.
...
IAM backend-api remains owned by `sdkwork-api-iam-assembly`. A host that selects both applications composes that dependency assembly alongside `sdkwork-api-cloudrouter-assembly`; this product route crate does not copy IAM handlers.

`CloudRouterBackendDomainInjector` projects `IamAppContext` from canonical `WebRequestContext`.
