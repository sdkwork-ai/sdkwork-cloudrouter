...
pub fn init_tracing_with_runtime_config(
    config: Option<&sdkwork_cloudrouter_config::ObservabilitySectionConfig>,
) -> Result<(), String> {
...
pub fn init_tracing_with_config(config: TracingConfig) {
    ...
    match initialization {
        ...
        TracingInitialization::SubscriberUnavailable(error) => tracing::warn!(
            error,
            "Cloud Router tracing subscriber could not be installed"
        ),
    }
}
...
mod tests {
    ...
    fn resolved_log_filter_prefers_env_then_config_then_info() {
        assert_eq!(
            "warn,sdkwork_cloudrouter=debug",
            resolved_log_filter(Some(" warn,sdkwork_cloudrouter=debug "), Some("info"))
        );
    ...
    fn tracing_config_reads_runtime_observability_policy() {
        let runtime = sdkwork_cloudrouter_config::ObservabilitySectionConfig {
            log_filter: Some(" info,sdkwork_cloudrouter=debug ".to_owned()),
