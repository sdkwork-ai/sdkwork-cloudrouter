# sdkwork-routes-cloudrouter-app-api Specs

Local component contract for the Cloud Router **app-api** Rust route crate.
...
IAM app-api remains owned by `sdkwork-api-iam-assembly`. A host that selects both applications composes that dependency assembly alongside `sdkwork-api-cloudrouter-assembly`; this product route crate does not copy IAM handlers.
...
`CloudRouterAppDomainInjector` projects `IamAppContext` from canonical `WebRequestContext`.
...
cargo test -p sdkwork-routes-cloudrouter-app-api cloud_router_app_domain_injector
