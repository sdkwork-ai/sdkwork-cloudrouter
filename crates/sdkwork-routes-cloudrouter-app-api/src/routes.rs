...
use sdkwork_cloudrouter_router_service::ports::AdminTransactionCenterSubject;
...
pub fn build_sdkwork_cloudrouter_router_app_api_router() -> Router {
    router()
}

pub async fn build_sdkwork_cloudrouter_router_app_api_router_from_env(
...
pub fn router() -> Router {
    merge_app_sdk_reference_router(router_with_database_status(None, None, None))
        ...
        .merge(sdkwork_cloudrouter_router_service::api::app_settings_router())
        .merge(sdkwork_cloudrouter_router_service::api::app_usage_logs_router())
        .merge(sdkwork_cloudrouter_router_service::api::app_gateway_traces_router())
        .merge(sdkwork_cloudrouter_router_service::api::app_notification_router())
...
fn router_with_database_status(
    ...
    readiness_check: Option<sdkwork_cloudrouter_http::ReadinessCheckFn>,
    deployment_mode: Option<DeploymentMode>,
) -> Router {
    match deployment_mode {
        Some(deployment_mode) => {
            sdkwork_cloudrouter_http::service_router_with_filtered_contract_routes_database_config_readiness_check_and_deployment_mode(
                SERVICE_NAME,
                sdkwork_cloudrouter_http::ApiSurface::App,
                config,
        ...
        None => sdkwork_cloudrouter_http::service_router_with_filtered_contract_routes_database_config_and_readiness_check(
            SERVICE_NAME,
            sdkwork_cloudrouter_http::ApiSurface::App,
            config,
            product_local_contract_operation,
...
fn is_appbase_dependency_contract_path(path: &str) -> bool {
    if is_cloudrouter_owned_iam_app_path(path) {
        return false;
    }
...
async fn finalize_product_router_with_federated_capabilities(
    ...
    if let Some(database_config) = database_config {
        ...
        .await
        .map_err(ProductCatalogRouterError::Config)?;
        let router = crate::drive_runtime::merge_federated_drive_app_router(
            router,
            database_config,
            subject_boundary_config.clone(),
        )
        .await
        .map_err(ProductCatalogRouterError::Config)?;
        let router = crate::assets_runtime::merge_federated_assets_app_router(
            router,
            database_config,
            subject_boundary_config.clone(),
        )
        .await
        .map_err(ProductCatalogRouterError::Config)?;
        let router = crate::skills_runtime::merge_federated_skills_app_router(
            router,
            database_config,
            subject_boundary_config.clone(),
        )
...
fn router_with_runtime_stores_and_database_status(runtime: AppRouterRuntime<'_>) -> Router {
    ...
    router = match app_site_settings_store {
        Some(store) => router.merge(
            sdkwork_cloudrouter_router_service::api::app_site_settings_router_with_store(store),
        ),
    ...
    router = match dashboard_read_store {
        Some(read_store) => sdkwork_cloudrouter_http::merge_web_framework_scoped_app_read_router(
            router,
            sdkwork_cloudrouter_router_service::api::app_dashboard_overview_router_with_read_store(
                read_store,
            ),
            subject_boundary_config.clone(),
        ),
    ...
    router = match usage_logs_read_store {
        Some(read_store) => sdkwork_cloudrouter_http::merge_web_framework_scoped_app_read_router(
            router,
            sdkwork_cloudrouter_router_service::api::app_usage_logs_router_with_read_store(
                read_store,
            ),
            subject_boundary_config.clone(),
        ),
        None => router.merge(sdkwork_cloudrouter_router_service::api::app_usage_logs_router()),
    };
    router = match gateway_traces_read_store {
        Some(read_store) => sdkwork_cloudrouter_http::merge_web_framework_scoped_app_read_router(
            router,
            sdkwork_cloudrouter_router_service::api::app_gateway_traces_router_with_read_store(
                read_store,
            ),
            subject_boundary_config.clone(),
        ),
        None => router.merge(sdkwork_cloudrouter_router_service::api::app_gateway_traces_router()),
    };
    router = match app_notification_store {
        Some(store) => sdkwork_cloudrouter_http::merge_web_framework_scoped_app_router(
            router,
            sdkwork_cloudrouter_router_service::api::app_notification_router_with_store(store),
            subject_boundary_config.clone(),
        ),
        None => router.merge(sdkwork_cloudrouter_router_service::api::app_notification_router()),
    };
    router = match app_chat_store {
        Some(store) => sdkwork_cloudrouter_http::merge_web_framework_scoped_app_router(
            router,
            sdkwork_cloudrouter_router_service::api::app_chat_router_with_store(
                store,
                Arc::clone(&entity_uuid_generator),
            ),
            subject_boundary_config.clone(),
        ),
        None => router.merge(sdkwork_cloudrouter_router_service::api::app_chat_router()),
    };
    router = match app_runtime_store {
        Some(store) => {
            ...
            let runtime_router = if let (Some(catalog), Some(gateway_client)) = (
                ...
                sdkwork_cloudrouter_router_service::api::app_runtime_router_with_store_and_gateway_client_and_runtime_stream_bus(
                    store,
                    Arc::clone(&entity_uuid_generator),
                    catalog,
                    gateway_client,
                ...
                sdkwork_cloudrouter_router_service::api::app_runtime_router_with_store_and_chat_stream_relay_and_runtime_stream_bus(
                    store,
                    Arc::clone(&entity_uuid_generator),
                    catalog,
                    chat_stream_relay,
                ...
                sdkwork_cloudrouter_router_service::api::app_runtime_router_with_store_and_runtime_stream_bus(
                    store,
                    Arc::clone(&entity_uuid_generator),
                    Arc::clone(&stream_bus),
                )
            };
            sdkwork_cloudrouter_http::merge_web_framework_scoped_app_router(
                router,
                runtime_router,
                subject_boundary_config.clone(),
            )
        }
        None => router.merge(sdkwork_cloudrouter_router_service::api::app_runtime_router()),
    };
    router = match app_routing_read_store {
        Some(read_store) => sdkwork_cloudrouter_http::merge_web_framework_scoped_app_router(
            router,
            sdkwork_cloudrouter_router_service::api::app_routing_router_with_read_store(read_store),
            subject_boundary_config.clone(),
        ),
    ...
    if let Some(api_key_runtime) = api_key_runtime {
        router = sdkwork_cloudrouter_http::merge_web_framework_scoped_app_router(
            router,
            sdkwork_cloudrouter_router_service::api::app_api_key_router_with_read_store_and_command_store(
    ...
    router = match settings_store {
        Some(store) => sdkwork_cloudrouter_http::merge_web_framework_scoped_app_router(
            router,
            sdkwork_cloudrouter_router_service::api::app_settings_router_with_store(
                store,
                Arc::new(OsApiKeySecretGenerator),
            ),
            subject_boundary_config.clone(),
        ),
        None => router.merge(sdkwork_cloudrouter_router_service::api::app_settings_router()),
    };
    router = match settlements_dashboard_read_store {
        Some(read_store) => sdkwork_cloudrouter_http::merge_web_framework_scoped_app_read_router(
            router,
            sdkwork_cloudrouter_router_service::api::app_settlements_dashboard_router_with_read_store(read_store),
            subject_boundary_config,
        ),
        None => router.merge(sdkwork_cloudrouter_router_service::api::app_settlements_dashboard_router()),
    };
...
async fn router_with_database_config_api_key_trusted_subject_app_session_and_startup_install_mode(
    ...
    sdkwork_cloudrouter_http::materialize_federated_database_env_from_config(&config);
    let subject_boundary_config =
        AppSubjectBoundaryConfig::new(trusted_subject_config.clone(), app_session_config.clone());
    ...
    let usage_settlement_worker_config =
        sdkwork_cloudrouter_router_service::application::resolve_usage_settlement_worker_config(
            runtime_toml,
        );
    let readiness_check =
...
async fn build_app_runtime_stream_bus(
        ...
        .await
    {
        Ok(bus) => {
            tracing::info!(
                explicit,
                key_prefix = key_prefix.as_deref().unwrap_or("cloudrouter"),
...
fn app_runtime_gateway_base_url(runtime_toml: Option<&RuntimeTomlConfig>) -> String {
    const APP_RUNTIME_GATEWAY_BASE_URL: &str = "SDKWORK_CLOUDROUTER_APP_RUNTIME_GATEWAY_BASE_URL";
    const EDGE_GATEWAY_BASE_URL: &str = "SDKWORK_CLOUDROUTER_EDGE_GATEWAY_BASE_URL";
    const DEFAULT_GATEWAY_BASE_URL: &str = "http://127.0.0.1:18080";

    sdkwork_cloudrouter_config::runtime::config_value(APP_RUNTIME_GATEWAY_BASE_URL, None)
        .or_else(|| {
            sdkwork_cloudrouter_config::runtime::config_value(
...
fn platform_transaction_center_subject() -> AdminTransactionCenterSubject {
    let tenant_id = std::env::var("SDKWORK_CLOUDROUTER_PLATFORM_TENANT_ID")
        .ok()
...
fn model_ranking_refresh_worker_config_from_env_or_toml(
    ...
    const ENABLED: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_REFRESH_WORKER_ENABLED";
    const TENANT_ID: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_TENANT_ID";
    const ORGANIZATION_ID: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_ORGANIZATION_ID";
    const RANK_SCOPE: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_RANK_SCOPE";
    const SNAPSHOT_PERIOD: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_SNAPSHOT_PERIOD";
    const LIMIT: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_LIMIT";
    const LOOKBACK_DAYS: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_LOOKBACK_DAYS";
    const INTERVAL_MILLIS: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_INTERVAL_MILLIS";
    const CACHE_MAX_AGE_SECONDS: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_CACHE_MAX_AGE_SECONDS";
    const RUN_TIMEOUT_MILLIS: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_RUN_TIMEOUT_MILLIS";
    const MAX_RETRY_ATTEMPTS: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_MAX_RETRY_ATTEMPTS";
    const RETRY_BACKOFF_MILLIS: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_RETRY_BACKOFF_MILLIS";
    const RUN_ON_STARTUP: &str = "SDKWORK_CLOUDROUTER_MODEL_RANKING_RUN_ON_STARTUP";
...
fn parse_optional_bool_config(
    ...
    sdkwork_cloudrouter_config::runtime::config_bool(name, config_value)
}
...
fn parse_non_negative_u32_config(
    ...
    Ok(sdkwork_cloudrouter_config::runtime::config_u32(name, config_value)?.unwrap_or(default))
}
...
fn parse_non_empty_string_config(
    ...
    let Some(value) = sdkwork_cloudrouter_config::runtime::config_value(name, config_value) else {
        return Ok(default);
    };
...
fn require_database_config(
    ...
    config.ok_or_else(|| {
        ...
        ProductCatalogRouterError::Config(
            format!(
                "SDKWORK_DATABASE_URL is required for sdkwork-cloudrouter-standalone-gateway startup so install checks can run.\n{help_text}"
            ),
        )
    })
}
...
mod tests {
    ...
    use sdkwork_cloudrouter_config::DeploymentMode;
    use sdkwork_cloudrouter_router_service::ports::{
    ...
    async fn app_runtime_catalog_refresh_defaults_to_low_pressure_interval() {
        ...
        let saved_refresh_interval =
            std::env::var("SDKWORK_CLOUDROUTER_PROVIDER_CATALOG_REFRESH_INTERVAL_MILLIS").ok();
        std::env::remove_var("SDKWORK_CLOUDROUTER_PROVIDER_CATALOG_REFRESH_INTERVAL_MILLIS");
    ...
    async fn model_ranking_refresh_worker_config_from_env_parses_runtime_policy() {
        let _guard = env_guard().lock().await;
        let names = [
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_REFRESH_WORKER_ENABLED",
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_TENANT_ID",
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_ORGANIZATION_ID",
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_RANK_SCOPE",
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_SNAPSHOT_PERIOD",
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_LIMIT",
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_LOOKBACK_DAYS",
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_INTERVAL_MILLIS",
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_CACHE_MAX_AGE_SECONDS",
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_RUN_TIMEOUT_MILLIS",
            "SDKWORK_CLOUDROUTER_MODEL_RANKING_MAX_RETRY_ATTEMPTS",
    ...
    async fn router_from_env_rejects_zero_config_server_placeholder_postgres() {
        ...
        let saved_deployment_mode = std::env::var("SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE").ok();
        let saved_config_file = std::env::var("SDKWORK_CLOUDROUTER_CONFIG_FILE").ok();
        let saved_snowflake_node_id = std::env::var("SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID").ok();
        let saved_api_key_pepper = std::env::var("SDKWORK_CLOUDROUTER_API_KEY_PEPPER").ok();
        let saved_trusted_subject_secret =
            std::env::var("SDKWORK_CLOUDROUTER_TRUSTED_SUBJECT_SECRET").ok();
        let saved_app_session_secret = std::env::var("SDKWORK_CLOUDROUTER_APP_SESSION_SECRET").ok();
        let saved_payment_webhook_secret =
            std::env::var("SDKWORK_CLOUDROUTER_PAYMENT_WEBHOOK_SECRET").ok();
        let config_path = unique_runtime_config_path();
        std::env::remove_var("SDKWORK_DATABASE_URL");
        std::env::set_var("SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE", "server");
        std::env::set_var("SDKWORK_CLOUDROUTER_CONFIG_FILE", &config_path);
        std::env::remove_var("SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID");
        std::env::set_var(
    ...
    async fn router_from_env_rejects_static_or_invalid_server_snowflake_node_id_before_database_bootstrap(
        ...
        for node_id in ["17", "not-a-node-id"] {
            ...
            std::env::set_var("SDKWORK_CLOUDROUTER_CONFIG_FILE", &config_path);
            std::env::set_var("SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID", node_id);
            ...
            assert!(
                error.contains("SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID"),
