use sdkwork_cloudrouter_config::{DatabaseConfig, DatabaseEngine, DeploymentMode};
...
mod tests {
    ...
    fn health_response_exposes_service_and_deployment_mode() {
        let health =
            HealthResponse::new("sdkwork-cloudrouter-edge-runtime", DeploymentMode::Desktop);

        assert_eq!("ok", health.status);
        assert_eq!("sdkwork-cloudrouter-edge-runtime", health.service);
    ...
    fn health_response_summarizes_database_config_without_leaking_url() {
        let config = DatabaseConfig::from_url_with_max_connections(
            "postgres://cloud_user:secret-password@127.0.0.1/cloud",
            32,
        )
        .unwrap();

        let health =
            HealthResponse::new("sdkwork-cloudrouter-admin-gateway", DeploymentMode::Server)
        ...
        assert!(!body.contains("cloud_user"));
