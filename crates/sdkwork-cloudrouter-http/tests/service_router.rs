...
use sdkwork_cloudrouter_config::{DatabaseConfig, DeploymentMode};
use sdkwork_cloudrouter_http::ApiSurface;
use serde_json::{Map, Value};
use tower::ServiceExt;

const APP_SDK_AUTHORITY_OPENAPI_JSON: &str =
    include_str!("../../../sdks/cloudrouter-app-sdk/openapi/cloudrouter-app-sdk.openapi.json");
const BACKEND_SDK_AUTHORITY_OPENAPI_JSON: &str = include_str!(
    "../../../sdks/cloudrouter-backend-sdk/openapi/cloudrouter-backend-sdk.openapi.json"
);
const OPEN_SDK_AUTHORITY_OPENAPI_JSON: &str =
    include_str!("../../../sdks/cloudrouter-open-sdk/openapi/cloudrouter-open-sdk.openapi.json");

#[tokio::test]
async fn service_router_exposes_standard_health_and_ready_endpoints() {
    let router = sdkwork_cloudrouter_http::service_router("sdkwork-cloudrouter-standalone-gateway");
...
async fn service_router_metrics_expose_canonical_bounded_request_contract() {
    let router = sdkwork_cloudrouter_http::service_router("sdkwork-cloudrouter-edge-runtime");
    let response = router
        .clone()
        .oneshot(
            Request::builder()
    ...
    sdkwork_cloudrouter_http::record_readiness_check(true);
...
async fn service_router_health_uses_the_resolved_deployment_mode_from_state() {
    let response = sdkwork_cloudrouter_http::service_router_with_deployment_mode(
...
async fn service_router_exposes_gateway_openapi_document() {
    let response = sdkwork_cloudrouter_http::service_router("sdkwork-cloudrouter-edge-runtime")
        .oneshot(
            Request::builder()
                .uri("/openapi.json")
                .body(Body::empty())
...
async fn service_router_exposes_s3_compatible_cloud_services_openapi_document() {
    let payload = fetch_runtime_openapi_json(
        sdkwork_cloudrouter_http::service_router("sdkwork-cloudrouter-edge-runtime"),
        "/cloud/v3/openapi.json",
    )
    .await;
...
async fn service_router_keeps_gateway_openapi_off_app_and_backend_root_paths() {
    let app_response = sdkwork_cloudrouter_http::service_router_with_contract_routes(
    ...
    let backend_response = sdkwork_cloudrouter_http::service_router_with_contract_routes(
    ...
    let app_payment_response = sdkwork_cloudrouter_http::service_router_with_contract_routes(
        "sdkwork-cloudrouter-standalone-gateway",
        ApiSurface::App,
    )
    .oneshot(
    ...
    let backend_payment_response = sdkwork_cloudrouter_http::service_router_with_contract_routes(
        "sdkwork-cloudrouter-admin-gateway",
        ApiSurface::Backend,
    )
    .oneshot(
...
async fn service_router_openapi_documents_match_sdk_authority_contracts() {
    ...
    let app_payload = fetch_runtime_openapi_json(
        sdkwork_cloudrouter_http::service_router_with_contract_routes(
            "sdkwork-cloudrouter-standalone-gateway",
            ApiSurface::App,
        ),
        "/app/v3/api/openapi.json",
...
async fn service_router_health_body_contains_service_identity() {
    let response = sdkwork_cloudrouter_http::service_router("sdkwork-cloudrouter-admin-gateway")
        .oneshot(
            Request::builder()
                .uri("/healthz")
                .body(Body::empty())
    ...
    assert_eq!("sdkwork-cloudrouter-admin-gateway", payload["service"]);
...
async fn service_router_health_body_contains_safe_database_status() {
    ...
    let router = sdkwork_cloudrouter_http::service_router_with_database_config(
        "sdkwork-cloudrouter-admin-gateway",
        Some(&database),
    );
...
fn default_security_headers_are_defined() {
    let headers = sdkwork_cloudrouter_http::default_security_headers();
