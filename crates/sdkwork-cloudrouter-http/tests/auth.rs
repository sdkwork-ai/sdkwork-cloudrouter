...
fn attach_trusted_request_subject_exposes_appbase_iam_context() {
    ...
    assert_eq!("sdkwork-cloudrouter", context.app_id);
}
...
fn app_session_claim_token_accepts_tenant_level_organization_zero() {
    ...
    let claims = AppSessionTokenClaims {
        ...
        app_id: "sdkwork-cloudrouter".to_owned(),
        ...
        permission_scope: vec!["cloudrouter.console.access".to_owned()],
        issued_at: 1_800_000_000,
        expires_at: 1_800_000_300,
        kid: None,
    };

    let token = sdkwork_cloudrouter_http::sign_app_session_token_with_claims(&config, &claims);
...
fn app_session_authorization_header_accepts_case_insensitive_bearer_scheme() {
    ...
    let parsed = sdkwork_cloudrouter_http::verify_app_session_authorization_header(
        &config,
        &format!("  bearer   {token}  "),
        1_800_000_001,
    )
