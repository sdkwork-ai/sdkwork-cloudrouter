...
use sdkwork_cloudrouter_config::{DeploymentRuntime, RuntimeTomlConfig};
...
fn metrics_bearer_authorized(request: &Request) -> bool {
    let Some(expected) = std::env::var("SDKWORK_CLOUDROUTER_METRICS_BEARER_TOKEN")
        .ok()
...
mod tests {
    ...
    fn runtime_toml_environment_is_normalized() {
        ...
        if std::env::var("SDKWORK_CLOUDROUTER_ROUTER_ENVIRONMENT").is_ok() {
            return;
        }
