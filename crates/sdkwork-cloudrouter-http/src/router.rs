...
use sdkwork_cloudrouter_contract::{ApiSurface, ContractManifest, ContractOperation};
use sdkwork_cloudrouter_health::DatabaseHealth;
...
fn service_router_with_optional_contract_operation_filter(
    ...
    let manifest = ContractManifest::from_embedded()
        .expect("embedded CloudRouter API contract manifest must be valid JSON");
    base_router()
