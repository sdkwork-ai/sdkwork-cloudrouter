...
const CLOUDROUTER_OPEN_API: &str = include_str!(concat!(
...
static OWNED_OPERATIONS: LazyLock<Vec<OwnedOperation>> = LazyLock::new(|| {
    ...
    append_tagged_operations(&mut operations, CLOUDROUTER_OPEN_API);
        ...
        OWNED_OPERATIONS
            ...
            .find(|operation| {
                operation.method == *method
                    && sdkwork_cloudrouter_contract::matches_path_pattern(
...
fn append_operations(
    ...
    let document: Value =
        serde_json::from_str(document).expect("embedded CloudRouter OpenAPI must be valid JSON");
    let paths = document
        .get("paths")
        .and_then(Value::as_object)
        .expect("embedded CloudRouter OpenAPI must declare paths");
