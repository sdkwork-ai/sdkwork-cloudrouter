...
use axum::Json;
use axum::Router;
use sdkwork_cloudrouter_config::is_production_like_runtime_environment;
...
pub fn cloud_web_framework_enabled_from_env() -> bool {
    if env_flag_enabled(
        std::env::var("SDKWORK_CLOUDROUTER_WEB_FRAMEWORK_LEGACY")
            .ok()
            .as_deref(),
    ) {
        return false;
    }
    match std::env::var("SDKWORK_CLOUDROUTER_WEB_FRAMEWORK_ENABLED")
        .ok()
        .as_deref()
    {
...
/// already attached by the outer pipeline. This is the cloud-specific bridge for legacy
/// SQL handlers and must not parse cloud app-session tokens.
...
pub fn ensure_production_web_framework_security_policy() {
    ...
    if env_flag_enabled(
        ...
        panic!(
            "SDKWORK_CLOUDROUTER_WEB_FRAMEWORK_LEGACY must not be enabled in production-like environments"
        );
    }
    if !cloud_web_framework_enabled_from_env() {
        panic!(
            "sdkwork-web-framework must remain enabled in production-like environments; do not set SDKWORK_CLOUDROUTER_WEB_FRAMEWORK_ENABLED=false"
        );
    }
}
...
/// Merges federated T1 app-api capability routers with Cloud app-session auth.
...
#[cfg(test)]
mod tests {
    use super::cloud_web_framework_enabled_from_env;

    #[test]
    fn cloud_web_framework_enabled_by_default() {
        let legacy = std::env::var("SDKWORK_CLOUDROUTER_WEB_FRAMEWORK_LEGACY")
            .ok()
        ...
        assert!(cloud_web_framework_enabled_from_env());
    }
}
