...
use sdkwork_cloudrouter_security::redact_secret;
...
fn iam_app_context_from_trusted_subject(subject: TrustedRequestSubject) -> IamAppContext {
    IamAppContext::new(
        ...
        format!("cloud-subject-{}", subject.user_id),
        "sdkwork-cloudrouter",
        Environment::Prod,
...
/// App-session boundary for federated T1 capability routers mounted into Cloud Router.
/// These routes must accept Cloud app-session dual tokens even when the outer app surface
