# SDKWork Cloud Http Component Specs

Local standards index for `sdkwork-cloudrouter-http`. Root SDKWork specs remain authoritative per [sdkwork-specs/README.md](../../../../sdkwork-specs/README.md).

## Component

| Field | Value |
| --- | --- |
| Name | `sdkwork-cloudrouter-http` |
| Type | `rust-crate` |
| Domain | `platform` |
| Capability | `router` |
...
Production app/backend surfaces use **sdkwork-web-framework** as the canonical HTTP auth and context pipeline. This crate provides Cloud-specific IAM resolver wiring and **legacy-only** subject bridges.
...
Migrated app-api SQL read handlers live in `sdkwork-cloudrouter-router-service/src/api/app_sql_subject.rs` and consume `WebRequestContext` / `TenantAppContext` per `WEB_FRAMEWORK_SPEC.md`.
...
| `SDKWORK_CLOUDROUTER_WEB_FRAMEWORK_ENABLED=false` | Off | Skip `WebFrameworkLayer` wrapping |
| `SDKWORK_CLOUDROUTER_WEB_FRAMEWORK_LEGACY=true` | Legacy session path | Use cloud app-session tokens; for integration tests and explicit rollback only |
...
lifecycle contracts. They must be implemented and verified in `sdkwork-iam`; Cloud Router must not
...
```bash
cargo test -p sdkwork-cloudrouter-http
cargo test -p sdkwork-cloudrouter-http --test auth
