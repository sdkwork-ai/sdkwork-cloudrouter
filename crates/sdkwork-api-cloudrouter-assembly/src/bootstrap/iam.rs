...
fn resolve_cloudrouter_app_root() -> PathBuf {
    for key in ["SDKWORK_APP_ROOT", "SDKWORK_CLOUDROUTER_APP_ROOT"] {
...
pub(super) async fn wire_iam_app_router() -> anyhow::Result<Router> {
    ...
    ensure_tenant_application_from_app_root(
        resolve_cloudrouter_app_root().as_path(),
        &EmbeddedApplicationBootstrapOptions {
...
#[cfg(test)]
mod tests {
    use super::resolve_cloudrouter_app_root;

    #[test]
    fn default_app_root_contains_application_manifest() {
        assert!(resolve_cloudrouter_app_root()
