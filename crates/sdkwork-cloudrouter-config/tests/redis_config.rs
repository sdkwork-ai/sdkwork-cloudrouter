...
"#,
    ...
    assert!(redis.enabled());
    assert_eq!(
        "rediss://cloudrouter:redis-secret@redis.internal:6380/3",
        redis.url()
    );
    assert_eq!(Some("redis.internal"), redis.host());
    assert_eq!(Some(6380), redis.port());
    assert_eq!(Some(3), redis.database());
    assert_eq!(Some("cloudrouter"), redis.username());
    assert_eq!(Some("cloudrouter-prod"), redis.key_prefix());
    assert!(redis.tls());
...
fn server_production_requires_enabled_redis_config() {
    use sdkwork_cloudrouter_config::{ensure_server_production_redis_config, DeploymentMode};
