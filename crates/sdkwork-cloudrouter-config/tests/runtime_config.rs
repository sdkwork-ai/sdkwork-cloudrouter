use sdkwork_cloudrouter_config::{DeploymentMode, RuntimeConfig, RuntimeTomlConfig};
...
fn runtime_config_uses_default_bind_and_desktop_mode_when_env_parts_are_absent() {
    ...
    assert_eq!("sdkwork-cloudrouter-edge-runtime", config.service_name);
...
fn runtime_config_rejects_blank_or_invalid_bind_address() {
    ...
    .unwrap_err();
    assert!(blank.contains("SDKWORK_CLOUDROUTER_GATEWAY_BIND"));
...
fn runtime_config_rejects_invalid_deployment_mode() {
    ...
    assert!(error.contains("SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE"));
}
...
[observability]
log_filter = "info,sdkwork_cloudrouter=debug"
log_format = "json"
log_ansi = false
log_target = true
...
port = 6380
database = 3
username = "cloudrouter"
password_file = "/etc/sdkwork/router/redis.secret"
key_prefix = "cloudrouter-prod"
tls = true
max_connections = 32
...
"#,
    ...
    assert_eq!(
        Some("info,sdkwork_cloudrouter=debug"),
    ...
    assert_eq!(Some("cloudrouter"), config.redis.username.as_deref());
    assert_eq!(
        Some("/etc/sdkwork/router/redis.secret"),
        config.redis.password_file.as_deref()
    );
    assert_eq!(Some("cloudrouter-prod"), config.redis.key_prefix.as_deref());
