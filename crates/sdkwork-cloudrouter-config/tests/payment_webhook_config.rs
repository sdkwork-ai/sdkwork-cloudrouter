use sdkwork_cloudrouter_config::{PaymentWebhookConfig, RuntimeTomlConfig};
...
fn payment_webhook_config_rejects_missing_blank_short_and_invalid_skew() {
    ...
    assert!(blank.contains("SDKWORK_CLOUDROUTER_PAYMENT_WEBHOOK_SECRET must not be blank"));
    ...
    assert!(invalid_skew.contains(
        "SDKWORK_CLOUDROUTER_PAYMENT_WEBHOOK_MAX_CLOCK_SKEW_SECONDS must be a positive integer"
    ));
