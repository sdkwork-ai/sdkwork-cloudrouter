...
fn rejects_invalid_provider_native_passthrough_json_config() {
    ...
    let malformed = config
        ...
        .unwrap_err();
    assert!(malformed.contains("SDKWORK_CLOUDROUTER_PROVIDER_PASSTHROUGH_JSON"));
...
fn rejects_partial_or_blank_openai_relay_config() {
    let missing_token =
        ProviderRelayConfig::from_optional_parts(Some("https://provider.example".to_owned()), None)
            .unwrap_err();
    assert!(missing_token.contains("SDKWORK_CLOUDROUTER_OPENAI_RELAY_BEARER_TOKEN"));

    let missing_url =
        ProviderRelayConfig::from_optional_parts(None, Some("sk-provider".to_owned())).unwrap_err();
    assert!(missing_url.contains("SDKWORK_CLOUDROUTER_OPENAI_RELAY_BASE_URL"));
    ...
    .unwrap_err();
    assert!(blank.contains("SDKWORK_CLOUDROUTER_OPENAI_RELAY_BASE_URL"));
}
...
fn rejects_provider_adapter_registry_json_without_gateway_token() {
    ...
    assert!(error.contains("SDKWORK_CLOUDROUTER_PROVIDER_ADAPTER_GATEWAY_TOKEN"));
}
...
"#,
    ...
    assert_eq!(
        &[ProviderPassthroughHeader::new("x-goog-api-client", "cloudrouter").unwrap()],
...
fn rejects_invalid_provider_secret_map_config() {
    ...
    assert!(malformed.contains("SDKWORK_CLOUDROUTER_PROVIDER_SECRET_MAP_JSON"));
