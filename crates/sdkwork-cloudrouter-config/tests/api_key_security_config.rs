use sdkwork_cloudrouter_config::{ApiKeySecurityConfig, RuntimeTomlConfig};
...
fn rejects_blank_or_short_api_key_pepper() {
    ...
    assert!(blank.contains("SDKWORK_CLOUDROUTER_API_KEY_PEPPER"));
...
fn api_key_secret_storage_mode_reads_env_var_and_rejects_invalid_values() {
    ...
    assert!(config.secret_storage_mode().is_ciphertext());
    std::env::remove_var(sdkwork_cloudrouter_config::ApiKeySecretStorageMode::ENV_SECRET_STORAGE);
