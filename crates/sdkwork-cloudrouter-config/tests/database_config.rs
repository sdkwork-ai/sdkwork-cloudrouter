...
[database]
engine = "sqlite"
url = "sqlite:///Users/example/.sdkwork/router/data/cloudrouter.sqlite"
max_connections = 1
"#,
    );
    ...
    assert!(config.url.ends_with("cloudrouter.sqlite"));
}
...
fn runtime_config_file_expands_password_file_environment_variables() {
    ...
    let _guard = EnvGuard::set(&[(
        "SDKWORK_CLOUDROUTER_TEST_SECRET_ROOT",
...
fn runtime_config_locations_expose_desktop_sqlite_database_paths() {
    let linux_desktop = RuntimeConfigLocation::for_platform("linux", RuntimeConfigProfile::Desktop);
    assert_eq!(
        PathBuf::from("~/.sdkwork/router/data/cloudrouter.sqlite"),
    ...
    let windows_desktop =
        RuntimeConfigLocation::for_platform("windows", RuntimeConfigProfile::Desktop);
    assert_eq!(
        PathBuf::from("%USERPROFILE%/.sdkwork/router/data/cloudrouter.sqlite"),
...
fn server_runtime_validation_rejects_placeholder_postgres_host_and_password() {
    ...
    for url in [
        ...
        "postgresql://sdkwork_cloudrouter_router:change-me@db.example.com:5432/sdkwork_cloudrouter_router?sslmode=require",
        "postgresql://sdkwork_cloudrouter_router:secret@db.example.com:5432/sdkwork_cloudrouter_router?sslmode=require",
        "postgresql://sdkwork_cloudrouter_router:change-me@db.internal:5432/sdkwork_cloudrouter_router?sslmode=require",
        "postgresql://sdkwork_cloudrouter_router:change-me@localhost:5432/sdkwork_cloudrouter_router",
...
fn runtime_config_profile_reads_runtime_toml_with_env_override() {
    let _env_lock = ENV_LOCK.lock().unwrap();
    let _guard = EnvGuard::set(&[("SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE", None)]);
...
fn parses_startup_install_mode_from_optional_environment_part() {
    ...
    assert!(
        StartupInstallMode::from_optional_part(Some("repair".to_owned()))
            .unwrap_err()
            .contains("SDKWORK_CLOUDROUTER_STARTUP_INSTALL_MODE")
    );
}
...
fn temp_root(label: &str) -> PathBuf {
    ...
    root.push("sdkwork-cloudrouter-config-tests");
    root.push(format!(
        "{}-{}",
        label,
        SystemTime::now()
