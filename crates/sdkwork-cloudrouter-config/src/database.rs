...
impl DatabaseConfig {
    ...
    pub const ENV_CONFIG_FILE: &'static str = "SDKWORK_CLOUDROUTER_CONFIG_FILE";
    ...
    fn from_canonical_database_env() -> Result<Option<Self>, String> {
        ...
        let standard = sdkwork_database_config::env::load_from_env("CLOUDROUTER")
    ...
    pub fn default_runtime_config_toml(
        ...
        let mut lines = vec![
            "# SdkWork CloudRouter runtime configuration.".to_owned(),
    ...
    pub fn startup_help_lines_for_location(
        ...
        match profile {
            RuntimeConfigProfile::Server => vec![
                ...
                "Set SDKWORK_CLOUDROUTER_CONFIG_FILE to override the runtime TOML location.".to_owned(),
                "Server/service deployments use external PostgreSQL by default.".to_owned(),
                format!(
