...
Read `sdkwork.app.config.json` only when changing Mini Program application behavior, runtime config, SDK wiring, release metadata, packaging, or app-owned capabilities.
...
- `sdkwork.app.config.json`: Mini Program application identity and release metadata.
- `.sdkwork/`: local application dictionary for skills, plugins, manifests, and AI workspace metadata.
- `specs/`: local Mini Program application/component contracts and narrowing rules.
- `etc/`: checked-in Mini Program source configuration (deployment index and profile sources) governed by `SOURCE_CONFIG_SPEC.md`.
- `packages/`: Mini Program package family for core, commons, shell, i18n, and console page packages.
- `src/`: WX app shell, page templates, and the bootstrap that assembles the console ports.
...
- Frontend/UI code: `../../../sdkwork-specs/FRONTEND_CODE_SPEC.md`, `../../../sdkwork-specs/FRONTEND_SPEC.md`, `../../../sdkwork-specs/UI_ARCHITECTURE_SPEC.md`, and `../../../sdkwork-specs/MINI_PROGRAM_APP_ARCHITECTURE_SPEC.md`.
- mini program application architecture: `../../../sdkwork-specs/APPLICATION_SPEC.md`, `../../../sdkwork-specs/APP_CLIENT_ARCHITECTURE_ALIGNMENT_SPEC.md`, and `../../../sdkwork-specs/MINI_PROGRAM_APP_ARCHITECTURE_SPEC.md`.
...
Run `node --test tests/mini-program-surface-contract.test.mjs` and the SDKWork frontend composition checker.
...
API work loads `../../../sdkwork-specs/API_SPEC.md` and its validators. List/search work loads `../../../sdkwork-specs/PAGINATION_SPEC.md` and `check-pagination.mjs`. Source configuration work loads `../../../sdkwork-specs/SOURCE_CONFIG_SPEC.md` and `check-source-config-standard.mjs`. Host adapter work loads `../../../sdkwork-specs/MINI_PROGRAM_APP_ARCHITECTURE_SPEC.md` and `../../../sdkwork-specs/APP_SDK_INTEGRATION_SPEC.md`. Link these authorities instead of copying their normative bodies into `AGENTS.md`.
