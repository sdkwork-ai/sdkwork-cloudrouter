...
} from "./packages/sdkwork-cloudrouter-pc-console-api-keys/src/accountGroups.ts";
import { formatGroupMultiplier } from "./packages/sdkwork-cloudroutes-pc-commons/src/components/GroupSelector.tsx";
...
test("console api key drawer uses the default group when no groups are available", async () => {
  const source = await import("node:fs/promises").then((fs) =>
    fs.readFile(new URL("./packages/sdkwork-cloudrouter-pc-console-api-keys/src/CreateKeyDrawer.tsx", import.meta.url), "utf8"),
  );
...
test("console account group selectors render group names while submitting group codes", async () => {
  const [drawerSource, listSource, usageSource] = await Promise.all([
    ...
    import("node:fs/promises").then((fs) =>
      fs.readFile(new URL("./packages/sdkwork-cloudrouter-pc-console-api-keys/src/ApiKeysView.tsx", import.meta.url), "utf8"),
    ),
    import("node:fs/promises").then((fs) =>
      fs.readFile(new URL("./packages/sdkwork-cloudrouter-pc-console-api-keys/src/usage-details/ApiKeyUsageDetailsDrawer.tsx", import.meta.url), "utf8"),
    ),
  ]);
...
test("console api key service reads SdkWork list totals from pageInfo.totalItems", async () => {
  const serviceSource = await import("node:fs/promises").then((fs) =>
    fs.readFile(new URL("./packages/sdkwork-cloudrouter-pc-console-api-keys/src/apiKeyService.ts", import.meta.url), "utf8"),
  );
...
test("console api key usage details profiles cover supported tool setup tabs", async () => {
  ...
  } = await import("./packages/sdkwork-cloudrouter-pc-console-api-keys/src/usage-details/toolProfiles.ts");
  ...
  const snippets = buildApiKeyUsageToolSnippets({
    apiKeyPlaceholder: "<YOUR_CLOUD_ROUTER_API_KEY>",
...
test("console api key usage details drawer never exposes persisted full-key copy", async () => {
  ...
  assert.doesNotMatch(source, /copyableKey/);
  assert.match(source, /<YOUR_CLOUD_ROUTER_API_KEY>/);
...
test("console api key i18n keys are present in English and Chinese resources", async () => {
  const source = await import("node:fs/promises").then((fs) =>
    fs.readFile(new URL("./packages/sdkwork-cloudrouter-pc-i18n/src/resources/console/api-keys.ts", import.meta.url), "utf8"),
  );
