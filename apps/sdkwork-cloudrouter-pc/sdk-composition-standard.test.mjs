...
test('portal workspace declares appbase, messaging and business owner backend SDK packages', () => {
  ...
  const commonsPackageJson = json('packages/sdkwork-cloudroutes-pc-commons/package.json');
...
test('portal workspace composes business capabilities through independent owner SDK clients', () => {
  const packageJson = json('package.json');
  const commonsPackageJson = json('packages/sdkwork-cloudroutes-pc-commons/package.json');
  const pcCorePackageJson = json('packages/sdkwork-cloudrouter-pc-core/package.json');
  const workspaceSource = repoSource('pnpm-workspace.yaml');
  const tsconfigSource = source('tsconfig.json');
  const typecheckSource = source('tsconfig.typecheck.json');
  const viteConfigSource = source('vite.config.ts');
  ...
  assert.equal(pcCorePackageJson.dependencies['@sdkwork/cloudrouter-backend-sdk'], undefined);
  ...
  assert.match(viteConfigSource, /cloudrouter-portal-pnpm-workspace-resolver/);
  assert.doesNotMatch(viteConfigSource, /find: 'cloudrouter-app-domain-transport-generated-typescript'/);
  assert.doesNotMatch(viteConfigSource, /find: '@sdkwork\/cloudrouter-app-sdk'/);
...
test('cloudrouter backend SDK is owner-only and does not regenerate business capability SDKs', () => {
  const appManifest = json('../../sdks/cloudrouter-app-sdk/sdk-manifest.json');
  const appComponentSpec = json('../../sdks/cloudrouter-app-sdk/specs/component.spec.json');
  const backendManifest = json('../../sdks/cloudrouter-backend-sdk/sdk-manifest.json');
  const appOpenapi = json('../../sdks/cloudrouter-app-sdk/openapi/cloudrouter-app-sdk.openapi.json');
  const backendOpenapi = json('../../sdks/cloudrouter-backend-sdk/openapi/cloudrouter-backend-sdk.openapi.json');
  ...
  for (const workspace of [
    ...
    assert.ok(
      ...
      `CloudRouter app SDK must declare ${workspace}`,
    );
  }
  ...
  for (const workspace of [
    ...
    assert.ok(backendDependencyWorkspaces.has(workspace), `CloudRouter backend SDK must declare ${workspace}`);
  }
  ...
  assert.equal(
    existsSync(new URL('../../sdks/cloudrouter-app-sdk/openapi/cloudrouter-app-domain-transport.openapi.json', portalRoot)),
    false,
    'CloudRouter app SDK must not publish an aggregate domain transport authority',
  );
  ...
  assert.ok(
    ...
    'CloudRouter provider inventory is a product extension and must remain in the product backend SDK',
  );

  assert.equal(
    existsSync(new URL('../../sdks/cloudrouter-app-sdk/cloudrouter-app-sdk-typescript/src/api/commerce.ts', portalRoot)),
    false,
    'CloudRouter app generated SDK must not contain a Commerce API module',
  );
  assert.equal(
    existsSync(new URL('../../sdks/cloudrouter-backend-sdk/cloudrouter-backend-sdk-typescript/src/api/commerce.ts', portalRoot)),
    false,
    'CloudRouter backend generated SDK must not contain a Commerce API module',
  );
  assert.equal(
    existsSync(new URL('../../sdks/cloudrouter-app-sdk/cloudrouter-app-sdk-typescript/src/domains/index.ts', portalRoot)),
    false,
    'CloudRouter app SDK must not expose a cross-domain aggregation subpath',
  );
  assert.equal(
    existsSync(new URL('../../sdks/cloudrouter-backend-sdk/cloudrouter-backend-sdk-typescript/src/domains/index.ts', portalRoot)),
    false,
  ...
  assert.doesNotMatch(
    ...
    'Commerce promotion resources must stay in Commerce backend SDK, not CloudRouter system SDK',
  );
});

test('admin host composes owner admin modules through the backend-admin core SDK boundary', () => {
  const appSource = source('src/App.tsx');
  const hostSource = source('src/admin/cloudRouterAdminHostMount.tsx');
  const permissionSource = source('packages/sdkwork-cloudrouter-pc-admin-shell/src/admin-route-permission-hints.ts');
  const menuSource = source('packages/sdkwork-cloudrouter-pc-admin-shell/src/adminModuleRegistry.ts');
  const portalPackage = json('package.json');
  const adminCorePackage = json('packages/sdkwork-cloudrouter-pc-admin-core/package.json');
  const adminCoreComponent = json('packages/sdkwork-cloudrouter-pc-admin-core/specs/component.spec.json');
  const adminCoreSdkSource = source('packages/sdkwork-cloudrouter-pc-admin-core/src/sdk/index.ts');
  const adminCoreInventorySource = source('packages/sdkwork-cloudrouter-pc-admin-core/src/composition/sdk-inventory.ts');
  const upstreamPackage = json('packages/sdkwork-cloudrouter-pc-admin-upstream/package.json');
  const upstreamService = source('packages/sdkwork-cloudrouter-pc-admin-upstream/src/upstreamService.ts');
  ...
  assert.equal((hostSource.match(/\broute\('/g) ?? []).length, 30);
  ...
  assert.deepEqual(
    ...
    ['cloudrouter-backend-sdk', 'sdkwork-models-backend-sdk'],
  );
  ...
  assert.match(upstreamService, /from '@sdkwork\/cloudrouter-pc-admin-core\/sdk'/);
  assert.match(upstreamService, /getCloudRouterBackendSdkClient/);
...
test('commons SDK client bootstrap composes appbase, product and open SDKs through standard credentials', () => {
  ...
  assert.match(sdkClientsSource, /from '@sdkwork\/cloudrouter-app-sdk'/);
  assert.match(sdkClientsSource, /from '@sdkwork\/cloudrouter-backend-sdk'/);
  assert.match(sdkClientsSource, /from '@sdkwork\/cloudrouter-open-sdk'/);
  assert.match(sdkClientsSource, /createTokenManager/);
  assert.match(sdkClientsSource, /getCloudRouterGlobalTokenManager/);
  ...
  assert.match(sdkClientsSource, /tokenManager:\s*resolveCloudRouterSdkTokenManager\(options\.tokenManager\)/);
  ...
  assert.match(messagingConfigSource, /resolveCloudRouterSdkTokenManager\(options\.tokenManager\)/);
  assert.doesNotMatch(messagingConfigSource, /VITE_CLOUDROUTER_APP_API_BASE_URL|\?\?\s*APP_API_PREFIX/);
...
test('commons SDK bootstrap keeps app composition separate and backend client owner-only', () => {
  ...
  assert.match(sdkClientsSource, /VITE_CLOUDROUTER_APP_API_BASE_URL/);
  assert.match(sdkClientsSource, /VITE_CLOUDROUTER_BACKEND_API_BASE_URL/);
  assert.match(sdkClientsSource, /tokenManager:\s*resolveCloudRouterSdkTokenManager\(options\.tokenManager\)/);
...
test('commerce admin services consume owner backend SDKs while preserving product extensions', () => {
  const membershipsService = source('packages/sdkwork-cloudrouter-pc-admin-memberships/src/membershipsService.ts');
  ...
  assert.match(membershipsService, /getCloudRouterBackendSdkClient\(\)\.recharges/);
  ...
  assert.doesNotMatch(marketingService, /getCloudRouterBackendSdkClient\(\)\.promotions/);
  ...
  assert.match(marketingService, /getCloudRouterBackendSdkClient\(\)\.billing\.referralStats\.list/);
  ...
  assert.match(paymentsService, /getCloudRouterBackendSdkClient\(\)\.payments\.providers\.list/);
...
test('IAM runtime uses the high-level appbase auth runtime while binding app SDK clients to the shared token manager', () => {
  const iamRuntimeSource = source('packages/sdkwork-cloudroutes-pc-commons/src/iam-runtime.ts');
...
test('appbase-owned app capabilities no longer call the product cloudrouter app SDK', () => {
  const sessionServiceSource = source('packages/sdkwork-cloudroutes-pc-commons/src/sessionService.ts');
  const portalSessionSource = source('packages/sdkwork-cloudroutes-pc-commons/src/portal-session.ts');
  const iamDirectorySource = source('packages/sdkwork-cloudroutes-pc-commons/src/iamDirectoryApiOperations.ts');
  const authSettingsSource = source('src/auth/cloudRouterAuthSettingsService.ts');
  const userServiceSource = source('packages/sdkwork-cloudrouter-pc-console-user/src/userService.ts');
  ...
  for (const [name, fileSource] of [
    ...
    ['cloudRouterAuthSettingsService', authSettingsSource],
    ['userService', userServiceSource],
  ]) {
    assert.match(fileSource, /getSdkworkAppbaseAppSdkClient/, `${name} must use appbase app SDK`);
  }

  assert.doesNotMatch(sessionServiceSource, /getCloudRouterAppSdkClient/);
  assert.doesNotMatch(portalSessionSource, /getCloudRouterAppSdkClient/);
  assert.doesNotMatch(iamDirectorySource, /getCloudRouterAppSdkClient/);
  assert.doesNotMatch(authSettingsSource, /getCloudRouterAppSdkClient/);
  assert.doesNotMatch(userServiceSource, /getCloudRouterAppSdkClient/);
  assert.doesNotMatch(userServiceSource, /@sdkwork\/cloudrouter-app-sdk/);

  assert.match(sessionServiceSource, /getSdkworkAppbaseAppSdkClient\(options\)\.auth\.sessions\.create/);
  assert.doesNotMatch(sessionServiceSource, /getCloudRouterAppSdkClient\(\)\.auth\.sessions\.create/);
...
test('app surface component spec declares permission inheritance and core SDK inventory', () => {
  ...
  const commonsSpec = json('packages/sdkwork-cloudroutes-pc-commons/specs/component.spec.json');
  const coreSpec = json('packages/sdkwork-cloudrouter-pc-core/specs/component.spec.json');
  const coreInventorySource = source('packages/sdkwork-cloudrouter-pc-core/src/composition/sdk-inventory.ts');
  ...
  assert.doesNotMatch(JSON.stringify(appSpec.contracts?.sdkInventory ?? []), /cloudrouter-app-sdk\.commerce/);
  assert.doesNotMatch(JSON.stringify(appSpec.contracts?.sdkInventory ?? []), /cloudrouter-backend-sdk\.commerce/);
  assert.doesNotMatch(JSON.stringify(appSpec.contracts?.sdkInventory ?? []), /cloudrouter-app-capability/);
  ...
  assert.equal(iamModuleManifest.moduleId, 'cloudrouter');
  assert.ok(iamModuleManifest.permissions.catalog.some((entry) => entry.code === 'cloudrouter.console.access'));
  assert.deepEqual(
    iamModuleManifest.roles.roleGrantExtensions,
    [
      { roleCode: 'org_admin', patterns: ['cloudrouter.*'] },
      { roleCode: 'platform_system_admin', patterns: ['cloudrouter.*'] },
    ],
    'the manager org_admin role and platform system admins must inherit the Cloud Router admin domain',
  );
});
