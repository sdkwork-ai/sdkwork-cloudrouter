...
export function CloudRouterAuthShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    document.documentElement.classList.add('sdkwork-cloudrouter-auth-active');
    document.body.classList.add('sdkwork-cloudrouter-auth-active');
    return () => {
      document.documentElement.classList.remove('light-mode');
      document.documentElement.classList.remove('sdkwork-cloudrouter-auth-active');
      document.documentElement.style.removeProperty('color-scheme');
      document.body.classList.remove('sdkwork-cloudrouter-auth-active');
    };
  ...
  const handleMinimize = () => {
    window.dispatchEvent(new CustomEvent('sdkwork-cloudrouter:window-control', {
  ...
  return (
    <div className="sdkwork-cloudrouter-auth-shell">
      {shouldRenderDesktopHeader ? (
        <header className="sdkwork-cloudrouter-auth-header drag-region">
          <div className="sdkwork-cloudrouter-auth-header-brand">
            <span className="sdkwork-cloudrouter-auth-header-mark">
              <Terminal size={12} />
            </span>
            <span>Cloud Router</span>
          </div>
          <div className="sdkwork-cloudrouter-auth-header-center" />
          <div className="sdkwork-cloudrouter-auth-header-actions no-drag">
            <button
              aria-label={isLightMode ? 'Switch to dark mode' : 'Switch to light mode'}
              className="sdkwork-cloudrouter-auth-theme-button"
              onClick={toggleTheme}
            ...
            {shouldRenderWindowControls ? (
              <div className="sdkwork-cloudrouter-auth-window-controls">
                <button
                  aria-label="Minimize window"
                  className="sdkwork-cloudrouter-auth-window-button"
      ...
      <main className="sdkwork-cloudrouter-auth-main">{children}</main>
    </div>
  );
}
