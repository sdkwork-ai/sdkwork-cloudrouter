...
import { fetchCloudRouterAuthRuntimeSettings } from './cloudRouterAuthSettingsService.ts';
...
export const DEFAULT_CLOUD_ROUTER_AUTH_RUNTIME_CONFIG: SdkworkAuthRuntimeConfig = {
...
export function useCloudRouterAuthRuntimeConfig(): SdkworkAuthRuntimeConfig {
  const [runtimeConfig, setRuntimeConfig] = useState(DEFAULT_CLOUD_ROUTER_AUTH_RUNTIME_CONFIG);

  useEffect(() => {
    let cancelled = false;

    fetchCloudRouterAuthRuntimeConfig()
      .then((config) => {
        if (!cancelled) {
      ...
      .catch(() => {
        if (!cancelled) {
          setRuntimeConfig(DEFAULT_CLOUD_ROUTER_AUTH_RUNTIME_CONFIG);
        }
      });
...
export async function fetchCloudRouterAuthRuntimeConfig(): Promise<SdkworkAuthRuntimeConfig> {
  return mergeCloudRouterAuthRuntimeConfig(await fetchCloudRouterAuthRuntimeSettings());
}

export function mergeCloudRouterAuthRuntimeConfig(record: ApiRecord): SdkworkAuthRuntimeConfig {
  readRequiredEnum(
    record,
    'qrLoginType',
    BACKEND_QR_LOGIN_TYPES,
    'Auth qrLoginType is required',
    'Unsupported auth qrLoginType',
  );
