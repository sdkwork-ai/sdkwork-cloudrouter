...
export const CLOUD_ROUTER_THEME_STORAGE_KEY = 'cloud-router-theme';
...
export function resolveInitialThemePreference(): ThemePreference {
  ...
  try {
    const storedTheme = window.localStorage.getItem(CLOUD_ROUTER_THEME_STORAGE_KEY);
...
export function persistThemePreference(theme: ThemePreference): void {
  ...
  try {
    window.localStorage.setItem(CLOUD_ROUTER_THEME_STORAGE_KEY, theme);
  } catch {
...
export function resolveInitialThemeColorPreference(): ThemeColorPreference {
  ...
  try {
    const storedThemeColor = window.localStorage.getItem(CLOUD_ROUTER_THEME_COLOR_STORAGE_KEY);
...
export function persistThemeColorPreference(themeColor: ThemeColorPreference): void {
  ...
  try {
    window.localStorage.setItem(CLOUD_ROUTER_THEME_COLOR_STORAGE_KEY, themeColor);
  } catch {
    // Storage can be unavailable in private or embedded contexts.
  }
}
