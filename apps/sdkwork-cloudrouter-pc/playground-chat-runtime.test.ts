...
} from "./packages/sdkwork-cloudrouter-pc-playground/src/components/chat/chatModelSelection.ts";
import {
  loadStoredChatSessions,
  mergeChatSessions,
  saveStoredChatConversation,
  saveStoredChatInFlightStream,
} from "./packages/sdkwork-cloudrouter-pc-playground/src/components/chat/chatLocalStore.ts";
import type { ChatMessage, ChatSessionSummary } from "./packages/sdkwork-cloudrouter-pc-playground/src/components/chat/chatTypes.ts";
import type {
...
} from "@sdkwork/cloudroutes-pc-commons/runtime-usage";
import { readRuntimeTextDelta } from "./packages/sdkwork-cloudroutes-pc-commons/src/runtime-stream-event.ts";
...
test("chat runtime test owns only the Cloud Router chat boundary", () => {
...
test("chat playground leaves shared markdown implementation behind its public package export", () => {
  ...
  const bubbleSource = readPortalFile(
    "./packages/sdkwork-cloudrouter-pc-playground/src/components/chat/ChatMessageBubble.tsx",
  );
...
test("chat runtime uses application SDK operations and never constructs a raw transport", () => {
  const operationsSource = readPortalFile(
    "./packages/sdkwork-cloudrouter-pc-playground/src/appRuntimeApiOperations.ts",
  );
  const serviceSource = readPortalFile(
    "./packages/sdkwork-cloudrouter-pc-playground/src/components/chat/chatService.ts",
  );
  const pageSource = readPortalFile(
    "./packages/sdkwork-cloudrouter-pc-playground/src/components/chat/ChatPage.tsx",
  );

  assert.match(operationsSource, /getCloudRouterAppSdkClient/);
...
test("chat stream composition uses the typed runtime stream helper", () => {
  const runtimeStreamSource = readPortalFile(
    "./packages/sdkwork-cloudrouter-pc-playground/src/runtimeStream.ts",
  );
  const commonsRuntimeSource = readPortalFile(
    "./packages/sdkwork-cloudroutes-pc-commons/src/runtime.ts",
  );
  const streamEventSource = readPortalFile(
    "./packages/sdkwork-cloudroutes-pc-commons/src/runtime-stream-event.ts",
  );

  assert.match(runtimeStreamSource, /@sdkwork\/cloudroutes-pc-commons\/runtime/);
