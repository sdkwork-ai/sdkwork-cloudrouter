...
const CONSOLE_LAYOUT_FILE = "./packages/sdkwork-cloudrouter-pc-console-shell/src/ConsoleLayout.tsx";
...
const simplifiedConsolePageFiles = [
  ...
  "./packages/sdkwork-cloudrouter-pc-console-user/src/UserView.tsx",
...
const removedConsolePageTitlePatterns = [
  {
    file: "./packages/sdkwork-cloudrouter-pc-console-gateway/src/GatewayView.tsx",
    pattern: /<h1[^>]*>\s*\{t\('console\.gateway\.title'/,
  },
  {
    file: "./packages/sdkwork-cloudrouter-pc-console-messages/src/MessagesView.tsx",
    pattern: /<h1[^>]*>\s*<Bell[^>]*>\s*\{t\('console\.messages\.title'/,
  },
  {
    file: "./packages/sdkwork-cloudrouter-pc-console-settings/src/SettingsView.tsx",
    pattern: /<h1[^>]*>\s*\{t\("console\.settings\.settingsview\.text\.18giiv0"/,
  },
  {
    file: "./packages/sdkwork-cloudrouter-pc-console-usage/src/UsageView.tsx",
    pattern: /<h1[^>]*>\s*\{t\('console\.usage\.title'/,
  },
  {
    file: "./packages/sdkwork-cloudrouter-pc-console-user/src/UserView.tsx",
...
test("console usage logs i18n resources include English and Chinese entries", () => {
  const resourceIndex = readPortalFile("./packages/sdkwork-cloudrouter-pc-i18n/src/resources/index.ts");
  const source = readPortalFile("./packages/sdkwork-cloudrouter-pc-i18n/src/resources/console/usage.ts");
...
test("console dashboard stays product-focused without read-only caveats", () => {
  const source = readPortalFile("./packages/sdkwork-cloudrouter-pc-console-dashboard/src/DashboardView.tsx");
...
test("console settlement page keeps menu copy in navigation without page title chrome", () => {
  ...
  const coreMessages = readPortalFile("./packages/sdkwork-cloudrouter-pc-i18n/src/resources/console/core.ts");
...
test("console message center stays product-focused without implementation caveats", { skip: !portalFileExists("./packages/sdkwork-cloudrouter-pc-console-messages/src/MessagesView.tsx") }, () => {
  const source = readPortalFile("./packages/sdkwork-cloudrouter-pc-console-messages/src/MessagesView.tsx");

  assert.doesNotMatch(source, /<h1[^>]*>\s*<Bell[^>]*>\s*\{t\('console\.messages\.title'/);
  assertNoImplementationCaveats(source);
});

test("console message center constrains the detail pane to the available viewport height", { skip: !portalFileExists("./packages/sdkwork-cloudrouter-pc-console-messages/src/MessagesView.tsx") }, () => {
  const source = readPortalFile("./packages/sdkwork-cloudrouter-pc-console-messages/src/MessagesView.tsx");
...
test("console gateway tooling stays product-focused without implementation caveats", () => {
  const source = readPortalFile("./packages/sdkwork-cloudrouter-pc-console-gateway/src/GatewayView.tsx");
...
test("console account and recharge surfaces are owned by T1 domain PC packages", () => {
  ...
  assert.doesNotMatch(appSource, /cloudrouter-pc-console-account/);
...
test("console user settings stay product-focused without implementation caveats", () => {
  const source = readPortalFile("./packages/sdkwork-cloudrouter-pc-console-user/src/UserView.tsx");

  assertNoImplementationCaveats(source);
});

test("console settings center constrains the settings panel to the available viewport height", () => {
  const source = readPortalFile("./packages/sdkwork-cloudrouter-pc-console-settings/src/SettingsView.tsx");
...
test("console appearance preferences support system mode and theme colors", () => {
  const settingsSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-console-settings/src/SettingsView.tsx");
  ...
  assert.match(themePreferenceSource, /CLOUD_ROUTER_THEME_COLOR_STORAGE_KEY/);
...
test("console wallet uses order-owned inline recharge while account wallet retains checkout navigation", () => {
  ...
  const consoleWalletSource = readPortalFile("./src/console-business/CloudRouterWalletPage.tsx");
  ...
  assert.match(consoleWalletSource, /getCloudRouterPointsRechargeService/);
...
test("admin guidance copy stays product-focused without implementation caveats", () => {
  const upstreamSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-admin-upstream/src/index.tsx");
...
test("console API key management keeps search on the left and create action on the right", () => {
  const source = readPortalFile("./packages/sdkwork-cloudrouter-pc-console-api-keys/src/ApiKeysView.tsx");
...
test("admin searchable list toolbars keep search before primary create actions", () => {
  const toolbarSpecs = [
    {
      file: "./packages/sdkwork-cloudrouter-pc-admin-user/src/index.tsx",
      toolbar: "data-admin-user-toolbar",
      search: "data-admin-user-search",
      primaryAction: "data-admin-user-primary-action",
    },
    {
      file: "./packages/sdkwork-cloudrouter-pc-admin-upstream/src/suppliersPage.tsx",
...
test("shared navigation notifications stay product-focused without read-only labels", () => {
  const source = readPortalFile("./packages/sdkwork-cloudroutes-pc-commons/src/components/Navbar.tsx");
...
test("i18n product copy avoids implementation contract wording for routed model guidance", () => {
  const source = readPortalFile("./packages/sdkwork-cloudrouter-pc-i18n/src/index.ts");
