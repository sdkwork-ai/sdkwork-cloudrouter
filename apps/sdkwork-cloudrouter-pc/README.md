...
SDKWork CloudRouter PC is the browser console for the Cloud Router product. Root `pnpm dev` starts the canonical standalone application runtime. Cloud profile commands are remote-client-only and start no local API process. Frontend business calls stay behind the portal service layer, generated SDKs, and surface-oriented SDKWork API entrypoints.

## Architecture

- Portal UI modules call local service boundaries.
- App business APIs use `@sdkwork/cloudrouter-app-sdk` for `/app/v3/api`.
...
```powershell
pnpm.cmd --dir apps/sdkwork-cloudrouter-pc install
```
...
- `VITE_CLOUDROUTER_*` and `VITE_API_BASE_URL` for browser-visible SDK paths
- `SDKWORK_CLOUDROUTER_BROWSER_DEV_PROXY_*_ORIGIN` for private dev-server proxy upstreams
...
Private edge-server settings (CSP, tool API rate limits, SDK archive fallback) belong in `.env.release` as `SDKWORK_CLOUDROUTER_EDGE_*` and `SDKWORK_CLOUDROUTER_TOOL_API_*`. See `.env.release.example` and `specs/application-env-standard.md`.
...
- Chromium must be installed: `pnpm.cmd --dir apps/sdkwork-cloudrouter-pc exec playwright install chromium`.

Run the full suite from the portal directory:

```powershell
pnpm.cmd --dir apps/sdkwork-cloudrouter-pc test:e2e
```

Run a single spec:

```powershell
pnpm.cmd --dir apps/sdkwork-cloudrouter-pc exec playwright test e2e/theme-switch.spec.ts
```
...
- `pnpm --filter sdkwork-cloudrouter-pc typecheck`
