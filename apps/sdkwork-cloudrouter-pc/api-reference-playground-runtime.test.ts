...
function withCloudRouterRuntimeEnv<T>(
  ...
  components: {
    schemas: {
      JsonObject: {
        ...
        description: "Provider-specific JSON payload accepted by Cloud Router.",
      },
...
const paymentAggregateSpec = JSON.parse(readFileSync(
  new URL("../../crates/sdkwork-cloudrouter-http/specs/payment-aggregate-openapi.json", import.meta.url),
  "utf8",
));
const paasSpec = JSON.parse(readFileSync(
  new URL("../../crates/sdkwork-cloudrouter-http/specs/paas-openapi.json", import.meta.url),
  "utf8",
));
const cloudStorageSpec = JSON.parse(readFileSync(
  new URL("../../crates/sdkwork-cloudrouter-http/specs/cloud-services-openapi.json", import.meta.url),
  "utf8",
));
...
test("api reference schema tabs source uses commons SDK prefix boundary instead of local business prefix literals", () => {
  ...
  assert.match(source, /from '@sdkwork\/cloudroutes-pc-commons\/runtime'/);
...
test("api reference keeps vendor multimodal endpoints under their OpenAI modality groups", async () => {
  const manifest: ApiSchemaTabsDocument = {
    cacheTtlSeconds: 30,
    tabs: [
      { id: "gateway", name: "Cloud Router Open API", order: 10, schemaUrls: ["/openapi.json"], defaultSchemaUrl: "/openapi.json" },
    ],
  };
  ...
  assert.deepEqual(endpoint.responseProperties, [
    {
      ...
      desc: "Provider-specific JSON payload accepted by Cloud Router.",
      required: false,
    },
  ]);
  assert.equal(endpoint.response, "{}");
...
test("sdk reference reuses schema tabs and maps tabs to generated SDK metadata", async () => {
  ...
  assert.equal(getGeneratedSdkMetadataForSystem("payment-open-api").packageName, "@sdkwork/cloudrouter-payment-sdk");
  assert.equal(getGeneratedSdkMetadataForSystem("iaas-open-api").packageName, "@sdkwork/cloudrouter-cloud-services-sdk");
  assert.equal(getGeneratedSdkMetadataForSystem("app-api").packageName, "@sdkwork/cloudrouter-app-sdk");
  assert.equal(getGeneratedSdkMetadataForSystem("backend-api").packageName, "@sdkwork/cloudrouter-backend-sdk");
  ...
  assert.equal(cloudConfig.packageName, "@sdkwork/cloudrouter-cloud-services-sdk");
  ...
  assert.equal(config.packageName, "@sdkwork/cloudrouter-backend-sdk");
});
...
test("sdk reference maps product open api tabs to generated open SDK metadata", async () => {
  ...
  assert.equal(getGeneratedSdkMetadataForSystem("knowledgebase-open-api").packageName, "@sdkwork/cloudrouter-open-sdk");
...
test("sdk reference generates llm open api SDKs from domain-root paths", () => {
  ...
  assert.equal(gatewayTypescriptSdk.initCode.includes('baseUrl: process.env.CLOUDROUTER_API_BASE_URL ?? "https://api.sdkwork.com"'), true);
...
test("sdk reference gateway base URL uses SDK-specific runtime override and falls back to public API base URL", () => {
  const inheritedConfig = withCloudRouterRuntimeEnv(
    {
      VITE_API_BASE_URL: "https://tenant.example.com/v1",
    },
    () => createGeneratedSdkToolConfig("llm-open-api", "typescript", "/openapi.json"),
  );
  const overriddenConfig = withCloudRouterRuntimeEnv(
    {
      VITE_API_BASE_URL: "https://tenant.example.com/v1",
      VITE_CLOUDROUTER_OPEN_API_BASE_URL: "https://open.example.com/v1",
    },
    () => createGeneratedSdkToolConfig("llm-open-api", "typescript", "/openapi.json"),
  );
...
test("sdk reference endpoint documentation supports multipart requests and binary responses", async () => {
  ...
  const sdkData = {
    name: "CloudRouterGatewaySdk",
    packageName: "@sdkwork/cloudrouter-gateway-sdk",
    baseUrl: "https://api.example.test",
  };
...
test("sdk reference generation uses app SDK instead of local tool API fetches", () => {
  ...
  const appSdkSource = readFileSync(
    new URL("../../sdks/cloudrouter-app-sdk/cloudrouter-app-sdk-typescript/generated/server-openapi/src/api/sdk-reference.ts", import.meta.url),
    "utf8",
  );
  ...
  assert.equal(serviceSource.includes('fetch("/api/generate-sdk"'), false);
  assert.match(serviceSource, /getCloudRouterAppSdkClient/u);
