...
export function runOwnedSourceTypecheck() {
  ...
  if (result.owned.length > 0) {
    ...
    console.error(`[typecheck] ${result.owned.length} Cloud Router owned-source diagnostics found.`);
    return 1;
  }

  console.log(
    `[typecheck] Cloud Router owned sources passed; ${result.external.length} external diagnostics were reported separately.`,
  );
  return 0;
}
