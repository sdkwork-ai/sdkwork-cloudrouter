...
test('owned source classification includes every Cloud Router repository surface', () => {
  assert.equal(
  ...
  assert.equal(
    isOwnedSourcePath(
      path.join(repositoryRoot, 'sdks', 'cloudrouter-app-sdk', 'cloudrouter-app-sdk-typescript', 'src', 'index.ts'),
      repositoryRoot,
    ),
    true,
  );
