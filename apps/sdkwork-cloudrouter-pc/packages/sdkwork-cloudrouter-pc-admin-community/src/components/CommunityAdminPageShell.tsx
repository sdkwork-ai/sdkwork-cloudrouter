...
interface CommunityAdminPageShellProps {
  isLoading: boolean;
  error: string | null;
  onRefresh: () => void;
  children: ReactNode;
...
export function CommunityAdminPageShell({
  isLoading,
  error,
  onRefresh,
  children,
  ...
  loadingTitle,
  errorTitle,
}: CommunityAdminPageShellProps) {
  const { t } = useTranslation();
  if (isLoading) {
    return (
      <BusinessStatePanel
        kind="loading"
        title={loadingTitle ?? t('admin.community.loading', 'Loading community records...')}
        className="min-h-48"
      />
    );
  }
  if (error) {
    return (
      <BusinessStatePanel
        kind="error"
        title={errorTitle ?? t('admin.community.error', 'Community data could not be loaded')}
        description={error}
        onRetry={onRefresh}
        className="min-h-48"
      />
