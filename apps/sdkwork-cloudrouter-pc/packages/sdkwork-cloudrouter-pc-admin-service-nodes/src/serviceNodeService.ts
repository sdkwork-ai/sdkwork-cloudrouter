...
type ServiceNodeListSdkParams = Parameters<ReturnType<typeof getCloudRouterBackendSdkClient>['system']['serviceNodes']['list']>[0];

export class ServiceNodeService {
  static async fetchNodes(params: ServiceNodeListParams = {}): Promise<ServiceNode[]> {
    const result = await getCloudRouterBackendSdkClient().system.serviceNodes.list(toListParams(params));
  ...
  static async createNode(input: ServiceNodeInput): Promise<ServiceNode> {
    const result = await getCloudRouterBackendSdkClient().system.serviceNodes.create(toCreateRequest(input));
  ...
  static async updateNode(nodeId: string, input: ServiceNodeInput): Promise<ServiceNode> {
    const result = await getCloudRouterBackendSdkClient().system.serviceNodes.update(
  ...
  static async updateNodeStatus(nodeId: string, status: ServiceNodeStatus): Promise<ServiceNode> {
    const result = await getCloudRouterBackendSdkClient().system.serviceNodes.status.update(
      requiredNodeId(nodeId),
  ...
  static async deleteNode(nodeId: string): Promise<void> {
    await getCloudRouterBackendSdkClient().system.serviceNodes.delete(requiredNodeId(nodeId));
  }
}
