...
export function CloudRouterAuthSettingsPage() {
  ...
  const loadSettings = useCallback(async (isActive: () => boolean = () => true) => {
    ...
    setLoadError(null);
    try {
      const record = await fetchCloudRouterAuthSettings();
      if (isActive()) {
  ...
  const saveSettings = async () => {
    ...
    setSaveSuccess(null);
    try {
      const saved = await updateCloudRouterAuthSettings(toAuthSettingsRequest(form));
