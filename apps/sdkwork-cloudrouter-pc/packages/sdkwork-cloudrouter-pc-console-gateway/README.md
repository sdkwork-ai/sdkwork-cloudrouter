...
This README is the SDKWork module entrypoint for `sdkwork-cloudrouter-pc-console-gateway`. The machine-readable component contract is `specs/component.spec.json`; canonical standards are under `../../../../../sdkwork-specs/`.
...
- Consumes `ai.gateway.traces.list` from the composed `@sdkwork/cloudrouter-app-sdk` through the `@sdkwork/cloudrouter-pc-console-core/sdk` boundary.
...
- `node --import ./scripts/register-portal-workspace-resolver.mjs --import tsx --test --test-name-pattern="console gateway" console-app-runtime.test.ts` from `apps/sdkwork-cloudrouter-pc`
- `pnpm exec vitest run packages/sdkwork-cloudrouter-pc-console-gateway/src/GatewayView.test.tsx` from `apps/sdkwork-cloudrouter-pc`
