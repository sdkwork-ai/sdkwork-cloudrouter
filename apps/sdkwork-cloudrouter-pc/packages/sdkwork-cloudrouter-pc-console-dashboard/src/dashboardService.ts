...
async function loadDashboardOverview(params: Record<string, string>): Promise<unknown> {
  return retrieveDashboardOverview(getCloudRouterAppSdkClient(), params);
}

async function loadTokenBankAccount(): Promise<unknown> {
  return getCloudRouterAccountAppService().tokenBank.account.retrieve();
}

async function retrieveDashboardOverview(
  client: ReturnType<typeof getCloudRouterAppSdkClient>,
