...
import { trim } from '@sdkwork/cloudroutes-pc-commons/sdkwork-utils';
import {
...
function readGenerationArtifactResource(record: ApiRecord): CloudRouterMediaResource | undefined {
...
function createGenerationArtifactAsset(
  {
    ...
    mimeType?: string;
    resource: CloudRouterMediaResource;
    thumbnail?: CloudRouterMediaResource;
  },
...
function cloneMediaResource(resource: CloudRouterMediaResource): CloudRouterMediaResource {
  const clone: CloudRouterMediaResource = { ...resource };
  if (resource.poster) {
