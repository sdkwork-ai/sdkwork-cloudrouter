...
This README is the SDKWork module entrypoint for `sdkwork-cloudroutes-pc-commons`. The machine-readable component contract is `specs/component.spec.json`; canonical standards are under `../../../../../sdkwork-specs/`.
...
- Messaging verification uses `@sdkwork/messaging-app-sdk` with the shared global token manager. Its base URL must come from `VITE_SDKWORK_MESSAGING_APP_API_BASE_URL` or `PORTAL_PUBLIC_SDK_BASE_URL`; it never falls back to the Cloud Router product API URL.
...
- `pnpm --filter sdkwork-cloudroutes-pc-commons typecheck`
