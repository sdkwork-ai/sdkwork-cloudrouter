...
export type CloudRouterMediaResource = MediaResource;
export type CloudRouterMediaKind = CloudRouterMediaResource['kind'];
type CloudRouterMediaChecksumAlgorithm = 'etag' | 'md5' | 'sha256';

export interface CloudRouterSdkMediaResource {
  access?: CloudRouterMediaResource['access'];
  ai?: CloudRouterMediaResource['ai'];
  altText?: string;
  checksum?: {
    algorithm: CloudRouterMediaChecksumAlgorithm;
    value: string;
  };
  ...
  height?: number;
  id?: string;
  kind: CloudRouterMediaResource['kind'];
  metadata?: JsonObject;
  mimeType?: string;
  objectBlobId?: string;
  poster?: CloudRouterSdkMediaResource;
  publicUrl?: string;
  sizeBytes?: string;
  source: CloudRouterMediaResource['source'];
  thumbnails?: CloudRouterSdkMediaResource[];
  title?: string;
  uri?: string;
  url?: string;
  variants?: CloudRouterSdkMediaResource[];
  width?: number;
}
...
export function toNullableExternalUrlMediaResource(
  value: string | null | undefined,
  kind: CloudRouterMediaKind = 'image',
): CloudRouterMediaResource | null | undefined {
  if (value === undefined) {
    return undefined;
  }
...
export function toSdkMediaResource(
  ...
  fieldName = 'media',
): CloudRouterSdkMediaResource | undefined {
  if (!value) {
    return undefined;
  }
...
export function readRequiredMediaResource(value: unknown, message: string): CloudRouterMediaResource {
  const resource = readMediaResource(value);
  if (!resource) {
...
export function toExternalUrlMediaResources(
  ...
  const resources = values
    ...
    .filter((value): value is CloudRouterMediaResource => value !== undefined);
  return resources;
}
...
function toRequiredSdkMediaResource(
  value: CloudRouterMediaResource,
  fieldName: string,
): CloudRouterSdkMediaResource {
...
function normalizeMediaChecksumAlgorithm(
  value: string,
  fieldName: string,
): CloudRouterMediaChecksumAlgorithm {
