...
export class AdminCacheService {
  static async fetchOverview(): Promise<CacheOverview> {
    const result = await getCloudRouterBackendSdkClient().system.cache.overview.retrieve();
  ...
  static async refreshAll(): Promise<CacheOperationOutcome> {
    const result = await getCloudRouterBackendSdkClient().system.cache.refresh();
  ...
  static async refreshInstance(instanceName: string): Promise<CacheOperationOutcome> {
    const result = await getCloudRouterBackendSdkClient().system.cache.instances.refresh(instanceName);
  ...
  static async deleteInstance(instanceName: string): Promise<void> {
    await getCloudRouterBackendSdkClient().system.cache.instances.delete(instanceName);
  }

  static async refreshNamespace(namespace: string): Promise<CacheOperationOutcome> {
    const result = await getCloudRouterBackendSdkClient().system.cache.namespaces.refresh(namespace);
  ...
  static async deleteNamespace(namespace: string): Promise<void> {
    await getCloudRouterBackendSdkClient().system.cache.namespaces.delete(namespace);
  }

  static async deleteKey(namespace: string, key: string): Promise<void> {
    await getCloudRouterBackendSdkClient().system.cache.namespaces.keys.delete(namespace, key);
  }
  ...
  static async listKeys(
    ...
    const result = await getCloudRouterBackendSdkClient().system.cache.namespaces.keys.list(namespace, {
      pageSize,
      ...(cursor ? { cursor } : {}),
    });
