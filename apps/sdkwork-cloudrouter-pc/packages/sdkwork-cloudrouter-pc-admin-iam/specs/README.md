# SDKWork Cloud Router Admin IAM Component Specs

This directory is the local standards index for `sdkwork-cloudrouter-pc-admin-iam`.
...
| Field | Value |
| --- | --- |
| Name | `sdkwork-cloudrouter-pc-admin-iam` |
| Type | `node-package` |
| Root | `sdkwork-cloudrouter/apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudrouter-pc-admin-iam` |
| Domain | `iam` |
...
- `src/index.tsx` (route workspaces)
- `src/iamAdminContribution.ts` (`./contribution`: module/menu/route/permission metadata)
- `src/i18n/index.ts` (`./i18n`: domain copy bundle)

## SDK Clients

- No generated SDK client class is declared at this component boundary.
- Remote calls flow through the shared `SdkworkIamService` built over the portal's
  `@sdkwork/iam-app-sdk` / `@sdkwork/iam-backend-sdk` clients.
