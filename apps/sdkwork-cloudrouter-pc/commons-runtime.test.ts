...
import { getLoadErrorMessage } from "./packages/sdkwork-cloudroutes-pc-commons/src/load-error.ts";
import { formatUserAgentDeviceLabel } from "./packages/sdkwork-cloudroutes-pc-commons/src/user-agent.ts";
import { createClientOperationToken, createIdempotencyParams } from "./packages/sdkwork-cloudroutes-pc-commons/src/idempotency.ts";
import {
  ...
  readRequiredString,
  readRequiredApiItem,
} from "./packages/sdkwork-cloudroutes-pc-commons/src/api-result.ts";
import {
...
import { normalizeGeneratedSdkBaseUrl } from "./packages/sdkwork-cloudroutes-pc-commons/src/sdk-base-url.ts";
import { formatRechargeCurrencyAmount } from "./packages/sdkwork-cloudroutes-pc-commons/src/recharge-math.ts";
import {
...
} from "./packages/sdkwork-cloudroutes-pc-commons/src/media-resource.ts";
import {
...
} from "./packages/sdkwork-cloudroutes-pc-commons/src/sdk-request-boundary.ts";
import { createAppSession, revokeAppSession } from "./packages/sdkwork-cloudroutes-pc-commons/src/sessionService.ts";
import { verifyCurrentPortalAdminAccess } from "./packages/sdkwork-cloudroutes-pc-commons/src/portal-session.ts";
import { API_BASE_URL } from "./packages/sdkwork-cloudroutes-pc-commons/src/utils/env.ts";
import { syntaxHighlightJson } from "./packages/sdkwork-cloudroutes-pc-commons/src/utils/index.ts";
import {
...
} from "./packages/sdkwork-cloudroutes-pc-commons/src/reference-sidebar-groups.ts";
import { readAdminResourceCollectionMeta } from "./packages/sdkwork-cloudroutes-pc-commons/src/components/AdminResourceCenter.tsx";
import { generateCodeSnippets } from "./packages/sdkwork-cloudrouter-pc-core/src/index.ts";
...
test("reference sidebar group element ids are safe and stable for aria controls", () => {
  assert.equal(
    createReferenceSidebarGroupElementId("api-reference-sidebar-group", "Cloud Router Open API", "Chat / Responses"),
    "api-reference-sidebar-group-cloud-router-open-api-chat-responses",
  );
  assert.equal(
...
test("generated SDK metadata declares independent runtime base URL variables for every SDK surface", () => {
  assert.equal(SDK_SYSTEM_CONFIG.gateway.runtimeEnvName, "VITE_CLOUDROUTER_OPEN_API_BASE_URL");
  assert.equal(SDK_SYSTEM_CONFIG.app.runtimeEnvName, "VITE_CLOUDROUTER_APP_API_BASE_URL");
  assert.equal(SDK_SYSTEM_CONFIG.backend.runtimeEnvName, "VITE_CLOUDROUTER_BACKEND_API_BASE_URL");
});

test("documents runtime adapter prefers documents app base URL, then cloudrouter app base URL, then APP_API_PREFIX", () => {
  const source = readPortalSource("./packages/sdkwork-cloudroutes-pc-commons/src/documents-reference-runtime-adapter.ts");

  const documentsEnvIndex = source.indexOf("VITE_SDKWORK_DOCUMENTS_APP_API_BASE_URL");
  const cloudRouterEnvIndex = source.indexOf("VITE_CLOUDROUTER_APP_API_BASE_URL");
  ...
  assert.ok(cloudRouterEnvIndex !== -1, "documents runtime adapter must retain VITE_CLOUDROUTER_APP_API_BASE_URL fallback");
  assert.ok(apiPrefixFallbackIndex !== -1, "documents runtime adapter must fall back to APP_API_PREFIX");
  assert.ok(documentsEnvIndex < cloudRouterEnvIndex, "documents runtime adapter must prefer the dedicated documents API base URL");
  assert.ok(cloudRouterEnvIndex < apiPrefixFallbackIndex, "documents runtime adapter must fall back to the cloudrouter app API base URL before APP_API_PREFIX");
});

test("documents runtime adapter caches a singleton SDK client bound to the cloudrouter token manager", () => {
  const source = readPortalSource("./packages/sdkwork-cloudroutes-pc-commons/src/documents-reference-runtime-adapter.ts");
  ...
  assert.match(source, /tokenManager: getCloudRouterGlobalTokenManager\(\),/);
  assert.match(source, /return documentsAppSdkClient as unknown as DocumentsAppSdkClient;/);
});

test("portal bootstrap mounts the documents runtime provider with the cloudrouter adapter", () => {
  const source = readPortalSource("./src/main.tsx");

  assert.match(source, /import \{ PortalQueryProvider, PortalErrorBoundary, cloudRouterDocumentsReferenceRuntime \} from '@sdkwork\/cloudroutes-pc-commons';/);
  assert.match(source, /import \{ DocumentsReferenceRuntimeProvider \} from '@sdkwork\/documents-pc-commons';/);
  assert.match(source, /<DocumentsReferenceRuntimeProvider value=\{cloudRouterDocumentsReferenceRuntime\}>/);
...
test("portal shell offsets embedded documents routes below the fixed navbar", () => {
  ...
  assert.match(shellSource, /sdkwork-cloudrouter-documents-host-offset flex-1/);
  assert.match(indexCssSource, /\.sdkwork-cloudrouter-documents-host-offset \{/);
  assert.match(indexCssSource, /padding-top: var\(--sdkwork-portal-navbar-height, 4rem\);/);
  assert.match(indexCssSource, /\.sdkwork-cloudrouter-documents-host-offset \.sdkwork-documents-shell-page-root \{/);
...
test("portal shell offsets playground routes below the fixed navbar", () => {
  ...
  assert.match(indexCssSource, /\.sdkwork-cloudrouter-playground-host-offset \{/);
...
test("portal i18n consumes documents catalogs from sdkwork-documents", () => {
  const resourcesSource = readPortalSource("./packages/sdkwork-cloudrouter-pc-i18n/src/resources/index.ts");
  ...
  assert.match(mainSource, /platformName: 'Cloud Router'/);
...
test("sdk clients use a static IAM runtime reset dependency so Vite can chunk the portal deterministically", () => {
  ...
  const iamRuntimeSource = readFileSync(
    new URL("./packages/sdkwork-cloudroutes-pc-commons/src/iam-runtime.ts", import.meta.url),
    "utf8",
  );
  ...
  assert.doesNotMatch(
    iamRuntimeSource,
    /SDKWORK_IAM_BOOTSTRAP_|readCloudRouterRuntimeEnv\('SDKWORK_APP_ID'\)/u,
    "Cloud Router IAM runtime must not read runtime identity scope from bootstrap env variables.",
  );
  assert.match(
    iamRuntimeSource,
    /CLOUD_ROUTER_IAM_RUNTIME_APP_ID[\s\S]*?readCloudRouterRuntimeEnv\('VITE_SDKWORK_APP_ID'\)[\s\S]*?\|\|\s*['"]sdkwork-cloudrouter['"]/u,
    "Cloud Router IAM runtime must resolve app id from manifest-derived VITE_SDKWORK_APP_ID with manifest fallback.",
  );
});
...
test("global token manager seeds bootstrap access token before login session exists", () => {
  ...
  try {
    assert.equal(getCloudRouterGlobalTokenManager().getAccessToken(), "bootstrap-access-token");
    assert.equal(getCloudRouterGlobalTokenManager().getAuthToken(), undefined);
  } finally {
...
test("stored login session replaces bootstrap access token in global token manager", () => {
  ...
  try {
    ...
    assert.equal(getCloudRouterGlobalTokenManager().getAccessToken(), "session-access-token");
    assert.equal(getCloudRouterGlobalTokenManager().getAuthToken(), "session-auth-token");
  } finally {
    if (previousAccessToken === undefined) {
      delete process.env.SDKWORK_ACCESS_TOKEN;
    } else {
...
test("open gateway SDK clients never inherit portal session tokens", () => {
  ...
  try {
    ...
    const implicitClient = getCloudRouterAiSdkClient() as unknown as {
    ...
    const explicitClient = createCloudRouterAiSdkClient({
...
test("commons exports an adaptive admin table shell with a fixed footer slot", () => {
  const shellSource = readFileSync(
    new URL("./packages/sdkwork-cloudroutes-pc-commons/src/components/AdminTableShell.tsx", import.meta.url),
    "utf8",
  );
  const indexSource = readFileSync(
    new URL("./packages/sdkwork-cloudroutes-pc-commons/src/index.ts", import.meta.url),
    "utf8",
  );
...
test("portal notification service composes the shared service with canonical pagination and app scope", () => {
  const serviceSource = readFileSync(
    new URL("./packages/sdkwork-cloudroutes-pc-commons/src/notificationService.ts", import.meta.url),
    "utf8",
  );
...
test("portal notification facade remains typed and backend SDK construction has no cross-domain compatibility overlay", () => {
  ...
  for (const marker of [
    ...
    "facade.catalog.spus",
    "CloudRouterBackendDomainTransportSdkClient",
  ]) {
  ...
  assert.match(
    sdkClientsSource,
    /createCloudRouterBackendSdkClient[\s\S]*?new SdkworkBackendClient\(buildBackendConfig\(options\)\)/,
  );
...
test("cloudrouter backend client constructs without loading business-domain SDK resources", () => {
  const client = createCloudRouterBackendSdkClient({
...
test("iam directory app operations keep one canonical params shape before the appbase SDK boundary", () => {
  const source = readFileSync(
    new URL("./packages/sdkwork-cloudroutes-pc-commons/src/iamDirectoryApiOperations.ts", import.meta.url),
    "utf8",
  );
...
test("runtime stream URL uses standard lower snake case query parameters", () => {
  const source = readFileSync(
    new URL("./packages/sdkwork-cloudroutes-pc-commons/src/runtime.ts", import.meta.url),
    "utf8",
  );
...
test("admin resource center collection metadata reads only the canonical pageSize field", () => {
  const source = readFileSync(
    new URL("./packages/sdkwork-cloudroutes-pc-commons/src/components/AdminResourceCenter.tsx", import.meta.url),
    "utf8",
  );
...
test("portal css stabilizes navbar notification dropdown empty state dimensions", () => {
  ...
  assert.ok(
    navbarSource.includes('className="cloud-router-navbar-notification-bell"'),
    "Navbar must pass the notification bell CSS scope class",
  );

  for (const marker of [
    ".cloud-router-navbar-notification-bell",
    ".cloud-router-navbar-notification-bell [role=\"menu\"]",
    "width: min(22rem, calc(100vw - 2rem));",
    "min-width: min(22rem, calc(100vw - 2rem));",
    ".cloud-router-navbar-notification-bell [role=\"menu\"] > div:nth-child(2)",
    "min-height: 7rem;",
    ".cloud-router-navbar-notification-bell [role=\"menu\"] > div:nth-child(2) > div:not(:has(*))",
    "white-space: normal;",
  ]) {
...
test("navbar localizes notification controls and uses runtime site branding", () => {
  ...
  const siteBrandingSource = readFileSync(
    new URL("./packages/sdkwork-cloudroutes-pc-commons/src/siteBranding.ts", import.meta.url),
    "utf8",
  );
  ...
  assert.doesNotMatch(navbarSource, />\s*Cloud Router\s*</u, "Navbar must render the configurable site name instead of hard-coded text");
});

test("footer renders configurable site branding and copyright", () => {
  const footerSource = readFileSync(
    new URL("./packages/sdkwork-cloudroutes-pc-commons/src/components/Footer.tsx", import.meta.url),
    "utf8",
  );
  ...
  assert.doesNotMatch(footerSource, />\s*Cloud Router\s*</u, "Footer must render the configurable site name instead of hard-coded text");
...
test("console layout keeps readable navigation labels and valid logout markup", () => {
  const source = readFileSync(
    new URL("./packages/sdkwork-cloudrouter-pc-console-shell/src/ConsoleLayout.tsx", import.meta.url),
    "utf8",
  );
...
test("console business routes compose T1 domain PC packages through Cloud Router extensions", () => {
  ...
  const walletSource = readPortalSource("./src/console-business/CloudRouterWalletPage.tsx");

  assert.match(appSource, /CloudRouterConsoleBusinessHostRoutes/);
  assert.match(appSource, /CloudRouterConsoleBusinessNavbarActions/);
  assert.match(mountSource, /CloudRouterWalletPage/);
  ...
  assert.doesNotMatch(appSource, /CloudRouterConsoleCommerceHostRoutes/);
  assert.doesNotMatch(appSource, /consoleCommerceViews/);
  assert.doesNotMatch(appSource, /cloudrouter-pc-console-wallet/);
  assert.doesNotMatch(appSource, /cloudrouter-pc-console-recharge/);
  assert.doesNotMatch(appSource, /cloudrouter-pc-console-commerce/);
});
...
test("console recharge exchange wording stays consistent in shell navigation and i18n resources", () => {
  ...
  const billingI18nSource = readFileSync(
    new URL("./packages/sdkwork-cloudrouter-pc-i18n/src/resources/console/billing.ts", import.meta.url),
    "utf8",
  );
  const rechargeI18nSource = readFileSync(
    new URL("./packages/sdkwork-cloudrouter-pc-i18n/src/resources/console/recharge.ts", import.meta.url),
    "utf8",
  );
      ...
      JSON.stringify({
        ...
        data: {
          ...
          context: {
            ...
            permissionScope: ["cloudrouter.console.access"],
...
function portalAdminSessionPayload(
  ...
  return {
    ...
    context: {
      ...
      userId: "user-admin",
      sessionId,
      permissionScope: options.permissionScope ?? ["cloudrouter.admin.access", "cloudrouter.system.read"],
    },
  };
}
...
test("portal admin access check authorizes the effective IAM permission scope without a business endpoint probe", async () => {
  for (const [permissionScope, expectedState] of [
    [["*"], "allowed"],
    [["cloudrouter.*"], "allowed"],
    [["cloudrouter.admin.access"], "allowed"],
    [["cloudrouter.console.access"], "forbidden"],
  ] as const) {
...
test("portal admin access check uses IAM session RBAC without a backend business probe", () => {
  const source = readFileSync(
    new URL("./packages/sdkwork-cloudroutes-pc-commons/src/portal-session.ts", import.meta.url),
    "utf8",
  );

  assert.match(source, /hasPortalAdminSurfaceAccess/);
  assert.match(source, /auth\.sessions\.current\.retrieve\(\)/);
  assert.doesNotMatch(source, /getCloudRouterBackendSdkClient/);
...
test("BusinessStatePanel resolves invalid or missing kind before reading style metadata", () => {
  const source = readFileSync(new URL("./packages/sdkwork-cloudroutes-pc-commons/src/components/BusinessState.tsx", import.meta.url), "utf8");
...
test("app-surface services do not supply client auth context selectors", () => {
  const sdkClientsSource = readPackageSource("packages/sdkwork-cloudroutes-pc-commons/src/sdk-clients.ts");
  const appSurfaceServiceSources = [
    "packages/sdkwork-cloudrouter-pc-admin-record/src/recordService.ts",
    "packages/sdkwork-cloudrouter-pc-playground/src/playgroundGenerationsService.ts",
  ];
