...
import { backendPromotionOffersList } from "./packages/sdkwork-cloudrouter-pc-admin-marketing/src/marketingService.ts";
import {
  fetchMembershipAdminPackageGroups,
  fetchMembershipAdminPackages,
  fetchMembershipAdminPlans,
} from "./packages/sdkwork-cloudrouter-pc-admin-memberships/src/membershipsService.ts";
...
test("console business host mounts T1 domain wallet, membership, coupon, checkout, and payment routes", () => {
  ...
  const membershipPageSource = readPortalFile("./src/console-business/CloudRouterMembershipPage.tsx");
  const tokenPlanPageSource = readPortalFile("./src/token-plan/CloudRouterTokenPlanPage.tsx");
  const tokenPlanSurfaceSource = readPortalFile("./src/token-plan/CloudRouterTokenPlanSurface.tsx");
  const shellSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-shell/src/AppShellLayout.tsx");
  const publicNavbarSource = readPortalFile("./packages/sdkwork-cloudroutes-pc-commons/src/components/Navbar.tsx");
  ...
  assert.match(appSource, /CloudRouterTokenPlanPage/);
  assert.match(tokenPlanPageSource, /CloudRouterTokenPlanSurface/);
  ...
  assert.match(tokenPlanSurfaceSource, /\.\.\.sdkworkSubscriptionCatalogHostComponents/);
  assert.match(tokenPlanSurfaceSource, /CloudRouterTokenPlanCheckoutModal/);
  assert.match(tokenPlanSurfaceSource, /checkoutModal:\s*CloudRouterTokenPlanCheckoutModal/);
  assert.match(tokenPlanSurfaceSource, /checkoutPort=\{getCloudRouterMembershipCheckoutService\(\)\}/);
  assert.match(tokenPlanSurfaceSource, /useTokenPlanMemberSummary/);
  assert.match(tokenPlanSurfaceSource, /export function CloudRouterTokenPlanSurface/);
  assert.match(tokenPlanSurfaceSource, /data-token-plan-surface/);
  assert.match(membershipPageSource, /CloudRouterTokenPlanSurface/);
  ...
  assert.match(navbarSource, /CloudRouterNavbarWalletEntry/);
  assert.doesNotMatch(navbarSource, /console\.navbar\.coupons/);
  assert.doesNotMatch(navbarSource, /TicketPercent/);
  assert.doesNotMatch(navbarSource, /CloudRouterNavbarTokenPlanEntry/);
  assert.doesNotMatch(navbarSource, /SdkworkTokenPlanHeaderEntry/);
  assert.match(readPortalFile("./src/console-business/CloudRouterWalletPage.tsx"), /@sdkwork\/account-pc-wallet/);
...
test("token plan purchases use order-owned checkout services and dialogs", () => {
  const modalSource = readPortalFile("./src/token-plan/CloudRouterTokenPlanCommerceModal.tsx");
  ...
  const i18nResourcesSource = readPortalFile(
    "./packages/sdkwork-cloudrouter-pc-i18n/src/resources/index.ts",
  );
  ...
  assert.match(modalSource, /getCloudRouterPointsRechargeService/);
  assert.match(modalSource, /service=\{getCloudRouterPointsRechargeService\(\)\}/);
  assert.match(modalSource, /SdkworkCouponRedemptionDialog/);
  assert.match(modalSource, /service=\{getCloudRouterCouponRechargeService\(\)\}/);
  assert.doesNotMatch(modalSource, /getCloudRouterCouponRechargeService\(\)\.redeem/);
  ...
  assert.match(providerSource, /getCloudRouterCouponRechargeService/);
  assert.match(providerSource, /getCloudRouterMembershipCheckoutService/);
...
test("Compute Credits balances and activity use the Token Bank account", () => {
  ...
  const dashboardSource = readPortalFile(
    "./packages/sdkwork-cloudrouter-pc-console-dashboard/src/dashboardService.ts",
  );
  ...
  const tokenBankSources = [
    readPortalFile("./src/console-business/CloudRouterNavbarWalletEntry.tsx"),
    readPortalFile("./src/console-business/CloudRouterNavbarWalletQuickPanel.tsx"),
    readPortalFile("./src/console-business/CloudRouterTokenBankBalancePanel.tsx"),
    readPortalFile("./src/console-business/CloudRouterTokenBankTransactionList.tsx"),
    readPortalFile("./src/console-business/CloudRouterWalletPage.tsx"),
    readPortalFile("./src/console-business/ConsoleAccountView.tsx"),
    readPortalFile("./src/token-plan/tokenPlanMemberSummary.ts"),
  ];

  assert.match(providerSource, /getCloudRouterAccountAppService/);
...
test("English and Chinese catalogs use the Compute Credits terminology", () => {
  const resourcesRoot = new URL(
    "./packages/sdkwork-cloudrouter-pc-i18n/src/resources/",
    import.meta.url,
  );
  ...
  const adminAnalyticsSource = readPortalFile(
    "./packages/sdkwork-cloudrouter-pc-admin-analytics/src/index.tsx",
  );
...
test("Playground delegates Token Plan UI to Agents and injects only host services", () => {
  ...
  assert.match(playgroundSource, /pointsRechargeService:\s*getCloudRouterPointsRechargeService\(\)/);
...
test("console wallet embeds the order-owned points recharge flow", () => {
  const walletSource = readPortalFile("./src/console-business/CloudRouterWalletPage.tsx");
  ...
  assert.match(walletSource, /getCloudRouterPointsRechargeService/);
...
test("recharge hosts provide server-expiration checkout copy", () => {
  const tokenPlanSource = readPortalFile("./src/token-plan/CloudRouterTokenPlanCommerceModal.tsx");
  const walletSource = readPortalFile("./src/console-business/CloudRouterWalletPage.tsx");
  const rechargeCatalogSource = readPortalFile(
    "./packages/sdkwork-cloudrouter-pc-i18n/src/resources/console/recharge.ts",
  );
...
test("console wallet derives authentication from the shared IAM session", () => {
  const walletSource = readPortalFile("./src/console-business/CloudRouterWalletPage.tsx");
  const withdrawSource = readPortalFile("./src/console-business/CloudRouterWithdrawDialog.tsx");
  const quickPanelSource = readPortalFile("./src/console-business/CloudRouterNavbarWalletQuickPanel.tsx");
  ...
  const accountSource = readPortalFile("./src/console-business/ConsoleAccountView.tsx");
  const walletEntrySource = readPortalFile("./src/console-business/CloudRouterNavbarWalletEntry.tsx");
...
test("app bootstrap wires T1 domain service providers from independent owner SDKs", () => {
  ...
  const providersSource = readPortalFile("./packages/sdkwork-cloudroutes-pc-commons/src/domain-service-providers.ts");

  assert.match(mainSource, /configureCloudRouterDomainServiceProviders\(\)/);
  assert.doesNotMatch(mainSource, /getCloudRouterAppSdkClient/);
...
test("federated commerce runtime mounts the complete Order gateway assembly", () => {
  const runtimeSource = readPortalFile("../../crates/sdkwork-routes-cloudrouter-app-api/src/commerce_runtime.rs");
