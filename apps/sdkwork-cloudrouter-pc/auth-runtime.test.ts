...
import {
  mergeCloudRouterAuthRuntimeConfig,
  DEFAULT_CLOUD_ROUTER_AUTH_RUNTIME_CONFIG,
} from "./src/auth/cloudRouterAuthConfig.ts";
import {
...
} from "./packages/sdkwork-cloudroutes-pc-commons/src/portal-auth.ts";
import {
...
import {
  createCloudRouterMessagingVerificationService,
} from "./packages/sdkwork-cloudroutes-pc-commons/src/messaging-verification-service.ts";

type AuthSettingsPageModule = typeof import("./packages/sdkwork-cloudrouter-pc-admin-site/src/CloudRouterAuthSettingsPage.tsx");
...
async function loadAuthSettingsPageModule(): Promise<AuthSettingsPageModule> {
  authSettingsPageModulePromise ??= import("./packages/sdkwork-cloudrouter-pc-admin-site/src/CloudRouterAuthSettingsPage.tsx");
  return authSettingsPageModulePromise;
}

type SdkClientsModule = typeof import("./packages/sdkwork-cloudroutes-pc-commons/src/sdk-clients.ts");
...
type PortalSessionModule = typeof import("./packages/sdkwork-cloudroutes-pc-commons/src/portal-session.ts");
...
async function loadSdkClientsModule(): Promise<SdkClientsModule> {
  sdkClientsModulePromise ??= import("./packages/sdkwork-cloudroutes-pc-commons/src/sdk-clients.ts");
...
async function loadPortalSessionModule(): Promise<PortalSessionModule> {
  portalSessionModulePromise ??= import("./packages/sdkwork-cloudroutes-pc-commons/src/portal-session.ts");
...
async function loadSdkSessionAuthRuntime() {
  const {
    ...
    handleCloudRouterSdkSessionAuthError,
    isCloudRouterSdkSessionAuthError,
...
function readI18nResourceFiles(): Array<{ relativePath: string; source: string }> {
  const resourcesRoot = new URL("./packages/sdkwork-cloudrouter-pc-i18n/src/resources/", import.meta.url);
  if (!existsSync(resourcesRoot)) {
    return [];
  }

  return readPortalSourceFiles("./packages/sdkwork-cloudrouter-pc-i18n/src/resources/");
}
...
function readI18nResourceSource(): string {
  return [
    readPortalFile("./packages/sdkwork-cloudrouter-pc-i18n/src/index.ts"),
...
function readAdminRegistrySource(): string {
  return readPortalFile("./packages/sdkwork-cloudrouter-pc-admin-shell/src/adminModuleRegistry.ts");
}

function readAdminLayoutSource(): string {
  return readPortalFile("./packages/sdkwork-cloudrouter-pc-admin-shell/src/AdminLayout.tsx");
}
...
function installPortalAuthRedirectWindow({
  ...
  Object.defineProperty(globalThis, "window", {
    configurable: true,
    value: {
      __CLOUDROUTER_ENV__: runtimeEnv,
      addEventListener: () => {},
...
async function importFreshAppSessionTokenModule(): Promise<typeof import("./packages/sdkwork-cloudroutes-pc-commons/src/app-session-token.ts")> {
  freshAppSessionTokenModuleImportIndex += 1;
  return import(`./packages/sdkwork-cloudroutes-pc-commons/src/app-session-token.ts?fresh=${freshAppSessionTokenModuleImportIndex}`);
}

test("portal exposes appbase auth routes as standalone React routes", () => {
  const appSource = readPortalFile("./src/App.tsx");
  const shellSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-shell/src/AppShellLayout.tsx");
  const authRouteSource = readPortalFile("./src/auth/CloudRouterAuthRoutes.tsx");
  const authShellSource = readPortalFile("./src/auth/CloudRouterAuthShell.tsx");
  const authAppearanceSource = readPortalFile("./src/auth/cloudRouterAuthAppearance.ts");
  const indexCssSource = readPortalFile("./src/index.css");

  assert.match(appSource, /lazyRoute\(\(\) => import\('\.\/auth\/CloudRouterAuthRoutes'\), 'CloudRouterAuthRoutes'\)/);
  assert.match(appSource, /<Route path="\/auth\/\*" element=\{<PortalAuthenticatedAuthRouteGuard><CloudRouterAuthRoutes \/><\/PortalAuthenticatedAuthRouteGuard>\} \/>/);
  assert.match(appSource, /PortalAuthenticatedAuthRouteGuard/);
  assert.match(shellSource, /pathname\.startsWith\('\/auth'\)/);
  assert.match(shellSource, /sdkwork-auth-route-fallback sdkwork-cloudrouter-auth-route-fallback fixed inset-0 z-\[60\] h-\[100dvh\] min-h-\[100dvh\]/);
  assert.doesNotMatch(appSource, /CloudRouterAuthOAuthCallbackRoute/);
  ...
  assert.match(authRouteSource, /CloudRouterAuthShell/);
  assert.match(authRouteSource, /resolveCloudRouterAuthAppearance/);
  assert.match(authRouteSource, /appearance=\{resolveCloudRouterAuthAppearance\(\)\}/);
  assert.match(authRouteSource, /viewportMode="flow"/);
  assert.match(authShellSource, /sdkwork-cloudrouter-auth-active/);
  assert.match(indexCssSource, /html\.sdkwork-cloudrouter-auth-active/);
  assert.match(indexCssSource, /position: fixed;/);
  assert.match(authRouteSource, /className="!bg-transparent"/);
  assert.match(authShellSource, /sdkwork-cloudrouter-auth-shell/);
  assert.match(authAppearanceSource, /sdkwork-cloudrouter-auth-aside-panel/);
  assert.match(authAppearanceSource, /var\(--sdkwork-cloudrouter-auth-content-text\)/);
  assert.match(indexCssSource, /\.sdkwork-cloudrouter-auth-shell \{/);
  ...
  assert.doesNotMatch(authRouteSource, /cloudRouterAuthController/);
  assert.match(authRouteSource, /from '\.\/cloudRouterAuthRuntime'/);
  assert.doesNotMatch(authRouteSource, /CloudRouterAuthOAuthCallbackRoute/);
  assert.match(authRouteSource, /basePath="\/auth"/);
  assert.match(authRouteSource, /locale=\{i18n\.language\}/);
  assert.match(authRouteSource, /getRuntime=\{getCloudRouterAuthRuntime\}/);
...
test("cloud router auth controller reuses appbase runtime while preserving app SDK boundary", () => {
  const controllerSource = readPortalFile("./src/auth/cloudRouterAuthController.ts");
  const runtimeAdapterSource = readPortalFile("./src/auth/cloudRouterAuthRuntime.ts");
  const routeSource = readPortalFile("./src/auth/CloudRouterAuthRoutes.tsx");
  const configSource = readPortalFile("./src/auth/cloudRouterAuthConfig.ts");
  const settingsServiceSource = readPortalFile("./src/auth/cloudRouterAuthSettingsService.ts");
  const adminSettingsServiceSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-admin-site/src/AuthSettingsService.ts");
  const iamRuntimeSource = readPortalFile("./packages/sdkwork-cloudroutes-pc-commons/src/iam-runtime.ts");
  const sdkClientsSource = readPortalFile("./packages/sdkwork-cloudroutes-pc-commons/src/sdk-clients.ts");

  assert.match(controllerSource, /createSdkworkIamRuntimeAuthController/);
  assert.match(controllerSource, /getCloudRouterAuthRuntime/);
  assert.doesNotMatch(controllerSource, /getCloudRouterIamRuntime/);
  assert.match(runtimeAdapterSource, /getCloudRouterIamRuntime/);
  assert.match(runtimeAdapterSource, /SdkworkIamRuntimeAuthRuntimeLike/);
  assert.match(runtimeAdapterSource, /getCloudRouterMessagingVerificationService/);
  assert.match(runtimeAdapterSource, /messaging:\s*getCloudRouterMessagingVerificationService\(\)/);
  ...
  assert.match(iamRuntimeSource, /credentialEntry:\s*\{[\s\S]*prepareTokens:\s*prepareCloudRouterCredentialEntryTokens/u);
  assert.doesNotMatch(iamRuntimeSource, /wrapCredentialEntryClient|skipWrap/u);
  assert.match(iamRuntimeSource, /prepareTokens:\s*prepareCloudRouterCredentialEntryTokens/);
  assert.match(iamRuntimeSource, /bindCloudRouterIamSessionProjection/);
  assert.match(iamRuntimeSource, /patchCloudRouterIamContextStore/);
  ...
  assert.match(iamRuntimeSource, /getCloudRouterAppSdkClient\(\)/);
  ...
  assert.match(sdkClientsSource, /function buildAppConfig\(options: CloudRouterAppSdkClientOptions\): SdkworkAppConfig \{\s*return \{[\s\S]*?tokenManager:\s*resolveCloudRouterSdkTokenManager\(options\.tokenManager\)/);
  assert.match(sdkClientsSource, /function buildBackendConfig\(options: CloudRouterBackendSdkClientOptions\): SdkworkBackendConfig \{\s*return \{[\s\S]*?tokenManager:\s*resolveCloudRouterSdkTokenManager\(options\.tokenManager\)/);
  assert.match(sdkClientsSource, /function buildAppbaseAppConfig\(options: SdkworkAppbaseAppSdkClientOptions\): SdkworkAppbaseAppConfig \{\s*return \{[\s\S]*?tokenManager:\s*resolveCloudRouterSdkTokenManager\(options\.tokenManager\)/);
  assert.match(sdkClientsSource, /function buildMessagingAppConfig\(options: SdkworkMessagingAppSdkClientOptions\): MessagingAppConfig \{\s*return \{[\s\S]*?tokenManager:\s*resolveCloudRouterSdkTokenManager\(options\.tokenManager\)/);
  ...
  assert.doesNotMatch(sdkClientsSource, /interface CloudRouterAiSdkClientOptions \{[\s\S]*?(?:authToken|accessToken)\?: string/);
  assert.doesNotMatch(iamRuntimeSource, /getCloudRouterAiSdkClient\(\)/);
  assert.doesNotMatch(iamRuntimeSource, /getCloudRouterBackendSdkClient\(\)/);
  ...
  assert.doesNotMatch(iamRuntimeSource, /app:\s*getCloudRouterAppSdkClient\(\)/);
  ...
  assert.doesNotMatch(controllerSource, /resetCloudRouterSdkClients/);
  ...
  assert.doesNotMatch(controllerSource, /Cloud Router app API contract does not expose password login/);
  assert.doesNotMatch(controllerSource, /getCloudRouterAppSdkClient\(\)\.auth\.login/);
  assert.doesNotMatch(controllerSource, /auth\.createAppSession/);
  assert.doesNotMatch(controllerSource, /getCloudRouterAppSdkClient\(\)\.user\.fetchUserProfile/);
  ...
  assert.match(routeSource, /getCloudRouterAuthRuntime/);
  assert.match(routeSource, /from '\.\/cloudRouterAuthRuntime'/);
  assert.doesNotMatch(routeSource, /cloudRouterAuthController/);
  assert.match(routeSource, /useCloudRouterAuthRuntimeConfig/);
  assert.match(routeSource, /runtimeConfig=\{runtimeConfig\}/);
  assert.doesNotMatch(routeSource, /const cloudRouterAuthRuntimeConfig/);
  assert.match(routeSource, /appearance=\{resolveCloudRouterAuthAppearance\(\)\}/);
  assert.match(routeSource, /CloudRouterAuthShell/);
  assert.match(configSource, /DEFAULT_CLOUD_ROUTER_AUTH_RUNTIME_CONFIG/);
  ...
  assert.match(configSource, /fetchCloudRouterAuthRuntimeSettings/);
  assert.doesNotMatch(configSource, /fetchCloudRouterAuthSettings/);
  assert.match(settingsServiceSource, /getSdkworkAppbaseAppSdkClient/);
  assert.doesNotMatch(settingsServiceSource, /getCloudRouterAppSdkClient/);
  assert.doesNotMatch(settingsServiceSource, /getCloudRouterBackendSdkClient/);
  ...
  assert.match(adminSettingsServiceSource, /getCloudRouterBackendSdkClient/);
...
test("auth runtime config applies backend IAM settings without tenant or organization being required", () => {
  const config = mergeCloudRouterAuthRuntimeConfig(authRuntimeSettingsFixture());
  ...
  assert.equal(DEFAULT_CLOUD_ROUTER_AUTH_RUNTIME_CONFIG.qrLoginEnabled, true);
  assert.equal(DEFAULT_CLOUD_ROUTER_AUTH_RUNTIME_CONFIG.verificationPolicy?.emailRegistrationVerificationRequired, false);
});
...
test("auth runtime config fails closed when backend omits required IAM runtime fields", () => {
  for (const [field, message] of [
    ...
    assert.throws(
      () => mergeCloudRouterAuthRuntimeConfig(settings),
      message,
    );
  }
});
...
test("auth runtime config fails closed when backend returns unsupported IAM runtime options", () => {
  for (const [patch, message] of [
    ...
    assert.throws(
      () => mergeCloudRouterAuthRuntimeConfig(authRuntimeSettingsFixture(patch)),
      message,
    );
  }
});
...
test("auth runtime config validates backend QR login types without exposing a retired client field", () => {
  for (const qrLoginType of ["web", "official", "mini"] as const) {
    const config = mergeCloudRouterAuthRuntimeConfig(authRuntimeSettingsFixture({ qrLoginType }));
    assert.equal("qrLoginType" in config, false);
  }
  assert.equal("qrLoginType" in DEFAULT_CLOUD_ROUTER_AUTH_RUNTIME_CONFIG, false);
});
...
test("auth runtime config fails closed when backend omits verification policy flags", () => {
  for (const [field, message] of [
    ...
    assert.throws(
      () => mergeCloudRouterAuthRuntimeConfig(authRuntimeSettingsFixture({ verificationPolicy })),
      message,
    );
  }
});

test("cloud router app auth composes the appbase IAM OAuth dependency SDK without regenerating dependency transport", () => {
  const contractSource = readPortalFile("../../docs/schema-registry/frontend-field-contracts.yaml");
  const appOpenApiSource = readPortalFile("../../generated/openapi/cloudrouter-app-openapi.json");
  const backendOpenApiSource = readPortalFile("../../generated/openapi/cloudrouter-backend-openapi.json");
  const appSdkInputSource = readPortalFile("../../sdks/cloudrouter-app-sdk/openapi/cloudrouter-app-sdk.sdkgen.json");
  ...
  for (const [path, method, operationId] of appbaseOwnedAppRoutes) {
    ...
    assert.equal(appSdkInput.paths?.[path], undefined, `cloudrouter app SDK input must not regenerate ${method.toUpperCase()} ${path}`);
  }
  ...
  assert.equal(existsSync(new URL("../../sdks/cloudrouter-app-sdk/cloudrouter-app-sdk-typescript/src/api/auth.ts", import.meta.url)), false);
...
test("portal exposes backend-backed admin auth settings configuration", () => {
  ...
  const settingsPageSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-admin-site/src/CloudRouterAuthSettingsPage.tsx");
  const settingsServiceSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-admin-site/src/AuthSettingsService.ts");
  const routeClassificationSource = readPortalFile("../../docs/schema-registry/frontend-route-classification.yaml");

  assert.match(adminHostSource, /lazyAdminRoute\(\(\) => import\('@sdkwork\/cloudrouter-pc-admin-site'\), 'CloudRouterAuthSettingsPage'\)/);
  assert.match(adminHostSource, /route\('settings', 'sdkwork-cloudrouter', '@sdkwork\/cloudrouter-pc-admin-site', \['cloudrouter-backend-sdk'\]/);
  assert.match(adminRegistrySource, /path:\s*'\/admin\/settings'/);
  assert.match(adminRegistrySource, /ShieldCheck/);
  assert.match(settingsPageSource, /fetchCloudRouterAuthSettings/);
  assert.match(settingsPageSource, /updateCloudRouterAuthSettings/);
  ...
  assert.match(settingsServiceSource, /getCloudRouterBackendSdkClient\(\)\.system\.auth\.settings\.retrieve\(\)/);
  assert.match(settingsServiceSource, /getCloudRouterBackendSdkClient\(\)\.system\.auth\.settings\.update\(input/);
  assert.doesNotMatch(settingsServiceSource, /\bfetch\s*\(/);
  assert.doesNotMatch(settingsServiceSource, /\baxios\b/);
  assert.doesNotMatch(settingsServiceSource, /\/backend\/v3\/api\/system\/auth\/settings/);
  ...
  assert.match(routeClassificationSource, /apps\/sdkwork-cloudrouter-pc\/packages\/sdkwork-cloudrouter-pc-admin-site\/src\/AuthSettingsService\.ts/);
});
...
test("navbar routes sign in through the auth module instead of bootstrapping sessions directly", () => {
  const navbarSource = readPortalFile("./packages/sdkwork-cloudroutes-pc-commons/src/components/Navbar.tsx");
  const portalAuthSource = readPortalFile("./packages/sdkwork-cloudroutes-pc-commons/src/portal-auth.ts");
  const sessionTokenSource = readPortalFile("./packages/sdkwork-cloudroutes-pc-commons/src/app-session-token.ts");
  ...
  assert.deepEqual(
    resolveProtectedPortalAccess({
      ...
      location: {
        ...
        search: "?project=cloud",
      },
    }),
    {
      allowed: false,
      redirectTo: "/auth/login?redirect=%2Fconsole%2Fapi-keys%3Fproject%3Dcloud%23keys",
...
test("cloud router app session persists across module reload in the same browser tab", async () => {
...
test("cloud router app session persists across browser tabs via shared localStorage", async () => {
  const storageHarness = createPortalSessionStorageHarness();
  const expiresAt = Math.floor(Date.now() / 1000) + 3600;
...
test("admin layout enforces route permission guard for protected admin pages", async () => {
  ...
  const guardSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-admin-shell/src/AdminRoutePermissionGuard.tsx");
  const permissionsSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-admin-shell/src/admin-menu-permissions.ts");
  const { isAdminRouteAllowed } = await import("./packages/sdkwork-cloudrouter-pc-admin-shell/src/admin-menu-permissions.ts");
  ...
  assert.equal(isAdminRouteAllowed("/admin/upstream", ["cloudrouter.admin.access"]), true);
  assert.equal(isAdminRouteAllowed("/admin/dashboard", ["cloudrouter.admin.access"]), true);
  assert.equal(isAdminRouteAllowed("/admin/dashboard", ["cloudrouter.*"]), true);
  assert.equal(isAdminRouteAllowed("/admin/upstream", ["*"]), true);
});

test("portal i18n keeps document language aligned with active locale", () => {
  const i18nSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-i18n/src/index.ts");
...
test("generated SDK auth errors clear the app session and redirect protected pages to login", async () => {
  ...
  try {
    ...
    assert.equal(isCloudRouterSdkSessionAuthError({
      code: "4010",
      msg: "app session token has expired",
    }), true);
    assert.equal(handleCloudRouterSdkSessionAuthError({
      code: "4010",
      msg: "app session token has expired",
    }), true);
...
test("generated SDK ProblemDetail invalid-session errors clear the app session", async () => {
  ...
  try {
    ...
    assert.equal(isCloudRouterSdkSessionAuthError(invalidSessionProblem), true);
    assert.equal(handleCloudRouterSdkSessionAuthError(invalidSessionProblem), true);
...
test("injected app SDK clients retain the invalid-session cleanup boundary", async () => {
  ...
  try {
    ...
    Object.defineProperty(host, "__SDKWORK_CLOUDROUTER_ROUTER_APP_SDK_CLIENT__", {
      configurable: true,
      value: {
        http: {
    ...
    if (injectedDescriptor) {
      Object.defineProperty(host, "__SDKWORK_CLOUDROUTER_ROUTER_APP_SDK_CLIENT__", injectedDescriptor);
    } else {
      delete host.__SDKWORK_CLOUDROUTER_ROUTER_APP_SDK_CLIENT__;
    }
    restoreWindow();
  }
});
...
test("dashboard SDK requests send the IAM dual-token headers", async () => {
  const { createCloudRouterAppSdkClient } = await loadSdkSessionAuthRuntime();
  const previousFetch = globalThis.fetch;
...
test("portal wires console and admin routes through the protected session guard", () => {
  ...
  const sharedAuthSource = readPortalFile("./packages/sdkwork-cloudroutes-pc-commons/src/portal-auth.ts");
  ...
  assert.match(guardSource, /@sdkwork\/cloudroutes-pc-commons\/runtime/);
  assert.doesNotMatch(guardSource, /sdkwork-cloudroutes-pc-commons\/runtime/);
...
test("console and admin logout revoke the current IAM session through the app SDK", () => {
  const consoleLayoutSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-console-shell/src/ConsoleLayout.tsx");
  const adminLayoutSource = readAdminLayoutSource();
  const sessionServiceSource = readPortalFile("./packages/sdkwork-cloudroutes-pc-commons/src/sessionService.ts");
...
test("cloud router i18n resources are split by business domain", () => {
  const indexSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-i18n/src/index.ts");
  ...
  const resourceIndex = resourceFiles.find((file) => file.relativePath === "./packages/sdkwork-cloudrouter-pc-i18n/src/resources/index.ts");
  const businessResourceFiles = resourceFiles.filter((file) => file.relativePath !== "./packages/sdkwork-cloudrouter-pc-i18n/src/resources/index.ts");
...
test("direct admin translation lookups without fallbacks have English and Chinese translations", () => {
  ...
  ].filter((file) => file.relativePath !== "./packages/sdkwork-cloudrouter-pc-i18n/src/index.ts");
...
test("upstream credentials never expose plaintext after submission", () => {
  const accountSource = readPortalFile("./packages/sdkwork-cloudrouter-pc-admin-upstream/src/accountsPage.tsx");
...
test("portal composes appbase auth and Tauri host packages through workspace installs", () => {
  ...
  const tauriBridgeSource = readPortalFile("./src/auth/cloudRouterTauriAuthHost.ts");
...
test("portal imports auth runtime session helpers through exported workspace subpaths", () => {
  ...
  const iamSessionProjectionSource = readPortalFile(
    "./packages/sdkwork-cloudroutes-pc-commons/src/iam-runtime-session-projection.ts",
  );
...
test("portal resolves T1 domain console packages through workspace installs", () => {
  ...
  assert.equal(packageJson.dependencies['@sdkwork/cloudrouter-pc-admin-inventory'], undefined);
  assert.equal(packageJson.dependencies['@sdkwork/cloudrouter-pc-admin-file-platform'], undefined);
...
test("portal typecheck remains scoped to cloud router packages after appbase workspace reuse", () => {
