...
import { GatewayService } from "./packages/sdkwork-cloudrouter-pc-console-gateway/src/gatewayService.ts";
import { NotificationService } from "./packages/sdkwork-cloudroutes-pc-commons/src/notificationService.ts";
import { MessagesService } from "./packages/sdkwork-cloudrouter-pc-console-messages/src/messagesService.ts";
...
test("navbar delegates popup queue and persisted acknowledgement state to the appbase notification component", () => {
  ...
  assert.match(navbarSource, /getPortalNotificationClient\(\)/, "Navbar must inject the generated Cloud Router app SDK client");
...
test("app SDK message contract exposes popup display flag", () => {
  const messageTypeSource = readPortalFile("../../sdks/cloudrouter-app-sdk/cloudrouter-app-sdk-typescript/generated/server-openapi/src/types/notification-item.ts");
...
test("console gateway service follows opaque cursor pagination returned by the generated app SDK", async () => {
  ...
  await withAppSdkResponses(
    ...
    async (captured) => {
      ...
      const firstUrl = new URL(captured[0].url, "https://cloudrouter.test");
      ...
      const continuationUrl = new URL(captured[1].url, "https://cloudrouter.test");
