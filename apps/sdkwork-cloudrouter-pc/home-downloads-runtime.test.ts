...
test("home hero release badge is driven by the download catalog version", () => {
  ...
  assert.ok(heroSource.includes("createCloudRouterDownloadCatalog"));
  assert.ok(heroSource.includes("cloudRouterDownloadCatalog"));
  ...
  assert.ok(i18nSource.includes("Cloud Router v{{version}} is now live"));
  assert.ok(i18nSource.includes("Cloud Router v{{version}} 现已发布"));
});
