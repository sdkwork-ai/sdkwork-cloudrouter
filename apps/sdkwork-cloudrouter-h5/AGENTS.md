...
- `etc/`: checked-in H5 source configuration (deployment index and profile sources) governed by `SOURCE_CONFIG_SPEC.md`.
- `packages/`: H5 package family for core, commons, shell, i18n, and console capability modules.
- `src/`: thin H5 bootstrap, providers, route assembly, and the app-level auth gate wiring.
...
Run `pnpm check` from this root for the fast surface verification.
