...
materializer, so existing unrelated SDK directories such as `sdks/cloudrouter-*`
are not read, rewritten, or deleted. The file SDK package does not materialize
`sdks/file-*` under the CloudRouter application SDK workspace; CloudRouter's
application-owned `sdks/` tree remains reserved for the three CloudRouter SDK
families.
