...
Read `sdkwork.app.config.json` only when changing shared Cloud Router client family application behavior, runtime config, SDK wiring, release metadata, packaging, or app-owned capabilities.
...
- `sdkwork.app.config.json`: shared Cloud Router client family application identity and release metadata.
- `.sdkwork/`: local application dictionary for skills, plugins, manifests, and AI workspace metadata.
- `specs/`: local shared Cloud Router client family application/component contracts and narrowing rules.
- `etc/`: checked-in shared Cloud Router client family source configuration (deployment index and profile sources) governed by `SOURCE_CONFIG_SPEC.md`.
- `packages/`: shared Cloud Router client family package family for shared contract, port, and service packages.
- `packages/`: shared contracts, service ports, and architecture-neutral services for the Cloud Router client roots.
...
- Shared client application architecture: `../../../sdkwork-specs/APPLICATION_SPEC.md`, `../../../sdkwork-specs/APP_CLIENT_ARCHITECTURE_ALIGNMENT_SPEC.md`, and `../../../sdkwork-specs/APP_CLIENT_ARCHITECTURE_ALIGNMENT_SPEC.md`.
...
Read `../../../sdkwork-specs/CODE_STYLE_SPEC.md` and `../../../sdkwork-specs/NAMING_SPEC.md` before code changes. Root `packages/` must stay thin; business screens, services, i18n, state, and route contributions belong in packages. Feature packages consume generated SDK clients through the core package public exports, not by importing generated SDK modules or creating raw HTTP clients.
...
Run `pnpm test` from this root to execute the family contract test.
...
API work loads `../../../sdkwork-specs/API_SPEC.md` and its validators. List/search work loads `../../../sdkwork-specs/PAGINATION_SPEC.md` and `check-pagination.mjs`. Source configuration work loads `../../../sdkwork-specs/SOURCE_CONFIG_SPEC.md` and `check-source-config-standard.mjs`. Host adapter work loads `../../../sdkwork-specs/APP_CLIENT_ARCHITECTURE_ALIGNMENT_SPEC.md` and `../../../sdkwork-specs/APP_SDK_INTEGRATION_SPEC.md`. Link these authorities instead of copying their normative bodies into `AGENTS.md`.
