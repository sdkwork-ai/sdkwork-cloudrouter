...
Read `sdkwork.app.config.json` only when changing HarmonyOS Mobile application behavior, runtime config, SDK wiring, release metadata, packaging, or app-owned capabilities.

- Application key: `cloudrouter-harmony-mobile`
- Bundle name: `com.sdkwork.cloudrouter.mobile`
...
- `sdkwork.app.config.json`: HarmonyOS Mobile application identity and release metadata.
- `.sdkwork/`: local application dictionary for skills, plugins, manifests, and AI workspace metadata.
- `specs/`: local HarmonyOS Mobile application/component contracts and narrowing rules.
- `etc/`: checked-in HarmonyOS Mobile source configuration (deployment index and profile sources) governed by `SOURCE_CONFIG_SPEC.md`.
- `config/`: runtime environment and host/platform descriptor examples.
- `entry/`: installable HarmonyOS entry/composition module — entry ability, bootstrap, SDK client construction, IAM runtime wiring, route registry, and host adapter registration.
- `packages/`: ArkTS/HAR package family for core, commons, shell, host, and console capability packages.
- `AppScope/`, `oh-package.json5`, `build-profile.json5`, `hvigor/`, `hvigorfile.ts`: deployable-root build metadata.
...
- ArkTS code: `../../../sdkwork-specs/ARKTS_CODE_SPEC.md` when that spec is present in the standards root; otherwise follow `../../../sdkwork-specs/CODE_STYLE_SPEC.md` and the HarmonyOS UI authority.
- Frontend/UI code: `../../../sdkwork-specs/FRONTEND_CODE_SPEC.md`, `../../../sdkwork-specs/FRONTEND_SPEC.md`, `../../../sdkwork-specs/UI_ARCHITECTURE_SPEC.md`, and `../../../sdkwork-specs/APP_HARMONY_NATIVE_UI_SPEC.md`.
- harmony-mobile application architecture: `../../../sdkwork-specs/APPLICATION_SPEC.md`, `../../../sdkwork-specs/APP_CLIENT_ARCHITECTURE_ALIGNMENT_SPEC.md`, and `../../../sdkwork-specs/HARMONY_APP_MOBILE_ARCHITECTURE_SPEC.md`.
...
Read `../../../sdkwork-specs/CODE_STYLE_SPEC.md` and `../../../sdkwork-specs/NAMING_SPEC.md` before code changes. Root `entry/` must stay thin: it wires composition only, and business pages live in capability packages. Capability packages consume generated SDK clients through the core package public exports, not by importing generated transport modules or creating raw HTTP clients.
...
Run `node --test tests/harmony-surface-contract.test.mjs` and the SDKWork frontend composition checker.

No HarmonyOS toolchain (DevEco Studio / HarmonyOS SDK / `hvigor` / `ohpm`) is installed in this workspace, and no ArkTS target is produced by the SDK generation chain yet. Both are declared as pending integration steps in `sdkwork.app.config.json#artifacts.installConfig.metadata`; until they land, verification is limited to the Node contract test and the repository gates.
...
API work loads `../../../sdkwork-specs/API_SPEC.md` and its validators. List/search work loads `../../../sdkwork-specs/PAGINATION_SPEC.md` and `check-pagination.mjs`. Source configuration work loads `../../../sdkwork-specs/SOURCE_CONFIG_SPEC.md` and `check-source-config-standard.mjs`. Locale resource work loads `../../../sdkwork-specs/I18N_SPEC.md`. Host adapter work loads `../../../sdkwork-specs/HARMONY_APP_MOBILE_ARCHITECTURE_SPEC.md` and `../../../sdkwork-specs/APP_SDK_INTEGRATION_SPEC.md`. Link these authorities instead of copying their normative bodies into `AGENTS.md`.
