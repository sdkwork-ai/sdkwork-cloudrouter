...
export function AiResourceSelectorModal({
  ...
  const resourceSearchQuery = searchQuery ?? localSearchQuery;
  const serverSearch = onSearchQueryChange !== undefined;
  const filteredResourceOptions = serverSearch ? options : options.filter(option => matchesAiResourceSelectorSearch(resourceSearchQuery, [
    option.displayName,
    option.resourceCode,
    option.resourceType,
    option.vendorCode,
      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between gap-3 border-t border-slate-200 p-5 dark:border-white/10">
          ...
          <button
            ...
            className="rounded-lg border border-slate-200 bg-slate-50 px-5 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-white/10 dark:bg-[#1a1a1a] dark:text-slate-300 dark:hover:bg-white/5"
          >
            {labels.done}
          </button>
        </div>
...
function AiResourceSelectorState({
  actionLabel,
  onAction,
  text,
}: {
  ...
  return (
    <div className="flex min-h-[240px] flex-col items-center justify-center gap-3 px-6 text-center text-sm text-slate-500 dark:text-slate-400">
      <span>{text}</span>
      {actionLabel && onAction ? (
        <button
          type="button"
          onClick={onAction}
          className="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 px-3 text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-white/10 dark:text-slate-200 dark:hover:bg-white/5"
        >
