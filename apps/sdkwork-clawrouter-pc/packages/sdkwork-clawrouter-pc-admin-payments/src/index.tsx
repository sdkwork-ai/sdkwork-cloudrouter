...
import { BarChart3, CreditCard, Pencil, Power, PowerOff, Receipt, ShieldCheck, Trash2 } from 'lucide-react';
                ...
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4" data-admin-payment-provider-compact-form>
                  ...
                  <div className="md:col-span-2 xl:col-span-4">
                    <div className="grid gap-2.5 md:grid-cols-2 xl:grid-cols-3">
                      <PaymentProviderAccountInput
                        label={t('admin.commerce.payments.providerAccounts.credentials.secretRef', 'Credential secret reference')}
                        placeholder="vault://payments/provider/environment/account/credential"
                        value={providerAccountForm.storedSecretRef}
                        onChange={(storedSecretRef) => setProviderAccountForm((current) => ({ ...current, storedSecretRef }))}
                        required
                      />
                      <PaymentProviderAccountInput
                        label={t('admin.commerce.payments.providerAccounts.credentials.certificateRef', 'Certificate reference')}
                        placeholder="vault://payments/provider/environment/account/certificate"
                        value={providerAccountForm.storedCertificateRef}
                        onChange={(storedCertificateRef) => setProviderAccountForm((current) => ({ ...current, storedCertificateRef }))}
                      />
                      <PaymentProviderAccountInput
                        label={t('admin.commerce.payments.providerAccounts.credentials.webhookSecretRef', 'Webhook secret reference')}
                        placeholder="vault://payments/provider/environment/account/webhook"
                        value={providerAccountForm.storedWebhookSecretRef}
                        onChange={(storedWebhookSecretRef) => setProviderAccountForm((current) => ({ ...current, storedWebhookSecretRef }))}
                      />
                      <div className="md:col-span-2 xl:col-span-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-800 dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-200">
                        {t('admin.commerce.payments.providerAccounts.credentials.referenceOnly', 'Store payment credentials in your secret manager first. This form accepts references only and never reads or uploads raw keys.')}
                      </div>
...
function toPaymentProviderAccountRequest(form: PaymentProviderAccountFormState): PaymentProviderAccountMutationInput {
  ...
  const secretRef = requiredText(form.storedSecretRef, 'secretRef');
  const certificateRef = form.storedCertificateRef.trim();
  const webhookSecretRef = form.storedWebhookSecretRef.trim();
  return {
    providerCode,
