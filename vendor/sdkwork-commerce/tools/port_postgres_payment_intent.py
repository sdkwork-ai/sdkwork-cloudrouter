#!/usr/bin/env python3
"""Port payment intent helpers from sqlite_payment_intent.rs to postgres_payment_intent.rs."""
...
SQLITE, POSTGRES = sqlite_postgres_pair("payment", "payment_intent")
...
def port_text(text: str) -> str:
    ...
    text = text.replace("pool: &sqlx::SqlitePool", "pool: &sqlx::PgPool")
    text = text.replace("use sqlx::{Row, Sqlite, Transaction};", "use sqlx::{Postgres, Row, Transaction};")
