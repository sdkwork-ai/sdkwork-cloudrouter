...
configureSdkworkCommerceSessionTokenProvider(readClawRouterCommerceSessionTokens);

let clawRouterDomainServicesInitPromise: Promise<void> | null = null;

function readClawRouterCommerceSessionTokens() {
...
export function initializeClawRouterDomainServiceProviders(): Promise<void> {
  if (!clawRouterDomainServicesInitPromise) {
    clawRouterDomainServicesInitPromise = import('./domain-service-providers.ts').then(({ configureClawRouterDomainServiceProviders }) => {
      configureClawRouterDomainServiceProviders(() => wrapCommerceAppSdkClient(getSdkworkCommerceAppSdkClient()));
    });
  }

  return clawRouterDomainServicesInitPromise;
}
