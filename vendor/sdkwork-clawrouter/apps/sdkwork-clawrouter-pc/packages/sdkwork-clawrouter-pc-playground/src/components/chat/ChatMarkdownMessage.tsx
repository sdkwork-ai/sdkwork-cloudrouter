  ...
  return (
    <div
      className={mergeClassNames(
        'chat-markdown min-w-0 max-w-full text-[15px] leading-7 tracking-normal',
        tone === 'user'
          ? 'text-slate-50'
          : tone === 'danger'
            ? 'text-red-100'
...
function ChatMarkdownBlockquote({
  ...
  return (
    <blockquote
      {...props}
      className={mergeClassNames(
        'my-4 rounded-xl border-l-2 px-4 py-3 first:mt-0 last:mb-0',
        tone === 'user'
          ? 'border-white/20 bg-white/[0.06] text-slate-100'
          : tone === 'danger'
            ? 'border-red-300/40 bg-red-400/10 text-red-100'
            : 'border-cyan-300/40 bg-white/[0.04] text-slate-200',
        className,
      )}
    />
  );
  ...
  if (display) {
    return (
      <div
        tabIndex={0}
        className={mergeClassNames(
          'katex-display my-4 max-w-full overflow-x-auto rounded-xl border px-4 py-3 text-center first:mt-0 last:mb-0',
          tone === 'user'
            ? 'border-white/12 bg-white/[0.06] text-slate-50'
            : tone === 'danger'
