...
fn app_api_direct_http_tests_use_standard_query_names() {
    ...
    for entry in
        std::fs::read_dir(&tests_dir).expect("sdkwork-clawrouter-app-api-server tests dir exists")
    {
        let entry = entry.expect("sdkwork-clawrouter-app-api-server test entry is readable");
        let path = entry.path();
        ...
        let source = std::fs::read_to_string(&path)
            .expect("sdkwork-clawrouter-app-api-server test is readable");
        for forbidden in [
            ("pageSize=", "page_size"),
