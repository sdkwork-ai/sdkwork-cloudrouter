...
impl NotificationApi {
    ...
    pub async fn notifications_list(&self, include_archived: Option<bool>, page: Option<&str>, page_size: Option<&str>) -> Result<NotificationsListResult, SdkworkError> {
    ...
    pub async fn notifications_acknowledge_create(&self, notification_id: &str, idempotency_key: &str) -> Result<NotificationsAcknowledgeCreateResult, SdkworkError> {
        let path = app_path(&format!("/notification/notifications/{}/acknowledge", serialize_path_parameter(notification_id, PathParameterSpec::new("notificationId", "simple", false))));
    ...
    pub async fn notifications_popup_seen_create(&self, notification_id: &str, idempotency_key: &str) -> Result<NotificationsPopupSeenCreateResult, SdkworkError> {
        let path = app_path(&format!("/notification/notifications/{}/popup_seen", serialize_path_parameter(notification_id, PathParameterSpec::new("notificationId", "simple", false))));
        let headers = build_request_headers(
            &[
                ("Idempotency-Key", HeaderParameterSpec::new(idempotency_key, "simple", false, None)),
            ],
