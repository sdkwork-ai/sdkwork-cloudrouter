...
async fn list_provider_accounts(
    ...
    let rows = sqlx::query(&format!(
        ...
        WHERE a.tenant_id = ?1
          ...
          AND (?5 IS NULL OR a.provider_code = ?5)
          AND (?6 IS NULL OR lower(a.account_code) LIKE ?6 OR lower(a.account_name) LIKE ?6 OR lower(a.provider_code) LIKE ?6)
...
async fn list_sender_identities(
    ...
    let rows = sqlx::query(&format!(
        ...
        WHERE s.tenant_id = ?1
          ...
          AND (?5 IS NULL OR s.provider_code = ?5)
...
async fn list_send_requests(
    ...
    let rows = sqlx::query(
        ...
        WHERE r.tenant_id = ?1
          ...
          AND (?4 IS NULL OR r.channel = ?4)
          AND (?5 IS NULL OR r.scene_code = ?5)
          AND (?6 IS NULL OR a.provider_code = ?6)
