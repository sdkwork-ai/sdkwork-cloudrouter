...
pub struct EdgeInProcessUpstreams {
    ...
    sdkwork_api_cloud_gateway_router: Option<Router>,
...
impl EdgeInProcessUpstreams {
    pub fn new(gateway_router: Router, backend_router: Router, app_router: Router) -> Self {
        Self {
            gateway_router,
            sdkwork_api_cloud_gateway_router: None,
            backend_router,
            app_router,
        }
    }

    pub fn with_sdkwork_api_cloud_gateway_router(mut self, router: Router) -> Self {
        self.sdkwork_api_cloud_gateway_router = Some(router);
        self
    }
    ...
    fn router_for_path(&self, path: &str) -> Option<Router> {
        if let Some(router) = self
            .sdkwork_api_cloud_gateway_router
            .clone()
            .filter(|_| sdkwork_api_cloud_gateway_surface_path(path))
        {
            return Some(router);
        }
        if sdkwork_api_cloud_gateway_dependency_path(path) {
            return Some(
                self.sdkwork_api_cloud_gateway_router
                    .clone()
