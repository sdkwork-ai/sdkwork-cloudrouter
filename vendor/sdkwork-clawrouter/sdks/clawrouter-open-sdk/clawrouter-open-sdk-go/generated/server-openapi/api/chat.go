...
func (a *ChatApi) List(limit *int, order *string, after *string, before *string, model *string, metadata *string) (sdktypes.OpenAiChatCompletionList, error) {
    query := BuildQueryString([]QueryParameterSpec{
        ...
        {Name: "metadata", Value: func() interface{} { if metadata == nil { return nil }; return *metadata }(), Style: "form", Explode: true, AllowReserved: false},
    })
    raw, err := a.client.Get(AppendQueryString(AiApiPath("/chat/completions"), query), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiChatCompletionList
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiChatCompletionList](raw)
}

// Create chat completion
func (a *ChatApi) Create(body sdktypes.OpenAiChatCompletionRequest) (sdktypes.OpenAiChatCompletion, error) {
    raw, err := a.client.Post(AiApiPath("/chat/completions"), body, nil, nil, "application/json")
    if err != nil {
        var zero sdktypes.OpenAiChatCompletion
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiChatCompletion](raw)
}

// Delete stored chat completion
func (a *ChatApi) Delete(completionId string) (sdktypes.DeleteResult, error) {
    raw, err := a.client.Delete(AiApiPath(fmt.Sprintf("/chat/completions/%s", SerializePathParameter(completionId, PathParameterSpec{Name: "completion_id", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
...
func (a *ChatApi) Retrieve(completionId string) (sdktypes.OpenAiChatCompletion, error) {
    raw, err := a.client.Get(AiApiPath(fmt.Sprintf("/chat/completions/%s", SerializePathParameter(completionId, PathParameterSpec{Name: "completion_id", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiChatCompletion
        return zero, err
    }
...
func (a *ChatApi) Update(completionId string, body sdktypes.OpenAiChatCompletionUpdateRequest) (sdktypes.OpenAiChatCompletion, error) {
    raw, err := a.client.Post(AiApiPath(fmt.Sprintf("/chat/completions/%s", SerializePathParameter(completionId, PathParameterSpec{Name: "completion_id", Style: "simple", Explode: false}))), body, nil, nil, "application/json")
    if err != nil {
        var zero sdktypes.OpenAiChatCompletion
        return zero, err
    }
...
func (a *ChatApi) ListMessages(completionId string, limit *int, order *string, after *string, before *string) (sdktypes.OpenAiChatCompletionMessageList, error) {
    query := BuildQueryString([]QueryParameterSpec{
        {Name: "limit", Value: func() interface{} { if limit == nil { return nil }; return *limit }(), Style: "form", Explode: true, AllowReserved: false},
        {Name: "order", Value: func() interface{} { if order == nil { return nil }; return *order }(), Style: "form", Explode: true, AllowReserved: false},
        {Name: "after", Value: func() interface{} { if after == nil { return nil }; return *after }(), Style: "form", Explode: true, AllowReserved: false},
    ...
    raw, err := a.client.Get(AppendQueryString(AiApiPath(fmt.Sprintf("/chat/completions/%s/messages", SerializePathParameter(completionId, PathParameterSpec{Name: "completion_id", Style: "simple", Explode: false}))), query), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiChatCompletionMessageList
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiChatCompletionMessageList](raw)
}
