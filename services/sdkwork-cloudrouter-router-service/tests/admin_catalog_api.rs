...
async fn admin_catalog_category_routes_use_standard_create_list_delete_semantics() {
    ...
    let router =
        sdkwork_cloudrouter_router_service::api::admin_catalog_router_with_store(store.clone());
...
impl AdminCatalogStore for TestAdminCatalogStore {
    ...
    fn initialize_category_seeds<'a>(
        &'a self,
        _command: sdkwork_cloudrouter_router_service::ports::AdminCategorySeedInitializeCommand,
    ) -> AdminCatalogFuture<
        'a,
        Vec<sdkwork_cloudrouter_router_service::ports::AdminCategorySeedInitializeSummary>,
    > {
