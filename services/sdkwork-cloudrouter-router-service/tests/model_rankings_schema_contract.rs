...
const SCHEMA_REGISTRY_TABLES: &str =
    include_str!("../../../generated/schema/registry/sdkwork-cloudrouter.tables.effective.yaml");
...
fn generated_schema_has_table_field_contract_for_model_ranking_refresh_and_reads() {
    ...
    for expected in [
        ...
        assert_contains(
            &runtime_schema,
            expected,
            "runtime ranking schema (cloud-router generated + sdkwork-models catalog baseline)",
        );
    }
}
...
fn schema_registry_declares_the_same_model_ranking_field_and_index_contract() {
    for expected in [
        ...
        "source_tables: - ai_metering_usage - ai_model - ai_model_rank_snapshot",
        "runtime_contract:",
        "scheduler_owner: sdkwork-cloudrouter-standalone-gateway",
        ...
        "idx_ai_metering_usage_model_occurred",
...
fn schema_manifest_preserves_model_ranking_semantic_contracts_for_release_audit() {
    ...
    let usage_fact = manifest_table(&manifest, "ai_metering_usage");
    assert_eq!(
        "occurred_at",
    ...
    assert_eq!(
        serde_json::json!(["ai_metering_usage", "ai_model", "ai_model_rank_snapshot"]),
