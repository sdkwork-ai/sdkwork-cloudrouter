...
async fn app_chat_create_conversation_uses_product_chat_namespace_and_store_contract() {
    ...
    let router = sdkwork_cloudrouter_router_service::api::app_chat_router_with_store(
        store.clone(),
...
async fn app_chat_default_router_fails_closed_without_store_and_redacts_configuration() {
    let router = sdkwork_cloudrouter_router_service::api::app_chat_router();
...
async fn app_chat_list_messages_rejects_noncanonical_ambiguous_and_invalid_cursor_queries() {
    ...
    for query in queries {
        let response = router
            ...
            .oneshot(chat_json_request(
                "GET",
                &format!("/app/v3/api/chat/conversations/chat-conversation-1/messages?{query}"),
                Body::empty(),
            ))
            .await
            .unwrap();
...
async fn app_chat_list_messages_fails_closed_when_store_cursor_does_not_advance() {
    ...
    *store.message_page_stalled.lock().unwrap() = true;
    ...
    let response = router
        .oneshot(chat_json_request(
            "GET",
            &format!("/app/v3/api/chat/conversations/chat-conversation-1/messages?cursor={cursor}"),
            Body::empty(),
        ))
        .await
        .unwrap();
