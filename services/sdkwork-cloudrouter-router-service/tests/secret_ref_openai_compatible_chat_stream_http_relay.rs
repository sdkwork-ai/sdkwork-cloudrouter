...
use sdkwork_cloudrouter_router_service::infrastructure::provider::SecretRefOpenAiCompatibleChatCompletionStreamRelay;
...
impl ProviderSecretResolver for MapSecretResolver {
    fn resolve_secret_value(
        ...
        self.secrets.get(secret_ref).cloned().ok_or_else(|| {
            sdkwork_cloudrouter_router_service::domain::DomainError::new("secret ref not found")
        })
    }
}
...
async fn secret_ref_chat_stream_relay_resolves_endpoint_and_secret_from_request_context() {
    ...
    let relay = SecretRefOpenAiCompatibleChatCompletionStreamRelay::for_local_development(
