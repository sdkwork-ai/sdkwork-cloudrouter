...
fn admin_marketing_store_does_not_reimplement_promotion_coupon_surface_locally() {
    ...
    for forbidden in [
        ...
        assert!(
            ...
            "cloudrouter admin marketing store must not re-implement promotion coupon SQL `{forbidden}`; sdkwork-promotion owns that surface via the federated commerce pool"
        );
    }
}
