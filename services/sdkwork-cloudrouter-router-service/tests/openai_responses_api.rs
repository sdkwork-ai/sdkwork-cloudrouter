...
async fn openai_responses_authenticates_validates_price_and_returns_honest_not_implemented() {
    ...
    let router = sdkwork_cloudrouter_router_service::api::openai_responses_router(
...
async fn openai_responses_invocation_plugins_cannot_override_account_before_relay() {
    ...
    let router =
        sdkwork_cloudrouter_router_service::api::openai_responses_router_with_relay_and_plugins(
            Arc::new(catalog_with_hashed_api_key(key_hash)),
            hasher,
...
async fn openai_responses_records_usage_after_provider_success() {
    ...
    let router = sdkwork_cloudrouter_router_service::api::openai_responses_router_with_relay_and_usage_recorder(
        Arc::new(catalog_with_hashed_api_key(key_hash)),
        hasher,
        relay,
        recorder,
...
async fn openai_responses_rejects_chat_only_model_before_fake_success() {
    ...
    let router =
        sdkwork_cloudrouter_router_service::api::openai_responses_router(Arc::new(catalog), hasher);
