...
fn explain_request(api_key_id: i64, account_group_id: Option<i64>) -> Request<Body> {
    ...
    common::web_framework_backend_request(
        "POST",
        &format!("/backend/v3/api/ai/upstream_account_groups/{account_group_id}/route_explain"),
...
async fn route_explain_requires_a_typed_admin_subject() {
    let router =
        sdkwork_cloudrouter_router_service::api::admin_route_explain_router(Arc::new(catalog()));
    let response = router
        .oneshot(
            Request::builder()
                .method("POST")
