...
async fn admin_record_route_lists_logs_and_normalizes_filters() {
    ...
    let router =
        sdkwork_cloudrouter_router_service::api::admin_record_router_with_store(store.clone());
...
async fn admin_record_route_rejects_missing_trusted_subject() {
    let router = sdkwork_cloudrouter_router_service::api::admin_record_router_with_store(Arc::new(
            ...
            *self.captured.lock().unwrap() = Some(query.clone());
            Ok(AdminRecordListPage {
                items: vec![AdminRecordLogItem {
                    ...
                    points: "0".to_owned(),
                    original_currency_amount: "0.012300".to_owned(),
                    original_currency_code: "USD".to_owned(),
