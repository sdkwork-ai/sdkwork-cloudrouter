...
impl StripePaymentHttpClient for RegistryStripeHttpClient {
    ...
    ) -> sdkwork_cloudrouter_router_service::application::PaymentAdapterFuture<'a, serde_json::Value>
    {
        Box::pin(async move {
            Ok(json!({
                "id": "pi_registry_1",
...
async fn registry_can_replace_mainstream_sandbox_adapter_with_configured_real_adapter() {
    ...
    let outcome = adapter
        .create_payment_intent(
            sdkwork_cloudrouter_router_service::application::PaymentCreateIntentRequest {
