...
use sdkwork_cloudrouter_router_service::infrastructure::provider::ProviderSecretMapResolver;
...
impl EmbeddingsRelay for RecordingEmbeddingsRelay {
    ...
    ) -> sdkwork_cloudrouter_router_service::ports::EmbeddingsRelayFuture<'a> {
        self.captured.lock().unwrap().push(request);
        Box::pin(async {
            Ok(
                sdkwork_cloudrouter_router_service::ports::EmbeddingsRelayResponse::json(
                    200,
                    json!({
                        "object": "list",
...
async fn openai_embeddings_registry_hit_calls_internal_adapter_without_direct_relay() {
    ...
    let adapter_relay =
        sdkwork_cloudrouter_router_service::infrastructure::provider::AdapterAwareEmbeddingsRelay::new(
            relay,
    ...
    let router = sdkwork_cloudrouter_router_service::api::openai_embeddings_router_with_relay(
