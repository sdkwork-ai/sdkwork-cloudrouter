...
async fn openai_embeddings_invocation_plugins_cannot_override_account_before_relay() {
    ...
    let router =
        sdkwork_cloudrouter_router_service::api::openai_embeddings_router_with_relay_and_plugins(
            Arc::new(catalog_with_hashed_api_key(key_hash)),
            hasher,
...
async fn openai_embeddings_records_usage_after_provider_success() {
    ...
    let router = sdkwork_cloudrouter_router_service::api::openai_embeddings_router_with_relay_and_usage_recorder(
        Arc::new(catalog_with_hashed_api_key(key_hash)),
        hasher,
        relay,
        recorder,
