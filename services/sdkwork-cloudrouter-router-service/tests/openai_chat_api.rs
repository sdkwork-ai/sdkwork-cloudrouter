...
fn catalog_with_regional_minimax_pricing_and_routes(key_hash: String) -> InMemoryPricingCatalog {
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("minimax_direct", 4001)
            .with_region_code("cn")
            .with_upstream_endpoint(
                Some("http://provider-proxy.internal/minimax"),
                Some("vault://providers/minimax/account/cn"),
...
async fn openai_chat_completions_authenticates_and_returns_honest_relay_not_implemented() {
    ...
    let router = sdkwork_cloudrouter_router_service::api::openai_chat_completions_router(
...
#[tokio::test]
async fn openai_chat_completions_routes_each_upstream_account_group_to_its_configured_channel_route(
) {
    let hasher =
        Arc::new(HmacSha256ApiKeySecretHasher::new("0123456789abcdef0123456789abcdef").unwrap());
    ...
    assert_eq!("openrouter-premium", captured[1].supplier_code);
    assert_eq!(
        Some("http://provider-proxy.internal/openrouter-premium"),
...
async fn openai_chat_completions_routes_catalog_model_through_channel_route_without_model_route() {
    ...
    assert_eq!(1, captured.len());
    assert_eq!(10, captured[0].group_id);
    assert_eq!(3002, captured[0].provider_account_id);
...
async fn openai_chat_completions_routes_alibaba_regional_model_through_region_scoped_group_channel_route(
    ...
    assert_eq!(1, captured.len());
    assert_eq!(10, captured[0].group_id);
    assert_eq!(3101, captured[0].provider_account_id);
...
async fn openai_chat_completions_rejects_group_policy_missing_chat_capability_without_global_fallback(
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("global-openrouter", 3003).with_upstream_endpoint(
            Some("http://provider-proxy.internal/global-openrouter"),
            Some("vault://providers/openrouter/account/global"),
        ),
    );
...
impl OpenAiInvocationPlugin for RecordingInvocationPlugin {
    ...
    fn on_route_fault<'a>(
        ...
        self.events.lock().unwrap().push(format!(
            "route_fault:{}:{}",
            route.supplier_code, fault.error_code
        ));
        Box::pin(async { Ok(()) })
    }
    ...
    fn on_route_success<'a>(
        ...
        self.events.lock().unwrap().push(format!(
            "route_success:{}:{}",
            route.supplier_code, outcome.status_code
        ));
        Box::pin(async { Ok(()) })
    }
}
...
impl OpenAiInvocationPlugin for BlockingInvocationPlugin {
    fn before_relay<'a>(
        ...
        self.events
            .lock()
            .unwrap()
            .push(format!("blocked_before_relay:{}", route.supplier_code));
        Box::pin(async {
...
async fn openai_chat_completions_fails_over_to_next_account_after_primary_relay_failure() {
    ...
    let router = sdkwork_cloudrouter_router_service::api::openai_chat_completions_router_with_relay_and_usage_recorder_and_plugins(
        Arc::new(catalog),
        hasher,
...
async fn openai_chat_completions_fails_over_after_retryable_provider_status() {
    ...
    let usage_records = Arc::new(Mutex::new(Vec::new()));
    let router =
        sdkwork_cloudrouter_router_service::api::openai_chat_completions_router_with_relay_and_usage_recorder(
            Arc::new(catalog),
            hasher,
    ...
    assert_eq!("openrouter", captured[0].supplier_code);
    assert_eq!("openrouter-fallback", captured[1].supplier_code);
    assert_eq!(3002, usage_records.lock().unwrap()[0].account_id);
}
...
async fn openai_chat_completions_uses_runtime_default_retry_policy_for_status_failover() {
    ...
    let captured = Arc::new(Mutex::new(Vec::new()));
    let usage_records = Arc::new(Mutex::new(Vec::new()));
    let router = sdkwork_cloudrouter_router_service::api::openai_chat_completions_router_with_relay_usage_recorder_plugins_and_runtime_config(
        Arc::new(catalog),
        hasher,
        Arc::new(RetryableStatusPrimaryRelay::new(
            Arc::clone(&captured),
...
async fn openai_chat_completions_fail_closed_strategy_stops_after_retryable_provider_status() {
    ...
    let captured = Arc::new(Mutex::new(Vec::new()));
    let usage_records = Arc::new(Mutex::new(Vec::new()));
    let router = sdkwork_cloudrouter_router_service::api::openai_chat_completions_router_with_relay_usage_recorder_plugins_and_failure_strategy(
        Arc::new(catalog),
        hasher,
        Arc::new(RetryableStatusPrimaryRelay::new(
            Arc::clone(&captured),
...
async fn openai_chat_completions_stream_fails_over_to_next_account_before_response_start() {
    ...
    let router = sdkwork_cloudrouter_router_service::api::openai_chat_completions_router_with_relays_and_usage_recorder_and_plugins(
        Arc::new(catalog),
        hasher,
...
async fn openai_chat_invocation_plugins_observe_route_and_relay_lifecycle() {
    ...
    let captured = Arc::new(Mutex::new(Vec::new()));
    let events = Arc::new(Mutex::new(Vec::new()));
    let router = sdkwork_cloudrouter_router_service::api::openai_chat_completions_router_with_relay_and_plugins(
        Arc::new(catalog_with_hashed_api_key(key_hash)),
        hasher,
...
async fn openai_chat_completions_records_non_stream_usage_after_provider_success() {
    ...
    assert_eq!(10, command.account_group_id);
    assert_eq!("standard-group", command.upstream_account_group_snapshot);
...
async fn openai_chat_completions_relays_stream_request_after_auth_model_and_price_validation() {
    ...
    let router =
        sdkwork_cloudrouter_router_service::api::openai_chat_completions_router_with_streaming_relay(
            Arc::new(catalog_with_hashed_api_key(key_hash)),
            hasher,
            stream_relay,
        );
...
async fn openai_chat_completions_records_failed_provider_status_trace_without_usage_fact() {
    ...
    assert_eq!("standard-group", trace.upstream_account_group_snapshot);
...
async fn openai_chat_completions_reports_stream_missing_usage_as_plugin_fault() {
    ...
    let router =
        sdkwork_cloudrouter_router_service::api::openai_chat_completions_router_with_relays_and_usage_recorder_and_plugins(
            Arc::new(catalog_with_hashed_api_key(key_hash)),
            hasher,
            Arc::new(RecordingRelay::new(Arc::new(Mutex::new(Vec::new())))),
            stream_relay,
