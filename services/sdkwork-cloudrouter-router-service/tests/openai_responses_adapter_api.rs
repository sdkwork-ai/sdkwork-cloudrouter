...
impl ResponsesRelay for RecordingResponsesRelay {
    ...
    ) -> sdkwork_cloudrouter_router_service::ports::ResponsesRelayFuture<'a> {
        self.captured.lock().unwrap().push(request);
        Box::pin(async {
            Ok(
                sdkwork_cloudrouter_router_service::ports::ResponsesRelayResponse::json(
                    200,
                    json!({
                        "id": "resp-direct",
                        "object": "response",
...
async fn openai_responses_registry_hit_calls_internal_adapter_without_direct_relay() {
    ...
    let adapter_relay =
        sdkwork_cloudrouter_router_service::infrastructure::provider::AdapterAwareResponsesRelay::new(
            relay,
    ...
    let router = sdkwork_cloudrouter_router_service::api::openai_responses_router_with_relay(
