...
impl ChatCompletionRelay for RecordingRelay {
    fn create_chat_completion<'a>(
        ...
        Box::pin(async {
            Ok(
                sdkwork_cloudrouter_router_service::ports::ChatCompletionRelayResponse::json(
                    200,
                    json!({
                        "id": "chatcmpl-direct",
...
impl ChatCompletionStreamRelay for RecordingStreamRelay {
    fn create_chat_completion_stream<'a>(
        &'a self,
        request: ChatCompletionRelayRequest,
    ) -> sdkwork_cloudrouter_router_service::ports::ChatCompletionStreamRelayFuture<'a> {
        self.captured.lock().unwrap().push(request);
        Box::pin(async {
...
async fn openai_chat_registry_hit_calls_internal_adapter_without_direct_relay() {
    ...
    let adapter_relay =
        sdkwork_cloudrouter_router_service::infrastructure::provider::AdapterAwareChatCompletionRelay::new(
            relay,
            Arc::new(ProviderAdapterRegistry::new(vec![adapter_route(
                fake_adapter.base_url.as_str(),
            )])),
            sdkwork_cloudrouter_provider_adapter_http::ProviderAdapterHttpClient::for_development("test-token"),
        )
...
async fn openai_chat_adapter_fails_closed_when_shared_response_memory_is_saturated() {
    ...
    let response_limit =
        sdkwork_cloudrouter_provider_adapter_http::ProviderAdapterHttpClient::MAX_BUFFERED_RESPONSE_BYTES;
...
async fn openai_chat_stream_registry_hit_calls_internal_adapter_without_direct_stream_relay() {
    ...
    let adapter_stream_relay =
        sdkwork_cloudrouter_router_service::infrastructure::provider::AdapterAwareChatCompletionStreamRelay::new(
            stream_relay,
