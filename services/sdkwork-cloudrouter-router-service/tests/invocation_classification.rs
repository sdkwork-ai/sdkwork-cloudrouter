...
fn classify_openai(
    ...
    sdkwork_cloudrouter_router_service::application::InvocationResource,
    InvocationBilling,
    InvocationRouting,
) {
...
fn rejects_provider_control_plane_routes() {
    for (method, path) in [
        ...
        assert_eq!(
            sdkwork_cloudrouter_router_service::application::InvocationErrorKind::ResourceClassification,
            error.kind
        );
    }
}
