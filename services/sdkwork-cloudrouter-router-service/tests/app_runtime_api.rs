...
async fn app_runtime_create_invocation_uses_product_runtime_namespace_and_store_contract() {
    ...
    let router = sdkwork_cloudrouter_router_service::api::app_runtime_router_with_store(
        store.clone(),
...
async fn app_runtime_stream_executes_openai_compatible_invocation_and_persists_delta_events() {
    ...
    let router =
        sdkwork_cloudrouter_router_service::api::app_runtime_router_with_store_and_chat_stream_relay(
            store.clone(),
            Arc::new(SequentialUuidGenerator::new(vec![
                "runtime-event-uuid-1",
...
async fn app_runtime_prefers_gateway_chat_completions_over_local_relay_when_both_are_configured() {
    ...
    let relay_requests = Arc::new(Mutex::new(Vec::new()));
    let router =
        sdkwork_cloudrouter_router_service::api::app_runtime_router_with_store_and_gateway_client_chat_stream_relay(
            store.clone(),
            Arc::new(SequentialUuidGenerator::new(vec!["runtime-event-uuid-1"])),
            Arc::new(TestRuntimeCatalog::without_model_upstream_route(
                "openai/gpt-5.5",
...
async fn app_runtime_gateway_executor_selects_lowest_route_capable_api_key_when_no_default_is_set()
{
    for _ in 0..24 {
        ...
        store.list_events_items.lock().unwrap().clear();
        let gateway_requests = Arc::new(Mutex::new(Vec::new()));
        let router = sdkwork_cloudrouter_router_service::api::app_runtime_router_with_store_and_gateway_client(
            store,
            Arc::new(SequentialUuidGenerator::new(vec!["runtime-event-uuid-1"])),
            Arc::new(TestRuntimeCatalog::with_api_keys(vec![
                TestRuntimeApiKeyFixture {
...
impl AppRuntimeStore for TestAppRuntimeStore {
    fn list_invocations<'a>(
        ...
        query: sdkwork_cloudrouter_router_service::ports::AppRuntimeInvocationQuery,
...
impl sdkwork_cloudrouter_router_service::ports::PricingCatalog for TestRuntimeCatalog {
    fn visit_models(
        &self,
        _vendor_code: Option<&str>,
        visitor: &mut dyn FnMut(&sdkwork_cloudrouter_router_service::domain::AiModel) -> bool,
    ) {
        let mut model = sdkwork_cloudrouter_router_service::domain::AiModel::new(
            &self.model,
            &self.model,
            &self.vendor_code,
            vec!["chat", "response"],
    ...
    fn list_model_upstream_routes(
        &self,
        model: &str,
    ) -> Vec<sdkwork_cloudrouter_router_service::domain::ModelUpstreamRoute> {
    ...
    fn list_upstream_account_routes(
        &self,
    ) -> Vec<sdkwork_cloudrouter_router_service::domain::UpstreamAccountRoute> {
        if !self.include_channel_route {
            return Vec::new();
        }
        vec![
            sdkwork_cloudrouter_router_service::domain::UpstreamAccountRoute::new("openai", 3001)
                .with_upstream_endpoint(Some("https://provider.example/v1"), Some("secret-ref"))
    ...
    fn resolve_model_mapping(
        &self,
        _source_model: &str,
        _context: &sdkwork_cloudrouter_router_service::domain::ResolveModelMappingContext,
    ) -> Option<sdkwork_cloudrouter_router_service::domain::ModelMappingRule> {
        None
    }

    fn list_api_keys(&self) -> Vec<sdkwork_cloudrouter_router_service::domain::GatewayApiKey> {
        ...
        let api_key = sdkwork_cloudrouter_router_service::domain::GatewayApiKey::new(
            101, 10, "sk-app", "hash",
        )
        .with_owner(TEST_TENANT_ID, TEST_ORGANIZATION_ID, TEST_USER_ID);
    ...
    fn list_upstream_account_groups(
        &self,
    ) -> Vec<sdkwork_cloudrouter_router_service::domain::UpstreamAccountGroup> {
        vec![
    ...
    fn list_model_prices(
        &self,
        model: &str,
        price_side: sdkwork_cloudrouter_router_service::domain::PriceSide,
        ...
        match price_side {
            sdkwork_cloudrouter_router_service::domain::PriceSide::OfficialReference => vec![
                sdkwork_cloudrouter_router_service::domain::ModelPrice::new_for_catalog_key(
                    &self.catalog_key,
                    &self.model,
            ...
            sdkwork_cloudrouter_router_service::domain::PriceSide::UpstreamCost => vec![
                sdkwork_cloudrouter_router_service::domain::ModelPrice::new_for_catalog_key(
                    &self.catalog_key,
                    &self.model,
    ...
    fn find_api_key(
        &self,
        api_key_id: i64,
    ) -> Option<sdkwork_cloudrouter_router_service::domain::GatewayApiKey> {
        self.list_api_keys()
            .into_iter()
    ...
    fn find_upstream_account_group(
        &self,
        group_id: i64,
    ) -> Option<sdkwork_cloudrouter_router_service::domain::UpstreamAccountGroup> {
    ...
    fn find_pricing_plan(
        &self,
        plan_code: &str,
    ) -> Option<sdkwork_cloudrouter_router_service::domain::PricingPlan> {
    ...
    fn find_model(
        &self,
        model: &str,
    ) -> Option<sdkwork_cloudrouter_router_service::domain::AiModel> {
    ...
    fn find_vendor(
        &self,
        vendor_code: &str,
    ) -> Option<sdkwork_cloudrouter_router_service::domain::ModelVendorDefinition> {
        (vendor_code == self.vendor_code).then(|| {
            sdkwork_cloudrouter_router_service::domain::ModelVendorDefinition::new(
                &self.vendor_code,
                sdkwork_cloudrouter_router_service::domain::ModelVendor::from_code(
                    &self.vendor_code,
                ),
                &self.vendor_code,
            )
    ...
    fn find_model_upstream_route(
        ...
        model: &str,
        supplier_code: &str,
    ) -> Option<sdkwork_cloudrouter_router_service::domain::ModelUpstreamRoute> {
...
impl sdkwork_cloudrouter_router_service::ports::UpstreamAccountRouteCatalog for TestRuntimeCatalog {
    fn shared_upstream_account_routes(
        &self,
    ) -> Arc<[sdkwork_cloudrouter_router_service::domain::UpstreamAccountRoute]> {
        <Self as sdkwork_cloudrouter_router_service::ports::PricingCatalog>::list_upstream_account_routes(
            self,
        )
        .into()
    }
...
impl AppRuntimeGatewayClient for RecordingGatewayRuntimeClient {
    ...
    ) -> sdkwork_cloudrouter_router_service::ports::AppRuntimeFuture<'a, AppRuntimeGatewayResponse>
    {
