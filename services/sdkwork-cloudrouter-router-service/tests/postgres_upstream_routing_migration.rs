...
async fn migration_repairs_legacy_columns_when_0003_history_already_exists() {
    ...
    let history = sqlx::query(
        "SELECT applied_by, execution_ms FROM ops_schema_migration_history WHERE module_id = 'cloudrouter' AND version = '0003'",
    )
    .fetch_one(&context.pool)
    .await
    .unwrap();
...
VALUES
    ('cloudrouter', '0003', 'sdkwork-dev-manual-fix', 0);
