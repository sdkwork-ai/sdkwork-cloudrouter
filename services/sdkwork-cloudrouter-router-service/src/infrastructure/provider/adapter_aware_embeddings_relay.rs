...
use sdkwork_cloudrouter_provider_adapter_http::{AdapterInvokeResult, ProviderAdapterHttpClient};
                ...
                ProviderInvocationMode::InternalHttpAdapter(route) => {
                    ...
                    match response {
                        AdapterInvokeResult::Buffered(response) => Ok(
                            EmbeddingsRelayResponse::json(response.status_code, response.body)
                                .with_memory_guard(memory_guard),
                        ),
                        AdapterInvokeResult::Streaming { .. } => Err(crate::domain::DomainError::new(
                            "provider adapter returned a streaming body for a non-streaming embeddings request",
                        )),
                    }
                }
            }
