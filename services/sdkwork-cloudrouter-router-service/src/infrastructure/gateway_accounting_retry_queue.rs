...
fn redis_queue_key_prefix(prefix: &str) -> String {
    ...
    format!("{{cloudrouter-gateway-accounting-retry}}:{prefix}")
}
...
mod tests {
    ...
    fn redis_queue_keys_share_a_cluster_hash_tag_and_preserve_tagged_prefixes() {
        ...
        assert_eq!(
            "{cloudrouter-production}:gateway-accounting-retry:stream",
            queue.stream
        );

        let tagged = RedisGatewayAccountingRetryQueue::new(
            "redis://127.0.0.1:6379/0",
            "cloudrouter:{tenant-a}::",
        )
        .expect("construct pre-tagged Redis retry queue");
        assert_eq!(
            "cloudrouter:{tenant-a}:gateway-accounting-retry:stream",
            tagged.stream
        );
        ...
        assert_eq!(
            Some("cloudrouter-gateway-accounting-retry"),
