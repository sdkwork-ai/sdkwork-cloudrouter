...
pub(super) async fn list_credentials(
    ...
    // Credentials are write-only on every read surface: rows map to masked
    // metadata and never decrypt `secret_ciphertext` into a response item.
...
async fn create_credential_in_transaction(
    ...
    let credential_id = next_cloud_runtime_id("upstream account credential")?;
