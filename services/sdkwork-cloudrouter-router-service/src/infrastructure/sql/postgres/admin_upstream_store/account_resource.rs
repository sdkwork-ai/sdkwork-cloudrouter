...
pub(super) async fn list(
    ...
    let rows = sqlx::query(sqlx::AssertSqlSafe(sql))
        ...
        .fetch_all(pool)
        .await
        .map_err(|error| store_error("failed to list upstream account resources", error))?;
    if rows.len() > MAX_NESTED_ITEMS {
        return Err(DomainError::new(format!(
            "upstream account resources exceed the bounded maximum of {MAX_NESTED_ITEMS} items"
        )));
    }
...
pub(super) async fn replace(
    ...
    for item in &items {
        let binding_id = next_cloud_runtime_id("upstream account resource")?;
        sqlx::query(
            r#"
            INSERT INTO ai_upstream_account_resource (
                ...
                account_id, resource_code, resource_group_code, grant_type, priority
            ) VALUES (
                $1, $2, $3, $4, $5, $6,
            ...
            ON CONFLICT (
                tenant_id, organization_id, account_id, resource_code, resource_group_code
            ) DO UPDATE SET
                ...
                deleted_at = NULL,
                deleted_by = NULL,
                version = ai_upstream_account_resource.version + 1,
        ...
        .execute(&mut *tx)
        .await
        .map_err(|error| store_error("failed to upsert upstream account resource", error))?;
    }
    ...
    record_routing_change(
        ...
        "upstream_account",
        account_id,
        "replace_upstream_account_resources",
...
async fn retire_omitted(
    ...
    sqlx::query(
        r#"
        UPDATE ai_upstream_account_resource binding
    ...
    .execute(&mut **tx)
    .await
    .map_err(|error| store_error("failed to retire omitted upstream account resources", error))?;
    Ok(())
}
...
async fn list_in_transaction(
    ...
    let rows = sqlx::query(sqlx::AssertSqlSafe(sql))
        ...
        .fetch_all(&mut **tx)
        .await
        .map_err(|error| store_error("failed to reload upstream account resources", error))?;
    rows.into_iter().map(map_resource_row).collect()
}
