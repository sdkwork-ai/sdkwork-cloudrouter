...
async fn list_api_key_rate_limits(
    ...
    let sql = api_key_rate_limit_select_sql(
        ...
        WHERE q.tenant_id = $1
          ...
          AND (
              $4 IS NULL
              OR LOWER(COALESCE(k.key_prefix, q.subject_ref_masked, '')) LIKE $4
              OR LOWER(COALESCE(k.user_id::text, '')) LIKE $4
          )
...
async fn insert_quota_policy(
    ...
    let id = next_cloud_runtime_id("ai_quota_policy")?;
    sqlx::query(
        r#"
...
async fn insert_config_snapshot(
    ...
    let id = next_cloud_runtime_id("ops_config_snapshot")?;
    sqlx::query(
        r#"
