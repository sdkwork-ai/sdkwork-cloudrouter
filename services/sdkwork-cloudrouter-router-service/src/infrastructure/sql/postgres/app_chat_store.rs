...
impl AppChatStore for PostgresAppChatStore {
    ...
    fn create_conversation<'a>(
        ...
        Box::pin(async move {
            ...
            let conversation_id = next_cloud_runtime_id("ai_chat_conversation")?;
            sqlx::query(
                r#"
...
async fn create_turn(
    ...
    let turn_id = next_cloud_runtime_id("ai_chat_turn")?;
    sqlx::query(
        r#"
    ...
    let message_id = next_cloud_runtime_id("ai_chat_message")?;
    ...
    .bind(&command.requested_at)
    .bind(&metadata)
    .bind(next_cloud_runtime_id("ai_chat_message_part")?)
    .execute(&mut *tx)
    .await
    .map_err(sql_error)?;
...
async fn update_existing_streaming_turn_response_outcome(
    ...
    .bind(turn_pk)
    .bind(output_item_pk)
    .bind(usage.input_tokens.saturating_sub(existing_usage.input_tokens))
    .bind(usage.output_tokens.saturating_sub(existing_usage.output_tokens))
    .bind(usage.cached_tokens.saturating_sub(existing_usage.cached_tokens))
    .bind(usage.reasoning_tokens.saturating_sub(existing_usage.reasoning_tokens))
    .bind(&usage.cost_amount)
...
async fn insert_context_snapshot(
    ...
    let context_snapshot_id = next_cloud_runtime_id("ai_chat_context_snapshot")?;
...
async fn insert_usage_link_for_message(
    ...
    .bind(&command.requested_at)
    .bind(metadata)
    .bind(next_cloud_runtime_id("ai_runtime_usage_link")?)
    .execute(&mut **tx)
    .await
    .map_err(sql_error)?;
    Ok(())
...
async fn insert_item(
    ...
    .bind(&command.requested_at)
    .bind(&metadata)
    .bind(next_cloud_runtime_id("ai_chat_item")?)
    .fetch_one(&mut **tx)
    .await
    .map_err(sql_error)
}
