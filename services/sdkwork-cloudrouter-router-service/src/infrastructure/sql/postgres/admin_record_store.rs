...
usage_by_request AS (
    SELECT
        ...
        CAST(COALESCE(SUM(COALESCE(debit_points, 0)), 0) AS TEXT) AS debit_points,
        CAST(COALESCE(SUM(COALESCE(original_currency_amount, 0)), 0) AS TEXT) AS original_currency_amount,
        MAX(original_currency_code) AS original_currency_code,
...
SELECT
    ...
    COALESCE(u.debit_points, '0') AS debit_points,
    COALESCE(u.original_currency_amount, '0') AS original_currency_amount,
    COALESCE(u.original_currency_code, '') AS original_currency_code,
...
fn row_to_log(row: sqlx::postgres::PgRow) -> Result<AdminRecordLogItem, DomainError> {
    Ok(AdminRecordLogItem {
        ...
        original_currency_amount: decimal_string_cell(
            ...
            6,
            "admin record original currency amount",
        )?,
        original_currency_code: string_cell(&row, "original_currency_code"),
