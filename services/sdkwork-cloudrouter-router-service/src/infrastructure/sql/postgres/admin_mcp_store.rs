...
async fn list_servers(
    ...
    let rows = sqlx::query(
        ...
        WHERE tenant_id = $1
          ...
          AND ($3::text IS NULL OR lower(server_key) LIKE $3 OR lower(name) LIKE $3 OR lower(COALESCE(description, '')) LIKE $3)
          AND ($4::text IS NULL OR transport = $4)
...
async fn create_server(
    ...
    let id = next_cloud_runtime_id("ai_mcp_server")?;
    ...
    load_server(pool, command.subject, id).await
}
...
async fn update_server(
    ...
    if let Some(category) = command.category_id {
        ...
        sqlx::query(
            ...
            SET category_id = $1, category_code = $2, updated_at = now()
...
async fn create_revision(
    ...
    let id = next_cloud_runtime_id("ai_mcp_server_revision")?;
    ...
    sqlx::query(
        r#"
        UPDATE ai_mcp_server
        SET latest_revision_id = $1, updated_at = now()
...
async fn check_health(
    ...
    let checked_at: String = sqlx::query_scalar(
        r#"
        SELECT CAST(COALESCE(last_checked_at::text, now()::text) AS TEXT)
        FROM ai_mcp_server
        WHERE tenant_id = $1 AND organization_id = $2 AND id = $3
        "#,
    )
...
async fn update_tool(
    ...
    if let Some(input_schema) = command.input_schema {
        sqlx::query(update_tool_sql("input_schema = $1::jsonb"))
    ...
    if let Some(output_schema) = command.output_schema {
        sqlx::query(update_tool_sql("output_schema = $1::jsonb"))
    ...
    if let Some(rate_limit_policy) = command.rate_limit_policy {
        sqlx::query(update_tool_sql("rate_limit_policy = $1::jsonb"))
...
async fn create_binding(
    ...
    let id = next_cloud_runtime_id("ai_mcp_binding")?;
...
async fn update_binding(
    ...
    sqlx::query(
        ...
        SET server_revision_id = $1,
            ...
            denied_tools = $6::jsonb,
            policy_json = $7::jsonb,
            priority = $8,
            enabled = $9,
            status = $10,
            snapshot_json = $11::jsonb,
            updated_at = now()
        WHERE tenant_id = $12
...
fn row_to_server(row: &sqlx::postgres::PgRow) -> DomainResult<AdminMcpServerItem> {
...
fn row_to_revision(row: &sqlx::postgres::PgRow) -> DomainResult<AdminMcpServerRevisionItem> {
...
fn row_to_tool(row: &sqlx::postgres::PgRow) -> DomainResult<AdminMcpToolItem> {
    Ok(AdminMcpToolItem {
        id: integer_cell(row, "id")?,
        uuid: string_cell(row, "uuid")?,
        tenant_id: integer_cell(row, "tenant_id")?,
...
fn row_to_binding(row: &sqlx::postgres::PgRow) -> DomainResult<AdminMcpBindingItem> {
...
fn update_server_sql(set_clause: &str) -> &'static str {
    match set_clause {
        ...
        "transport = $1" => {
            r#"
            UPDATE ai_mcp_server
            SET transport = $1, updated_at = now()
            WHERE tenant_id = $2 AND organization_id = $3 AND id = $4 AND deleted_at IS NULL
            "#
        }
        "visibility = $1" => {
...
fn update_tool_sql(set_clause: &str) -> &'static str {
    match set_clause {
        ...
        "input_schema = $1::jsonb" => {
            r#"
            UPDATE ai_mcp_tool
            SET input_schema = $1::jsonb, updated_at = now()
            WHERE tenant_id = $2 AND organization_id = $3 AND id = $4 AND deleted_at IS NULL
            "#
        }
        "output_schema = $1::jsonb" => {
            r#"
            UPDATE ai_mcp_tool
            SET output_schema = $1::jsonb, updated_at = now()
            WHERE tenant_id = $2 AND organization_id = $3 AND id = $4 AND deleted_at IS NULL
            "#
        }
        "risk_level = $1" => {
        ...
        "rate_limit_policy = $1::jsonb" => {
            r#"
            UPDATE ai_mcp_tool
            SET rate_limit_policy = $1::jsonb, updated_at = now()
            WHERE tenant_id = $2 AND organization_id = $3 AND id = $4 AND deleted_at IS NULL
            "#
        }
        "sort_weight = $1" => {
            r#"
            UPDATE ai_mcp_tool
            SET sort_weight = $1, updated_at = now()
            WHERE tenant_id = $2 AND organization_id = $3 AND id = $4 AND deleted_at IS NULL
            "#
        }
...
async fn current_timestamp(pool: &PgPool) -> DomainResult<String> {
    sqlx::query_scalar("SELECT CAST(now() AS TEXT)")
        .fetch_one(pool)
        .await
        .map_err(store_error)
}
