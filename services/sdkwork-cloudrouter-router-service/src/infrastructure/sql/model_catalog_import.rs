...
fn vendor_native_video_descriptor(vendor_code: &str) -> Option<EndpointDescriptor> {
    let descriptor = match vendor_code {
        ...
        // `sdkwork-models/models/bytedance/<region>/vendor.json`), whose video
        // models are the `doubao-seedance-*` family. Ark submits an async task
        // and polls it, so a seedance video model binds to the
...
mod tests {
    ...
    fn image_models_bind_to_their_vendor_native_endpoint() {
        ...
        // catalog surface is Volcengine Ark (`/api/v3/...`), while Jimeng is its
        // own consumer host (`/v1/...`). Collapsing them, as the old
        // `"bytedance" | "jimeng"` arm did, bound every Ark image model to a
        // path Ark does not serve.
    ...
    fn video_models_bind_to_their_vendor_native_endpoint() {
        ...
        // against a non-existent endpoint.
        //
        // `alibaba`, `luma_ai`, `pixverse` and `zhipu` moved out of this list:
        // each publishes a video surface the project declares *and* whose own
        // video models declare `apiFormat = vendor_native`, so keeping them
        // here would have been the assertion defending the bug. They are
        // covered by the native table above instead.
    ...
    fn music_models_bind_to_their_vendor_native_endpoint() {
        ...
        // Mureka publishes its own song surface and all 8 of its music models
        // declare `vendor_native`; before its arm existed they all collapsed
        // onto the Suno compatibility face, which speaks a different song
        // protocol. `mureka` therefore left the Suno list below.
        let mureka = model_endpoint_descriptor(&music_model_of("mureka", "vendor_native"));
        assert_eq!(
            mureka.endpoint_code, "mureka.music_generation",
            "mureka music model must reach its own native music route"
        );
        assert_eq!(mureka.protocol_code, "vendor_native");
    ...
    fn every_capability_binds_only_to_a_declared_vendor_native_endpoint() {
        ...
        const DECLARED_VENDOR_NATIVE_ENDPOINTS: &[(&str, &str)] = &[
            ...
            // ByteDance's catalog surface is Volcengine Ark, not Jimeng. The two
            // were aliased together until the seedance video models were found
        ...
        const NOT_BOUND_BY_DESCRIPTOR: &[&str] = &[
            ...
            // endpoints. Same shape as the poll surfaces above: the caller
            // arrives holding a task id produced by a prior create call, so no
            // model's `primaryCapability` selects them. Each has a classifier
            // arm in both copies and a grant in its `official.<vendor>.full`
            // group; they are declared for the classifier, not bound.
            "alibaba.video_generation_task_query",
            "luma_ai.video_generation_task_query",
            "mureka.music_generation_task_query",
            "pixverse.video_generation_task_query",
            ...
            //
            // Verified by sweeping `sdkwork-models/models/<vendor>/**/models/*.json`:
            // `deepseek` chat ×8, `meituan` chat ×2, `moonshot` chat ×6 + code ×6,
            // `stepfun` chat ×3, `tencent` chat ×9 + reasoning ×1, `xai` chat ×5 +
            // image ×3 + reasoning ×2 + video ×2, `xiaomi` audio ×8 + chat ×10,
            // `alibaba` chat ×12 + embedding ×4 + image ×1, `zhipu` chat ×8 +
            // embedding ×2 + image ×2 — all `openai_compatible`.
            "alibaba.chat_completions",
            "alibaba.embeddings",
            "alibaba.image_generation",
