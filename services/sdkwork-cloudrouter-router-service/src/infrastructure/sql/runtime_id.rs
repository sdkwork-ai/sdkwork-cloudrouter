...
const CLOUD_RUNTIME_NODE_ID_ENV: &str = "SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID";
const MAX_CLOUD_RUNTIME_NODE_ID: u16 = 1023;
    ...
    METRIC
        .get_or_init(|| {
            let metric = prometheus::IntGauge::new(
                "cloudrouter_runtime_id_generator_ready",
    ...
    METRIC
        .get_or_init(|| {
            let metric = prometheus::IntCounterVec::new(
                prometheus::Opts::new(
                    "cloudrouter_runtime_id_failures_total",
...
static CLOUD_RUNTIME_ID_MANAGER: OnceLock<RuntimeIdManager> = OnceLock::new();
...
pub async fn bootstrap_cloud_runtime_id_generator(
    pool: &DatabasePool,
    service_name: &str,
...
pub(crate) fn cloud_runtime_id_is_healthy() -> bool {
...
/// `SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID` is accepted only for single-process desktop development.
pub fn validate_cloud_runtime_id_configuration(
...
fn runtime_id_manager() -> &'static RuntimeIdManager {
    CLOUD_RUNTIME_ID_MANAGER.get_or_init(RuntimeIdManager::new)
}
...
fn build_local_development_generator() -> Result<SnowflakeIdGenerator, RuntimeIdConfigurationError>
{
    ...
    SnowflakeIdGenerator::new(node_id.value()).map_err(|error| {
        RuntimeIdConfigurationError::new(format!(
            "{CLOUD_RUNTIME_NODE_ID_ENV} is invalid for Cloud runtime IDs: {error:?}"
        ))
    })
}
...
fn spawn_runtime_id_lease_recovery(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if cloud_runtime_id_is_healthy() {
...
fn configured_runtime_node_id_from_env() -> Result<Option<String>, RuntimeIdConfigurationError> {
    Ok(match std::env::var(CLOUD_RUNTIME_NODE_ID_ENV) {
        Ok(value) => Some(value),
        Err(std::env::VarError::NotPresent) => None,
        Err(std::env::VarError::NotUnicode(_)) => {
            return Err(RuntimeIdConfigurationError::new(format!(
                "{CLOUD_RUNTIME_NODE_ID_ENV} must contain valid Unicode"
            )));
        }
    })
}
    ...
    F: FnOnce() -> Result<u16, RuntimeIdConfigurationError>,
{
    ...
    match configured_runtime_node_id(configured_node_id, deployment_mode)? {
        ...
        None => {
            ...
            if node_id > MAX_CLOUD_RUNTIME_NODE_ID {
                return Err(RuntimeIdConfigurationError::new(format!(
                    "desktop development Snowflake node id must be between 0 and {MAX_CLOUD_RUNTIME_NODE_ID}"
                )));
            }
...
fn configured_runtime_node_id(
    ...
    if deployment_mode != DeploymentMode::Desktop {
        return Err(RuntimeIdConfigurationError::new(format!(
            "{CLOUD_RUNTIME_NODE_ID_ENV} is a desktop development override and must not be configured for {}; clustered runtimes use database-leased node IDs",
            deployment_mode.as_str()
        )));
    }
    Ok(Some(node_id))
...
fn parse_runtime_node_id(value: &str) -> Result<u16, RuntimeIdConfigurationError> {
    ...
    if value.is_empty() {
        return Err(RuntimeIdConfigurationError::new(format!(
            "{CLOUD_RUNTIME_NODE_ID_ENV} must be an integer between 0 and {MAX_CLOUD_RUNTIME_NODE_ID}"
        )));
    }
...
fn local_development_node_id() -> Result<u16, RuntimeIdConfigurationError> {
    ...
    Ok(u16::from_le_bytes(bytes) & MAX_CLOUD_RUNTIME_NODE_ID)
}
...
mod tests {
    ...
    fn standard_postgres_config_materializes_canonical_only_workspace_search_path() {
        ...
        let config = sdkwork_cloudrouter_config::DatabaseConfig {
            engine: sdkwork_cloudrouter_config::DatabaseEngine::Postgres,
        ...
        for value in ["", "not-a-number", "1024"] {
            let error =
                validate_runtime_node_id_configuration(Some(value), DeploymentMode::Desktop)
                    ...
                    .to_string();
            assert!(error.contains(CLOUD_RUNTIME_NODE_ID_ENV));
        }
    }
    ...
    fn runtime_id_metrics_are_registered_with_bounded_failure_labels() {
        ...
        let readiness = metric_families
            .iter()
            .find(|family| family.get_name() == "cloudrouter_runtime_id_generator_ready")
        ...
        let failures = metric_families
            .iter()
            .find(|family| family.get_name() == "cloudrouter_runtime_id_failures_total")
