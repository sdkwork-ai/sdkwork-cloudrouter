...
pub fn shared_password_login_rate_limiter(
    ...
    let deployment_mode =
        sdkwork_cloudrouter_config::DeploymentMode::from_env_or_runtime_toml(runtime_toml)
    ...
    if deployment_mode != sdkwork_cloudrouter_config::DeploymentMode::Desktop
        && sdkwork_cloudrouter_config::is_production_like_runtime_environment(runtime_toml)
...
impl PasswordLoginRateLimiter {
    ...
    pub fn try_with_redis_config(redis_config: Option<&RedisConfig>) -> Self {
        let distributed = redis_config.and_then(|config| {
            let prefix = format!(
                "{}:password-login",
                config.key_prefix().unwrap_or("cloudrouter")
            );
