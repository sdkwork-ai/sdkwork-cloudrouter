...
/// Alert on `cloudrouter_rate_limit_redis_degraded == 1` to catch silent
    ...
    GAUGE
        .get_or_init(|| {
            let gauge = IntGauge::with_opts(
                Opts::new(
                    "cloudrouter_rate_limit_redis_degraded",
...
impl GatewayInvocationRateLimiter {
    ...
    pub fn try_with_redis_config_and_instances(
        ...
        let (distributed, degraded) = match redis_config {
            Some(config) => {
                let prefix = config.key_prefix().unwrap_or("cloudrouter").to_owned();
