...
impl UsageSettlementWorkerConfig {
    pub fn validate_for_deployment(&self) -> Result<(), String> {
        ...
        Err(
            "usage settlement worker requires SDKWORK_CLOUDROUTER_USAGE_SETTLEMENT_TENANT_ID > 0 or explicit SDKWORK_CLOUDROUTER_USAGE_SETTLEMENT_PLATFORM_SCOPE=true when enabled"
                .to_owned(),
        )
    }
}
...
fn platform_settlement_scope_allowed() -> bool {
    std::env::var("SDKWORK_CLOUDROUTER_USAGE_SETTLEMENT_PLATFORM_SCOPE")
        .ok()
        .map(|value| {
            matches!(
        ...
        .get_or_init(|| {
            let metric = prometheus::HistogramVec::new(
                prometheus::HistogramOpts::new(
                    "cloudrouter_usage_settlement_duration_seconds",
