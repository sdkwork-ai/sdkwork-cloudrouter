# SDKWork Cloud Product Component Specs

This directory is the local standards index for `sdkwork-cloudrouter-router-service`.
...
| Field | Value |
| --- | --- |
| Name | `sdkwork-cloudrouter-router-service` |
| Type | `rust-crate` |
| Root | `sdkwork-cloudrouter/services/sdkwork-cloudrouter-router-service` |
| Domain | `platform` |
| Capability | `router` |
| Languages | `rust` |
...
`cloudrouter_runtime_id_failures_total{operation,reason}` with bounded operational labels. Raw
...
`SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID` is a single-process desktop development override only and is
...
Cloud Router runtime role is not an accepted workaround.
