...
fn category_row_from_query(row: &sqlx::sqlite::SqliteRow) -> DomainResult<CategoryListRow> {
    Ok(CategoryListRow {
...
async fn fetch_category_ancestor_rows(
    ...
    let seed_placeholders = seed_ids
        .iter()
        .enumerate()
    ...
    let sql = format!(
        ...
        WITH RECURSIVE ancestors AS (
            ...
            WHERE c.tenant_id = CAST(?1 AS TEXT)
              AND (c.organization_id = CAST(?2 AS TEXT) OR c.organization_id IS NULL)
        )
        SELECT DISTINCT
            id,
            category_no,
    ...
    for seed_id in seed_ids {
        query = query.bind(seed_id);
    }

    let rows = query.fetch_all(pool).await.map_err(store_error)?;
