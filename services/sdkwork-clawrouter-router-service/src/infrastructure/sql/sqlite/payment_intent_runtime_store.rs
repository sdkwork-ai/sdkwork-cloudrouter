...
#[derive(Debug, Clone)]
pub struct SqlitePaymentIntentRuntimeStore {
    pool: SqlitePool,
}

impl SqlitePaymentIntentRuntimeStore {
    pub fn new(pool: SqlitePool) -> Self {
        Self { pool }
    }
}

impl PaymentIntentRuntimeStore for SqlitePaymentIntentRuntimeStore {
...
impl PaymentRefundRuntimeStore for SqlitePaymentIntentRuntimeStore {
...
async fn load_refund_by_idempotency(
    ...
    let row = sqlx::query(
        ...
        SELECT id, tenant_id, organization_id, payment_intent_id, payment_attempt_id, refund_no,
               amount, currency_code, provider_code, reason, status, idempotency_key, created_at, updated_at
...
async fn load_refund_items(
    ...
    let rows = sqlx::query(
        ...
        SELECT id, tenant_id, organization_id, refund_id, order_item_id, quantity,
               refund_amount, tax_refund_amount, shipping_refund_amount, created_at
...
async fn finish_refund_attempt(
    ...
    let row = sqlx::query(
        ...
        SELECT id, tenant_id, organization_id, refund_attempt_no, refund_id, provider_code,
               ...
               status, failure_code, failure_message, submitted_at, succeeded_at, failed_at,
...
fn intent_from_row(row: &sqlx::sqlite::SqliteRow) -> DomainResult<PaymentIntentRuntimeRecord> {
...
fn refund_item_from_row(row: &sqlx::sqlite::SqliteRow) -> DomainResult<PaymentRefundItemRecord> {
...
fn refund_from_row(row: &sqlx::sqlite::SqliteRow) -> DomainResult<PaymentRefundRuntimeRecord> {
    let status = match string_cell(row, "status").as_str() {
