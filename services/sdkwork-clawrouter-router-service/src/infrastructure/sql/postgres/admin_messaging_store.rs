...
#[derive(Debug, Clone)]
pub struct PostgresAdminMessagingStore {
    pool: PgPool,
}

impl PostgresAdminMessagingStore {
    pub fn new(pool: PgPool) -> Self {
        Self { pool }
    }
}

impl AdminMessagingStore for PostgresAdminMessagingStore {
...
async fn create_provider_account(
    ...
    .is_none()
    {
        sqlx::query(
            ...
            VALUES
                ($1, $2, $3, 1, $4, $5, $6, $7, $8::jsonb, $9, $10, $11, $12, $13, $14, $15)
            "#,
        )
        .bind(stable_uuid(
...
async fn list_templates(
    ...
    let rows = sqlx::query(
        ...
        WHERE t.tenant_id = $1
          ...
          AND ($4::text IS NULL OR t.channel = $4)
          AND ($5::text IS NULL OR t.scene_code = $5)
          AND ($6::text IS NULL OR lower(t.template_code) LIKE $6 OR lower(t.template_name) LIKE $6)
...
async fn create_template(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, $2, $3, 1, $4, 1, $5, $6, $7, $8::jsonb, $9, $10)
        "#,
    )
    .bind(stable_uuid(
...
async fn list_route_rules(
    ...
    let rows = sqlx::query(&format!(
        ...
        WHERE r.tenant_id = $1
          ...
          AND ($6::text IS NULL OR lower(r.rule_code) LIKE $6)
        ORDER BY r.priority ASC, r.updated_at DESC NULLS LAST, r.id DESC
        LIMIT $7 OFFSET $8
        "#,
...
async fn create_route_rule(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, $2, $3, 1, $4, $5, $6, $7, $8, $9, $10, $11, $12::jsonb, $13)
        "#,
    )
    .bind(stable_uuid(
...
async fn list_suppressions(
    ...
    let rows = sqlx::query(&format!(
        ...
        WHERE s.tenant_id = $1
          ...
          AND ($3::text IS NULL OR {status_label} = $3)
          AND ($4::text IS NULL OR s.channel = $4)
          AND ($5::text IS NULL OR s.target_hash = $5)
          AND ($6::text IS NULL OR s.reason_code = $6)
        ORDER BY s.updated_at DESC NULLS LAST, s.id DESC
        LIMIT $7 OFFSET $8
        "#,
        status_label = status_label_sql("s.status")
...
async fn create_suppression(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, $2, $3, 1, $4, $5, $6, $7, $8, $9, $10::timestamptz, $11::timestamptz, $12, $13, $14)
        "#,
    )
    .bind(stable_uuid(
...
async fn list_rate_limit_buckets(
    ...
    let rows = sqlx::query(&format!(
        ...
        WHERE b.tenant_id = $1
          ...
          AND ($3::text IS NULL OR b.scene_code = $3)
          AND ($4::text IS NULL OR b.channel = $4)
          AND ($5::text IS NULL OR b.target_hash = $5)
          AND ($6::text IS NULL OR b.ip_hash = $6)
          AND ($7::text IS NULL OR b.device_hash = $7)
...
async fn list_verification_policies(
    ...
    let rows = sqlx::query(&format!(
        ...
        WHERE p.tenant_id = $1
          ...
          AND ($3::text IS NULL OR {status_label} = $3)
          AND ($4::text IS NULL OR p.default_channel = $4)
          AND ($5::text IS NULL OR p.scene_code = $5)
          AND ($6::text IS NULL OR lower(p.scene_code) LIKE $6 OR lower(COALESCE(p.scene_name, '')) LIKE $6)
...
async fn increment_rate_limit_bucket(
    ...
    sqlx::query(
        ...
        VALUES
            ($1, $2, $3, 1, $4, $5, $6, '*', '*',
             date_trunc('hour', CURRENT_TIMESTAMP), 3600, $7, 0, $8, CURRENT_TIMESTAMP, $9)
    ...
    .execute(pool)
    .await
    .map_err(|error| write_error("failed to upsert messaging rate-limit bucket", error))?;
    Ok(())
}
...
async fn load_policy_lookup(
    ...
    sqlx::query(
        ...
        WHERE tenant_id = $1
          AND organization_id = $2
          AND deleted_at IS NULL
          AND (($3::bigint IS NOT NULL AND id = $3) OR scene_code = $4)
        LIMIT 1
        "#,
    )
    .bind(subject.tenant_id)
