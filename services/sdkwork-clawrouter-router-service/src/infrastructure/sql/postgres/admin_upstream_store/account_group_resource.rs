...
pub(super) async fn replace(
    ...
    let mut tx = pool.begin().await.map_err(|error| {
        store_error("failed to begin account group resource replacement", error)
    })?;
    ...
    for item in &items {
        let binding_id = next_claw_runtime_id("upstream account group resource")?;
        sqlx::query(
            r#"
        ...
        .execute(&mut *tx)
        .await
        .map_err(|error| store_error("failed to upsert account group resource", error))?;
    }
    ...
    tx.commit().await.map_err(|error| {
        store_error("failed to commit account group resource replacement", error)
    })?;
    Ok(result)
}
...
async fn retire_omitted(
    ...
    .execute(&mut **tx)
    .await
    .map_err(|error| store_error("failed to retire omitted account group resources", error))?;
    Ok(())
}
...
async fn list_in_transaction(
    ...
    let rows = sqlx::query(&sql)
        ...
        .fetch_all(&mut **tx)
        .await
        .map_err(|error| store_error("failed to reload account group resources", error))?;
    rows.into_iter().map(map_resource_row).collect()
}
