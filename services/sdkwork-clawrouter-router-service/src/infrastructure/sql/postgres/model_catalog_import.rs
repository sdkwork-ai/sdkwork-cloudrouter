...
async fn load_vendor_ids(conn: &mut PgConnection) -> Result<BTreeMap<String, i64>, sqlx::Error> {
    ...
    Ok(rows
        .into_iter()
        .map(|row| Ok((row.get("vendor_code"), postgres_row_i64(&row, "id")?)))
...
async fn load_model_ids(conn: &mut PgConnection) -> Result<BTreeMap<String, i64>, sqlx::Error> {
    ...
    Ok(rows
        .into_iter()
        .map(|row| Ok((row.get("catalog_key"), postgres_row_i64(&row, "id")?)))
        .collect::<Result<BTreeMap<_, _>, sqlx::Error>>()?)
}
...
async fn load_modality_ids(conn: &mut PgConnection) -> Result<BTreeMap<String, i64>, sqlx::Error> {
    ...
    Ok(rows
        .into_iter()
        .map(|row| Ok((row.get("modality_code"), postgres_row_i64(&row, "id")?)))
        .collect::<Result<BTreeMap<_, _>, sqlx::Error>>()?)
}
...
async fn load_api_endpoint_ids(
    ...
    Ok(rows
        .into_iter()
        .map(|row| Ok((row.get("endpoint_code"), postgres_row_i64(&row, "id")?)))
        .collect::<Result<BTreeMap<_, _>, sqlx::Error>>()?)
}
