...
use sdkwork_clawrouter_router_service::ports::{ResponsesRelay, ResponsesRelayRequest};
use serde_json::json;
...
impl ResponsesRelay for RecordingResponsesRelay {
    ...
    ) -> sdkwork_clawrouter_router_service::ports::ResponsesRelayFuture<'a> {
        self.captured.lock().unwrap().push(request);
        Box::pin(async {
            Ok(
                sdkwork_clawrouter_router_service::ports::ResponsesRelayResponse::json(
                    200,
                    json!({
                        "id": "resp-direct",
                        "object": "response",
...
async fn openai_responses_registry_hit_calls_internal_adapter_without_direct_relay() {
    ...
    let adapter_relay =
        sdkwork_clawrouter_router_service::infrastructure::provider::AdapterAwareResponsesRelay::new(
            relay,
