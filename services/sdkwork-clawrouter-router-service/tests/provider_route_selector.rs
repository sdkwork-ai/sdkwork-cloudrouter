use sdkwork_clawrouter_router_service::application::{
    ...
    SelectUpstreamAccountRouteQuery, SelectProviderRouteQuery,
};
use sdkwork_clawrouter_router_service::domain::{
    AiModel, BillingMeter, UpstreamAccountGroup, DecimalValue, GatewayApiKey,
    GatewayApiKeyAccountGroupBinding, ModelMappingBindingType, ModelMappingRule, ModelPrice,
    ModelUpstreamRoute, ModelVendor, ModelVendorDefinition, Money, PriceSide, PricingPlan,
    UpstreamAccountRoute, ResolveModelMappingContext, RouteCandidate, RoutingCapability,
...
fn add_callable_route_for_api(
    ...
    catalog.add_provider_route(
        ...
        .with_upstream_endpoint(
            Some(format!("http://provider-proxy.internal/{supplier_code}")),
            Some(format!("vault://providers/{supplier_code}/account/main")),
        ),
    );
    catalog.add_price(
...
fn add_callable_credential_route(
    ...
    let secret_ref = format!("vault://providers/{supplier_code}/account/{credential_id}");
    let base_url = format!("http://provider-proxy.internal/{supplier_code}/{credential_id}");
    catalog.add_provider_route(
        ModelUpstreamRoute::new_for_catalog_key(
            "openai/gpt-4o-mini",
            "gpt-4o-mini",
        ...
        .with_upstream_endpoint(Some(base_url.clone()), Some(secret_ref.clone())),
    );
...
fn add_target_catalog_model(catalog: &mut InMemoryPricingCatalog) {
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("anthropic-direct", 4001).with_upstream_endpoint(
            Some("http://provider-proxy.internal/anthropic"),
            Some("vault://providers/anthropic/account/main"),
        ),
    );
...
fn add_group_policy_rule(
    ...
    catalog.add_routing_rule(
        ...
        .with_candidate_account_groups(candidate_account_groups)
...
fn select_channel_route_query(route_key: &str) -> SelectUpstreamAccountRouteQuery {
...
#[test]
fn selector_prefers_upstream_account_group_policy_over_global_policy() {
    ...
    assert_eq!(3002, selection.route.account_id);
...
fn selector_prefers_policy_matching_the_request_capability() {
    ...
    assert_eq!(3001, selection.route.account_id);
    assert_eq!(Some(2), selection.policy_id);
    assert_eq!(Some(202), selection.rule_id);
}
...
fn selector_uses_configured_fallback_chain_without_legacy_cross_pool_fallback() {
    ...
    assert_eq!(3002, selection.route.account_id);
    assert_eq!("openrouter-fallback", selection.route.supplier_code);
}
...
fn selector_plan_includes_primary_and_enabled_fallback_candidates() {
    ...
    let account_ids = plan
        .routes
        .iter()
        .map(|selection| selection.route.account_id)
        .collect::<Vec<_>>();
    assert_eq!(vec![3001, 3002], account_ids);
...
fn selector_plan_respects_policy_fallback_mode_none() {
    ...
    let account_ids = plan
        ...
        .map(|selection| selection.route.account_id)
        .collect::<Vec<_>>();
    assert_eq!(vec![3001], account_ids);
}
...
fn selector_prices_candidate_routes_with_canonical_catalog_key_and_region_context() {
    ...
    assert_eq!(4001, selection.route.account_id);
    assert_eq!("minimax_cn_direct", selection.route.supplier_code);
...
fn selector_selects_channel_route_by_route_key_without_model_pricing() {
    ...
    assert_eq!(3001, selection.route.account_id);
    assert_eq!("openrouter-main", selection.route.supplier_code);
    assert_eq!(Some(2), selection.policy_id);
    assert_eq!(Some(202), selection.rule_id);
}
...
fn selector_synthetic_model_route_preserves_channel_route_region_context() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("minimax-upstream", 4001)
            .with_region_code("cn")
            .with_upstream_endpoint(
    ...
    assert_eq!("minimax-upstream", plan.routes[0].route.supplier_code);
    assert_eq!(4001, plan.routes[0].route.account_id);
}
...
fn selector_routes_group_bound_channel_route_by_route_key_without_explicit_policy_rule() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-group-bound", 3002)
            .with_upstream_endpoint(
                Some("http://provider-proxy.internal/openrouter-group-bound"),
                Some("vault://providers/openrouter-group-bound/account/main"),
            )
            .with_resource_scoped_account_group_binding(10, 1, 100, Vec::<String>::new(), vec!["llm"]),
    );
    ...
    assert_eq!(3002, selection.route.account_id);
    assert_eq!("openrouter-group-bound", selection.route.supplier_code);
    assert_eq!(None, selection.policy_id);
    assert_eq!(None, selection.rule_id);
}

#[test]
fn selector_prefers_upstream_account_group_channel_route_over_global_policy() {
    let mut catalog = base_catalog();
...
fn selector_restricts_channel_route_candidates_to_group_channel_bindings() {
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-group-bound", 3002)
            ...
            .with_account_group_binding(10, 1, 100),
    );
    add_group_policy_rule(
        &mut catalog,
        2,
...
fn selector_routes_slash_native_catalog_model_through_resource_scoped_channel_route() {
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter", 3101)
            .with_upstream_endpoint(
    ...
    assert_eq!(3101, selection.route.account_id);
    assert_eq!("openrouter", selection.route.supplier_code);
    assert_eq!(
...
fn selector_routes_regional_catalog_model_through_group_bound_channel_route_region_scope() {
    ...
    assert_eq!(3101, selection.route.account_id);
    assert_eq!("dashscope", selection.route.supplier_code);
    assert_eq!(
...
#[test]
fn selector_explains_when_channel_route_exists_but_upstream_account_group_has_no_matching_binding() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter", 3001)
            .with_upstream_endpoint(
            ...
            .with_resource_scoped_account_group_binding(99, 1, 100, Vec::<String>::new(), vec!["llm"]),
    );
    ...
    assert!(!message.contains("account_group_id"), "{message}");
    assert!(!message.contains("channel_routes_loaded"), "{message}");
    assert!(!message.contains("any_account_group_bindings"), "{message}");
    assert!(
...
#[test]
fn selector_restricts_model_route_account_group_bindings_by_api_scope() {
    let mut catalog = base_catalog();
    add_callable_route(
        &mut catalog,
        3001,
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-other-model", 3001)
            .with_upstream_endpoint(
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-bound-model", 3002)
            .with_upstream_endpoint(
    ...
    assert_eq!(3002, selection.route.account_id);
    assert_eq!("openrouter-bound-model", selection.route.supplier_code);
}
...
fn selector_restricts_upstream_account_account_group_bindings_by_capability_scope() {
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-image-only", 3001)
            ...
            .with_resource_scoped_account_group_binding(10, 1, 100, Vec::<String>::new(), vec!["image"]),
    );
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-llm", 3002)
            .with_upstream_endpoint(
                Some("http://provider-proxy.internal/openrouter-llm"),
                Some("vault://providers/openrouter-llm/account/main"),
            )
            .with_resource_scoped_account_group_binding(10, 50, 1, Vec::<String>::new(), vec!["llm"]),
    );
    add_group_policy_rule(
        &mut catalog,
        2,
    ...
    assert_eq!(3002, selection.route.account_id);
    assert_eq!("openrouter-llm", selection.route.supplier_code);
}
...
fn selector_restricts_upstream_account_account_group_bindings_by_api_scope() {
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-files-api", 3002)
            .with_upstream_endpoint(
                Some("http://provider-proxy.internal/openrouter-files-api"),
                Some("vault://providers/openrouter-files-api/account/main"),
            )
            .with_resource_scoped_account_group_binding(10, 50, 1, vec!["api.openai.files"], vec!["llm"]),
    );
    add_group_policy_rule(
        &mut catalog,
        2,
    ...
    assert_eq!(3002, selection.route.account_id);
    assert_eq!("openrouter-files-api", selection.route.supplier_code);
}
...
fn selector_rejects_route_key_alias_when_standard_api_code_scope_does_not_match() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-route-key-alias", 3001)
            .with_upstream_endpoint(
...
fn selector_routes_provider_native_channel_by_exact_standard_api_code_only() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("kling-generic-image", 3001)
            .with_upstream_endpoint(
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("kling-video", 3002)
            .with_upstream_endpoint(
    ...
    assert_eq!(3002, selection.route.account_id);
    assert_eq!("kling-video", selection.route.supplier_code);
}
...
fn selector_matches_modeless_channel_route_when_api_scope_matches() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-files-api", 3001)
            .with_upstream_endpoint(
                Some("http://provider-proxy.internal/openrouter-files-api"),
                Some("vault://providers/openrouter-files-api/account/main"),
            )
            .with_resource_scoped_account_group_binding(10, 1, 100, vec!["openai.files"], vec!["network"]),
    );
    add_group_policy_rule(
        &mut catalog,
        2,
...
fn selector_allows_capability_scoped_channel_route_when_api_scope_is_unrestricted() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-network", 3001)
            .with_upstream_endpoint(
                Some("http://provider-proxy.internal/openrouter-network"),
                Some("vault://providers/openrouter-network/account/main"),
            )
            .with_resource_scoped_account_group_binding(10, 1, 100, Vec::<String>::new(), vec!["network"]),
    );
    add_group_policy_rule(
        &mut catalog,
        2,
    ...
    assert_eq!(3001, selection.route.account_id);
    assert_eq!("openrouter-network", selection.route.supplier_code);
}
...
fn selector_matches_upstream_account_account_group_bindings_by_standard_api_code() {
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-files-other-api", 3001)
            ...
            .with_resource_scoped_account_group_binding(10, 1, 100, vec!["openai.responses"], vec!["llm"]),
    );
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-files-api", 3002)
            .with_upstream_endpoint(
            ...
            .with_resource_scoped_account_group_binding(10, 50, 1, vec!["openai.files"], vec!["llm"]),
    );
    add_group_policy_rule(
        &mut catalog,
        2,
...
#[test]
fn selector_restricts_model_account_group_bindings_by_api_scope() {
    let mut catalog = base_catalog();
    add_callable_route_for_api(
        &mut catalog,
        3001,
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-responses-only", 3001)
            .with_upstream_endpoint(
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-chat-completions", 3002)
            .with_upstream_endpoint(
    ...
    assert_eq!(3002, selection.route.account_id);
    assert_eq!("openrouter-chat-completions", selection.route.supplier_code);
}
...
fn selector_prefers_bound_channel_route_priority_over_rule_candidate_weight() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-low-priority", 3001)
            .with_upstream_endpoint(
    ...
    assert_eq!(3002, selection.route.account_id);
    assert_eq!("openrouter-high-priority", selection.route.supplier_code);
}
...
fn selector_uses_bound_channel_route_weight_when_priorities_match() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-lower-weight", 3001)
            .with_upstream_endpoint(
    ...
    assert_eq!(3002, selection.route.account_id);
    assert_eq!("openrouter-higher-weight", selection.route.supplier_code);
}

#[test]
fn selector_rejects_channel_route_when_account_group_bindings_exist_but_not_for_upstream_account_group() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-other-group", 3001)
            .with_upstream_endpoint(
                Some("http://provider-proxy.internal/openrouter-other-group"),
                Some("vault://providers/openrouter-other-group/account/main"),
            )
            .with_account_group_binding(99, 1, 100),
    );
    add_group_policy_rule(
        &mut catalog,
        2,
...
#[test]
fn selector_rejects_model_route_when_account_group_bindings_exist_but_not_for_upstream_account_group() {
    let mut catalog = base_catalog();
    add_callable_route(
        &mut catalog,
        3001,
...
fn selector_chooses_route_from_explicit_api_key_upstream_account_account_group_bindings() {
    ...
    catalog.add_upstream_account_route(
        UpstreamAccountRoute::new("openrouter-premium", 3002)
            .with_upstream_endpoint(
    ...
    assert_eq!(3002, selection.route.account_id);
    assert_eq!("openrouter-premium", selection.route.supplier_code);
...
fn selector_falls_back_to_global_default_channel_route_when_group_has_no_route_key_rule() {
    ...
    assert_eq!(3001, selection.route.account_id);
    assert_eq!("openrouter-default", selection.route.supplier_code);
...
fn selector_rejects_channel_route_candidate_without_callable_endpoint() {
    let mut catalog = base_catalog();
    catalog.add_upstream_account_route(UpstreamAccountRoute::new("openrouter-main", 3001));
    add_group_policy_rule(
        &mut catalog,
        2,
        201,
...
fn catalog_resolves_global_model_mapping_before_route_selection() {
    ...
    assert_eq!("anthropic-direct", selection.routes[0].route.supplier_code);
}
...
#[test]
fn catalog_model_mapping_matches_upstream_account_group_and_account_codes_from_context() {
    let mut catalog = base_catalog();
    catalog.add_model_mapping(
        ModelMappingRule::new(
            10,
