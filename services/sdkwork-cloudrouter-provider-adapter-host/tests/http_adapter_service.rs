...
async fn adapter_service_exposes_health_and_manifest() {
    let router = sdkwork_cloudrouter_provider_adapter_host::router_with_adapters(
...
async fn adapter_service_default_manifest_composes_provider_packages_without_false_endpoint_claims()
{
    ...
    let cloud_storage = providers
        .iter()
        .find(|provider| provider["package"] == "sdkwork-cloudrouter-storage")
    ...
    let cloud_iaas = providers
        .iter()
        .find(|provider| provider["package"] == "sdkwork-cloudrouter-iaas")
...
fn adapter_service_gateway_token_can_be_read_from_env_file() {
    ...
    let token = sdkwork_cloudrouter_provider_adapter_host::gateway_token_from_env().unwrap();
...
fn adapter_service_bind_addr_uses_default_or_env_override() {
    ...
    let default_bind = sdkwork_cloudrouter_provider_adapter_host::bind_addr_from_env().unwrap();
    assert_eq!("0.0.0.0:39110", default_bind);

    std::env::set_var(
        sdkwork_cloudrouter_provider_adapter_host::ENV_PROVIDER_ADAPTER_BIND,
        "127.0.0.1:49110",
    );
    let override_bind = sdkwork_cloudrouter_provider_adapter_host::bind_addr_from_env().unwrap();
...
fn adapter_service_bind_addr_rejects_invalid_env_override() {
    ...
    let error = sdkwork_cloudrouter_provider_adapter_host::bind_addr_from_env().unwrap_err();
...
fn clear_bind_env() {
    std::env::remove_var(sdkwork_cloudrouter_provider_adapter_host::ENV_PROVIDER_ADAPTER_BIND);
}

fn unique_secret_path(name: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!(
        "cloudrouter-{name}-{}-{}.secret",
        std::process::id(),
...
fn cloud_storage_openapi_operations() -> BTreeMap<String, CloudStorageOpenApiOperation> {
    let spec: serde_json::Value = serde_json::from_str(include_str!(
        "../../../crates/sdkwork-cloudrouter-http/specs/cloud-services-openapi.json"
    ))
