...
fn require_postgres_installer_database(config: &DatabaseConfig) -> anyhow::Result<()> {
    ...
    Err(InstallerCliError::InvalidArgument(
        "cloudrouterctl requires PostgreSQL because Cloud Router server data is authoritative"
            .to_owned(),
    )
    .into())
}
...
fn database_connection_error_message(database_url: &str, error: impl std::fmt::Display) -> String {
    format!(
        ...
        sdkwork_cloudrouter_router_service::infrastructure::sql::pool::POSTGRES_POOL_ACQUIRE_TIMEOUT_SECONDS
    )
}
...
fn read_reset_admin_password() -> anyhow::Result<String> {
    let password = std::env::var(SDKWORK_CLOUDROUTER_ADMIN_RESET_PASSWORD_ENV)
        .ok()
        .map(|value| value.trim().to_owned())
        .filter(|value| !value.is_empty())
        .ok_or_else(|| {
            InstallerCliError::InvalidArgument(format!(
                "{SDKWORK_CLOUDROUTER_ADMIN_RESET_PASSWORD_ENV} is required for reset-admin"
            ))
        })?;
    if password.len() < 8 {
        return Err(InstallerCliError::InvalidArgument(
...
mod tests {
    ...
    fn installer_rejects_sqlite_as_non_authoritative_server_storage() {
        ...
        assert!(error
            .to_string()
            .contains("cloudrouterctl requires PostgreSQL"));
    }
}
