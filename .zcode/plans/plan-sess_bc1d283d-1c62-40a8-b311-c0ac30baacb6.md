...
- SDK 链:`apis/...openapi.json` → `@sdkwork/cloudrouter-app-sdk`(生成物)→ console-core/sdk → 前端

## 实施方案

### 1. 配置(sdkwork-cloudrouter-config `api_key.rs`)
- `ApiKeySecurityConfig` 增加 `secret_storage_mode: ApiKeySecretStorageMode`(Plaintext 默认 | Ciphertext)
- 环境变量 `SDKWORK_CLOUDROUTER_API_KEY_SECRET_STORAGE=plaintext|ciphertext`,默认 `plaintext`;ciphertext 模式 AEAD 密钥由现有 `SDKWORK_CLOUDROUTER_API_KEY_PEPPER` 经领域分离 KDF 派生

### 2. 数据库(cloudrouter 模块)
- 基线 DDL `0001_cloudrouter_baseline.sql` + 新迁移 `0008_add_api_key_secret_storage.up.sql`,给 `iam_gateway_api_key` 加列:
...
- 新增 `ApiKeySecretCodec` trait + `RingAeadApiKeySecretCodec`(仿照凭据 codec):`encode(context, secret)` / `decode(context, key_id, ciphertext)`,AAD 绑定 (tenant_id, organization_id, api_key_id),KDF 域 `sdkwork-cloudrouter:api-key-secret:v1`
...
### 6. API 契约 + SDK
- 更新 `apis/app-api/cloudrouter/cloudrouter-app-api.openapi.json`:`AppApiKeyItem` 增加 `rawKey`(nullable string)
- 用 `cloudrouter-sdk-generation` skill 重新生成 `@sdkwork/cloudrouter-app-sdk`
