...
class CloudRouterSdkGuardianTest(unittest.TestCase):
    def write_sdk(
        self,
        root: Path,
        sdk_dir: str,
        ...
        *,
        ...
        sdkgen_input_path_line = (
            ...
            if sdk_dir == "cloudrouter-open-sdk"
            else ""
        )
        ...
        if sdk_dir == "cloudrouter-backend-sdk":
            sdk_source = (
        ...
        if sdk_dir == "cloudrouter-app-sdk":
    ...
    def write_portal_sdk_boundary(self, root: Path) -> None:
        open_sdk = root / "sdks" / "cloudrouter-open-sdk" / "cloudrouter-open-sdk-typescript"
        ...
        (portal / "package.json").write_text(
            json.dumps(
                {
                    "dependencies": {
                        "@sdkwork/cloudrouter-app-sdk": "workspace:*",
                        "@sdkwork/cloudrouter-backend-sdk": "workspace:*",
                        "@sdkwork/cloudrouter-open-sdk": "workspace:*",
                    }
                }
            )
            + "\n",
        ...
        (commons / "src" / "sdk-clients.ts").write_text(
            ...
            "export function createCloudRouterAppSdkClient() { return new SdkworkAppClient({ baseUrl: '' }); }\n"
            "export function createCloudRouterBackendSdkClient() { return new SdkworkBackendClient({ baseUrl: '' }); }\n"
            "export function createCloudRouterAiSdkClient() { return new SdkworkAiClient({ baseUrl: '' }); }\n",
            encoding="utf-8",
        )
    ...
    def test_reports_missing_required_sdk_system_and_wrong_package_name(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.write_sdk(root, "cloudrouter-app-sdk", "@sdkwork/wrong", "app", "SdkworkAppClient", "/app/v3/api")
            self.write_portal_sdk_boundary(root)
            ...
            self.assertIn(f"generated SDK family is missing: {root / 'sdks' / 'cloudrouter-backend-sdk'}", result.messages)

    def test_reports_missing_required_open_sdk_system(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.write_sdk(root, "cloudrouter-app-sdk", "@sdkwork/cloudrouter-app-sdk", "app", "SdkworkAppClient", "/app/v3/api")
            self.write_sdk(
                root,
                "cloudrouter-backend-sdk",
                "@sdkwork/cloudrouter-backend-sdk",
            ...
            self.assertFalse(result.ok)
            self.assertIn(f"generated SDK family is missing: {root / 'sdks' / 'cloudrouter-open-sdk'}", result.messages)
    ...
    def test_reports_unexpected_fourth_sdk_system(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write_portal_sdk_boundary(root)
            (root / "sdks" / "cloudrouter-admin-sdk").mkdir(parents=True)

            result = CloudRouterSdkGuardian(root=root).run()

            self.assertFalse(result.ok)
            self.assertIn(
                f"unexpected generated SDK family is present: {root / 'sdks' / 'cloudrouter-admin-sdk'}",
                result.messages,
            )
    ...
    def test_reports_sdk_family_without_official_multilanguage_generation_matrix(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write_portal_sdk_boundary(root)
            family = root / "sdks" / "cloudrouter-app-sdk"
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk sdk-manifest.json must list official SDK language flutter",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk sdk-manifest.json must list official SDK language python",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk bin/generate-sdk.mjs must support --language language selection",
                result.messages,
            )
    ...
    def test_reports_sdk_family_legacy_single_derived_spec_field(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write_portal_sdk_boundary(root)
            assembly_path = root / "sdks" / "cloudrouter-app-sdk" / "sdk-manifest.json"
            assembly = json.loads(assembly_path.read_text(encoding="utf-8"))
            assembly["derivedSpec"] = "openapi/cloudrouter-app-sdk.sdkgen.json"
            assembly_path.write_text(json.dumps(assembly) + "\n", encoding="utf-8")
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk sdk-manifest.json must not declare legacy derivedSpec; use derivedSpecs",
                result.messages,
            )
    ...
    def test_reports_sdk_family_generator_without_generated_transport_cleanup(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write_portal_sdk_boundary(root)
            script_path = root / "sdks" / "cloudrouter-app-sdk" / "bin" / "generate-sdk.mjs"
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk bin/generate-sdk.mjs must clean non-TypeScript generated transport output after generation",
                result.messages,
            )
    ...
    def test_reports_sdk_family_generator_that_bypasses_authority_openapi_contract(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write_portal_sdk_boundary(root)
            (root / "sdks" / "cloudrouter-app-sdk" / "bin" / "generate-sdk.mjs").write_text(
                "const OFFICIAL_LANGUAGES = ['typescript', 'flutter', 'rust', 'java', 'csharp', 'swift', 'kotlin', 'go', 'python'];\n"
                "const sdkFamily = 'cloudrouter-app-sdk';\n"
                "const authorityInputPath = `sdks/${sdkFamily}/openapi/${sdkFamily}.openapi.json`;\n"
                "const sdkgenInputPath = `sdks/${sdkFamily}/openapi/${sdkFamily}.sdkgen.json`;\n"
                "function strictTypeScriptArgs() {\n"
                ...
                "}\n"
                "console.log('--language');\n"
                "console.log('generated/openapi/cloudrouter-app-openapi.json');\n"
                "console.log('sdks/${sdkFamily}/${sdkFamily}-${language}/generated/server-openapi');\n",
                encoding="utf-8",
            )
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk bin/generate-sdk.mjs strictTypeScriptArgs() must generate from openapi/${sdkFamily}.openapi.json",
                result.messages,
            )
    ...
    def test_reports_sdk_family_generator_that_mixes_authority_and_derived_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk bin/generate-sdk.mjs strictTypeScriptArgs() must not generate from openapi/${sdkFamily}.sdkgen.json",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk bin/generate-sdk.mjs generatorArgs(language) must not generate from openapi/${sdkFamily}.sdkgen.json",
                result.messages,
            )
    ...
    def test_reports_app_backend_generator_that_declares_unused_sdkgen_input_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk bin/generate-sdk.mjs must not declare sdkgenInputPath because generation uses the authority OpenAPI",
                result.messages,
            )
    ...
    def test_reports_open_sdk_generator_that_omits_sdkgen_input_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write_portal_sdk_boundary(root)
            script_path = root / "sdks" / "cloudrouter-open-sdk" / "bin" / "generate-sdk.mjs"
            script_path.write_text(
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-open-sdk bin/generate-sdk.mjs must declare sdkgenInputPath because generation uses the derived sdkgen contract",
                result.messages,
            )
    ...
    def test_reports_authority_input_path_that_points_to_sdkgen_contract(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk bin/generate-sdk.mjs authorityInputPath must point to openapi/${sdkFamily}.openapi.json",
                result.messages,
            )
    ...
    def test_reports_open_sdk_sdkgen_input_path_that_points_to_authority_contract(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-open-sdk bin/generate-sdk.mjs sdkgenInputPath must point to openapi/${sdkFamily}.sdkgen.json",
                result.messages,
            )
    ...
    def test_reports_sdk_family_openapi_when_it_is_not_synchronized_with_generated_openapi(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (generated_openapi / "cloudrouter-app-openapi.json").write_text(
                '{"openapi":"3.1.2","info":{"title":"generated-app","version":"0.1.0"},"paths":{}}\n',
                encoding="utf-8",
            )
            (generated_openapi / "cloudrouter-backend-openapi.json").write_text(
                '{"openapi":"3.1.2","info":{"title":"generated-backend","version":"0.1.0"},"paths":{}}\n',
                encoding="utf-8",
            )
            app_family_openapi = root / "sdks" / "cloudrouter-app-sdk" / "openapi" / "cloudrouter-app-sdk.openapi.json"
            backend_family_openapi = root / "sdks" / "cloudrouter-backend-sdk" / "openapi" / "cloudrouter-backend-sdk.openapi.json"
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk openapi/cloudrouter-app-sdk.openapi.json must stay synchronized with owner-only generated/openapi/cloudrouter-app-openapi.json",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-backend-sdk openapi/cloudrouter-backend-sdk.openapi.json must stay synchronized with owner-only generated/openapi/cloudrouter-backend-openapi.json",
                result.messages,
            )
    ...
    def test_reports_wrong_sdk_type_and_client_without_enforcing_url_prefix(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.write_sdk(root, "cloudrouter-app-sdk", "@sdkwork/cloudrouter-app-sdk", "backend", "WrongClient", "/wrong")
            self.write_sdk(
                root,
                "cloudrouter-backend-sdk",
                "@sdkwork/cloudrouter-backend-sdk",
            ...
            self.assertFalse(result.ok)
            self.assertIn("cloudrouter-app-sdk-typescript sdkwork-sdk.json sdkType must be app", result.messages)
            self.assertIn("cloudrouter-app-sdk-typescript src/sdk.ts must export SdkworkAppClient", result.messages)
            self.assertNotIn(
                "cloudrouter-app-sdk-typescript src/api/paths.ts must contain /app/v3/api",
                result.messages,
            )
    ...
    def test_reports_non_standard_runtime_export_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            package_path = root / "sdks" / "cloudrouter-app-sdk" / "cloudrouter-app-sdk-typescript" / "package.json"
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk-typescript package.json main must be dist/index.cjs",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript package.json exports[.].require must stay inside SDK package: ../dist/index.cjs",
                result.messages,
            )
    ...
    def test_reports_generated_transport_copied_into_composed_facade(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(
                "cloudrouter-app-sdk-typescript composed root must not copy generated control-plane file .sdkwork/sdkwork-generator-manifest.json",
                result.messages,
            )
    ...
    def test_reports_unexported_generated_api_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk-typescript must not contain unexported generated API artifact: src/api/coupon.ts",
                result.messages,
            )
    ...
    def test_reports_generated_type_file_missing_from_type_index(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-backend-sdk-typescript src/types/index.ts must export AdminSkillItem from ./admin-skill-item",
                result.messages,
            )
    ...
    def test_reports_weak_public_sdk_common_empty_operation_types_and_forbidden_no_data_type(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk-typescript src/types/common.ts must not re-export PageResult",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript src/types/no-data.ts is forbidden; no-data operations use PlusApiResult",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript src/types/no-data.ts must not declare NoData",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript src/types/disable-skill-request.ts must not expose "
                "DisableSkillRequest as Record<string, unknown>; use Record<string, never>",
                result.messages,
            )
            self.assertNotIn(
                "cloudrouter-app-sdk-typescript src/types/metadata-bag.ts must not expose MetadataBag as "
    ...
    def test_reports_no_data_index_export(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk-typescript src/types/index.ts must not export NoData from ./no-data",
                result.messages,
            )
    ...
    def test_reports_non_standard_sdk_build_script(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            app_package_path = root / "sdks" / "cloudrouter-app-sdk" / "cloudrouter-app-sdk-typescript" / "package.json"
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk-typescript package.json scripts.build must be node custom/build-runtime.mjs",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript package.json scripts.dev must be node custom/build-runtime.mjs",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript custom/build-runtime.mjs is required for SDK runtime builds",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript package.json devDependencies must not include vite",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript package.json devDependencies must not include vite-plugin-dts",
                result.messages,
            )
    ...
    def test_reports_public_app_model_catalog_private_pricing_type_regression(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk-typescript AppModelCatalogPriceAvailability.status must be 'reference' | 'unavailable'",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript AppModelCatalogPriceAvailability.status must not expose public available",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript AppModelCatalogItem must not expose public private pricing field lowestUpstreamCostUnitPrice",
                result.messages,
            )
            for sensitive_field in (
                ...
                self.assertIn(
                    "cloudrouter-app-sdk-typescript AppModelCatalogPriceAvailability must not expose public private "
    ...
    def test_reports_app_sdk_query_parameter_standard_regression(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-app-sdk-typescript src/api/ai.ts must expose SDK search text as q, not searchQuery/search_query/keyword/search",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript src/api/ai.ts must send URL search text as q, not search_query/searchQuery/keyword/search",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript src/api/ai.ts must expose vendorCodes as string[] for multi-value query filters",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-app-sdk-typescript src/api/ai.ts must serialize vendor_codes with style=form and explode=false",
                result.messages,
            )
    ...
    def test_reports_backend_ecosystem_skill_resource_tree_regression(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "cloudrouter-backend-sdk-typescript src/api/ecosystem.ts must expose resource class EcosystemApi",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-backend-sdk-typescript src/api/ecosystem.ts must expose resource member public readonly package: EcosystemSkillsPackageApi;",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-backend-sdk-typescript src/api/ecosystem.ts EcosystemSkillsApi must expose async list(",
                result.messages,
            )
            self.assertIn(
                "cloudrouter-backend-sdk-typescript src/api/ecosystem.ts must use standard resource-tree methods, not async enableSkill(",
                result.messages,
            )
    ...
    def test_reports_missing_portal_sdk_boundary(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn("portal package.json must depend on @sdkwork/cloudrouter-app-sdk", result.messages)
            self.assertIn("portal package.json must depend on @sdkwork/cloudrouter-open-sdk", result.messages)
            self.assertIn("portal commons package.json must depend on @sdkwork/cloudrouter-backend-sdk", result.messages)
            self.assertIn("portal commons package.json must depend on @sdkwork/cloudrouter-open-sdk", result.messages)
            self.assertIn("portal SDK boundary is missing: apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudroutes-pc-commons/src/sdk-clients.ts", result.messages)
    ...
    def test_reports_portal_ui_root_that_exports_sdk_clients(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(result.ok)
            self.assertIn(
                "portal commons UI root must not export ./sdk-clients; use sdkwork-cloudroutes-pc-commons/runtime: "
                "apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudroutes-pc-commons/src/index.ts",
                result.messages,
            )
