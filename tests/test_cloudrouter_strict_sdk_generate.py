...
class CloudRouterStrictSdkGenerateTest(unittest.TestCase):
            ...
            json.dumps(
                {
                    ...
                    "components": {
                        "schemas": {
                            ...
                            "PlusApiResult": {
                                "description": "Base Cloud Router response envelope.",
                                "type": "object",
                                "additionalProperties": False,
                                "required": ["code"],
                                "properties": {
                            ...
                            "DeleteSessionResult": {
                                "description": "Delete session result schema exposed by Cloud Router.",
                                "type": "object",
                                "additionalProperties": False,
                                "required": ["code"],
                                "properties": {
                            ...
                            "AdminApiKeysMapResponse": {
                                "description": "Admin api keys map response schema exposed by Cloud Router.",
                                "type": "object",
    ...
    def test_dry_run_strict_generation_removes_page_result_and_closes_empty_dtos(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual("cloudrouter-app-sdk-generated-typescript", package_json["name"])
    ...
    def test_apply_generation_runs_project_runtime_standardizer(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
            family = temp_root / "sdks" / "cloudrouter-app-sdk"
            output = family / "cloudrouter-app-sdk-typescript"
            spec_path = self.write_openapi(temp_root)
            ...
            self.assertTrue((family / "openapi" / "cloudrouter-app-sdk.openapi.json").is_file())
            self.assertTrue((family / "openapi" / "cloudrouter-app-sdk.sdkgen.json").is_file())
            ...
            self.assertEqual("cloudrouter-app-sdk", assembly["workspace"])
            self.assertEqual("cloudrouter-app-sdk-typescript", assembly["languages"][0]["workspace"])
    ...
    def test_open_sdk_apply_generation_preserves_authority_openapi_snapshot(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            temp_root = Path(tmp)
            family = temp_root / "sdks" / "cloudrouter-open-sdk"
            output = family / "cloudrouter-open-sdk-typescript"
            authority_source = temp_root / "apps" / "sdkwork-cloudrouter-pc" / "public" / "openapi.json"
            ...
            sdkgen_path = family / "openapi" / "cloudrouter-open-sdk.sdkgen.json"
            ...
            family_authority = json.loads(
                (family / "openapi" / "cloudrouter-open-sdk.openapi.json").read_text(encoding="utf-8")
            )
            family_sdkgen = json.loads(
                (family / "openapi" / "cloudrouter-open-sdk.sdkgen.json").read_text(encoding="utf-8")
            )
