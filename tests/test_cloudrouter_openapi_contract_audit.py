...
class CloudRouterOpenApiContractAuditTest(unittest.TestCase):
    def write_specs(
        self,
        root: Path,
        *,
        ...
        (openapi_dir / "cloudrouter-app-openapi.json").write_text(
            json.dumps(app_spec or self.valid_spec("app"), ensure_ascii=False, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
        (openapi_dir / "cloudrouter-backend-openapi.json").write_text(
    ...
    def valid_spec(self, surface: str) -> dict:
        ...
        return {
            ...
            "info": {
                "title": f"Cloud Router {surface} API",
                "version": "0.1.0",
    ...
    def test_accepts_precise_app_and_backend_specs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = CloudRouterOpenApiContractAudit(root=root).run()
