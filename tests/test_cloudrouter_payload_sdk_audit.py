...
class CloudRouterPayloadSdkAuditTest(unittest.TestCase):
    ...
    def write_sdk(self, root: Path, *, generic_method: bool = False) -> None:
        base = root / "sdks" / "cloudrouter-app-sdk" / "cloudrouter-app-sdk-typescript"
        ...
        backend = root / "sdks" / "cloudrouter-backend-sdk" / "cloudrouter-backend-sdk-typescript" / "src"
    ...
    def read_app_spec(self, root: Path) -> dict:
        return json.loads((root / "generated" / "openapi" / "cloudrouter-app-openapi.json").read_text(encoding="utf-8"))

    def write_app_spec(self, root: Path, spec: dict) -> None:
        (root / "generated" / "openapi" / "cloudrouter-app-openapi.json").write_text(
    ...
    def test_accepts_payload_schema_openapi_and_sdk_closure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = CloudRouterPayloadSdkAudit(root=root).run()
    ...
    def test_accepts_sdkwork_v3_unwrapped_response_without_result_type(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.write_openapi(root)
            self.write_sdk(root)
            base = root / "sdks" / "cloudrouter-app-sdk" / "cloudrouter-app-sdk-typescript" / "src"
    ...
    def test_accepts_multipart_payload_schema_with_request_dto_sdk_body(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (root / "sdks" / "cloudrouter-app-sdk" / "cloudrouter-app-sdk-typescript" / "src" / "api").mkdir(parents=True)
            (root / "sdks" / "cloudrouter-app-sdk" / "cloudrouter-app-sdk-typescript" / "src" / "types").mkdir(parents=True)
            (root / "sdks" / "cloudrouter-backend-sdk" / "cloudrouter-backend-sdk-typescript" / "src" / "api").mkdir(parents=True)
            (root / "sdks" / "cloudrouter-backend-sdk" / "cloudrouter-backend-sdk-typescript" / "src" / "types").mkdir(parents=True)
            ...
            (root / "generated" / "openapi" / "cloudrouter-backend-openapi.json").write_text(
                json.dumps({"openapi": "3.1.0", "paths": {}, "components": {"schemas": {}}}) + "\n",
                encoding="utf-8",
            )

            sdk_base = root / "sdks" / "cloudrouter-app-sdk" / "cloudrouter-app-sdk-typescript" / "src"
    ...
    def test_ignores_non_exposed_derived_frontend_operations(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            CloudRouterOpenApiGenerator(root=root).write()
            app = root / "sdks" / "cloudrouter-app-sdk" / "cloudrouter-app-sdk-typescript" / "src"
            backend = root / "sdks" / "cloudrouter-backend-sdk" / "cloudrouter-backend-sdk-typescript" / "src"
    ...
    def test_accepts_generated_sdk_method_with_tag_suffix_removed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            CloudRouterOpenApiGenerator(root=root).write()
            base = root / "sdks" / "cloudrouter-backend-sdk" / "cloudrouter-backend-sdk-typescript"
            (base / "src" / "api").mkdir(parents=True, exist_ok=True)
            (base / "src" / "types").mkdir(parents=True, exist_ok=True)
    ...
    def test_dependency_operation_matches_normalized_path_parameter_names(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            app_family = root / "sdks" / "cloudrouter-app-sdk"
    ...
    def test_accepts_optional_request_body_when_request_schema_is_declared(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            backend = root / "sdks" / "cloudrouter-backend-sdk" / "cloudrouter-backend-sdk-typescript"
