...
from tools.cloudrouter_sdk_runtime_standardizer import SdkRuntimeStandardizer


class SdkRuntimeStandardizerTest(unittest.TestCase):
    def standardizer(self, root: Path, sdk_directories: tuple[str, ...] = ("cloudrouter-app-sdk", "cloudrouter-backend-sdk")) -> SdkRuntimeStandardizer:
    ...
    def test_standardizes_generated_sdk_runtime_build_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for sdk_dir in ("cloudrouter-app-sdk", "cloudrouter-backend-sdk"):
                base = self.sdk_base(root, sdk_dir)
                self.assertTrue(
                    {
                        base / "package.json",
...
"""
    ...
    def test_standardizes_primary_generated_http_client(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            base = self.sdk_base(root, "cloudrouter-app-sdk")
            source = """import { BaseHttpClient, withRetry } from '@sdkwork/sdk-common';
...
"""
            ...
            updated = self.standardizer(root, ("cloudrouter-app-sdk",))._standardize_generated_http_clients(
                "cloudrouter-app-sdk",
                base,
            )
    ...
    def test_removes_retired_domain_transport_directories(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            updated = self.standardizer(root, ("cloudrouter-backend-sdk",)).run()
    ...
    def test_materializes_thin_typescript_facade_from_generated_server_openapi(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (generated / "package.json").write_text(
                json.dumps(
                    {
                        "name": "cloudrouter-backend-sdk-generated-typescript",
                        "version": "0.1.0",
                        "private": True,
                ...
                json.dumps(
                    {
                        "language": "typescript",
                        "sdkType": "backend",
                        "packageName": "cloudrouter-backend-sdk-generated-typescript",
                    }
                )
                + "\n",
                encoding="utf-8",
            ...
            self.standardizer(root, ("cloudrouter-backend-sdk",)).run()
            ...
            package = json.loads((base / "package.json").read_text(encoding="utf-8"))
            self.assertEqual("@sdkwork/cloudrouter-backend-sdk", package["name"])
            ...
            self.assertEqual("@sdkwork/cloudrouter-backend-sdk", root_metadata["packageName"])
            self.assertEqual("backend", root_metadata["sdkType"])

    def test_standardizes_app_multipart_methods_to_request_dto_body(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            base = self.write_minimal_typescript_sdk(root, "cloudrouter-app-sdk", "@sdkwork/cloudrouter-app-sdk")
            generated_openapi = root / "generated" / "openapi"
            generated_openapi.mkdir(parents=True, exist_ok=True)
            sdk_openapi = root / "sdks" / "cloudrouter-app-sdk" / "openapi"
            ...
            (sdk_openapi / "cloudrouter-app-sdk.sdkgen.json").write_text(
            ...
            self.standardizer(root, ("cloudrouter-app-sdk",)).run()
    ...
    def test_syncs_family_openapi_snapshots_without_typescript_workspace(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse((root / "sdks" / "cloudrouter-app-sdk" / "cloudrouter-app-sdk-typescript").exists())

    def test_writes_open_sdk_derived_spec_without_recursive_schema_cycles(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.write_minimal_typescript_sdk(root, "cloudrouter-open-sdk", "@sdkwork/cloudrouter-open-sdk")
            source_path = root / "apps" / "sdkwork-cloudrouter-pc" / "public" / "openapi.json"
            ...
            self.standardizer(root, ("cloudrouter-open-sdk",)).run()

            family = root / "sdks" / "cloudrouter-open-sdk"
            authority = json.loads((family / "openapi" / "cloudrouter-open-sdk.openapi.json").read_text(encoding="utf-8"))
            sdkgen = json.loads((family / "openapi" / "cloudrouter-open-sdk.sdkgen.json").read_text(encoding="utf-8"))
            ...
            self.assertEqual("openapi/cloudrouter-open-sdk.sdkgen.json", assembly["generationInputSpec"])
            self.assertNotIn("derivedSpec", assembly)
            self.assertEqual(
                {"sdk-generator": "openapi/cloudrouter-open-sdk.sdkgen.json"},
    ...
    def test_standardizes_app_and_backend_typescript_generation_to_authority_openapi(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for sdk_dir in ("cloudrouter-app-sdk", "cloudrouter-backend-sdk"):
                ...
                self.assertIn("'tools.cloudrouter_sdk_runtime_standardizer'", composed_sync_body)
    ...
    def test_verify_script_checks_family_generation_input_contract(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.standardizer(
                root,
                ("cloudrouter-app-sdk", "cloudrouter-backend-sdk", "cloudrouter-open-sdk"),
            ).run()

            expected_generation_inputs = {
                "cloudrouter-app-sdk": "openapi/cloudrouter-app-sdk.openapi.json",
                "cloudrouter-backend-sdk": "openapi/cloudrouter-backend-sdk.openapi.json",
                "cloudrouter-open-sdk": "openapi/cloudrouter-open-sdk.sdkgen.json",
            }
            expected_derived_specs = {
                "cloudrouter-app-sdk": "{}",
                "cloudrouter-backend-sdk": "{}",
                "cloudrouter-open-sdk": '{"sdk-generator":"openapi/cloudrouter-open-sdk.sdkgen.json"}',
            }
                    ...
                    json.dumps(
                        {
                            ...
                            "sdk": {
                                ...
                                "version": "0.1.0",
                                "language": "typescript",
                                "sdkType": "app" if sdk_dir == "cloudrouter-app-sdk" else "backend",
    ...
    def test_standardizes_api_index_to_export_full_modules_for_parameter_types(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for sdk_dir, package_name, path_export in (
                ("cloudrouter-app-sdk", "@sdkwork/cloudrouter-app-sdk", "appApiPath"),
                ("cloudrouter-backend-sdk", "@sdkwork/cloudrouter-backend-sdk", "backendApiPath"),
            ):
                base = self.sdk_base(root, sdk_dir)
                (base / "src" / "api").mkdir(parents=True, exist_ok=True)
                (base / "package.json").write_text(
    ...
    def test_removes_unexported_generated_api_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for sdk_dir, package_name, exported_name, stale_name in (
                ("cloudrouter-app-sdk", "@sdkwork/cloudrouter-app-sdk", "coupons", "coupon"),
                ("cloudrouter-backend-sdk", "@sdkwork/cloudrouter-backend-sdk", "provider-secrets", "provider-secret"),
            ):
                base = self.sdk_base(root, sdk_dir)
                (base / "src" / "api").mkdir(parents=True, exist_ok=True)
                (base / "dist" / "api").mkdir(parents=True, exist_ok=True)
            ...
            for sdk_dir, exported_name, stale_name in (
                ("cloudrouter-app-sdk", "coupons", "coupon"),
                ("cloudrouter-backend-sdk", "provider-secrets", "provider-secret"),
            ):
                base = self.sdk_base(root, sdk_dir)
...
class CloudRouterAppSdkIamOwnerOperationsTest(unittest.TestCase):
    def test_owner_only_openapi_keeps_cloudrouter_user_settings_operations(self) -> None:
        from tools.cloudrouter_sdk_runtime_standardizer import SdkRuntimeStandardizer
