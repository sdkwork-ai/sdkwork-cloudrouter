...
class CloudRouterOpenApiGeneratorTest(unittest.TestCase):
    ...
    def test_generates_surface_specific_openapi_specs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            generator = CloudRouterOpenApiGenerator(root=root)
            app_spec = generator.generate("app")
            backend_spec = generator.generate("backend")

            self.assertEqual("3.1.2", app_spec["openapi"])
            self.assertEqual("SDKWork Cloud Router App API", app_spec["info"]["title"])
            ...
            self.assertEqual("SDKWork Cloud Router Backend API", backend_spec["info"]["title"])
    ...
    def test_emits_problem_detail_error_responses_and_domain_extensions(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            app_spec = CloudRouterOpenApiGenerator(root=root).generate("app")
    ...
    def test_emits_path_parameters_request_body_and_query_marker(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            backend_spec = CloudRouterOpenApiGenerator(root=root).generate("backend")
    ...
    def test_openapi_generation_never_emits_integer_int64_json_contracts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for surface in ("app", "backend"):
                spec = CloudRouterOpenApiGenerator(root=root).generate(surface)
                integer_int64_nodes = [
                    node
    ...
    def test_backend_contract_overrides_restore_typed_request_and_response_data(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            override_path = root / CloudRouterOpenApiGenerator.BACKEND_CONTRACT_OVERRIDES
    ...
    def test_dependency_sdk_authority_operations_are_subtracted_exactly(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            app_sdk_root = root / "sdks" / "cloudrouter-app-sdk"
    ...
    def test_backend_contract_overrides_reject_stale_operation_paths(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "stale backend contract override operation"):
                CloudRouterOpenApiGenerator(root=root).generate("backend")
    ...
    def test_models_catalog_excludes_operations_marked_not_openapi_exposed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            catalog = CloudRouterOpenApiGenerator(root=root).generate_models_catalog("app")
    ...
    def test_writes_and_checks_specs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                {
                    "app": root / "generated" / "openapi" / "cloudrouter-app-openapi.json",
                    "backend": root / "generated" / "openapi" / "cloudrouter-backend-openapi.json",
                    "api-authority-app": root / "apis" / "app-api" / "cloudrouter" / "cloudrouter-app-api.openapi.json",
                    "api-authority-backend": root / "apis" / "backend-api" / "cloudrouter" / "cloudrouter-backend-api.openapi.json",
                    "models-catalog-app": root / "generated" / "openapi" / "cloudrouter-models-catalog-app-openapi.json",
                    "models-catalog-backend": root / "generated" / "openapi" / "cloudrouter-models-catalog-backend-openapi.json",
                },
                outputs,
            )
            ...
            self.assertIn(f"cloudrouter app OpenAPI spec is stale: {outputs['app']}", result.messages)
