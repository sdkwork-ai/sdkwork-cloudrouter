...
class ConsoleGatewayBackendRuntimeStandardTest(unittest.TestCase):
    def test_console_gateway_operation_is_backed_by_real_app_api_router(self) -> None:
        ...
        self.assertIn("internal_problem", app_gateway)
        self.assertIn("redact_error_message", app_gateway)
        self.assertIn("tracing::error!", app_gateway)
        self.assertNotIn("PlusApiResult", app_gateway)
        self.assertNotIn("problem_from_wire_code", app_gateway)
        self.assertNotIn('"5000"', app_gateway)
        self.assertIn("assert_eq!(50001, payload[\"code\"])", app_gateway)
    ...
    def test_console_gateway_ui_is_read_only_until_command_contract_exists(self) -> None:
        ...
        self.assertIn("setTraces((current) => [...current, ...page.items])", gateway_view)
        self.assertIn("trace.id}:${trace.time}:${index}", gateway_view)
        self.assertNotIn("mergeUniqueTraces", gateway_view)
