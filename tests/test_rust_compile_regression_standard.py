...
class RustCompileRegressionStandardTest(unittest.TestCase):
    def test_app_api_key_create_normalizes_borrowed_fields_before_owned_modalities(self) -> None:
        source = (
            ROOT / "services" / "sdkwork-cloudrouter-router-service" / "src" / "api" / "app_api_keys.rs"
    ...
    def test_invoice_routes_reuse_the_process_database_identity_without_iam_env_rewrites(self) -> None:
        app_crate = ROOT / "crates" / "sdkwork-routes-cloudrouter-app-api"
        ...
        self.assertNotIn("apply_unified_cloud_postgres_env", app_source)
        ...
        self.assertNotIn("apply_unified_cloud_postgres_env", runtime)
