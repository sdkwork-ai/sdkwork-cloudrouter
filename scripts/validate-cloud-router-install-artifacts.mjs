...
function printHelp() {
  console.log(`Usage: node scripts/validate-cloud-router-install-artifacts.mjs --package-id <id> --artifact-path <path> [options]
...
function validateInstallArtifact(settings) {
  ...
  if (!artifactName.startsWith(`cloudrouter-${packageItem.artifactId}-${packageItem.version}.`)) {
...
function validateDebianArtifact(packageItem, artifactBytes) {
  ...
  if (packageItem.deploymentMode === 'service') {
    requireText(postinst, `chown root:root ${LINUX_SERVICE_RUNTIME_ROOT} ${LINUX_SERVICE_RUNTIME_ROOT}/bin /usr/bin/cloudrouter /usr/bin/cloudrouterctl`, 'postinst runtime binary ownership', issues);
    ...
    if (postinst.includes(`${LINUX_SERVICE_CONFIG_ROOT}/database.secret`) || postinst.includes('systemctl enable cloudrouter.service')) {
  ...
  const legacyOptRoot = `./${['opt', 'cloudrouter'].join('/')}`;
  ...
  requireTarEntry(dataEntries, './usr/bin/cloudrouter', 'file', 0o755, issues);
  requireTarEntry(dataEntries, './usr/bin/cloudrouterctl', 'file', 0o755, issues);
  ...
  requireTarEntry(dataEntries, `.${LINUX_SERVICE_RUNTIME_ROOT}/bin/cloudrouter`, 'file', 0o755, issues);
  requireTarEntry(dataEntries, `.${LINUX_SERVICE_RUNTIME_ROOT}/bin/cloudrouterctl`, 'file', 0o755, issues);
  ...
  requireParentBeforeChild(dataEntryNames, './usr/bin', './usr/bin/cloudrouter', issues);
  requireParentBeforeChild(dataEntryNames, `.${LINUX_SERVICE_RUNTIME_ROOT}`, `.${LINUX_SERVICE_RUNTIME_ROOT}/bin/cloudrouter`, issues);
  ...
  if (packageItem.deploymentMode === 'service') {
    ...
    requireText(systemdUnit, 'ExecStart=/usr/bin/cloudrouter', 'systemd start command', issues);
...
function validateNativeManifest(packageItem, manifest) {
  ...
  requireManifestPath(nativeInstall, 'files.binary', '/usr/bin/cloudrouter', issues);
  requireManifestPath(nativeInstall, 'files.installer', '/usr/bin/cloudrouterctl', issues);
  requireManifestPath(nativeInstall, 'files.privateBinary', `${LINUX_SERVICE_RUNTIME_ROOT}/bin/cloudrouter`, issues);
...
function validateWindowsNativeManifest(packageItem, manifest) {
  ...
  requireManifestPath(nativeInstall, 'files.binary', `${WINDOWS_INSTALL_ROOT}/bin/cloudrouter.exe`, issues);
  requireManifestPath(nativeInstall, 'files.installer', `${WINDOWS_INSTALL_ROOT}/bin/cloudrouterctl.exe`, issues);
