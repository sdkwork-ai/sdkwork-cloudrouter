...
const GATEWAY_ROOT = path.join(WORKSPACE_ROOT, 'crates', 'sdkwork-cloudrouter-edge-runtime');
...
const REQUIRED_SOURCE_MARKERS = Object.freeze([
  ...
  },
  {
    ...
    file: path.join(WORKSPACE_ROOT, 'crates', 'sdkwork-cloudrouter-test-support', 'src', 'lib.rs'),
  ...
  },
  {
    ...
    markers: ['sdkwork_cloudrouter_test_support::assert_server_generated_request_id'],
...
function runGatewayRequestIdentityTests() {
  execSync(
    'cargo test -p sdkwork-cloudrouter-edge-runtime request_identity',
    {
      cwd: WORKSPACE_ROOT,
      stdio: 'inherit',
      env: process.env,
  ...
  execSync(
    'cargo test -p sdkwork-cloudrouter-edge-runtime --test invocation_router',
    {
      cwd: WORKSPACE_ROOT,
      stdio: 'inherit',
      env: process.env,
...
function main() {
  ...
  console.log('[check-cloud-router-gateway-request-identity] gateway request identity alignment ok');
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === __filename) {
  try {
    ...
    console.error(`[check-cloud-router-gateway-request-identity] ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
