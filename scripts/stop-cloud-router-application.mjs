...
Stop SDKWork Cloud Router development processes that own the current workspace ports.
...
export async function stopWorkspaceProcesses({
  ...
  if (processIds.length === 0) {
    console.log('[stop-cloud-router-application] no current workspace listeners found');
  ...
  for (const processId of processIds) {
    if (dryRun) {
      console.log(`[stop-cloud-router-application] would stop PID ${processId} for ${targetSummary}`);
      continue;
    }
...
if (process.argv[1] && path.resolve(process.argv[1]) === __filename) {
  main().catch((error) => {
    console.error(`[stop-cloud-router-application] ${error.message}`);
    process.exit(1);
  });
}
