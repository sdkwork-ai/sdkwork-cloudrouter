...
function printHelp() {
  console.log(`Usage: node scripts/clean-cloud-router-workspace.mjs [options]
...
function buildCleanPlan({
  ...
  if (nodeModules) {
    relativePaths.push(path.join('apps', 'sdkwork-cloudrouter-pc', 'node_modules'));
  }
...
async function removeEntry(entry, { dryRun = false } = {}) {
  ...
  console.error(`[clean-cloud-router-workspace] remove ${entry.relativePath}`);
...
async function removeEntries(entries, {
  ...
  for (const entry of entries) {
    try {
      ...
      logWarning(`[clean-cloud-router-workspace] failed ${entry.relativePath}: ${message}`);
    }
  }
  return failures;
}
...
async function main() {
  ...
  if (failures.length > 0) {
    console.error(`[clean-cloud-router-workspace] completed with ${failures.length} cleanup warning(s)`);
  }
}

if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replaceAll('\\', '/'))) {
  main().catch((error) => {
    console.error(`[clean-cloud-router-workspace] ${error.message}`);
    process.exit(1);
  });
}
