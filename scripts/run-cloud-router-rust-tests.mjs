...
const PACKAGE_BY_SERVICE_DIR = Object.freeze({
  'sdkwork-cloudrouter-admin-gateway': 'sdkwork-cloudrouter-admin-gateway',
  'sdkwork-cloudrouter-standalone-gateway': 'sdkwork-cloudrouter-standalone-gateway',
  'sdkwork-cloudrouter-edge-runtime': 'sdkwork-cloudrouter-edge-runtime',
  'sdkwork-cloudrouter-standalone-gateway': 'sdkwork-cloudrouter-standalone-gateway',
  'sdkwork-cloudrouter-installer': 'sdkwork-cloudrouter-installer',
  'sdkwork-cloudrouter-router-service': 'sdkwork-cloudrouter-router-service',
});
const PACKAGE_WORKSPACE_ROOTS = Object.freeze(['services', 'crates']);
const PROFILE_BY_SERVICE_PACKAGE = Object.freeze({
  'sdkwork-cloudrouter-admin-gateway': 'admin-api',
  'sdkwork-cloudrouter-standalone-gateway': 'app-api',
  'sdkwork-cloudrouter-edge-runtime': 'gateway',
  'sdkwork-cloudrouter-installer': 'runtime',
  'sdkwork-cloudrouter-router-service': 'runtime',
});
const PROFILE_BY_PATH_PREFIX = Object.freeze([
  ['crates/sdkwork-cloudrouter-config/', 'quick'],
  ['crates/sdkwork-cloudrouter-security/', 'quick'],
  ['crates/sdkwork-cloudrouter-test-support/', 'smoke'],
  ['crates/sdkwork-cloudrouter-http/', 'runtime'],
  ['services/sdkwork-cloudrouter-provider-adapter/', 'product-relay'],
  ['crates/sdkwork-cloudrouter-edge-runtime/', 'gateway'],
  ['crates/sdkwork-api-cloudrouter-standalone-gateway/', 'gateway'],
  ['services/sdkwork-cloudrouter-standalone-gateway/', 'app-api'],
  ['Cargo.toml', 'quick'],
  ['Cargo.lock', 'quick'],
  ['.cargo/', 'quick'],
]);
...
function printHelp() {
  console.log(`Usage: node scripts/run-cloud-router-rust-tests.mjs <profile> [options]
...
function buildCargoEnv(env, { targetDir, buildJobs }) {
  ...
  if (!stepEnv.SDKWORK_CLOUDROUTER_HTTP_OPENAPI_BUILD_MODE) {
    stepEnv.SDKWORK_CLOUDROUTER_HTTP_OPENAPI_BUILD_MODE = 'copy';
  }
  return stepEnv;
}
...
function buildQuickSteps(env, settings) {
  return [
    ...
    cargoStep(
      ...
      ['test', '-p', 'sdkwork-cloudrouter-config', '--test', 'redis_config'],
      env,
      settings,
    ),
    cargoStep(
...
function buildSmokeSteps(env, settings) {
  return [
    cargoStep(
      ...
      ['test', '-p', 'sdkwork-cloudrouter-test-support', '--lib', 'standard_runtime_subject_helpers_create_verifiable_tokens_and_signatures'],
      env,
      settings,
    ),
    cargoStep(
...
function buildAdminApiSteps(env, settings) {
  return [
    cargoStep('admin api health tests', ['test', '-p', 'sdkwork-cloudrouter-admin-gateway', '--test', 'health'], env, settings),
    cargoStep(
      'admin api contract route tests',
      ['test', '-p', 'sdkwork-cloudrouter-admin-gateway', '--test', 'contract_routes'],
      env,
      settings,
    ),
    cargoStep(
      'admin api messaging route integration tests',
      ['test', '-p', 'sdkwork-cloudrouter-admin-gateway', '--test', 'messaging_route'],
      env,
      settings,
    ),
    cargoStep(
      'admin api product model route tests',
      ['test', '-p', 'sdkwork-cloudrouter-admin-gateway', '--test', 'product_model_route'],
      env,
      settings,
    ),
  ];
...
function buildAppApiSteps(env, settings) {
  return [
    cargoStep('app api health tests', ['test', '-p', 'sdkwork-cloudrouter-standalone-gateway', '--test', 'health'], env, settings),
    cargoStep(
      'app api contract route tests',
      ['test', '-p', 'sdkwork-cloudrouter-standalone-gateway', '--test', 'contract_routes'],
      env,
      settings,
    ),
    cargoStep(
      'app api query contract tests',
      ['test', '-p', 'sdkwork-cloudrouter-standalone-gateway', '--test', 'api_query_contract'],
      env,
      settings,
    ),
    cargoStep(
      'app api session route tests',
      ['test', '-p', 'sdkwork-cloudrouter-standalone-gateway', '--test', 'app_session_route'],
      env,
      settings,
    ),
  ];
...
function buildGatewaySteps(env, settings) {
  return [
    cargoStep('gateway health tests', ['test', '-p', 'sdkwork-cloudrouter-edge-runtime', '--test', 'health'], env, settings),
    cargoStep(
      'gateway edge server tests',
      ['test', '-p', 'sdkwork-cloudrouter-edge-runtime', '--test', 'edge_server'],
      env,
      settings,
    ),
    cargoStep(
      'gateway invocation router tests',
      ['test', '-p', 'sdkwork-cloudrouter-edge-runtime', '--test', 'invocation_router'],
      env,
      settings,
    ),
    cargoStep(
      'gateway provider adapter invocation tests',
      ['test', '-p', 'sdkwork-cloudrouter-edge-runtime', '--test', 'provider_adapter_invocation'],
      env,
      settings,
    ),
    cargoStep(
      'gateway provider adapter streaming tests',
      ['test', '-p', 'sdkwork-cloudrouter-edge-runtime', '--test', 'provider_adapter_passthrough_streaming'],
      env,
      settings,
    ),
    cargoStep(
...
function runStep(step, { dryRun = false } = {}) {
  ...
  console.error(`[run-cloud-router-rust-tests] ${step.label}: ${commandLine(step)}`);
  return new Promise((resolve, reject) => {
    ...
    child.on('exit', (code, signal) => {
      ...
      console.error(`[run-cloud-router-rust-tests] ${step.label}: completed in ${elapsedSeconds}s`);
      resolve();
    });
  });
}
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replaceAll('\\', '/'))) {
  main().catch((error) => {
    console.error(`[run-cloud-router-rust-tests] ${error.message}`);
    process.exit(1);
  });
}
