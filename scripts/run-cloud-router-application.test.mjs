...
import {
  ...
  resolveCloudRouterBusinessSpecsRoot,
} from './cloud-router-layout.mjs';

const workspaceRoot = path.resolve(import.meta.dirname, '..');
const portalRoot = path.join(workspaceRoot, 'apps', 'sdkwork-cloudrouter-pc');
...
const defaultDevSqliteDatabaseUrl = 'sqlite://target/dev/cloudrouter.sqlite';
...
function seedMinimalCloudRouterWorkspaceFixture(fixtureRoot) {
  writeFixtureFile(
    fixtureRoot,
  ...
  writeFixtureFile(
    fixtureRoot,
    'apps/sdkwork-cloudrouter-pc/package.json',
    JSON.stringify({ name: 'sdkwork-cloudrouter-pc-fixture', private: true }, null, 2),
  );
}
...
async function createRustAutoFixtureRepo() {
  ...
  writeFixtureFile(repoDir, 'services/sdkwork-cloudrouter-router-service/src/api/app_runtime.rs');
  writeFixtureFile(repoDir, 'services/sdkwork-cloudrouter-router-service/tests/app_runtime_api.rs');
  writeFixtureFile(repoDir, 'services/sdkwork-cloudrouter-router-service/tests/postgres_app_runtime_sql_contract.rs');
  writeFixtureFile(repoDir, 'services/sdkwork-cloudrouter-router-service/tests/sqlite_app_runtime_store.rs');
  writeFixtureFile(repoDir, 'services/sdkwork-cloudrouter-router-service/tests/openai_compatible_http_relay.rs');
  writeFixtureFile(repoDir, 'crates/sdkwork-cloudrouter-edge-runtime/src/edge_server.rs');
  writeFixtureFile(repoDir, 'crates/sdkwork-cloudrouter-edge-runtime/src/runtime.rs');
  writeFixtureFile(repoDir, 'crates/sdkwork-cloudrouter-edge-runtime/tests/edge_server.rs');
...
test('root package exposes pnpm application entrypoints', () => {
  ...
  const canonicalBrowser = 'pnpm exec sdkwork-app dev --runtime-target browser --deployment-profile standalone';
  ...
  assert.equal(rootPackage.scripts.dev, 'pnpm dev:standalone');
  assert.equal(
    rootPackage.scripts['dev:standalone'],
    'pnpm exec sdkwork-app dev --deployment-profile standalone',
  );
  assert.equal(
    rootPackage.scripts['dev:cloud'],
  ...
  assert.equal(
    rootPackage.scripts['_sdkwork:test'],
    'pnpm topology:validate && pnpm test:topology && node scripts/run-cloud-router-application.test.mjs',
  );
  ...
  assert.equal(rootPackage.scripts['_sdkwork:build'], 'node scripts/build-cloud-router-production.mjs');
  assert.equal(
    rootPackage.scripts.check,
    'pnpm exec sdkwork-app check',
  );
  assert.match(rootPackage.scripts['_sdkwork:check'], /check:application-env/u);
  assert.equal(
    rootPackage.scripts['check:application-env'],
    'node --test scripts/lib/cloud-router-browser-env-contract.test.mjs scripts/lib/cloud-router-edge-env-contract.test.mjs scripts/dev/cloud-router-application-env.test.mjs scripts/dev/ensure-cloud-router-env.test.mjs scripts/write-release-env.test.mjs scripts/release-environment-validation.test.mjs && node scripts/check-cloud-router-application-env.mjs',
  );
  assert.equal(
    rootPackage.scripts.start,
    'node scripts/start-cloud-router-production.mjs',
  );
  assert.equal(
  ...
  assert.equal(
    ...
    'node scripts/update-cloud-router-downloads.mjs',
  );
  assert.equal(
    rootPackage.scripts['downloads:check'],
    'node scripts/update-cloud-router-downloads.mjs --check',
  );
  assert.equal(
  ...
  assert.equal(
    ...
    'node scripts/manage-cloud-router-database.mjs',
  );
  assert.equal(
    rootPackage.scripts['db:status'],
    'cargo run --manifest-path ../sdkwork-database/Cargo.toml -p sdkwork-database-cli -- --app-root . status',
  );
  assert.equal(
    rootPackage.scripts['db:init'],
    'cargo run --manifest-path ../sdkwork-database/Cargo.toml -p sdkwork-database-cli -- --app-root . init',
  );
  assert.equal(
    rootPackage.scripts['db:upgrade'],
    'node scripts/manage-cloud-router-database.mjs upgrade',
  );
  assert.equal(
    rootPackage.scripts['db:ensure'],
    'node scripts/manage-cloud-router-database.mjs ensure',
  );
  assert.equal(
  ...
  assert.equal(rootPackage.scripts.format, 'node scripts/cargo-fmt-workspace.mjs');
  assert.equal(rootPackage.scripts['format:check'], 'node scripts/cargo-fmt-workspace.mjs --check');
  assert.equal(
  ...
  assert.equal(
    ...
    'node scripts/verify-cloud-router-application.mjs --fast',
  );
  assert.equal(
    rootPackage.scripts['verify:precommit'],
    'node scripts/verify-cloud-router-application.mjs --precommit',
  );
  assert.equal(
    rootPackage.scripts['verify:ci'],
    'node scripts/verify-cloud-router-application.mjs --ci',
  );
  assert.equal(
    rootPackage.scripts['verify:parallel'],
    'node scripts/verify-cloud-router-application.mjs --parallel',
  );
  assert.equal(
    rootPackage.scripts['test:rust:auto'],
    'node scripts/run-cloud-router-rust-tests.mjs auto',
  );
  assert.equal(
    rootPackage.scripts['test:rust:smoke'],
    'node scripts/run-cloud-router-rust-tests.mjs smoke',
  );
  assert.equal(
    rootPackage.scripts['test:rust:quick'],
    'node scripts/run-cloud-router-rust-tests.mjs quick',
  );
  assert.equal(
    rootPackage.scripts['test:rust:admin-api'],
    'node scripts/run-cloud-router-rust-tests.mjs admin-api',
  );
  assert.equal(
    rootPackage.scripts['test:rust:app-api'],
    'node scripts/run-cloud-router-rust-tests.mjs app-api',
  );
  assert.equal(
    rootPackage.scripts['test:rust:gateway'],
    'node scripts/run-cloud-router-rust-tests.mjs gateway',
  );
  assert.equal(
    rootPackage.scripts['test:rust:product-relay'],
    'node scripts/run-cloud-router-rust-tests.mjs product-relay',
  );
  assert.equal(
    rootPackage.scripts['test:rust:runtime'],
    'node scripts/run-cloud-router-rust-tests.mjs runtime',
  );
  assert.equal(
    rootPackage.scripts['test:rust:full'],
    'node scripts/run-cloud-router-rust-tests.mjs full',
  );
  assert.equal(
    rootPackage.scripts['test:rust:measure'],
    'node scripts/measure-cloud-router-test-targets.mjs',
  );
  assert.equal(
    rootPackage.scripts['test:rust:stop'],
    'node scripts/stop-cloud-router-test-processes.mjs',
  );
  assert.equal(
    rootPackage.scripts['clean:fast'],
    'node scripts/clean-cloud-router-workspace.mjs',
  );
  assert.equal(
  ...
  assert.equal(
    ...
    'node scripts/build-cloud-router-native-installer.mjs',
  );
  assert.equal(
    rootPackage.scripts['install:native:check'],
    'node scripts/build-cloud-router-native-installer.mjs --check --dry-run --all',
  );
  assert.equal(
  ...
  assert.equal(
    rootPackage.scripts['skills:seed:mirror-cloudhub'],
    'node scripts/mirror-cloudhub-skills-seed.mjs --fetch',
  );
  assert.equal(
    rootPackage.scripts['skills:seed:check'],
    'node scripts/mirror-cloudhub-skills-seed.mjs --check',
  );
  assert.equal(
...
test('release verification docs invoke the custom tooling test runner directly', () => {
  for (const relativePath of [
    'docs/release/CHANGELOG.md',
    'docs/release/2026-05-17-v0.3.0.md',
    'docs/architecture/tech/TECH-2026-05-17-v0-3-0.md',
    'docs/architecture/tech/TECH-changelog.md',
  ]) {
    const content = readFileSync(path.join(workspaceRoot, relativePath), 'utf8');
    assert.ok(!content.includes('node --test scripts/run-cloud-router-application.test.mjs'));
    assert.ok(content.includes(
      'node scripts/run-cloud-router-application.test.mjs --test-name-pattern',
    ));
  }
});

test('pnpm dev delegates to the canonical standalone application lifecycle', () => {
  ...
  assert.match(rootPackage.scripts['dev:browser:postgres:standalone'], /sdkwork-app dev/u);
  assert.match(rootPackage.scripts['dev:browser:postgres:standalone'], /--runtime-target browser/u);
...
test('application metadata does not own a platform gateway host or catalog', () => {
  ...
  assert.equal(componentSpec.integration?.foundationApiGateway, undefined);
  ...
  const forbiddenGatewayCatalogs = listFilesRecursive(path.join(workspaceRoot, 'specs'))
    ...
    .filter((relativePath) =>
      /(^|\/)(api-gateway-catalog|gateway-catalog|foundation-api-catalog)\.(json|ya?ml|toml)$/iu.test(relativePath)
    );
  assert.deepEqual(
...
test('product app and admin API servers do not keep direct foundation API runtime debts', () => {
  ...
  for (const relativePath of [
    'services/sdkwork-cloudrouter-standalone-gateway/Cargo.toml',
    'services/sdkwork-cloudrouter-admin-gateway/Cargo.toml',
  ]) {
    ...
    for (const crateName of forbiddenFoundationRuntimeCrates) {
      assert.doesNotMatch(
        ...
        `${relativePath} must not depend on ${crateName}; dependency APIs enter through canonical assemblies`,
      );
    }
  }
  ...
  for (const crateName of [
    ...
    assert.doesNotMatch(
      ...
      `root Cargo.toml must not keep unused ${crateName}; dependency APIs enter through canonical assemblies`,
    );
  }
  ...
  const productFoundationAdapters = listFilesRecursive(
    path.join(workspaceRoot, 'services', 'sdkwork-cloudrouter-router-service', 'src', 'api'),
    '.rs',
  )
    .map((filePath) => slashPath(path.relative(workspaceRoot, filePath)))
  ...
  assert.deepEqual(
    ...
    [],
    'sdkwork-cloudrouter-router-service must not retain product-local appbase backend IAM API adapters; IAM remains owned by its canonical assembly',
  );

  const productApiModule = readFileSync(
    path.join(workspaceRoot, 'services', 'sdkwork-cloudrouter-router-service', 'src', 'api', 'mod.rs'),
    'utf8',
  );
  for (const marker of [
    'admin_appbase_backend_iam_directory_router_with_read_store',
    ...
    assert.equal(
      ...
      false,
      `sdkwork-cloudrouter-router-service api module must not re-export product-local appbase backend IAM adapter ${marker}`,
    );
  }
});

test('product sqlite integration tests consume the workspace test-support crate instead of path-including helpers', () => {
  const productCargoToml = readFileSync(
    path.join(workspaceRoot, 'services', 'sdkwork-cloudrouter-router-service', 'Cargo.toml'),
    'utf8',
  );
  assert.match(
    productCargoToml,
    /sdkwork-cloudrouter-test-support\.workspace\s*=\s*true/u,
  );

  const rustTestFiles = listFilesRecursive(
    path.join(workspaceRoot, 'services', 'sdkwork-cloudrouter-router-service', 'tests'),
    '.rs',
  );
...
test('rust test runner exposes isolated daily-maintenance profiles', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-rust-tests.mjs')).href
  );
  ...
  assert.equal(quick.steps[0].env.SDKWORK_CLOUDROUTER_HTTP_OPENAPI_BUILD_MODE, 'copy');
  assert.equal('CARGO_BUILD_JOBS' in quick.steps[0].env, false);
  assert.equal(quick.steps.some((step) => step.args.includes('sdkwork-cloudrouter-config')), true);
  ...
  assert.equal(smoke.steps[0].args.includes('sdkwork-cloudrouter-test-support'), true);
  ...
  assert.equal(appApi.steps[0].args.includes('sdkwork-cloudrouter-standalone-gateway'), true);
  ...
  assert.equal(runtime.steps[0].env.SDKWORK_CLOUDROUTER_HTTP_OPENAPI_BUILD_MODE, 'copy');
  ...
  assert.equal(
    full.steps.some((step) =>
      ...
      && step.args.includes('sdkwork-cloudrouter-router-service')
  ...
  const autoExactTestTarget = module.buildRustTestPlan(
    module.parseArgs([
      'auto',
      '--changed-file',
      'services/sdkwork-cloudrouter-router-service/tests/openai_compatible_http_relay.rs',
    ]),
    {
      env: {},
      platform: 'linux',
  ...
  const autoGatewayServiceChange = module.buildRustTestPlan(
    module.parseArgs([
      'auto',
      '--changed-file',
      'crates/sdkwork-cloudrouter-edge-runtime/src/edge_server.rs',
    ]),
    {
      env: {},
      platform: 'linux',
  ...
  const autoProductServiceChange = module.buildRustTestPlan(
    module.parseArgs([
      'auto',
      '--changed-file',
      'services/sdkwork-cloudrouter-router-service/src/api/app_runtime.rs',
    ]),
    {
      env: {},
      platform: 'linux',
  ...
  const autoProductCommonModuleChange = module.buildRustTestPlan(
    module.parseArgs([
      'auto',
      '--changed-file',
      'services/sdkwork-cloudrouter-router-service/tests/common/mod.rs',
    ]),
    {
      env: {},
      platform: 'linux',
  ...
  try {
    ...
    await runGit(stagedFixtureRepo, ['add', 'services/sdkwork-cloudrouter-router-service/tests/openai_compatible_http_relay.rs']);
    writeFixtureFile(
      stagedFixtureRepo,
      'crates/sdkwork-cloudrouter-edge-runtime/src/runtime.rs',
  ...
  try {
    ...
    await runGit(baseRefFixtureRepo, ['add', 'crates/sdkwork-cloudrouter-edge-runtime/src/edge_server.rs']);
...
test('rust test process cleanup scopes Windows stops to repository-local targets', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'stop-cloud-router-test-processes.mjs')).href
  );
  ...
  const candidates = module.selectStoppableProcesses(
    [
      ...
      {
        Id: 2,
        ProcessName: 'sdkwork-cloudrouter-admin-gateway',
        Path: path.join(workspaceRoot, 'target-rust-tests', 'quick', 'debug', 'sdkwork-cloudrouter-admin-gateway.exe'),
      },
      {
        Id: 3,
...
test('nginx deployment spec documents the sdkwork site-family path convention', () => {
  const specsRoot = resolveCloudRouterBusinessSpecsRoot(workspaceRoot);
...
test('rust target measurement plan covers known slow integration surfaces', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'measure-cloud-router-test-targets.mjs')).href
  );
  ...
  assert.equal(
    plan.steps.some(
      (step) =>
        step.packageName === 'sdkwork-cloudrouter-admin-gateway' &&
  ...
  assert.equal(
    plan.steps.some(
      (step) =>
        step.packageName === 'sdkwork-cloudrouter-edge-runtime' &&
  ...
  assert.equal(
    plan.steps.some(
      (step) =>
        step.packageName === 'sdkwork-cloudrouter-router-service' &&
        step.testTarget === 'openai_compatible_http_relay',
    ),
    true,
  );
  assert.equal(plan.steps[0].env.SDKWORK_CLOUDROUTER_HTTP_OPENAPI_BUILD_MODE, 'copy');
...
test('installation documentation covers release, source, initialization, usage, and valid local links', () => {
  ...
  assert.ok(zhRelease.includes('cloudrouter-linux-x64-archive-0.3.0.tar.gz'));
  assert.ok(enRelease.includes('cloudrouter-linux-x64-archive-0.3.0.tar.gz'));
  ...
  assert.ok(zhRelease.includes('sudo apt install ./cloudrouter-linux-x64-server-0.3.0.deb'));
  assert.ok(enRelease.includes('sudo apt install ./cloudrouter-linux-x64-server-0.3.0.deb'));
  ...
  assert.equal(zhRelease.includes('sudo systemctl enable --now cloudrouter'), false);
  assert.equal(enRelease.includes('sudo systemctl enable --now cloudrouter'), false);
  assert.ok(enRelease.includes('/usr/bin/cloudrouter'));
  ...
  assert.ok(zhRelease.includes('./bin/cloudrouterctl ensure'));
  assert.ok(enRelease.includes('./bin/cloudrouterctl ensure'));
  ...
  assert.ok(zhUsage.includes('SDK 包版本独立于 Cloud Router release 版本'));
  assert.ok(enUsage.includes('SDK package versions are independent from Cloud Router release versions'));
  assert.ok(rootReadme.includes('Cloud development commands are remote-client-only'));
  ...
  assert.match(
    ...
    /Use\s+`pnpm dev:server` for PostgreSQL-backed product server debugging/u,
  );
  assert.ok(postgresqlDevelopment.includes('Desktop packages and desktop user data still use SQLite by default.'));
  assert.ok(postgresqlDevelopment.includes('~/.sdkwork/router/data/cloudrouter.sqlite'));
...
test('download catalog generator derives homepage JSON from release version and package matrix', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'update-cloud-router-downloads.mjs')).href
  );

  const catalog = module.createCloudRouterDownloadCatalog({
  ...
  assert.equal(
    ...
    'https://downloads.example.test/releases/v1.2.3/cloudrouter-windows-x64-desktop-1.2.3.msi',
  );
  assert.equal(
    actionsById.get('server-linux-x64')?.href,
    'https://downloads.example.test/releases/v1.2.3/cloudrouter-linux-x64-server-1.2.3.deb',
  );
...
test('verification plan checks the release download catalog before expensive suites', async () => {
  ...
  const downloadsCheckIndex = plan.findIndex((step) => step.label === 'cloud router download catalog check');
...
test('app store seed updater defaults to file seed updates and gates database sync behind explicit flag', async () => {
  ...
  assert.equal(defaults.appsRoot, resolveCloudRouterBusinessAppsRoot(workspaceRoot));
  ...
  assert.equal(checkPlan.steps.find((step) => step.name === 'export-platform-app-seed').mode, 'check');
...
test('application scripts keep commercial default ports and reject obsolete aliases', () => {
  ...
  const productionStarter = readFileSync(path.join(workspaceRoot, 'scripts', 'start-cloud-router-production.mjs'), 'utf8');
...
test('application scripts wire SDKWork application env standard checks and entrypoints', () => {
  ...
  const verifyScript = readFileSync(
    path.join(workspaceRoot, 'scripts', 'verify-cloud-router-application.mjs'),
    'utf8',
  );
  const productRunner = readFileSync(
    path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs'),
    'utf8',
  );
  ...
  assert.equal(rootPackage.scripts.check, 'pnpm exec sdkwork-app check');
  assert.match(rootPackage.scripts['_sdkwork:check'], /check:application-env/u);
  ...
  assert.match(productRunner, /ensureCloudRouterBrowserProductionEnv/u);
  assert.match(productRunner, /ensureCloudRouterBrowserDevelopmentEnv/u);
});
...
test('portal runtime is served by Rust edge server without Node server entrypoint', () => {
  ...
  assert.match(portalPackage.scripts['_sdkwork:client:browser'], /vite\.js --configLoader native/u);
  ...
  assert.equal(portalPackage.scripts.start, 'node ../../scripts/start-cloud-router-production.mjs');
  assert.equal(rootPackage.scripts.start, 'node scripts/start-cloud-router-production.mjs');
});

test('Rust standalone gateway owns configurable portal CSP and static delivery policy', () => {
  const edgeServerSource = readFileSync(
    path.join(workspaceRoot, 'crates', 'sdkwork-cloudrouter-edge-runtime', 'src', 'edge_server.rs'),
    'utf8',
  );
  const gatewayPortalSource = readFileSync(
    path.join(workspaceRoot, 'crates', 'sdkwork-api-cloudrouter-standalone-gateway', 'src', 'portal.rs'),
    'utf8',
  );
  ...
  assert.ok(gatewayPortalSource.includes('build_portal_content_security_policy'));
  assert.ok(gatewayPortalSource.includes('portal_file_path'));
  assert.ok(gatewayPortalSource.includes('inject_portal_runtime_env_script'));
  ...
  assert.ok(readmeSource.includes('SDKWORK_CLOUDROUTER_EDGE_CSP_CONNECT_SRC'));
  assert.ok(readmeSource.includes('[portal.security]'));
  assert.ok(readmeSource.includes('SDKWORK_CLOUDROUTER_TOOL_API_RATE_LIMIT_REQUESTS'));
  assert.ok(readmeSource.includes('SDKWORK_CLOUDROUTER_TOOL_API_SDK_ARCHIVE_ROOT'));
...
test('portal env example defaults to same-origin SDKWork API entrypoint paths', () => {
  const envExample = readFileSync(
    path.join(workspaceRoot, 'apps', 'sdkwork-cloudrouter-pc', '.env.example'),
    'utf8',
  );
...
test('cloud router application launcher preserves forwarded mode arguments after --', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
  );

  const parsed = module.parseCloudRouterProductArgs(['server', '--', '--help']);
...
test('cloud router application launcher drops repeated pnpm separator inside forwarded arguments', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
  );

  const parsed = module.parseCloudRouterProductArgs([
    'server',
    '--',
    '--database-url',
    'sqlite://target/dev/cloudrouter.sqlite',
    '--',
    '--dry-run',
  ]);
...
test('cloud router application launcher help distinguishes workspace PostgreSQL from desktop SQLite runtime', async () => {
  const { stdout } = await execFileAsync(process.execPath, [
    path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs'),
    '--help',
  ], { cwd: workspaceRoot });
...
test('cloud router application launcher parses dev env file before forwarded workspace arguments', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
  );
...
test('cloud router dev database env helper prefers split fields over stale process URL', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'cloud-router-dev-database-env.mjs')).href
  );
  ...
  const resolved = module.resolveCloudRouterDevDatabaseEnv({ env: merged, defaultDatabase: 'none' });
  assert.equal(
...
test('cloud router dev database env helper resolves split PostgreSQL fields', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'cloud-router-dev-database-env.mjs')).href
  );

  const splitConfig = module.resolveCloudRouterDevDatabaseEnv({
    env: {
      SDKWORK_DATABASE_ENGINE: 'postgresql',
      SDKWORK_DATABASE_HOST: '127.0.0.1',
  ...
  assert.equal(
    ...
    'postgresql://sdkwork_ai_dev:router%20pass@127.0.0.1:15432/sdkwork_ai_dev?sslmode=disable',
  );
  ...
  assert.throws(
    () => module.resolveCloudRouterDevDatabaseEnv({
      env: {
  ...
  const explicitConfig = module.resolveCloudRouterDevDatabaseEnv({
    env: {
      SDKWORK_DATABASE_URL:
  ...
  const defaultConfig = module.resolveCloudRouterDevDatabaseEnv({ env: {} });
...
test('cloud router dev database env helper enforces workspace identity and secret files', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'cloud-router-dev-database-env.mjs')).href
  );
  const fixtureRoot = createFixtureDir('cloud-router-database-password-file');
  const passwordFile = path.join(fixtureRoot, 'database.secret');
  writeFixtureFile(fixtureRoot, 'database.secret', 'file secret');

  const passwordFileConfig = module.resolveCloudRouterDevDatabaseEnv({
    env: {
      SDKWORK_DATABASE_ENGINE: 'postgresql',
      SDKWORK_DATABASE_HOST: '127.0.0.1',
      SDKWORK_DATABASE_PORT: '5432',
      ...
      SDKWORK_DATABASE_PASSWORD_FILE: passwordFile,
      SDKWORK_DATABASE_SSL_MODE: 'require',
    },
  });
  assert.equal(
    passwordFileConfig.databaseUrl,
    'postgresql://sdkwork_ai_prod:file%20secret@127.0.0.1:5432/sdkwork_ai_prod?sslmode=require',
  );
  ...
  assert.throws(
    () => module.resolveCloudRouterDevDatabaseEnv({
      env: {
        SDKWORK_DATABASE_ENGINE: 'postgresql',
        SDKWORK_DATABASE_HOST: '127.0.0.1',
        SDKWORK_DATABASE_NAME: 'cloudrouter_dev',
        SDKWORK_DATABASE_USERNAME: 'cloudrouter_dev',
        SDKWORK_DATABASE_PASSWORD: 'secret',
      },
    }),
    /SDKWORK_DATABASE_NAME must use a canonical workspace identity/u,
  );
  assert.throws(
    () => module.resolveCloudRouterDevDatabaseEnv({
      env: {
        ...
        SDKWORK_DATABASE_HOST: '127.0.0.1',
        SDKWORK_DATABASE_NAME: 'sdkwork_ai_dev',
        SDKWORK_DATABASE_SCHEMA: 'cloudrouter_dev',
        SDKWORK_DATABASE_USERNAME: 'sdkwork_ai_dev',
        SDKWORK_DATABASE_PASSWORD: 'secret',
      },
    }),
    /SDKWORK_DATABASE_SCHEMA must equal workspace database sdkwork_ai_dev/u,
  );
  const retiredCloudDatabaseUrl = ['SDKWORK', 'CLOUD', 'DATABASE', 'URL'].join('_');
  assert.throws(
    () => module.resolveCloudRouterDevDatabaseEnv({
      env: {
        SDKWORK_DATABASE_URL:
          'postgresql://sdkwork_ai_dev:secret@127.0.0.1/sdkwork_ai_dev',
        [retiredCloudDatabaseUrl]: 'postgresql://retired',
      },
    }),
    /is retired/u,
  );

  const sqliteConfig = module.resolveCloudRouterDevDatabaseEnv({
    env: {
      SDKWORK_DATABASE_ENGINE: 'sqlite',
      SDKWORK_DATABASE_FILE: 'target/dev/client.sqlite',
    },
  });
  assert.equal(sqliteConfig.kind, 'sqlite');
  assert.equal(sqliteConfig.databaseUrl, 'sqlite:target/dev/client.sqlite');
  rmSync(fixtureRoot, { recursive: true, force: true });
});

test('cloud router dev postgres env example documents split database fields', () => {
  ...
  const ignoredLines = new Set(ignored.split(/\r?\n/u));
  const envExample = readFileSync(envExamplePath, 'utf8');

  assert.equal(existsSync(envExamplePath), true);
  assert.ok(ignoredLines.has('.env'));
  assert.ok(ignoredLines.has('.env.*'));
  assert.ok(ignoredLines.has('!.env.*.example'));
  assert.ok(!ignoredLines.has('!.env.postgres'));
...
test('cloud router application launcher loads dev env file into server workspace env', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
  );
  const fixtureRoot = createFixtureDir('cloud-router-dev-postgres-env');
  ...
  const plan = module.createCloudRouterProductLaunchPlan({
    workspaceRoot,
    mode: 'server',
    install: false,
    platform: 'linux',
  ...
  assert.equal(
    ...
    'postgresql://sdkwork_ai_dev:env%20file%20pass@[::1]:15433/sdkwork_ai_dev?sslmode=disable',
  );
  assert.equal(serverStep.env.SDKWORK_DATABASE_MAX_CONNECTIONS, '15');
});

test('cloud router application launcher loads default PostgreSQL dev profile from workspace files', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
  );
  const fixtureRoot = createFixtureDir('cloud-router-default-dev-postgres-env');
  seedMinimalCloudRouterWorkspaceFixture(fixtureRoot);
  writeFixtureFile(
    fixtureRoot,
  ...
  writeFixtureFile(fixtureRoot, 'apps/sdkwork-cloudrouter-pc/node_modules/.bin/vite', '');

  const examplePlan = module.createCloudRouterProductLaunchPlan({
  ...
  assert.equal(
    ...
    'postgresql://sdkwork_ai_dev:example_dev_pass@[::1]:15432/sdkwork_ai_dev?sslmode=disable',
  );
  assert.equal(examplePlan[0].env.SDKWORK_DATABASE_MAX_CONNECTIONS, '9');
  assert.equal(examplePlan[0].env.SDKWORK_DATABASE_SCHEMA, 'sdkwork_ai_dev');
  ...
  const overridePlan = module.createCloudRouterProductLaunchPlan({
    workspaceRoot: fixtureRoot,
    mode: 'server',
    install: false,
    platform: 'linux',
  ...
  assert.equal(
    ...
    'postgresql://sdkwork_ai_dev:local_override_pass@[::1]:25432/sdkwork_ai_dev?sslmode=require',
  );
  assert.equal(overridePlan[0].env.SDKWORK_DATABASE_MAX_CONNECTIONS, '11');
  rmSync(fixtureRoot, { recursive: true, force: true });
});

test('cloud router application launcher reinstalls portal dependencies when command shims are missing', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
  );
  const fixtureRoot = createFixtureDir('cloud-router-stale-portal-install');
  seedMinimalCloudRouterWorkspaceFixture(fixtureRoot);
  writeFixtureFile(
    fixtureRoot,
    '.env.postgres.example',
  ...
  writeFixtureFile(fixtureRoot, 'apps/sdkwork-cloudrouter-pc/node_modules/vite/package.json', '{}');
  ...
  assert.deepEqual(plan[0].args, ['--dir', 'apps/sdkwork-cloudrouter-pc', 'install']);
  assert.equal(plan[1].label, 'server development workspace');
  rmSync(fixtureRoot, { recursive: true, force: true });
});

test('cloud router application launcher defaults explicit server dev to PostgreSQL only', async () => {
  await withIsolatedDevDatabaseEnv(async () => {
    const module = await import(
      pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
    );
    const fixtureRoot = createFixtureDir('cloud-router-default-server-dev');
    seedMinimalCloudRouterWorkspaceFixture(fixtureRoot);
    writeFixtureFile(fixtureRoot, 'apps/sdkwork-cloudrouter-pc/node_modules/.bin/vite', '');

    try {
      const serverPlan = module.createCloudRouterProductLaunchPlan({
        workspaceRoot: fixtureRoot,
        mode: 'server',
        install: false,
        platform: 'linux',
      ...
      const desktopPlan = module.createCloudRouterProductLaunchPlan({
        workspaceRoot: fixtureRoot,
        mode: 'desktop',
        install: false,
        platform: 'linux',
      ...
      assert.equal(desktopStep.env.SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE, 'desktop');
    } finally {
      rmSync(fixtureRoot, { recursive: true, force: true });
    }
  });
...
test('cloud router application launcher keeps explicit client and desktop modes gateway-backed', async () => {
  await withIsolatedDevDatabaseEnv(async () => {
    const module = await import(
      pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
    );

    const clientPlan = module.createCloudRouterProductLaunchPlan({
      workspaceRoot,
      mode: 'client',
      install: false,
      platform: 'linux',
    ...
    assert.equal(findPlanStep(desktopPlan, 'desktop development workspace').env.SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE, 'desktop');
  });
});

test('cloud router workspace client-only launch plan starts only the remote API client portal', async () => {
  const module = await import(
  ...
  assert.deepEqual(plan.steps.map((step) => step.name), ['portal']);
  ...
  assert.equal(portalStep.env.SDKWORK_CLOUDROUTER_BROWSER_DEV_PROXY_OPEN_API_ORIGIN, 'http://127.0.0.1:3902');
  assert.equal(portalStep.env.SDKWORK_CLOUDROUTER_BROWSER_DEV_PROXY_BACKEND_API_ORIGIN, 'http://127.0.0.1:3902');
  assert.equal(portalStep.env.SDKWORK_CLOUDROUTER_BROWSER_DEV_PROXY_APP_API_ORIGIN, 'http://127.0.0.1:3902');
  assert.equal(portalStep.env.VITE_CLOUDROUTER_OPEN_API_BASE_URL, 'http://127.0.0.1:3902/v1');
  assert.equal(portalStep.env.VITE_CLOUDROUTER_APP_API_BASE_URL, 'http://127.0.0.1:3902/app/v3/api');
  assert.equal(portalStep.env.VITE_CLOUDROUTER_BACKEND_API_BASE_URL, 'http://127.0.0.1:3902/backend/v3/api');
...
test('cloud router workspace launch plan defaults to all-in-one Rust edge runtime', async () => {
  await withIsolatedDevDatabaseEnv(async () => {
    const module = await import(
      pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
    );
    ...
    const installerBinary = module.cloudRouterDevInstallerBinaryPath(workspaceRoot);
    ...
    assert.equal(refreshStep.env.SDKWORK_CLOUDROUTER_STARTUP_INSTALL_MODE, 'ensure');
    assert.equal(refreshStep.env.SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID, undefined);
    assert.equal(
      refreshStep.env.SDKWORK_MODELS_CATALOG_ROOT,
      path.join(workspaceRoot, '..', 'sdkwork-models'),
    );
    assert.equal(plan.steps.some((step) => step.name === 'platform-api-host'), false);
    assert.deepEqual(portalStep.args, [
      '--dir',
      'apps/sdkwork-cloudrouter-pc',
      'run',
      '_sdkwork:client:browser',
    ]);
    ...
    assert.equal(serverStep.env.SDKWORK_CLOUDROUTER_EDGE_CSP_CONNECT_SRC, '');
    assert.equal(serverStep.env.SDKWORK_CLOUDROUTER_TOOL_API_SDK_GENERATOR_BASE_URL, '');
    assert.equal(serverStep.env.SDKWORK_CLOUDROUTER_TOOL_API_SDK_GENERATOR_API_KEY, '');
    assert.equal(
...
test('cloud router server development plan does not synthesize static Snowflake node ids', async () => {
  const previousNodeId = process.env.SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID;
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
  );

  try {
    delete process.env.SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID;
    ...
    for (const stepName of ['installer', 'model-catalog-refresh', 'server']) {
      assert.equal(
        plan.steps.find((step) => step.name === stepName).env.SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID,
        undefined,
      );
    }

    process.env.SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID = '17';
    const overriddenPlan = module.buildWorkspaceCommandPlan(settings, { workspaceRoot, platform: 'linux' });
    for (const stepName of ['installer', 'model-catalog-refresh', 'server']) {
      assert.equal(
        overriddenPlan.steps.find((step) => step.name === stepName).env.SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID,
        '17',
      );
    }
  } finally {
    if (previousNodeId === undefined) {
      ...
      process.env.SDKWORK_CLOUDROUTER_SNOWFLAKE_NODE_ID = previousNodeId;
    }
  }
});

test('cloud router workspace launch plan honors SDKWORK_DATABASE_URL from dev env', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
  );
  ...
  try {
    process.env.SDKWORK_DATABASE_URL =
      'postgresql://sdkwork_ai_dev:env_pass@127.0.0.1:15434/sdkwork_ai_dev?sslmode=disable';
...
test('cloud router workspace launch plan resolves split PostgreSQL env fields directly', async () => {
  await withIsolatedDevDatabaseEnv(async () => {
    const module = await import(
      pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
    );
    ...
    process.env.SDKWORK_DATABASE_NAME = 'sdkwork_ai_dev';
    process.env.SDKWORK_DATABASE_SCHEMA = 'sdkwork_ai_dev';
    process.env.SDKWORK_DATABASE_USERNAME = 'sdkwork_ai_dev';
    ...
    assert.equal(
      settings.databaseUrl,
      'postgresql://sdkwork_ai_dev:direct%20pass@127.0.0.1:15435/sdkwork_ai_dev?sslmode=disable',
    );
    for (const step of serviceSteps) {
...
test('cloud router workspace launch plan supports local split-process debugging', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
  );
  ...
  const portalStep = plan.steps.find((step) => step.name === 'portal');
  const serverStep = plan.steps.find((step) => step.name === 'server');
  assert.equal(gatewayStep.env.SDKWORK_CLOUDROUTER_GATEWAY_BIND, '0.0.0.0:19080');
  assert.equal(appApiStep.env.SDKWORK_CLOUDROUTER_APP_RUNTIME_GATEWAY_BASE_URL, 'http://127.0.0.1:19080');
  assert.equal(portalStep.env.PORTAL_PUBLIC_SDK_BASE_URL, undefined);
  assert.equal(portalStep.env.SDKWORK_CLOUDROUTER_BROWSER_DEV_PROXY_OPEN_API_ORIGIN, 'http://127.0.0.1:19080');
  assert.equal(portalStep.env.SDKWORK_CLOUDROUTER_BROWSER_DEV_PROXY_BACKEND_API_ORIGIN, 'http://127.0.0.1:18081');
  assert.equal(portalStep.env.SDKWORK_CLOUDROUTER_BROWSER_DEV_PROXY_APP_API_ORIGIN, 'http://127.0.0.1:18082');
  assert.equal(serverStep.env.PORTAL_PUBLIC_SDK_BASE_URL, 'http://127.0.0.1:3902');
  assert.equal(serverStep.env.SDKWORK_CLOUDROUTER_ALL_IN_ONE_RUNTIME, '0');
  assert.equal(serverStep.env.SDKWORK_CLOUDROUTER_EDGE_GATEWAY_BASE_URL, 'http://127.0.0.1:19080');
});

test('cloud router workspace reports occupied service ports before startup', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
  );
...
test('cloud router workspace can recheck the portal bind after backend startup', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
  );
...
test('cloud router workspace terminates Windows child process trees', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
  );
  const spawned = [];
...
test('cloud router workspace checks service ports before running installer steps', () => {
...
test('cloud router workspace rechecks backend ports after installer steps', () => {
  const workspaceStarter = readFileSync(
    path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs'),
    'utf8',
  );
...
test('cloud router workspace rechecks the portal port immediately before launching it', () => {
  const workspaceStarter = readFileSync(
    path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs'),
    'utf8',
  );
...
test('cloud router workspace supports custom edge server and direct portal binds', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
  );
  ...
  assert.equal(portalStep.env.SDKWORK_CLOUDROUTER_PORTAL_BIND, '0.0.0.0:13900');
  assert.equal(serverStep.env.SDKWORK_CLOUDROUTER_SERVER_BIND, '0.0.0.0:12900');
  assert.equal(serverStep.env.SDKWORK_CLOUDROUTER_EDGE_PORTAL_BASE_URL, 'http://127.0.0.1:13900');
});

test('cloud router workspace uses one resolved model catalog root for refresh and services', async () => {
...
test('cloud router workspace pins startup install ownership to installer steps', async () => {
  const previousStartupInstallMode = process.env.SDKWORK_CLOUDROUTER_STARTUP_INSTALL_MODE;
  process.env.SDKWORK_CLOUDROUTER_STARTUP_INSTALL_MODE = 'ensure';
  try {
    const module = await import(
      pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
    );
    ...
    const modesByStep = new Map(
      plan.steps
        ...
        .map((step) => [step.name, step.env.SDKWORK_CLOUDROUTER_STARTUP_INSTALL_MODE]),
    );
    ...
    if (previousStartupInstallMode === undefined) {
      delete process.env.SDKWORK_CLOUDROUTER_STARTUP_INSTALL_MODE;
    } else {
      process.env.SDKWORK_CLOUDROUTER_STARTUP_INSTALL_MODE = previousStartupInstallMode;
    }
  }
});

test('cloud router workspace constrains explicit SQLite dev database without overriding explicit database tuning', async () => {
  const previousMaxConnections = process.env.SDKWORK_DATABASE_MAX_CONNECTIONS;
  const previousSettlementWorker = process.env.SDKWORK_CLOUDROUTER_USAGE_SETTLEMENT_WORKER_ENABLED;
  const previousRankingStartup = process.env.SDKWORK_CLOUDROUTER_MODEL_RANKING_RUN_ON_STARTUP;
  try {
    delete process.env.SDKWORK_DATABASE_MAX_CONNECTIONS;
    delete process.env.SDKWORK_CLOUDROUTER_USAGE_SETTLEMENT_WORKER_ENABLED;
    delete process.env.SDKWORK_CLOUDROUTER_MODEL_RANKING_RUN_ON_STARTUP;
    const module = await import(
      pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
    );
    ...
    for (const step of sqlitePlan.steps.filter((step) =>
      ...
      assert.equal(step.env.SDKWORK_CLOUDROUTER_USAGE_SETTLEMENT_WORKER_ENABLED, 'false');
      assert.equal(step.env.SDKWORK_CLOUDROUTER_MODEL_RANKING_RUN_ON_STARTUP, 'false');
    }

    process.env.SDKWORK_DATABASE_MAX_CONNECTIONS = '8';
    process.env.SDKWORK_CLOUDROUTER_USAGE_SETTLEMENT_WORKER_ENABLED = 'true';
    process.env.SDKWORK_CLOUDROUTER_MODEL_RANKING_RUN_ON_STARTUP = 'true';
    ...
    for (const step of tunedPlan.steps.filter((step) =>
      ...
      assert.equal(step.env.SDKWORK_CLOUDROUTER_USAGE_SETTLEMENT_WORKER_ENABLED, 'true');
      assert.equal(step.env.SDKWORK_CLOUDROUTER_MODEL_RANKING_RUN_ON_STARTUP, 'true');
    }
  } finally {
    if (previousMaxConnections === undefined) {
      delete process.env.SDKWORK_DATABASE_MAX_CONNECTIONS;
    ...
    if (previousSettlementWorker === undefined) {
      ...
      process.env.SDKWORK_CLOUDROUTER_USAGE_SETTLEMENT_WORKER_ENABLED = previousSettlementWorker;
    }
    if (previousRankingStartup === undefined) {
      delete process.env.SDKWORK_CLOUDROUTER_MODEL_RANKING_RUN_ON_STARTUP;
    } else {
      process.env.SDKWORK_CLOUDROUTER_MODEL_RANKING_RUN_ON_STARTUP = previousRankingStartup;
    }
  }
});

test('cloud router workspace provides Redis host and port defaults for server dev services', async () => {
  const previousRedisEnabled = process.env.SDKWORK_CLOUDROUTER_REDIS_ENABLED;
  const previousRedisHost = process.env.SDKWORK_CLOUDROUTER_REDIS_HOST;
  const previousRedisPort = process.env.SDKWORK_CLOUDROUTER_REDIS_PORT;
  const previousRedisDatabase = process.env.SDKWORK_CLOUDROUTER_REDIS_DATABASE;
  const previousRedisUrl = process.env.SDKWORK_CLOUDROUTER_REDIS_URL;
  try {
    delete process.env.SDKWORK_CLOUDROUTER_REDIS_ENABLED;
    delete process.env.SDKWORK_CLOUDROUTER_REDIS_HOST;
    delete process.env.SDKWORK_CLOUDROUTER_REDIS_PORT;
    delete process.env.SDKWORK_CLOUDROUTER_REDIS_DATABASE;
    delete process.env.SDKWORK_CLOUDROUTER_REDIS_URL;
    const module = await import(
      pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
    );
    ...
    for (const step of plan.steps.filter((step) =>
      ...
      assert.equal(step.env.SDKWORK_CLOUDROUTER_REDIS_ENABLED, 'true');
      assert.equal(step.env.SDKWORK_CLOUDROUTER_REDIS_HOST, '127.0.0.1');
      assert.equal(step.env.SDKWORK_CLOUDROUTER_REDIS_PORT, '6379');
      assert.equal(step.env.SDKWORK_CLOUDROUTER_REDIS_DATABASE, '0');
      assert.equal(step.env.SDKWORK_CLOUDROUTER_REDIS_URL, undefined);
    }

    process.env.SDKWORK_CLOUDROUTER_REDIS_URL = 'redis://cache.internal:6380/3';
    process.env.SDKWORK_CLOUDROUTER_REDIS_HOST = 'stale-redis.internal';
    process.env.SDKWORK_CLOUDROUTER_REDIS_PORT = '6381';
    process.env.SDKWORK_CLOUDROUTER_REDIS_DATABASE = '4';
    ...
    for (const step of urlPlan.steps.filter((step) =>
      ...
      assert.equal(step.env.SDKWORK_CLOUDROUTER_REDIS_URL, 'redis://cache.internal:6380/3');
      assert.equal(step.env.SDKWORK_CLOUDROUTER_REDIS_HOST, undefined);
      assert.equal(step.env.SDKWORK_CLOUDROUTER_REDIS_PORT, undefined);
      assert.equal(step.env.SDKWORK_CLOUDROUTER_REDIS_DATABASE, undefined);
    }
  } finally {
    if (previousRedisEnabled === undefined) {
      delete process.env.SDKWORK_CLOUDROUTER_REDIS_ENABLED;
    } else {
      process.env.SDKWORK_CLOUDROUTER_REDIS_ENABLED = previousRedisEnabled;
    }
    if (previousRedisHost === undefined) {
      delete process.env.SDKWORK_CLOUDROUTER_REDIS_HOST;
    } else {
      process.env.SDKWORK_CLOUDROUTER_REDIS_HOST = previousRedisHost;
    }
    if (previousRedisPort === undefined) {
      delete process.env.SDKWORK_CLOUDROUTER_REDIS_PORT;
    } else {
      process.env.SDKWORK_CLOUDROUTER_REDIS_PORT = previousRedisPort;
    }
    if (previousRedisDatabase === undefined) {
      delete process.env.SDKWORK_CLOUDROUTER_REDIS_DATABASE;
    } else {
      process.env.SDKWORK_CLOUDROUTER_REDIS_DATABASE = previousRedisDatabase;
    }
    if (previousRedisUrl === undefined) {
      delete process.env.SDKWORK_CLOUDROUTER_REDIS_URL;
    } else {
      process.env.SDKWORK_CLOUDROUTER_REDIS_URL = previousRedisUrl;
    }
  }
});
...
test('admin reset wrapper maps release mode through production runtime config and requires a password', async () => {
  ...
  assert.equal(step.env.SDKWORK_CLOUDROUTER_ADMIN_RESET_PASSWORD, 'Admin-Release-Reset-Password-2026!');
  assert.equal(step.env.SDKWORK_CLOUDROUTER_CONFIG_FILE, configFile);
  assert.equal(step.env.SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE, 'server');
  assert.equal(
    step.env.SDKWORK_DATABASE_URL,
    'postgresql://sdkwork_ai_prod:secret@db.internal:5432/sdkwork_ai_prod?sslmode=require',
  );
...
test('database management wrapper maps pnpm init and upgrade commands to the installer', async () => {
  ...
  assert.equal(initStep.env.SDKWORK_CLOUDROUTER_CONFIG_FILE, configFile);
  ...
  assert.equal(rootDbPlan.steps[0].env.SDKWORK_CLOUDROUTER_CONFIG_FILE, configFile);
});
...
test('database management example config documents structured PostgreSQL fields', () => {
  const examplePath = path.join(workspaceRoot, 'etc', 'cloudrouter.database.example.toml');
...
test('cloud router workspace rejects obsolete portal dev bind option', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
  );
...
test('cloud router workspace supports explicit Rust server forwarding target URLs', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'dev', 'start-workspace.mjs')).href
  );
  ...
  assert.equal(serverEnv.SDKWORK_CLOUDROUTER_EDGE_GATEWAY_BASE_URL, 'http://gateway.internal:18080');
  assert.equal(serverEnv.SDKWORK_CLOUDROUTER_EDGE_BACKEND_API_BASE_URL, 'https://admin.internal');
  assert.equal(serverEnv.SDKWORK_CLOUDROUTER_EDGE_APP_API_BASE_URL, 'http://app.internal:18082');
});
...
test('workspace dry-run output uses server and portal bind names without obsolete aliases', async () => {
  ...
  assert.ok(textOutput.includes('SDKWORK_CLOUDROUTER_SERVER_BIND=0.0.0.0:12900'));
  assert.ok(textOutput.includes('SDKWORK_CLOUDROUTER_PORTAL_BIND=0.0.0.0:13900'));
...
test('workspace launch plan exposes explicit forwarded header trust settings', async () => {
  ...
  assert.equal(defaultServerEnv.SDKWORK_CLOUDROUTER_EDGE_EXTERNAL_SCHEME, 'http');
  assert.equal(defaultServerEnv.SDKWORK_CLOUDROUTER_EDGE_TRUST_FORWARDED_HEADERS, '0');
  ...
  assert.equal(serverEnv.SDKWORK_CLOUDROUTER_EDGE_EXTERNAL_SCHEME, 'https');
  assert.equal(serverEnv.SDKWORK_CLOUDROUTER_EDGE_TRUST_FORWARDED_HEADERS, '1');
  assert.equal(dryRun.externalScheme, 'https');
  assert.equal(dryRun.trustForwardedHeaders, true);
  assert.ok(textOutput.includes('SDKWORK_CLOUDROUTER_EDGE_EXTERNAL_SCHEME=https'));
  assert.ok(textOutput.includes('SDKWORK_CLOUDROUTER_EDGE_TRUST_FORWARDED_HEADERS=1'));
...
test('cloud router application launcher desktop mode runs install-checked workspace and installs portal dependencies when requested', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
  );
  ...
  assert.equal(plan[1].env.SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE, 'desktop');
});

test('cloud router application launcher service mode runs install-checked workspace with service flags', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
  );
  ...
  assert.equal(serviceStep.env.SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE, 'desktop');
  assert.equal(serviceStep.env.SDKWORK_CLOUDROUTER_SERVICE_MODE, '1');
  assert.equal(serviceStep.env.SDKWORK_CLOUDROUTER_PORTAL_START_HIDDEN, '1');
  assert.equal(serviceStep.shell, false);
});

test('cloud router application launcher forwards workspace arguments into server mode', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
  );
...
test('cloud router application launcher server plan prints human-readable access matrix by default', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'run-cloud-router-application.mjs')).href
  );
...
test('production starter supports help, dry-run, and full edge access matrix', async () => {
  ...
  const artifacts = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'cloud-router-production-artifacts.mjs')).href
  );
  ...
  const env = module.resolveStartProductionEnv(
    {
      SDKWORK_CLOUDROUTER_SERVER_BIND: '0.0.0.0:12900',
  ...
  assert.equal(env.SDKWORK_CLOUDROUTER_EDGE_BACKEND_API_BASE_URL, 'https://admin.internal');
  assert.equal(env.SDKWORK_CLOUDROUTER_EDGE_APP_API_BASE_URL, 'http://app.internal:18082');
  assert.equal(env.SDKWORK_CLOUDROUTER_EDGE_EXTERNAL_SCHEME, 'https');
  assert.equal(env.SDKWORK_CLOUDROUTER_EDGE_TRUST_FORWARDED_HEADERS, '1');
  ...
  assert.equal(env.SDKWORK_CLOUDROUTER_TOOL_API_RATE_LIMIT_REQUESTS, '120');
  assert.equal(env.SDKWORK_CLOUDROUTER_TOOL_API_RATE_LIMIT_WINDOW_SECONDS, '60');
  assert.equal(
    env.SDKWORK_CLOUDROUTER_TOOL_API_SDK_ARCHIVE_ROOT,
    path.join(workspaceRoot, 'apps', 'sdkwork-cloudrouter-pc', 'dist', 'sdk-archives'),
  );
  ...
  assert.equal(
    ...
    path.join(
      ...
      'target-codex',
      'release',
      'sdkwork-api-cloudrouter-standalone-gateway.exe',
    ),
  );
  assert.deepEqual(
    module.resolveStartProductionCommand(
      { ...env, SDKWORK_CLOUDROUTER_GATEWAY_BIN: 'D:\\prod\\cloudrouter.exe' },
      'win32',
      workspaceRoot,
    ),
    {
      command: 'D:\\prod\\cloudrouter.exe',
      args: [],
      source: 'env',
    },
  );
  ...
  assert.ok(lines.includes('[start-production]   SDKWORK_CLOUDROUTER_TOOL_API_RATE_LIMIT_REQUESTS=120'));
  assert.ok(lines.includes('[start-production]   SDKWORK_CLOUDROUTER_TOOL_API_RATE_LIMIT_WINDOW_SECONDS=60'));
  assert.ok(lines.some((line) => (
    line.startsWith('[start-production]   SDKWORK_CLOUDROUTER_TOOL_API_SDK_ARCHIVE_ROOT=')
    && line.includes(path.join('apps', 'sdkwork-cloudrouter-pc', 'dist', 'sdk-archives'))
  )));
  assert.ok(lines.includes('[start-production]   SDKWORK_CLOUDROUTER_EDGE_EXTERNAL_SCHEME=https'));
  assert.ok(lines.includes('[start-production]   SDKWORK_CLOUDROUTER_EDGE_TRUST_FORWARDED_HEADERS=1'));
});
...
test('production starter defaults to all-in-one runtime without forwarding targets', async () => {
  ...
  assert.equal(env.SDKWORK_CLOUDROUTER_EDGE_SERVER, '1');
  assert.equal(env.SDKWORK_CLOUDROUTER_ALL_IN_ONE_RUNTIME, '1');
  assert.equal(env.SDKWORK_CLOUDROUTER_APP_RUNTIME_GATEWAY_BASE_URL, 'http://127.0.0.1:12900');
...
test('cloud router application help does not present distributed dev as a standard workflow example', async () => {
...
test('production starter auto-initializes desktop SQLite runtime config', async () => {
  ...
  assert.equal(result.databaseUrl, `sqlite://${slashPath(path.join(env.HOME, '.sdkwork', 'router', 'data', 'cloudrouter.sqlite'))}`);
...
test('production starter initializes server PostgreSQL runtime config template', async () => {
  ...
  assert.equal(result.env.SDKWORK_CLOUDROUTER_CONFIG_FILE, configFile);
  assert.equal(result.env.SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE, 'server');
  assert.equal(
  ...
  assert.equal(
    ...
    slashPath(path.join(fixtureRoot, 'etc', 'Data', 'cloudrouter.sqlite')),
  );
...
test('production starter expands password_file environment variables', async () => {
  ...
  writeFileSync(configFile, [
    ...
    'database = "sdkwork_ai_prod"',
    'username = "sdkwork_ai_prod"',
    'password_file = "${SDKWORK_CLOUDROUTER_TEST_SECRET_ROOT}/database.secret"',
    'ssl_mode = "require"',
    'max_connections = 16',
    '',
    '[paths]',
  ...
  const result = module.prepareStartProductionRuntimeConfig({
    baseEnv: {
      HOME: path.join(fixtureRoot, 'home'),
      SDKWORK_CLOUDROUTER_TEST_SECRET_ROOT: secretRoot,
    },
    settings: module.parseStartProductionArgs([
      '--deployment-mode',
      'server',
  ...
  assert.equal(result.action, 'existing');
  assert.equal(result.databaseEngine, 'postgresql');
  assert.equal(result.databaseUrl, 'postgresql://sdkwork_ai_prod:real-password@db.internal:5432/sdkwork_ai_prod?sslmode=require');
  assert.equal(result.blockingIssue, null);
  rmSync(fixtureRoot, { recursive: true, force: true });
});
...
test('production starter help documents automatic runtime config initialization', async () => {
  const { stdout } = await execFileAsync(process.execPath, [
    path.join(workspaceRoot, 'scripts', 'start-cloud-router-production.mjs'),
    '--help',
  ], {
    cwd: workspaceRoot,
    maxBuffer: 1024 * 1024,
...
test('production build creates portal assets and Rust edge release artifact', async () => {
  ...
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'build-cloud-router-production.mjs')).href
  );

  assert.equal(rootPackage.scripts.build, 'pnpm exec sdkwork-app build');
  assert.equal(rootPackage.scripts['_sdkwork:build'], 'node scripts/build-cloud-router-production.mjs');
  ...
  assert.deepEqual(plan[0].args, ['-B', '-m', 'tools.cloudrouter_gateway_openapi_generator']);
  assert.deepEqual(plan[1].args, ['--dir', 'sdks/cloudrouter-app-sdk/cloudrouter-app-sdk-typescript/generated/server-openapi', 'build']);
  assert.deepEqual(plan[2].args, ['--dir', 'sdks/cloudrouter-backend-sdk/cloudrouter-backend-sdk-typescript/generated/server-openapi', 'build']);
  assert.deepEqual(plan[3].args, ['--dir', 'sdks/cloudrouter-open-sdk/cloudrouter-open-sdk-typescript/generated/server-openapi', 'build']);
  assert.equal(plan[1].attempts, 2);
  assert.equal(plan[2].attempts, 2);
  assert.equal(plan[3].attempts, 2);
  assert.deepEqual(plan[4].args, ['--dir', 'apps/sdkwork-cloudrouter-pc', 'build']);
  assert.deepEqual(plan[5].args, ['scripts/archive-cloud-router-sdks.mjs']);
  ...
  assert.ok(
    module.renderProductionBuildPlan(plan, { CARGO_TARGET_DIR: 'target-codex' }, 'win32', workspaceRoot)
      .some((line) => line.includes('target-codex') && line.includes('sdkwork-api-cloudrouter-standalone-gateway.exe')),
  );
  assert.ok(
    module.renderProductionBuildPlan(plan, { CARGO_TARGET_DIR: 'target-codex' }, 'win32', workspaceRoot)
      .some((line) => line.includes('target-codex') && line.includes('sdkwork-cloudrouter-edge-runtime.exe')),
  );
  assert.ok(
    module.renderProductionBuildPlan(plan, { CARGO_TARGET_DIR: 'target-codex' }, 'win32', workspaceRoot)
  ...
  const buildProductionSource = readFileSync(
    path.join(workspaceRoot, 'scripts', 'build-cloud-router-production.mjs'),
    'utf8',
  );
...
test('install package planner covers platforms, architectures, modes, fast init, and security defaults', async () => {
  ...
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'plan-cloud-router-install-packages.mjs')).href
  );
  ...
  assert.equal(plan.product, 'sdkwork-cloudrouter');
  assert.equal(plan.packageName, 'cloudrouter');
  assert.equal(plan.runtimeName, 'cloudrouter');
  assert.equal(plan.displayName, 'SdkWork CloudRouter');
  ...
  assert.equal(windowsService.archiveName, 'cloudrouter-windows-x64-server-0.1.0.zip');
  assert.equal(windowsService.binaryName, 'cloudrouter.exe');
  assert.equal(windowsService.installerBinaryName, 'cloudrouterctl.exe');
  assert.deepEqual(windowsService.serviceIntegration, {
    kind: 'windows-service',
    manifest: 'service/windows/cloudrouter.xml',
  });
  assert.ok(windowsService.artifacts.some((artifact) =>
    artifact.kind === 'edge-binary' && artifact.path === 'bin/cloudrouter.exe'
  ));
  assert.ok(windowsService.artifacts.some((artifact) =>
    artifact.kind === 'installer-binary' && artifact.path === 'bin/cloudrouterctl.exe'
  ));
  assert.ok(windowsService.artifacts.some((artifact) =>
  ...
  assert.ok(windowsService.initCommands.includes('.\\bin\\cloudrouterctl.exe ensure'));
  assert.ok(windowsService.initCommands.includes('.\\bin\\cloudrouterctl.exe refresh-catalog --force'));
  assert.equal(windowsService.startCommand, '.\\bin\\cloudrouter.exe');
  ...
  assert.equal(windowsService.redisPolicy.envOverrides.includes('SDKWORK_CLOUDROUTER_REDIS_HOST'), true);
  assert.equal(windowsService.redisPolicy.envOverrides.includes('SDKWORK_CLOUDROUTER_REDIS_PORT'), true);
  assert.equal(windowsService.redisPolicy.envOverrides.includes('SDKWORK_CLOUDROUTER_REDIS_DATABASE'), true);
  assert.equal(windowsService.redisPolicy.envOverrides.includes('SDKWORK_CLOUDROUTER_REDIS_URL'), true);
  assert.equal(windowsService.redisPolicy.envOverrides.includes('SDKWORK_CLOUDROUTER_REDIS_KEY_PREFIX'), true);
  assert.equal(windowsService.redisPolicy.envOverrides.includes('SDKWORK_CLOUDROUTER_REDIS_TLS'), true);
  assert.equal(windowsService.redisPolicy.envOverrides.includes('SDKWORK_CLOUDROUTER_REDIS_MAX_CONNECTIONS'), true);
  assert.equal(windowsService.redisPolicy.envOverrides.includes('SDKWORK_CLOUDROUTER_REDIS_CONNECT_TIMEOUT_MILLIS'), true);
  assert.equal(windowsService.redisPolicy.envOverrides.includes('SDKWORK_CLOUDROUTER_REDIS_COMMAND_TIMEOUT_MILLIS'), true);
  assert.equal(windowsService.redisPolicy.envOverrides.includes('SDKWORK_CLOUDROUTER_REDIS_POOL_IDLE_TIMEOUT_SECONDS'), true);
  assert.equal(windowsService.redisPolicy.keyPrefix, 'cloudrouter');
  ...
  assert.equal(linuxContainer.containerIntegration.entrypoint, '/opt/sdkwork/router/bin/cloudrouter');
  ...
  assert.ok(linuxContainer.initCommands.includes('./bin/cloudrouterctl ensure'));
  ...
  assert.equal(windowsContainer.containerIntegration.entrypoint, 'C:/sdkwork/router/bin/cloudrouter.exe');
...
test('install package archive builder creates manifest-backed archives without local release secrets', async () => {
  ...
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'build-cloud-router-install-package.mjs')).href
  );
  ...
  writeFileSync(path.join(stagingRoot, 'bin', 'cloudrouter.exe'), 'gateway-binary');
  writeFileSync(path.join(stagingRoot, 'bin', 'cloudrouterctl.exe'), 'installer-binary');
  ...
  try {
    ...
    assert.equal(buildPlan.archivePath, path.join(outputDir, 'cloudrouter-windows-x64-archive-0.1.0.zip'));
    ...
    assert.equal(result.archive.file, 'cloudrouter-windows-x64-archive-0.1.0.zip');
    ...
    assert.equal(result.manifest.installConfiguration.redis.keyPrefix, 'cloudrouter');
    ...
    assert.equal(aggregateManifest.archives[0].file, 'cloudrouter-windows-x64-archive-0.1.0.zip');
...
test('install package manifests distinguish schema version dates from generation timestamps', async () => {
  ...
  const explicitGeneratedAt = module.resolveManifestGeneratedAt({
    env: { SDKWORK_CLOUDROUTER_RELEASE_GENERATED_AT: '2026-05-16T08:05:57Z' },
  });
  ...
  const packageItem = {
    ...
    archiveName: 'cloudrouter-linux-x64-server-0.3.0.tar.gz',
    binaryName: 'cloudrouter',
    installerBinaryName: 'cloudrouterctl',
    startCommand: './bin/cloudrouter',
  ...
  const aggregate = module.createAggregateManifest(
    buildPlan,
    {
      file: 'cloudrouter-linux-x64-server-0.3.0.deb',
...
test('install package builder emits service and container deployment packages from the shared plan', async () => {
  ...
  mkdirSync(path.join(stagingRoot, 'bin'), { recursive: true });
  mkdirSync(path.join(stagingRoot, 'portal', 'dist', 'sdk-archives'), { recursive: true });
  writeFileSync(path.join(stagingRoot, 'bin', 'cloudrouter'), 'gateway-binary');
  writeFileSync(path.join(stagingRoot, 'bin', 'cloudrouterctl'), 'installer-binary');
  writeFileSync(path.join(stagingRoot, 'portal', 'dist', 'index.html'), '<!doctype html>');
  writeFileSync(path.join(stagingRoot, 'portal', 'dist', 'sdk-archives', 'sdk.zip'), 'sdk-archive');
  writeFileSync(path.join(stagingRoot, '.env.release.example'), 'PORTAL_PUBLIC_API_BASE_URL=/v1\n');
  ...
  try {
    ...
    assert.ok(servicePlan.entries.some((entry) =>
      entry.generated && entry.archivePath === 'service/linux/cloudrouter.service'
    ));
    ...
    assert.ok(serviceConfigTemplate.includes('key_prefix = "cloudrouter"'));
    ...
    assert.equal(
      serviceResult.manifest.generatedArtifacts.some((artifact) =>
        artifact.path === 'service/linux/cloudrouter.service'
      ),
      true,
    );
    ...
    assert.deepEqual(serviceResult.manifest.installConfiguration.requestLimits, {
      ...
      envOverrides: [
        'SDKWORK_CLOUDROUTER_ADMIN_APP_JSON_BODY_MAX_BYTES',
        'SDKWORK_CLOUDROUTER_ADMIN_SKILL_JSON_BODY_MAX_BYTES',
        'SDKWORK_CLOUDROUTER_PAYMENT_CALLBACK_BODY_MAX_BYTES',
        'SDKWORK_CLOUDROUTER_GATEWAY_INVOCATION_BODY_MAX_BYTES',
      ],
    });
    ...
    assert.equal(metadata.entrypoint, '/opt/sdkwork/router/bin/cloudrouter');
    ...
    assert.ok(!containerInstallGuide.includes('--secret cloudrouter-postgres-password'));
    assert.equal(
    ...
    writeFileSync(path.join(windowsStagingRoot, 'bin', 'cloudrouter.exe'), 'gateway-binary');
    writeFileSync(path.join(windowsStagingRoot, 'bin', 'cloudrouterctl.exe'), 'installer-binary');
    ...
    assert.equal(windowsContainerResult.archive.file, 'cloudrouter-windows-x64-container-0.1.0.zip');
    assert.equal(
    ...
    assert.ok(desktopInstallGuide.includes('/usr/bin/cloudrouterctl ensure'));
    assert.ok(desktopInstallGuide.includes('/usr/bin/cloudrouterctl refresh-catalog --force'));
    assert.ok(desktopInstallGuide.includes('/usr/bin/cloudrouter'));
  } finally {
    rmSync(fixtureRoot, { recursive: true, force: true });
  }
});
...
test('generated install guides use native desktop install paths across platforms', async () => {
  ...
  assert.ok(windowsGuide.includes('& "C:/Program Files/sdkwork/router/bin/cloudrouterctl.exe" ensure'));
  assert.ok(windowsGuide.includes('& "C:/Program Files/sdkwork/router/bin/cloudrouterctl.exe" refresh-catalog --force'));
  assert.ok(windowsGuide.includes('& "C:/Program Files/sdkwork/router/bin/cloudrouter.exe"'));
  assert.ok(!windowsGuide.includes('.\\bin\\cloudrouterctl.exe ensure'));

  const macosGuide = module.createInstallGuide(macosDesktop);
  assert.ok(macosGuide.includes('/opt/sdkwork/router/bin/cloudrouterctl ensure'));
  assert.ok(macosGuide.includes('/opt/sdkwork/router/bin/cloudrouterctl refresh-catalog --force'));
  assert.ok(macosGuide.includes('/opt/sdkwork/router/bin/cloudrouter'));
  assert.ok(!macosGuide.includes('./bin/cloudrouterctl ensure'));
});
...
test('macOS service packages run initialization through a launchd runner', async () => {
  ...
  try {
    ...
    assert.ok(servicePlan.entries.some((entry) =>
      entry.archivePath === 'service/macos/cloudrouter-service-runner'
      && entry.generated
    ...
    assert.equal(tarEntries.get('service/macos/cloudrouter-service-runner')?.mode, 0o755);
    const runnerText = readTarEntryText(tarBytes, 'service/macos/cloudrouter-service-runner');
    assert.ok(runnerText.includes('/Library/Application Support/sdkwork/router/bin/cloudrouterctl ensure'));
    assert.ok(runnerText.includes('/Library/Application Support/sdkwork/router/bin/cloudrouterctl refresh-catalog --force'));
    assert.ok(runnerText.includes('exec /Library/Application Support/sdkwork/router/bin/cloudrouter "$@"'));

    const plistText = readTarEntryText(tarBytes, 'service/macos/com.sdkwork.cloudrouter.plist');
    assert.ok(plistText.includes('/Library/Application Support/sdkwork/router/service/macos/cloudrouter-service-runner'));
    assert.ok(!plistText.includes('/Library/Application Support/sdkwork/router/bin/cloudrouter</string>'));
  } finally {
    rmSync(fixtureRoot, { recursive: true, force: true });
  }
});
...
test('install package archive builder emits tar.gz bytes for non-Windows packages', async () => {
  ...
  try {
    ...
    assert.equal(result.archive.file, 'cloudrouter-linux-arm64-archive-0.1.0.tar.gz');
    assert.equal(archiveBytes[0], 0x1f);
    assert.equal(archiveBytes[1], 0x8b);
    const tarBytes = gunzipSync(archiveBytes);
    const tarEntries = readTarEntries(tarBytes);
    assert.equal(tarEntries.get('bin/cloudrouter')?.mode, 0o755);
    assert.equal(tarEntries.get('bin/cloudrouterctl')?.mode, 0o755);
...
test('native installer builder emits apt-installable Debian packages for Linux service mode', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'build-cloud-router-native-installer.mjs')).href
  );
  ...
  try {
    ...
    assert.equal(plan.installerName, 'cloudrouter-linux-x64-server-0.1.0.deb');
    assert.deepEqual(module.validateNativeInstallerBuildPlan(plan), []);

    const result = await module.buildNativeInstaller(plan);
    assert.equal(result.installer.file, 'cloudrouter-linux-x64-server-0.1.0.deb');
    ...
    assert.equal(result.manifest.nativeInstall.files.installer, '/usr/bin/cloudrouterctl');
    ...
    assert.equal(result.manifest.nativeInstall.service.name, 'cloudrouter.service');
    ...
    assert.ok(controlText.includes('Package: cloudrouter'));
    ...
    assert.ok(!postinstText.includes('systemctl enable --now cloudrouter.service'));
    ...
    assert.ok(systemdText.includes('EnvironmentFile=-/etc/sdkwork/router/cloudrouter.env'));
    assert.ok(systemdText.includes('ExecStartPre=/usr/bin/cloudrouterctl ensure'));
    assert.ok(systemdText.includes('ExecStartPre=/usr/bin/cloudrouterctl refresh-catalog --force'));
    ...
    assert.deepEqual(aggregateManifest.archives.map((archive) => archive.file), [
      'cloudrouter-linux-x64-server-0.1.0.deb',
    ]);

    const { stdout, stderr } = await execFileAsync(process.execPath, [
      path.join(workspaceRoot, 'scripts', 'validate-cloud-router-install-artifacts.mjs'),
      '--package-id',
      'linux-x64-service',
      '--artifact-path',
...
test('native installer builder keeps Linux desktop packages user-scoped and self-describing', async () => {
  ...
  try {
    ...
    const result = await module.buildNativeInstaller(plan);
    assert.equal(result.installer.file, 'cloudrouter-linux-x64-desktop-0.1.0.deb');
...
test('native installer builder CLI validates cross-platform service and desktop installers in dry-run mode', async () => {
  ...
  const validator = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'validate-cloud-router-install-artifacts.mjs')).href
  );
  const { stdout, stderr } = await execFileAsync(process.execPath, [
    path.join(workspaceRoot, 'scripts', 'build-cloud-router-native-installer.mjs'),
    '--all',
    '--check',
    '--dry-run',
    '--json',
  ...
  assert.ok(payload.plans.some((plan) => plan.installerName === 'cloudrouter-linux-x64-server-0.3.0.deb'));
  assert.ok(payload.plans.some((plan) => plan.installerName === 'cloudrouter-windows-arm64-desktop-0.3.0.msi'));
  ...
  assert.equal(linuxService.nativeInstallLayout.service.name, 'cloudrouter.service');
  assert.equal(linuxService.nativeInstallLayout.commands.start, 'sudo systemctl start cloudrouter');
...
test('sdkwork workflow validates native installer payload layouts before upload', () => {
  ...
  assert.ok(validateSource.includes('node scripts/validate-cloud-router-install-artifacts.mjs'));
...
test('sdkwork workflow can opt into CDN download links without making CDN mandatory', () => {
  ...
  assert.ok(buildSource.includes('CLOUDROUTER_DOWNLOAD_CDN_BASE_URL'));
  assert.ok(buildSource.includes('if ($env:CLOUDROUTER_DOWNLOAD_CDN_BASE_URL)'));
  assert.ok(buildSource.includes('--cdn-base-url'));
  assert.ok(buildSource.includes('node scripts/update-cloud-router-downloads.mjs @downloadArgs'));
});
...
test('install package tar writer supports long production asset paths', async () => {
  ...
  const longPath = 'portal/dist/assets/admin/operations/runtime/chunks/sdkwork-cloudrouter-pc-admin-operations-runtime-production-entrypoint-bundle-abcdef1234567890.js';
...
test('install package archive builder CLI emits pure JSON when requested', async () => {
  ...
  try {
    const { stdout, stderr } = await execFileAsync(process.execPath, [
      path.join(workspaceRoot, 'scripts', 'build-cloud-router-install-package.mjs'),
      '--package-id',
      'windows-x64-archive',
      '--staging-root',
      stagingRoot,
...
test('install init smoke validates fast initialization without starting dev services', async () => {
  ...
  try {
    ...
    assert.equal(smokePlan.databaseUrl, 'postgresql://sdkwork_ai_prod@release-smoke.invalid:5432/sdkwork_ai_prod');
    ...
    assert.ok(smokePlan.steps.some((step) =>
      step.id === 'database-ensure' && step.command === './bin/cloudrouterctl ensure'
    ));
    assert.ok(smokePlan.steps.some((step) =>
      step.id === 'catalog-refresh' && step.command === './bin/cloudrouterctl refresh-catalog --force'
    ));
    assert.ok(!smokePlan.steps.some((step) =>
    ...
    assert.ok(writtenEnv.includes('SDKWORK_CLOUDROUTER_CONFIG_FILE='));
    assert.ok(!writtenEnv.includes('SDKWORK_DATABASE_URL="sqlite://'));
    assert.ok(!writtenEnv.includes('SDKWORK_DATABASE_URL='));
    ...
    assert.ok(writtenConfig.includes('username = "sdkwork_ai_prod"'));
    ...
    assert.equal(desktopSmokePlan.databaseUrl, `sqlite://${path.join(fixtureRoot, 'desktop', 'cloudrouter-install-init.sqlite').replaceAll('\\', '/')}`);
    assert.ok(desktopSmokePlan.databasePath.endsWith('cloudrouter-install-init.sqlite'));
...
test('install init smoke resolves installer binaries from package root and rejects missing package roots', async () => {
  ...
  writeFileSync(path.join(packageRoot, 'bin', 'cloudrouterctl'), 'installer-binary');
  ...
  try {
    const smokePlan = module.createInstallInitSmokePlan({
      ...
      installerBin: 'bin/cloudrouterctl',
      requireInstaller: true,
    });
    assert.equal(smokePlan.packageRoot, packageRoot);
    assert.equal(smokePlan.installerBin, path.join(packageRoot, 'bin', 'cloudrouterctl'));
    assert.deepEqual(module.validateInstallInitSmokePlan(smokePlan), []);
...
test('production SDK archiver creates deterministic ZIP artifacts for generated SDKs', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'archive-cloud-router-sdks.mjs')).href
  );
  ...
  assert.deepEqual(module.defaultSdkArchiveSpecs().map((spec) => spec.archiveName), [
    ...
    'sdkwork-cloudrouter-open-sdk-typescript-0.1.0.zip',
  ]);
  ...
  const manifest = module.buildSdkArchiveManifest(
    ...
    new Map([
      ['sdkwork-cloudrouter-app-sdk-typescript-0.1.0.zip', { size: 1200, sha256: 'a'.repeat(64) }],
      ['sdkwork-cloudrouter-backend-sdk-typescript-0.1.0.zip', { size: 2200, sha256: 'b'.repeat(64) }],
      ['sdkwork-cloudrouter-open-sdk-typescript-0.1.0.zip', { size: 3200, sha256: 'c'.repeat(64) }],
    ]),
  );
  ...
  assert.equal(manifest.archives[0].packageName, '@sdkwork/cloudrouter-app-sdk');
  assert.equal(manifest.archives[1].packageName, '@sdkwork/cloudrouter-backend-sdk');
  assert.equal(manifest.archives[2].packageName, '@sdkwork/cloudrouter-open-sdk');
...
test('Rust edge SDK archive tool API is constrained to generated SDK packages', () => {
  ...
  assert.ok(edgeServerSource.includes('sdkwork-cloudrouter-app-sdk-typescript-0.1.0.zip'));
  assert.ok(edgeServerSource.includes('sdkwork-cloudrouter-backend-sdk-typescript-0.1.0.zip'));
  assert.ok(edgeServerSource.includes('@sdkwork/cloudrouter-app-sdk'));
  assert.ok(edgeServerSource.includes('@sdkwork/cloudrouter-backend-sdk'));
  assert.ok(edgeServerSource.includes('SdkworkAppClient'));
  assert.ok(edgeServerSource.includes('unsupported_sdk_archive'));
  assert.ok(!edgeServerSource.includes('CloudRouterSDK'));
  assert.ok(!edgeServerSource.includes('sdkwork-cloudrouter-sdk'));
  assert.ok(!edgeServerSource.includes('"sdkwork-cloudrouter-sdk"'));
  assert.ok(!edgeServerSource.includes('sdkwork-cloudrouter-sdk-typescript-1.0.0.zip'));
  assert.ok(!edgeServerTestSource.includes('CloudRouterSDK'));
  assert.ok(!edgeServerTestSource.includes('@sdkwork/cloudrouter-sdk'));
  assert.ok(!edgeServerTestSource.includes('sdkwork-cloudrouter-sdk-typescript-1.0.0.zip'));
});
...
test('API router product chain is covered from portal services through SDK and Rust edge', () => {
  ...
  const apiKeyServiceSource = readFileSync(
    path.join(
      workspaceRoot,
      'apps',
      'sdkwork-cloudrouter-pc',
  ...
  const routingApiOperationsSource = readFileSync(
    path.join(
      workspaceRoot,
      'apps',
      'sdkwork-cloudrouter-pc',
  ...
  const appApiSource = readFileSync(
    path.join(workspaceRoot, 'crates', 'sdkwork-routes-cloudrouter-app-api', 'src', 'routes.rs'),
    'utf8',
  );
  const appRoutingReadSource = readFileSync(
    path.join(workspaceRoot, 'services', 'sdkwork-cloudrouter-router-service', 'src', 'api', 'app_routing.rs'),
    'utf8',
  );
  ...
  const gatewayRuntimeSource = readFileSync(
    path.join(workspaceRoot, 'crates', 'sdkwork-cloudrouter-edge-runtime', 'src', 'runtime.rs'),
    'utf8',
  );
  const openaiChatSource = readFileSync(
    path.join(workspaceRoot, 'services', 'sdkwork-cloudrouter-router-service', 'src', 'api', 'openai_chat.rs'),
    'utf8',
  );
  const openaiChatTestSource = readFileSync(
    path.join(workspaceRoot, 'crates', 'sdkwork-cloudrouter-edge-runtime', 'tests', 'openai_chat_route.rs'),
    'utf8',
  );
  const appAiServiceSurface = [
    modelServiceSource,
    playgroundServiceSource,
    appRuntimeApiOperationsSource,
    apiKeyServiceSource,
    routingApiOperationsSource,
  ].join('\n');
  ...
  const requiredAppRouterOperations = [
    {
      ...
      manifestSource: 'apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudrouter-pc-models/src/modelService.ts',
      portalRoute: '/models',
      sdkPath: '/ai/models',
    ...
    },
    {
      ...
      method: 'GET',
      manifestPath: '/app/v3/api/ai/routing/account_groups',
      manifestSource: 'apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudrouter-pc-console-api-keys/src/apiKeyService.ts',
      portalRoute: '/console/api-keys',
      sdkPath: '/ai/routing/account_groups',
      rustSource: appRoutingReadSource,
      rustRoutePath: '/app/v3/api/ai/routing/account_groups',
      skipEdgeSmokeCheck: true,
    },
    {
      operation: 'fetchRoutingApiKeys',
      method: 'GET',
      manifestPath: '/app/v3/api/ai/routing/api_keys',
      manifestSource: 'apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudroutes-pc-commons/src/routingApiOperations.ts',
      portalRoute: '/console/api-keys',
    ...
    {
      operation: 'fetchRoutingRequestTraces',
      method: 'GET',
      manifestPath: '/app/v3/api/ai/routing/request_traces',
      manifestSource: 'apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudroutes-pc-commons/src/routingApiOperations.ts',
      portalRoute: '/console/gateway',
    ...
    {
      operation: 'fetchRoutingUsage',
      method: 'GET',
      manifestPath: '/app/v3/api/ai/routing/usage',
      manifestSource: 'apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudroutes-pc-commons/src/routingApiOperations.ts',
      portalRoute: '/console/usage',
  ...
  const sdkServiceCallByOperation = {
    ...
    fetchGroups: 'getCloudRouterAppSdkClient().ai.routing.accountGroups.list()',
    fetchRoutingApiKeys: 'getCloudRouterAppSdkClient().ai.routing.apiKeys.list(',
    fetchRoutingRequestTraces: 'getCloudRouterAppSdkClient().ai.routing.requestTraces.list(',
    fetchRoutingUsage: 'getCloudRouterAppSdkClient().ai.routing.usage.retrieve()',
  };

  const sdkMethodByOperation = {
    fetchModels: 'async list(',
    fetchGenerationHistory: 'async list(',
    fetchGroups: 'async list(',
    fetchRoutingApiKeys: 'async list(',
    fetchRoutingRequestTraces: 'async list(',
    fetchRoutingUsage: 'async retrieve(',
  };

  const sdkOperationIdByOperation = {
    fetchModels: 'models.list',
    fetchGenerationHistory: 'generations.list',
    fetchGroups: 'routing.accountGroups.list',
    fetchRoutingApiKeys: 'routing.apiKeys.list',
    fetchRoutingRequestTraces: 'routing.requestTraces.list',
    fetchRoutingUsage: 'routing.usage.retrieve',
  };
  ...
  for (const operation of requiredAppRouterOperations) {
    const manifestOperation = manifest.operations.find((entry) =>
      entry.source === operation.manifestSource
      ...
      && entry.sdk_family === 'cloudrouter-app-sdk',
    );
    assert.ok(manifestOperation, `${operation.operation} must be declared in the app manifest`);
    assert.equal(manifestOperation.route, operation.portalRoute);
  ...
  const playgroundModelGroupsOperation = manifest.operations.find((entry) =>
    entry.source === 'apps/sdkwork-cloudrouter-pc/packages/sdkwork-cloudrouter-pc-playground/src/playgroundService.ts'
  ...
  assert.equal(playgroundModelGroupsOperation.sdk_family, 'cloudrouter-app-sdk');
  ...
  assert.ok(
    !appAiServiceSurface.includes(`getCloudRouterAppSdkClient().ai.${['playground', 'models'].join('.')}.list(`),
  ...
  assert.equal(openapi.paths['/app/v3/api/ai/models'], undefined);
...
test('portal SDK reference uses real generated SDK package metadata for downloads', () => {
  ...
  assert.ok(sdkClientBoundarySource.includes('CLOUDROUTER_APP_SDK_REFERENCE_METADATA'));
  assert.ok(sdkClientBoundarySource.includes('CLOUDROUTER_BACKEND_SDK_REFERENCE_METADATA'));
  assert.ok(documentsReferenceAdapterSource.includes('SDK_SYSTEM_CONFIG'));
  assert.ok(documentsReferenceAdapterSource.includes('cloudRouterDocumentsReferenceRuntime'));
  ...
  assert.ok(sdkMetadataSurface.includes('sdkwork-cloudrouter-app-sdk-typescript-0.1.0.zip'));
  assert.ok(sdkMetadataSurface.includes('sdkwork-cloudrouter-backend-sdk-typescript-0.1.0.zip'));
  ...
  assert.ok(!referenceSurface.includes('@sdkwork/cloudrouter-sdk'));
  assert.ok(!referenceSurface.includes('@sdkwork/cloudrouter-management-sdk'));
  assert.ok(!referenceSurface.includes('@sdkwork/cloudrouter-portal-sdk'));
...
test('portal model catalog API examples use the generated app SDK package', () => {
  ...
  assert.ok(modelCatalogSource.includes('createCloudRouterAppSdkModelExample'));
  assert.ok(sdkClientBoundarySource.includes("@sdkwork/cloudrouter-app-sdk"));
  assert.ok(sdkClientBoundarySource.includes('SdkworkAppClient'));
  assert.ok(sdkClientBoundarySource.includes('/app/v3/api'));
  assert.ok(!modelCatalogSource.includes("@sdkwork/cloudrouter-sdk"));
  assert.ok(!modelCatalogSource.includes('CloudRouterClient'));
});
...
test('postgres integration runner can plan an ephemeral Docker database', async () => {
  ...
  const plan = module.createPostgresIntegrationPlan(
    ...
    { SDKWORK_CLOUDROUTER_POSTGRES_TEST_PORT: '15439' },
    workspaceRoot,
  );
  ...
  assert.deepEqual(plan.steps[1].args, [
    'compose',
    '-p',
    'sdkwork-cloudrouter-postgres-test',
    '-f',
...
test('verification plan runs full Rust tests through the scoped Rust test runner', async () => {
  ...
  assert.deepEqual(rustWorkspaceTests.args, [
    'scripts/run-cloud-router-rust-tests.mjs',
    'full',
    '--target-dir',
    'target-verify',
    '--test-threads',
...
test('precommit verification plan keeps commit-time checks lightweight and staged-aware', async () => {
  ...
  assert.deepEqual(labels, [
    ...
    'cloud router download catalog check',
  ...
  assert.deepEqual(commandLines.at(-1), 'node scripts/run-cloud-router-rust-tests.mjs auto --staged');
...
test('ci verification plan extends precommit with rust format and admin api integration gates', async () => {
  ...
  assert.ok(labels.includes('relay retired admin surfaces guard'));
  assert.ok(labels.includes('admin route registry runtime tests'));
  ...
  assert.equal(
    commandLines.at(-1),
    `${module.pnpmCommand()} --dir apps/sdkwork-cloudrouter-pc typecheck`,
  );
  assert.ok(labels.includes('portal frontend typecheck'));
  assert.ok(!labels.includes('production artifact build'));
  assert.ok(!labels.includes('schema quality gate'));
...
test('fast verification plan refreshes SDK dist before low-cost Codex iteration checks', async () => {
  ...
  assert.deepEqual(commandLines, [
    ...
    'node --test --experimental-test-isolation=none scripts/verify-cloud-router-topology.test.mjs',
    ...
    'pnpm.cmd --dir sdks/cloudrouter-app-sdk/cloudrouter-app-sdk-typescript/generated/server-openapi build',
...
test('verification plan can include edge dev server smoke through explicit environment opt-in', async () => {
  ...
  const plan = module.buildVerificationPlan(
    { skipRustTests: true, skipPythonTests: true, skipSchemaGate: true },
    { CLOUDROUTER_VERIFY_EDGE_DEV_SMOKE: '1' },
  );
  assert.ok(plan.some((step) => step.label === 'edge dev server smoke'));
});
...
test('verification plan can include edge dev server smoke when explicitly requested', async () => {
  ...
  const portalReadme = readFileSync(
    path.join(workspaceRoot, 'apps', 'sdkwork-cloudrouter-pc', 'README.md'),
    'utf8',
  );
  ...
  assert.ok(commandLines.includes(
    'node --experimental-strip-types apps/sdkwork-cloudrouter-pc/vite-config-runtime.test.ts',
  ));
  ...
  assert.ok(
    smokeSource.includes("process.env.CLOUDROUTER_EDGE_DEV_SMOKE_TIMEOUT_MS ?? '900000'"),
    'edge dev smoke default timeout must allow full seed install and explicit product server services to start on Windows',
  );
  assert.ok(smokeSource.includes('CLOUDROUTER_EDGE_DEV_SMOKE_REQUIRED'));
  ...
  assert.ok(rootReadme.includes('CLOUDROUTER_EDGE_DEV_SMOKE_REQUIRED="1"'));
  ...
  assert.ok(portalReadme.includes('CLOUDROUTER_EDGE_DEV_SMOKE_REQUIRED="1"'));
});
...
test('edge dev smoke validates the current gateway and surface OpenAPI contract shapes', () => {
  ...
  const backendOpenApi = JSON.parse(
    readFileSync(path.join(workspaceRoot, 'generated', 'openapi', 'cloudrouter-backend-openapi.json'), 'utf8'),
  );
...
test('edge dev smoke isolates SQLite and validates public app model browse data', () => {
  ...
  assert.ok(smokeSource.includes('SDKWork Cloud Router'));
...
test('workspace cleanup plan defaults to rebuildable local artifacts only', async () => {
  const module = await import(
    pathToFileURL(path.join(workspaceRoot, 'scripts', 'clean-cloud-router-workspace.mjs')).href
  );
  ...
  assert.ok(relativePaths.includes(path.join('apps', 'sdkwork-cloudrouter-pc', '.turbo')));
  assert.ok(relativePaths.includes(path.join('apps', 'sdkwork-cloudrouter-pc', 'dist')));
  assert.ok(!relativePaths.includes('target'));
  assert.ok(!relativePaths.includes(path.join('apps', 'sdkwork-cloudrouter-pc', 'node_modules')));
  ...
  assert.ok(deepRelativePaths.includes(path.join('apps', 'sdkwork-cloudrouter-pc', 'node_modules')));
});
...
test('release preflight publishes a single release environment contract', async () => {
  ...
  assert.equal(module.RELEASE_ENVIRONMENT_CONTRACT.version, 5);
  ...
  const result = module.buildReleasePreflightReport({
    ...
    probes: {
      ...
      releaseVerifyEvidence: {
        found: true,
        path: 'docs/release/test-release.md',
        verifyEvidence: true,
        skipped: false,
      },
    },
...
test('release env writer creates a dotenv file from the executable contract without leaking values', async () => {
  ...
  assert.equal(plan.safeSummary, 'release env file would be written with 17 release profile variables');
  assert.doesNotMatch(plan.content, /SDKWORK_DATABASE_URL=/u);
  ...
  assert.doesNotMatch(plan.content, /SDKWORK_ACCESS_TOKEN=/u);
...
test('release env writer CLI check prints only a safe summary', async () => {
  ...
  assert.ok(stdout.includes('release env file would be written with 17 release profile variables'));
...
test('release env writer CLI writes a local dotenv file without leaking values', async () => {
  ...
  try {
    ...
    assert.doesNotMatch(written, /SDKWORK_DATABASE_URL=/u);
...
test('global and application environment contracts document Cloud Router runtime config standards', () => {
  const specsRoot = resolveCloudRouterBusinessSpecsRoot(workspaceRoot);
  ...
  for (const key of [
    'SDKWORK_CLOUDROUTER_ROUTER_CONFIG_PROFILE',
  ...
  for (const content of [environmentSpec, deploymentSpec]) {
    assert.ok(content.includes('SdkWork Cloud Router'));
    assert.ok(content.includes('SDKWORK_CLOUDROUTER_CONFIG_FILE'));
    assert.ok(content.includes('SDKWORK_DATABASE_URL'));
    assert.ok(content.includes('SDKWORK_CLOUDROUTER_REDIS_HOST'));
    assert.ok(content.includes('SDKWORK_CLOUDROUTER_REDIS_PORT'));
    assert.ok(content.includes('SDKWORK_CLOUDROUTER_REDIS_DATABASE'));
    assert.ok(content.includes('SDKWORK_CLOUDROUTER_REDIS_URL'));
    assert.ok(content.includes('SDKWORK_CLOUDROUTER_REDIS_KEY_PREFIX'));
    assert.ok(content.includes('SDKWORK_CLOUDROUTER_REDIS_TLS'));
    assert.ok(content.includes('SDKWORK_CLOUDROUTER_REDIS_MAX_CONNECTIONS'));
    assert.ok(content.includes('SDKWORK_CLOUDROUTER_REDIS_CONNECT_TIMEOUT_MILLIS'));
    assert.ok(content.includes('SDKWORK_CLOUDROUTER_REDIS_COMMAND_TIMEOUT_MILLIS'));
  ...
  assert.ok(environmentSpec.includes('`pnpm dev:desktop` default to PostgreSQL, standalone, and development.'));
  ...
  assert.ok(deploymentSpec.includes('Desktop packages must keep declared client-local data on SQLite by default.'));
  assert.ok(deploymentSpec.includes('The PostgreSQL development profile belongs to'));
  assert.ok(deploymentSpec.includes('standalone dev orchestration and any launched backend service runtime; it must'));
  assert.ok(deploymentSpec.includes('not change the installed desktop package default or the desktop user data'));
  ...
  assert.ok(deploymentSpec.includes('cloudrouterctl ensure'));
  assert.ok(deploymentSpec.includes('cloudrouterctl refresh-catalog --force'));
});
...
test('release preflight strict mode fails missing release environment and app dirty state', async () => {
  ...
  const result = module.buildReleasePreflightReport({
    ...
    probes: {
      ...
      rootStatusLines: [' M spring-ai-plus-business/apps/sdkwork-cloudrouter/scripts/release-preflight.mjs'],
      commandVersions: {
        git: 'git version 2.51.0',
        node: 'v24.11.1',
        pnpm: '10.33.0',
...
test('verification plan includes all commercial contract guardians before tests', async () => {
  ...
  const commandLines = plan.map((step) => `${step.command} ${step.args.join(' ')}`);
  const toolingTestsIndex = commandLines.indexOf('node scripts/run-cloud-router-application.test.mjs');
  ...
  assert.deepEqual(commandLines.slice(toolingTestsIndex, toolingTestsIndex + 16), [
    ...
    'python -B -m tools.cloudrouter_gateway_openapi_generator --check',
...
test('verification plan verifies production portal through Rust edge server without Node server tests', async () => {
  ...
  assert.ok(commandLines.includes(
    'cargo test -p sdkwork-cloudrouter-edge-runtime --test edge_server edge_server_can_serve_portal_dist_without_node_server',
  ));
...
test('portal service command results must not fabricate returned entities from empty objects', () => {
  const serviceRoot = path.join(workspaceRoot, 'apps', 'sdkwork-cloudrouter-pc', 'packages');
  const serviceFiles = [
    'sdkwork-cloudrouter-pc-admin-memberships/src/membershipsService.ts',
    'sdkwork-cloudrouter-pc-admin-marketing/src/marketingService.ts',
    'sdkwork-cloudrouter-pc-admin-ratelimit/src/ratelimitService.ts',
    'sdkwork-cloudrouter-pc-admin-service-nodes/src/serviceNodeService.ts',
...
test('portal mutable entity services must require backend stable ids', () => {
  ...
  const commonsSource = readFileSync(
    path.join(portalRoot, 'packages', 'sdkwork-cloudroutes-pc-commons', 'src', 'api-result.ts'),
    'utf8',
  );
  const guardedServices = [
    {
      file: path.join('sdkwork-cloudrouter-pc-admin-ratelimit', 'src', 'ratelimitService.ts'),
      requiredMessages: [
    ...
    {
      file: path.join('sdkwork-cloudrouter-pc-admin-marketing', 'src', 'marketingService.ts'),
      requiredMessages: ['Referral stat id is required'],
      forbidden: [/id:\s*readString\(item,\s*['"]id['"]\)/u],
    },
    {
      file: path.join('sdkwork-cloudrouter-pc-admin-dashboard', 'src', 'dashboardService.ts'),
      requiredMessages: ['Recent usage trace id is required'],
      forbidden: [/id:\s*readString\(item,\s*['"]id['"]\)/u],
    },
    {
      file: path.join('sdkwork-cloudrouter-pc-admin-monitor', 'src', 'monitorService.ts'),
      requiredMessages: ['System node id is required', 'Alert id is required'],
      forbidden: [/id:\s*readString\(item,\s*['"]id['"]\)/u],
    },
    {
      file: path.join('sdkwork-cloudrouter-pc-admin-record', 'src', 'recordService.ts'),
      requiredMessages: ['Log record id is required'],
      forbidden: [/id:\s*readString\(item,\s*['"]id['"]\)/u],
    },
    {
      file: path.join('sdkwork-cloudrouter-pc-console-usage', 'src', 'usageService.ts'),
...
test('portal dev scripts run Vite without a Node server entrypoint', () => {
  const portalPackage = JSON.parse(
    readFileSync(path.join(workspaceRoot, 'apps', 'sdkwork-cloudrouter-pc', 'package.json'), 'utf8'),
  );
  const viteConfig = readFileSync(
    path.join(workspaceRoot, 'apps', 'sdkwork-cloudrouter-pc', 'vite.config.ts'),
    'utf8',
  );
  ...
  assert.ok(!portalPackage.scripts['_sdkwork:client:browser'].includes('tsx'));
  assert.ok(portalPackage.scripts['_sdkwork:client:browser'].includes('--configLoader native'));
...
test('portal workspace packages declare ESM module metadata', () => {
  const packagesRoot = path.join(workspaceRoot, 'apps', 'sdkwork-cloudrouter-pc', 'packages');
...
test('standalone portal Vite dev server proxies API paths using topology profile fallbacks', () => {
  ...
  assert.ok(viteConfig.includes('SDKWORK_CLOUDROUTER_BROWSER_DEV_PROXY_OPEN_API_ORIGIN'));
  assert.ok(viteConfig.includes('SDKWORK_CLOUDROUTER_BROWSER_DEV_PROXY_BACKEND_API_ORIGIN'));
  assert.ok(viteConfig.includes('SDKWORK_CLOUDROUTER_BROWSER_DEV_PROXY_APP_API_ORIGIN'));
  assert.ok(viteConfig.includes('VITE_SDKWORK_CLOUDROUTER_ROUTER_APPLICATION_OPEN_HTTP_URL'));
  assert.ok(viteConfig.includes('VITE_SDKWORK_CLOUDROUTER_ROUTER_PLATFORM_API_GATEWAY_HTTP_URL'));
...
test('portal build script uses native Vite config loading', () => {
  ...
  const buildScript = readFileSync(
    path.join(workspaceRoot, 'apps', 'sdkwork-cloudrouter-pc', 'scripts', 'build-portal.mjs'),
    'utf8',
  );
...
test('verification plan includes portal frontend typecheck', async () => {
  ...
  const sdkBuilds = [
    ['app SDK runtime build', ['--dir', 'sdks/cloudrouter-app-sdk/cloudrouter-app-sdk-typescript/generated/server-openapi', 'build']],
    ['backend SDK runtime build', ['--dir', 'sdks/cloudrouter-backend-sdk/cloudrouter-backend-sdk-typescript/generated/server-openapi', 'build']],
    ['open SDK runtime build', ['--dir', 'sdks/cloudrouter-open-sdk/cloudrouter-open-sdk-typescript/generated/server-openapi', 'build']],
  ];
...
test('verification plan includes production artifact build and bundle budget audit', async () => {
  ...
  assert.ok(commandLines.includes(
    'node apps/sdkwork-cloudrouter-pc/scripts/audit-bundle-budget.mjs',
  ));
});
...
test('verification plan includes real browser DOM smoke after production HTTP smoke', async () => {
  ...
  const browserSmokeSource = readFileSync(
    path.join(workspaceRoot, 'apps', 'sdkwork-cloudrouter-pc', 'scripts', 'smoke-production-browser.mjs'),
    'utf8',
  );
  ...
  assert.match(
    browserSmokeSource,
    /parsePositiveIntegerEnv\("CLOUDROUTER_EDGE_STARTUP_TIMEOUT_MS", 900_000\)/,
  );
  ...
  assert.match(browserSmokeSource, /CLOUDROUTER_BROWSER_SMOKE_REQUIRED/);
  ...
  assert.match(browserSmokeSource, /CLOUDROUTER_BROWSER_DEBUG_PORT/);
  ...
  assert.match(browserSmokeSource, /VITE_CLOUDROUTER_OPEN_API_BASE_URL/);
  ...
  assert.match(browserSmokeSource, /VITE_CLOUDROUTER_APP_API_BASE_URL/);
  ...
  assert.match(browserSmokeSource, /CLOUDROUTER_API_KEY/);
  ...
  assert.match(browserSmokeSource, /window\.__BROWSER_SMOKE_CLIPBOARD__\?\.includes\("CLOUDROUTER_API_KEY"\)/);
  ...
  assert.match(browserSmokeSource, /window\.__CLOUDROUTER_ENV__/);
}
);
...
test('verification plan refreshes generated SDK runtimes after production smoke and before portal runtime suites', async () => {
  ...
  assert.deepEqual(plan[runtimeAppSdkRefreshIndex].args, [
    '--dir',
    'sdks/cloudrouter-app-sdk/cloudrouter-app-sdk-typescript/generated/server-openapi',
    'build',
  ]);
  assert.deepEqual(plan[runtimeBackendSdkRefreshIndex].args, [
    '--dir',
    'sdks/cloudrouter-backend-sdk/cloudrouter-backend-sdk-typescript/generated/server-openapi',
    'build',
  ]);
  assert.deepEqual(plan[runtimeOpenSdkRefreshIndex].args, [
    '--dir',
    'sdks/cloudrouter-open-sdk/cloudrouter-open-sdk-typescript/generated/server-openapi',
    'build',
  ]);
});
...
test('verification plan includes portal models runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    'node --experimental-strip-types apps/sdkwork-cloudrouter-pc/models-runtime.test.ts',
  ));
});
...
test('verification plan includes portal commons runtime tests before route runtime tests', async () => {
  ...
  assert.ok(commandLines.includes(
    'node --experimental-strip-types apps/sdkwork-cloudrouter-pc/commons-runtime.test.ts',
  ));
});
...
test('verification plan includes portal auth runtime tests before route runtime tests', async () => {
  ...
  assert.ok(commandLines.includes(
    `${module.pnpmCommand()} --dir apps/sdkwork-cloudrouter-pc exec tsx auth-runtime.test.ts`,
  ));
});
...
test('verification plan includes portal home downloads runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    'node --experimental-strip-types apps/sdkwork-cloudrouter-pc/home-downloads-runtime.test.ts',
  ));
});
...
test('verification plan includes portal api reference playground runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    `${module.pnpmCommand()} --dir apps/sdkwork-cloudrouter-pc exec tsx api-reference-playground-runtime.test.ts`,
  ));
});
...
test('verification plan includes portal api reference SSR smoke before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    'node apps/sdkwork-cloudrouter-pc/api-reference-ssr-smoke.test.cjs',
  ));
});
...
test('verification plan includes portal playground chat runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    `${module.pnpmCommand()} --dir apps/sdkwork-cloudrouter-pc exec vitest run playground-chat-runtime.test.ts --config vite.config.ts --pool vmThreads`,
  ));
});
...
test('production browser smoke keeps backend SDK interception for portal session routes', () => {
  ...
  assert.ok(smokeSource.includes('sdkwork.cloudRouter.appSession.v1'));
...
test('verification plan includes portal api key runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    'node --experimental-strip-types apps/sdkwork-cloudrouter-pc/api-key-runtime.test.ts',
  ));
});
...
test('verification plan includes portal commerce business runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    `${module.pnpmCommand()} --dir apps/sdkwork-cloudrouter-pc exec tsx commerce-business-runtime.test.ts`,
  ));
});
...
test('verification plan includes portal console app runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    `${module.pnpmCommand()} --dir apps/sdkwork-cloudrouter-pc exec tsx console-app-runtime.test.ts`,
  ));
});
...
test('verification plan includes portal console routing runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    'node --experimental-strip-types apps/sdkwork-cloudrouter-pc/console-routing-runtime.test.ts',
  ));
});
...
test('verification plan includes portal admin model runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    'node --experimental-strip-types apps/sdkwork-cloudrouter-pc/admin-model-runtime.test.ts',
  ));
});
...
test('verification plan includes portal admin ratelimit runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    'node --experimental-strip-types apps/sdkwork-cloudrouter-pc/admin-ratelimit-runtime.test.ts',
  ));
});
...
test('verification plan includes portal admin operations runtime tests before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    'node --experimental-strip-types apps/sdkwork-cloudrouter-pc/admin-operations-runtime.test.ts',
  ));
});
...
test('verification plan includes portal models SSR smoke before broad suites', async () => {
  ...
  assert.ok(commandLines.includes(
    'node apps/sdkwork-cloudrouter-pc/models-ssr-smoke.test.cjs',
  ));
});
