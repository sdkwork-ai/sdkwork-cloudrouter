...
function printHelp() {
  console.log(`Usage: node scripts/verify-cloud-router-application.mjs [options]

Run the standard sdkwork-cloudrouter verification sequence.
...
function shouldRunEdgeDevSmoke(settings, env = process.env) {
  if (settings.skipEdgeDevSmoke || isEnabled(env.CLOUDROUTER_EDGE_DEV_SMOKE_SKIP)) {
    return false;
  }
  return (
    settings.withEdgeDevSmoke === true
    || isEnabled(env.CLOUDROUTER_VERIFY_EDGE_DEV_SMOKE)
    || isEnabled(env.CLOUDROUTER_EDGE_DEV_SMOKE_REQUIRED)
  );
}
...
function cargoVerifyEnv(env = process.env) {
  const verifyEnv = {
    ...env,
    CARGO_TARGET_DIR: env.CLOUDROUTER_VERIFY_CARGO_TARGET_DIR || env.CARGO_TARGET_DIR || 'target-verify',
    CARGO_INCREMENTAL: '0',
...
const COMMERCIAL_CONTRACT_GUARDIANS = [
  ...
  ['cloudrouter generated SDK guard', 'tools.cloudrouter_sdk_guardian'],
  ['cloudrouter project skill guard', 'tools.cloudrouter_skill_guardian'],
  ...
  ['gateway openapi freshness check', 'tools.cloudrouter_gateway_openapi_generator', ['--check']],
  ['openapi precision audit', 'tools.cloudrouter_openapi_precision_audit'],
  ['payload SDK audit', 'tools.cloudrouter_payload_sdk_audit'],
  ...
  return [
    ...
    },
    {
      ...
      args: [
        ...
        'scripts/verify-cloud-router-topology.test.mjs',
      ],
      env,
    },
    {
...
function buildFastVerificationPlan(env = process.env) {
  return [
    ...
    {
      label: 'cloud router download catalog check',
      command: pnpmCommand(),
    ...
    ...buildTopologyVerificationPlan(env),
    {
      ...
      command: 'node',
      args: ['scripts/run-cloud-router-application.test.mjs'],
      env,
    },
...
function buildPrecommitVerificationPlan(env = process.env) {
  return [
    ...
    },
    {
      ...
      command: 'node',
      args: ['scripts/run-cloud-router-rust-tests.mjs', 'auto', '--staged'],
      env,
    },
  ];
}
...
function buildCiVerificationPlan(env = process.env, settings = {}) {
  ...
  plan.push({
    ...
    args: [
      ...
      '-p',
      'sdkwork-cloudrouter-http',
      '-p',
      'sdkwork-cloudrouter-security',
      '-p',
      'sdkwork-cloudrouter-test-support',
      '-p',
  ...
  plan.push({
    ...
    command: pnpmCommand(),
    args: ['--dir', 'apps/sdkwork-cloudrouter-pc', 'typecheck'],
    env,
  });
  return plan;
}
...
function buildVerificationPlan(settings, env = process.env) {
  ...
  plan.push({
    ...
    command: 'node',
    args: ['--experimental-strip-types', 'apps/sdkwork-cloudrouter-pc/vite-config-runtime.test.ts'],
    env,
  });
  ...
  plan.push({
    ...
    command: 'node',
    args: ['apps/sdkwork-cloudrouter-pc/scripts/audit-bundle-budget.mjs'],
    env,
  });
  plan.push({
    label: 'portal production edge smoke',
    command: 'cargo',
    args: ['test', '-p', 'sdkwork-cloudrouter-edge-runtime', '--test', 'edge_server', 'edge_server_can_serve_portal_dist_without_node_server'],
    env: rustEnv,
  });
  plan.push({
    label: 'portal production browser DOM smoke',
    command: 'node',
    args: ['apps/sdkwork-cloudrouter-pc/scripts/smoke-production-browser.mjs'],
    env: rustEnv,
  });
  plan.push({
  ...
  plan.push({
    ...
    command: 'node',
    args: ['--experimental-strip-types', 'apps/sdkwork-cloudrouter-pc/commons-runtime.test.ts'],
    env,
  });
  plan.push({
    label: 'portal auth runtime tests',
  ...
  plan.push({
    ...
    command: 'node',
    args: ['--experimental-strip-types', 'apps/sdkwork-cloudrouter-pc/models-runtime.test.ts'],
    env,
  });
  plan.push({
    label: 'portal rankings runtime tests',
    command: 'node',
    args: ['--experimental-strip-types', 'apps/sdkwork-cloudrouter-pc/rankings-runtime.test.ts'],
    env,
  });
  plan.push({
    label: 'portal home downloads runtime tests',
    command: 'node',
    args: ['--experimental-strip-types', 'apps/sdkwork-cloudrouter-pc/home-downloads-runtime.test.ts'],
    env,
  });
  plan.push({
    label: 'portal api reference playground runtime tests',
    command: pnpmCommand(),
    args: ['--dir', 'apps/sdkwork-cloudrouter-pc', 'exec', 'tsx', 'api-reference-playground-runtime.test.ts'],
    env,
  });
  plan.push({
    label: 'portal api reference SSR smoke tests',
    command: 'node',
    args: ['apps/sdkwork-cloudrouter-pc/api-reference-ssr-smoke.test.cjs'],
    env,
  });
  plan.push({
  ...
  plan.push({
    ...
    command: 'node',
    args: ['--experimental-strip-types', 'apps/sdkwork-cloudrouter-pc/api-key-runtime.test.ts'],
    env,
  });
  plan.push({
    label: 'portal commerce business runtime tests',
    command: pnpmCommand(),
    args: ['--dir', 'apps/sdkwork-cloudrouter-pc', 'exec', 'tsx', 'commerce-business-runtime.test.ts'],
    env,
  });
  plan.push({
    label: 'portal console app runtime tests',
    command: pnpmCommand(),
    args: ['--dir', 'apps/sdkwork-cloudrouter-pc', 'exec', 'tsx', 'console-app-runtime.test.ts'],
    env,
  });
  plan.push({
  ...
  plan.push({
    ...
    command: 'node',
    args: ['--experimental-strip-types', 'apps/sdkwork-cloudrouter-pc/admin-model-runtime.test.ts'],
    env,
  });
  plan.push({
    label: 'portal admin ratelimit runtime tests',
    command: 'node',
    args: ['--experimental-strip-types', 'apps/sdkwork-cloudrouter-pc/admin-ratelimit-runtime.test.ts'],
    env,
  });
  plan.push({
    label: 'portal admin operations runtime tests',
    command: 'node',
    args: ['--experimental-strip-types', 'apps/sdkwork-cloudrouter-pc/admin-operations-runtime.test.ts'],
    env,
  });
  plan.push({
    label: 'portal models SSR smoke tests',
    command: 'node',
    args: ['apps/sdkwork-cloudrouter-pc/models-ssr-smoke.test.cjs'],
    env,
  });
...
function runStep(step, { dryRun = false } = {}) {
  ...
  console.error(`[verify-cloud-router-application] ${step.label}: ${commandLine}`);
...
async function runStepGroup(group, { dryRun = false, concurrency = 4 } = {}) {
  ...
  console.error(
    `[verify-cloud-router-application] parallel group: ${group.steps.length} steps, concurrency ${concurrency}`,
  );
  let nextIndex = 0;
  let firstError = null;
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replaceAll('\\', '/'))) {
  main().catch((error) => {
    console.error(`[verify-cloud-router-application] ${error.message}`);
    process.exit(1);
  });
}
