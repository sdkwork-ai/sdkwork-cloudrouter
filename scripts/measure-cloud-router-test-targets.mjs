...
const DEFAULT_TARGETS = Object.freeze([
  ['sdkwork-cloudrouter-admin-gateway', 'messaging_route'],
  ['sdkwork-cloudrouter-standalone-gateway', 'api_query_contract'],
  ['sdkwork-cloudrouter-edge-runtime', 'invocation_router'],
  ['sdkwork-cloudrouter-edge-runtime', 'provider_adapter_passthrough_streaming'],
  ['sdkwork-cloudrouter-edge-runtime', 'edge_server'],
  ['sdkwork-cloudrouter-edge-runtime', 'provider_adapter_invocation'],
  ['sdkwork-cloudrouter-router-service', 'openai_compatible_http_relay'],
  ['sdkwork-cloudrouter-router-service', 'openai_compatible_chat_stream_http_relay'],
  ['sdkwork-cloudrouter-router-service', 'openai_compatible_embeddings_http_relay'],
  ['sdkwork-cloudrouter-router-service', 'openai_compatible_responses_http_relay'],
  ['sdkwork-cloudrouter-router-service', 'openai_chat_adapter_api'],
  ['sdkwork-cloudrouter-router-service', 'openai_embeddings_adapter_api'],
  ['sdkwork-cloudrouter-router-service', 'openai_responses_adapter_api'],
  ['sdkwork-cloudrouter-router-service', 'postgres_pricing_catalog_loader'],
  ['sdkwork-cloudrouter-edge-runtime', 'database_installer_startup'],
]);

function printHelp() {
  console.log(`Usage: node scripts/measure-cloud-router-test-targets.mjs [options]
...
function runStep(step, { dryRun = false } = {}) {
  ...
  console.error(`[measure-cloud-router-test-targets] ${step.label}: ${commandLine(step)}`);
  return new Promise((resolve) => {
    ...
    child.on('exit', (code, signal) => {
      ...
      console.error(`[measure-cloud-router-test-targets] ${step.label}: ${status} in ${durationSeconds}s`);
      resolve({
        packageName: step.packageName,
        testTarget: step.testTarget,
        command: commandLine(step),
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replaceAll('\\', '/'))) {
  main().catch((error) => {
    console.error(`[measure-cloud-router-test-targets] ${error.message}`);
    process.exit(1);
  });
}
