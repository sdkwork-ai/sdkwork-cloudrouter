...
const CLOUD_EDGE_DEFAULT_HSTS_ENABLED = false;
const CLOUD_EDGE_DEFAULT_HSTS_MAX_AGE_SECONDS = 31_536_000;
const CLOUD_EDGE_DEFAULT_HSTS_INCLUDE_SUBDOMAINS = true;
const CLOUD_EDGE_DEFAULT_HSTS_PRELOAD = false;
const CLOUD_EDGE_DEFAULT_CSP_FRAME_SRC = ['https://player.bilibili.com'];
...
function printHelp() {
  console.log(`Usage: node scripts/build-cloud-router-install-package.mjs [options]
...
function createGeneratedServiceEntries(packageItem) {
  ...
  if (packageItem.platform === 'macos') {
    entries.push({
      archivePath: 'service/macos/cloudrouter-service-runner',
      sourcePath: null,
      generated: true,
...
function validateInstallPackageBuildPlan(buildPlan) {
  ...
  if (buildPlan.package.deploymentMode === 'service') {
    ...
    if (buildPlan.package.platform === 'macos' && !buildPlan.entries.some((entry) =>
      ...
      issues.push(`${buildPlan.package.id} must generate executable service/macos/cloudrouter-service-runner`);
    }
  }
...
function resolveManifestGeneratedAt({ env = process.env, now = new Date() } = {}) {
  const explicitGeneratedAt = String(env.SDKWORK_CLOUDROUTER_RELEASE_GENERATED_AT ?? '').trim();
  if (explicitGeneratedAt) {
    return normalizeManifestTimestamp(explicitGeneratedAt, 'SDKWORK_CLOUDROUTER_RELEASE_GENERATED_AT');
  }
  ...
  return {
    ...
    portal: {
      ...
      security: {
        ...
        hstsEnabled: CLOUD_EDGE_DEFAULT_HSTS_ENABLED,
        hstsMaxAgeSeconds: CLOUD_EDGE_DEFAULT_HSTS_MAX_AGE_SECONDS,
        hstsIncludeSubdomains: CLOUD_EDGE_DEFAULT_HSTS_INCLUDE_SUBDOMAINS,
        hstsPreload: CLOUD_EDGE_DEFAULT_HSTS_PRELOAD,
        cspFrameSrc: [...CLOUD_EDGE_DEFAULT_CSP_FRAME_SRC],
      },
...
function installConfigurationCommands(packageItem) {
  ...
  if (packageItem.deploymentMode === 'container') {
    ...
    return {
      build: 'docker build -f container/Containerfile -t cloudrouter:local .',
      run: `docker run --rm -p 3900:3900 -v "${configMount}" -v "${passwordMount}" -v "${redisPasswordMount}" cloudrouter:local`,
    };
  }
  return {
...
function installConfigurationNextSteps(packageItem) {
  ...
  if (policy.defaultEngine === 'postgresql') {
    ...
    if (packageItem.platform === 'linux' && packageItem.deploymentMode === 'service') {
      steps.push(
        'Start the service with sudo systemctl start cloudrouter after PostgreSQL is reachable.',
...
function createInstallGuide(packageItem) {
  ...
  lines.push(
    ...
    `Set SDKWORK_CLOUDROUTER_CONFIG_FILE to use a custom config file path; otherwise use ${policy.configFile.path}.`,
    'Set SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE to server for archive, service, and container deployments, or desktop for desktop deployments.',
    '',
  );
  ...
  if (packageItem.deploymentMode === 'desktop') {
    lines.push(
      ...
      'Use SDKWORK_CLOUDROUTER_CONFIG_FILE only when an administrator needs to manage a non-default config location.',
      '',
    );
  }
...
function createRuntimeConfigTemplate(packageItem) {
  ...
  const secretRoot = packageItem.platform === 'windows'
    ...
    : packageItem.platform === 'linux'
      ? LINUX_SERVICE_CONFIG_ROOT
      : String(policy.configFile.path).replace(/\/cloudrouter\.toml$/u, '');
  const lines = [
  ...
  lines.push(
    ...
    `hsts_enabled = ${CLOUD_EDGE_DEFAULT_HSTS_ENABLED ? 'true' : 'false'}`,
    `hsts_max_age_seconds = ${CLOUD_EDGE_DEFAULT_HSTS_MAX_AGE_SECONDS}`,
    `hsts_include_subdomains = ${CLOUD_EDGE_DEFAULT_HSTS_INCLUDE_SUBDOMAINS ? 'true' : 'false'}`,
    `hsts_preload = ${CLOUD_EDGE_DEFAULT_HSTS_PRELOAD ? 'true' : 'false'}`,
    `csp_frame_src = [${CLOUD_EDGE_DEFAULT_CSP_FRAME_SRC.map((origin) => `"${origin}"`).join(', ')}]`,
    '',
    '[portal.tools]',
    ...
    '# cloudrouterctl and the gateway accept only the canonical "standard" seed',
    '# profile (the installer validates seed_profile and rejects other values).',
...
function createServiceManifest(packageItem) {
  if (packageItem.platform === 'windows') {
    return [
      ...
      `  <env name="SDKWORK_CLOUDROUTER_CONFIG_FILE" value="${packageItem.databasePolicy.configFile.path.replaceAll('/', '\\')}"/>`,
      '  <env name="SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE" value="server"/>',
  ...
  if (packageItem.platform === 'macos') {
    ...
    return [
      ...
      '<dict>',
      '  <key>Label</key>',
      '  <string>com.sdkwork.cloudrouter</string>',
      '  <key>ProgramArguments</key>',
      '  <array>',
      `    <string>${runtimeRoot}/service/macos/cloudrouter-service-runner</string>`,
      '  </array>',
      '  <key>WorkingDirectory</key>',
      `  <string>${runtimeRoot}</string>`,
      '  <key>EnvironmentVariables</key>',
      '  <dict>',
      '    <key>SDKWORK_CLOUDROUTER_CONFIG_FILE</key>',
      `    <string>${packageItem.databasePolicy.configFile.path}</string>`,
      '    <key>SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE</key>',
  ...
  return [
    ...
    `EnvironmentFile=-${LINUX_SERVICE_CONFIG_ROOT}/cloudrouter.env`,
    `Environment=SDKWORK_CLOUDROUTER_CONFIG_FILE=${packageItem.databasePolicy.configFile.path}`,
    'Environment=SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE=server',
...
function createServiceRunner(packageItem) {
  ...
  return [
    '#!/bin/sh',
    'set -eu',
    `export SDKWORK_CLOUDROUTER_CONFIG_FILE="${packageItem.databasePolicy.configFile.path}"`,
    'export SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE=server',
...
function createContainerfile(packageItem) {
  ...
  if (packageItem.platform === 'windows') {
    return [
      ...
      `ENV SDKWORK_CLOUDROUTER_CONFIG_FILE="${packageItem.databasePolicy.configFile.path}"`,
