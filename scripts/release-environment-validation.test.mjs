...
import { CLOUD_ROUTER_RELEASE_EDGE_ENV_KEY_ORDER } from './lib/cloud-router-edge-env-contract.mjs';
...
test('releaseEnvironmentIssues rejects invalid edge rate limit values', () => {
  const issues = releaseEnvironmentIssues({
    ...validReleaseEnv,
    SDKWORK_CLOUDROUTER_TOOL_API_RATE_LIMIT_WINDOW_SECONDS: '-1',
  });
  assert.ok(
    issues.some((issue) => issue.includes('SDKWORK_CLOUDROUTER_TOOL_API_RATE_LIMIT_WINDOW_SECONDS')),
  );
});
