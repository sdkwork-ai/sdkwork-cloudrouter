...
export function ensureCloudRouterNodeDeps({ repoRoot = DEFAULT_REPO_ROOT } = {}) {
  ...
  if (!existsSync(path.join(siblingRoot, 'package.json'))) {
    throw new Error(
      ...
      + 'Clone sdkwork-app-topology next to sdkwork-cloudrouter-router, then run pnpm install.',
    );
  }
  ...
  console.error(
    '[sdkwork-cloudrouter-dev] Root node_modules is missing @sdkwork/app-topology. Running pnpm install...',
  );
