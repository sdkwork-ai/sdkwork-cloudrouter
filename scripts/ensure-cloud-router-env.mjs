...
function printHelp() {
  console.log(`Usage: node scripts/ensure-cloud-router-env.mjs [options]

Ensures SDKWork-standard env profiles; only dev creates a local bootstrap token overlay.

Lifecycles:
  dev           apps/sdkwork-cloudrouter-pc/.env.development
  build         apps/sdkwork-cloudrouter-pc/.env.production
  start         .env.release and apps/sdkwork-cloudrouter-pc/.env.production
...
function main() {
  ...
  try {
    ...
    console.error(`[ensure-cloud-router-env] ${error.message}`);
    printHelp();
    process.exit(1);
  }
  ...
  const results = ensureCloudRouterEnvForLifecycle(options.lifecycle, {
    workspaceRoot: options.workspaceRoot,
    deploymentProfile: options.deploymentProfile,
    dryRun: options.dryRun,
  });

  console.log('[ensure-cloud-router-env] application env ready');
