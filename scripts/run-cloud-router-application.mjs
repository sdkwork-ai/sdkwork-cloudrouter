...
import { ensureCloudRouterBrowserDevelopmentEnv, ensureCloudRouterBrowserProductionEnv } from './dev/cloud-router-application-env.mjs';
import {
...
function installCandidatesForMode(mode) {
  switch (mode) {
    ...
    case 'browser':
    case 'check':
      return [toPortablePath(path.join('apps', 'sdkwork-cloudrouter-pc'))];
    default:
      return [];
  }
}
...
export function parseCloudRouterProductArgs(argv) {
  const result = {
    mode: 'desktop',
    install: false,
    dryRun: false,
...
function portalDesktopEnv(env) {
  return {
    ...env,
    SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE: 'desktop',
  };
}
...
function portalServiceEnv(env) {
  return {
    ...
    SDKWORK_CLOUDROUTER_SERVICE_MODE: '1',
    SDKWORK_CLOUDROUTER_PORTAL_START_HIDDEN: '1',
  };
}
...
export function createCloudRouterProductLaunchPlan({
  ...
  const portalRelativeDir = toPortablePath(path.join('apps', 'sdkwork-cloudrouter-pc'));
  ...
  switch (mode) {
    ...
    default:
      throw new Error(
        `Unsupported cloud router application mode: ${mode}. Expected one of client, desktop, service, server, plan, check, browser.`,
      );
  }
}

function printHelp() {
  console.log(`Usage: node scripts/run-cloud-router-application.mjs [mode] [options] [mode-args...]

Start sdkwork-cloudrouter through a root pnpm-compatible entrypoint.
...
Database profiles:
  ...
  Use pnpm dev:desktop:sqlite to validate client-local SQLite behavior.
...
async function printDryRun(step) {
  console.error(`[run-cloud-router-application] ${formatCommand(step)}`);
...
async function main() {
  const settings = parseCloudRouterProductArgs(process.argv.slice(2));
  if (settings.help) {
    printHelp();
    return;
  }

  const plan = createCloudRouterProductLaunchPlan({
    mode: settings.mode,
...
if (process.argv[1] && path.resolve(process.argv[1]) === __filename) {
  main().catch((error) => {
    console.error(`[run-cloud-router-application] ${error.message}`);
    process.exit(1);
  });
}
