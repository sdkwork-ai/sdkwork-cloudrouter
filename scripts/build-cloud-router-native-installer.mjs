...
} from './build-cloud-router-install-package.mjs';
import {
  DEFAULT_VERSION,
  PACKAGE_NAME,
...
function printHelp() {
  console.log(`Usage: node scripts/build-cloud-router-native-installer.mjs [options]
...
async function buildNativeInstaller(plan) {
  ...
  // Input snapshot cache (PACKAGING_SPEC §4): when every staged source entry
  // is unchanged since the last build and the installer + manifest already
  // exist, reuse them instead of re-reading/gzipping multi-hundred-MB
  // binaries. The snapshot records size+mtime per staged artifact; stale
  // snapshots (missing files, changed inputs, absent installer) always
  // rebuild.
  const snapshot = await collectNativeInstallerSnapshot(plan);
  const snapshotPath = `${plan.installerPath}.snapshot.json`;
  let cached = false;
  if (await nativeInstallerCacheHits(plan, snapshot, snapshotPath)) {
    console.log(`[native-installer-build] inputs unchanged; reusing cached installer ${path.basename(plan.installerPath)}`);
    cached = true;
  } else {
...
/// Snapshots every staged artifact (source path, size, mtime) so the next
/// build can skip re-packaging when inputs are unchanged.
async function collectNativeInstallerSnapshot(plan) {
  const entries = [];
  for (const entry of plan.archiveBuildPlan.entries) {
    if (entry.archivePath === PACKAGE_MANIFEST_FILE) {
      continue;
    }
    if (!entry.sourcePath) {
      // Generated entries are deterministic for a version; capture the
      // generated bytes digest so a builder change invalidates the cache.
      const data = createGeneratedArtifactBytes(plan.archiveBuildPlan, entry);
      entries.push({
        path: entry.archivePath,
        size: data.length,
        mtimeMs: 0,
        generatedDigest: sha256(data),
      });
      continue;
    }
    let stat;
    try {
      ...
      // Missing staged source: force rebuild (the caller will fail loudly).
  ...
  return {
    schemaVersion: '2026-08-08.native-installer-snapshot.v1',
    packageId: plan.package.id,
    version: plan.package.version,
...
async function nativeInstallerCacheHits(plan, snapshot, snapshotPath) {
  try {
  ...
  for (const entry of snapshot.entries) {
    ...
    if (prior.size !== entry.size || prior.mtimeMs !== entry.mtimeMs) return false;
    if (prior.generatedDigest !== entry.generatedDigest) return false;
  }
  // The installer file must be non-empty.
...
function createLinuxNativeInstallLayout(packageItem) {
  ...
  return createBaseNativeInstallLayout(packageItem, {
    ...
    service: isService
      ? {
        manager: 'systemd',
        name: 'cloudrouter.service',
        unitPath: '/lib/systemd/system/cloudrouter.service',
...
function createMacosNativeInstallLayout(packageItem) {
  ...
  if (isService) {
    files.launchDaemon = '/Library/LaunchDaemons/com.sdkwork.cloudrouter.plist';
    files.serviceRunner = `${serviceInstallRoot}/service/macos/cloudrouter-service-runner`;
  }
  ...
  return createBaseNativeInstallLayout(packageItem, {
    ...
    service: isService
      ? {
        manager: 'launchd',
        name: 'com.sdkwork.cloudrouter',
        unitPath: '/Library/LaunchDaemons/com.sdkwork.cloudrouter.plist',
    ...
    commands: isService
      ? {
        ...
        start: 'sudo launchctl bootstrap system /Library/LaunchDaemons/com.sdkwork.cloudrouter.plist',
        status: 'sudo launchctl print system/com.sdkwork.cloudrouter',
      }
      : {
...
function createWindowsNativeInstallLayout(packageItem) {
  ...
  if (isService) {
    files.serviceManifest = `${installRoot}/service/windows/cloudrouter.xml`;
  }
  ...
  return createBaseNativeInstallLayout(packageItem, {
    ...
    service: isService
      ? {
        ...
        name: 'cloudrouter',
        unitPath: `${installRoot}/service/windows/cloudrouter.xml`,
        enabledOnInstall: false,
        startedOnInstall: false,
      }
      : null,
    ...
    commands: isService
      ? {
        ...
        start: 'sc.exe start cloudrouter',
        status: 'sc.exe query cloudrouter',
      }
      : {
...
function createDebianControl(plan) {
  ...
  return [
    ...
    'Depends: libssl3, libc6, libgcc-s1',
...
function createDebianPostinst(plan) {
  ...
  return [
    ...
    '  systemctl enable cloudrouter.service >/dev/null 2>&1 || true',
    'fi',
...
function createDebianPrerm(plan) {
  ...
  return [
    ...
    '  systemctl stop cloudrouter.service >/dev/null 2>&1 || true',
    '  systemctl disable cloudrouter.service >/dev/null 2>&1 || true',
    'fi',
    'exit 0',
    '',
  ].join('\n');
...
function debianInstallPathForArchivePath(plan, archivePath) {
  ...
  if (normalized === 'service/macos/cloudrouter-service-runner') {
    return plan.package.deploymentMode === 'service'
      ? `${LINUX_NATIVE_INSTALL_ROOT}/service/macos/cloudrouter-service-runner`
      : null;
  }
  ...
  if (normalized === 'service/linux/cloudrouter.service') {
    return plan.package.deploymentMode === 'service'
      ? '/lib/systemd/system/cloudrouter.service'
      : null;
  }
...
async function buildMacosPkg(plan, fileEntries) {
  ...
  await execFileAsync('pkgbuild', [
    ...
    scriptsRoot,
    '--identifier',
    `cloud.sdkwork.cloudrouter.${plan.package.deploymentMode}`,
    '--version',
...
function macosInstallPathForArchivePath(plan, archivePath) {
  ...
  if (normalized === 'service/macos/com.sdkwork.cloudrouter.plist') {
    return plan.package.deploymentMode === 'service'
      ? '/Library/LaunchDaemons/com.sdkwork.cloudrouter.plist'
      : null;
  }
  if (normalized === 'INSTALL.md') {
...
function createMacosPostinstall(plan) {
  ...
  const serviceSetup = plan.package.deploymentMode === 'service'
    ? [
      ...
      '  chmod 0644 /Library/LaunchDaemons/com.sdkwork.cloudrouter.plist || true',
      'fi',
    ]
    : [
      `mkdir -p ${MACOS_NATIVE_SHARED_ROOT}/config`,
      `echo "CloudRouter desktop config is user-scoped. Run ${POSIX_INSTALL_ROOT}/bin/cloudrouterctl ensure as the target user before first start."`,
    ];
  return [
    '#!/bin/sh',
    'set -e',
...
async function buildWindowsMsi(plan, fileEntries) {
  ...
  const wixSourcePath = path.join(buildRoot, 'cloudrouter.wxs');
