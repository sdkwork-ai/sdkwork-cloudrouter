...
const CLOUDHUB_LIST_API = 'https://cloudhub.ai/api/v1/skills';
const CLOUDHUB_DETAIL_API_PREFIX = 'https://cloudhub.ai/api/v1/skills/';
const RAW_INDEX_RELATIVE_PATH = 'data/skills/cloudhub/raw/index.json';
const RAW_DETAILS_RELATIVE_PATH = 'data/skills/cloudhub/raw/details';
const NORMALIZED_MANIFEST_RELATIVE_PATH = 'data/skills/cloudhub/manifest.json';
const GENERATED_AT = '2026-05-10T00:00:00Z';
const OFFICIAL_CATEGORY_ID = 1901;
const CLOUDHUB_CATEGORY_ID = 1902;
const OFFICIAL_PACKAGE_ID = 7101;
const CLOUDHUB_PACKAGE_ID = 7201;
const CLOUDHUB_SKILL_ID_START = 8201;
const CLOUDHUB_ASSET_WIDTH = 1200;
const CLOUDHUB_ASSET_HEIGHT = 720;
const DEFAULT_PAGE_SIZE = 200;
const DEFAULT_SEED_LIMIT = null;
const DEFAULT_DETAIL_CONCURRENCY = 8;
const DEFAULT_HTTP_TIMEOUT_MS = 30_000;
const PREFERRED_CLOUDHUB_SKILL_SLUGS = [
  'mcp',
  'browser-use',
  'browser',
  'playwright',
...
function printHelp() {
  console.log(`Usage: node scripts/mirror-cloudhub-skills-seed.mjs [options]

Mirror CloudHub skills into local seed data. Runtime startup reads only local data/skills files.

Options:
  --fetch                 Fetch the CloudHub public full list API and detail API into local raw mirror files.
  ...
  --workspace-root <path> Workspace root, default current sdkwork-cloudrouter root.
  ...
  --seed-limit <number|all>
                         Number of CloudHub community skills projected into startup seed, default all mirrored details.
  ...
  --sort <field>          CloudHub list sort field, default createdAt.
  --json                  Print machine-readable summary.
  -h, --help              Show this help.

Examples:
  pnpm skills:seed:mirror-cloudhub
  pnpm skills:seed:mirror-cloudhub -- --max-items 200
...
export function parseCloudHubMirrorArgs(argv) {
  const settings = {
    fetch: false,
    fromMirror: false,
    check: false,
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (arg) {
      ...
      case '--':
        break;
      default:
        throw new Error(`unknown CloudHub skill seed option: ${arg}`);
    }
  }
...
export function buildCloudHubMirrorCommandPlan(settings, { workspaceRoot = settings.workspaceRoot } = {}) {
  const mode = settings.check ? 'check' : settings.fetch ? 'fetch-and-project' : 'project-from-mirror';
  const steps = [];
  if (settings.fetch) {
    steps.push({
      name: 'mirror-cloudhub-raw-skills',
      mode,
      api: CLOUDHUB_LIST_API,
  ...
  steps.push({
    name: settings.check ? 'check-cloudhub-local-seed' : 'project-cloudhub-local-seed',
    mode,
    ...
    try {
      const response = await fetch(url, {
        ...
        headers: {
          ...
          'user-agent': 'sdkwork-cloudrouter-skill-mirror/1.0',
        },
      });
      if (!response.ok) {
        const body = await response.text();
        throw new Error(`CloudHub request failed ${response.status} ${response.statusText}: ${url}\n${body.slice(0, 500)}`);
      }
...
async function fetchCloudHubRawMirror(settings) {
  const rawRoot = path.join(settings.workspaceRoot, 'data', 'skills', 'cloudhub', 'raw');
  ...
  while (!checkpoint || cursor !== null || pageNo === 0) {
    ...
    if (seenRequestCursors.has(`${requestCursor ?? ''}`)) {
      throw new Error(`CloudHub pagination cursor repeated before page ${pageNo}; refusing infinite mirror loop`);
    }
    seenRequestCursors.add(`${requestCursor ?? ''}`);
    const url = new URL(CLOUDHUB_LIST_API);
    ...
    await writeMirrorCheckpoint(settings.workspaceRoot, {
      schemaVersion: 'cloudhub-skills-mirror-checkpoint.v1',
      source: {
        listApi: CLOUDHUB_LIST_API,
        detailApiTemplate: `${CLOUD_HUB_DETAIL_API_TEMPLATE()}`,
        sort: settings.sort,
        pageSize: settings.pageSize,
      },
      fetchedAt,
  ...
  if (!settings.json) {
    console.error(`[skills-seed] fetching detail payloads for ${uniqueItems.length} mirrored CloudHub skills`);
  }
  ...
  await mapWithConcurrency(uniqueItems, settings.detailConcurrency, async (item) => {
    ...
    try {
      const detail = await fetchJson(`${CLOUDHUB_DETAIL_API_PREFIX}${encodeURIComponent(slug)}`, {
        timeoutMs: settings.httpTimeoutMs,
      });
      await writeJson(detailPath, detail);
    } catch (error) {
      await writeJson(errorPath, {
        schemaVersion: 'cloudhub-skill-detail-error.v1',
        slug,
        url: `${CLOUDHUB_DETAIL_API_PREFIX}${encodeURIComponent(slug)}`,
        fetchedAt: new Date().toISOString(),
  ...
  const index = {
    schemaVersion: 'cloudhub-skills-mirror.v1',
    ...
    items: uniqueItems.map((item) => {
      ...
      return {
        ...item,
        rawDetailPath: slug ? `data/skills/cloudhub/raw/details/${mirrorFileName(slug)}` : null,
        rawErrorPath: slug ? `data/skills/cloudhub/raw/errors/${mirrorFileName(slug)}` : null,
      };
    }),
  };
...
async function loadMirrorCheckpoint(workspaceRoot) {
  const checkpoint = await readJsonIfExists(path.join(workspaceRoot, 'data', 'skills', 'cloudhub', 'raw', 'checkpoint.json'));
  if (!checkpoint || checkpoint.schemaVersion !== 'cloudhub-skills-mirror-checkpoint.v1') {
    return null;
  }
  return checkpoint;
}
...
async function writeMirrorCheckpoint(workspaceRoot, checkpoint) {
  await writeJson(path.join(workspaceRoot, 'data', 'skills', 'cloudhub', 'raw', 'checkpoint.json'), checkpoint);
}

function CLOUD_HUB_DETAIL_API_TEMPLATE() {
  return `${CLOUD_HUB_DETAIL_API_BASE()}{slug}`;
}

function CLOUD_HUB_DETAIL_API_BASE() {
  return CLOUDHUB_DETAIL_API_PREFIX;
}
...
function scoreDetail(detail) {
  ...
  const preferredIndex = PREFERRED_CLOUDHUB_SKILL_SLUGS.indexOf(slug);
  const preferredScore = preferredIndex >= 0 ? (PREFERRED_CLOUDHUB_SKILL_SLUGS.length - preferredIndex) * 1_000_000 : 0;
...
function shortCloudHubSeedUuid(kind, slug, suffix = '') {
  ...
  const base = `skill-${kind}-cloudhub-`;
...
function deriveFeatureLines(detail) {
  ...
  const source = summary || changelog || 'Community skill mirrored from CloudHub.';
...
function deriveFrameworks(detail) {
  ...
  const frameworks = ['CloudHub', 'Agent Skill'];
...
function deriveCapabilities(detail) {
  ...
  const capabilities = ['cloudhub.metadata'];
...
function buildOfficialCategories() {
  return [
    ...
    {
      id: CLOUDHUB_CATEGORY_ID,
      uuid: 'skill-category-cloudhub-community',
      name: 'CloudHub Community',
      description: 'Community skills mirrored from CloudHub into local SDKWork seed data for offline startup and review.',
      shopId: 0,
      type: 19,
      groupName: 'community',
      code: 'cloudhub-community',
      tags: ['cloudhub', 'community', 'mirror'],
      icon: mediaResource('store'),
      sortWeight: 20,
      parentId: null,
      path: '/skills/cloudhub-community',
      visible: true,
      status: 1,
    },
  ];
...
function buildPackages() {
  return [
    ...
    {
      id: CLOUDHUB_PACKAGE_ID,
      uuid: 'skill-package-cloudhub-community-mirror',
      userId: 0,
      packageKey: 'cloudhub-community-mirror',
      name: 'CloudHub Community Mirror',
      summary: 'Local projection of community skills mirrored from the CloudHub public catalog.',
      description: 'CloudHub community skills are crawled into a local raw mirror and projected as metadata-only marketplace entries until their execution artifacts are reviewed.',
      icon: mediaResource('https://cdn.sdkwork.example/skills/packages/cloudhub-community-mirror/icon.png'),
      cover: mediaResource('https://cdn.sdkwork.example/skills/packages/cloudhub-community-mirror/cover.png'),
      categoryId: CLOUDHUB_CATEGORY_ID,
      enabled: true,
      featured: false,
      sortWeight: 20,
      tags: ['cloudhub', 'community', 'mirror'],
...
async function buildSkillSeedBundle(settings) {
  ...
  if (missingDetails.length > 0) {
    const errorRoot = path.join(settings.workspaceRoot, 'data', 'skills', 'cloudhub', 'raw', 'errors');
    const unresolved = [];
    ...
    if (unresolved.length > 0) {
      throw new Error(`CloudHub raw mirror is incomplete; missing detail or error files for: ${unresolved.slice(0, 20).join(', ')}`);
    }
  }
  ...
  const selectedDetails = selectCloudHubSeedDetails(details, settings.seedLimit);
  ...
  const normalizedManifest = {
    schemaVersion: 'sdkwork-cloudhub-skills-projection.v1',
...
async function refreshRawMirrorDetailStatus(workspaceRoot, rawIndex) {
  const rawRoot = path.join(workspaceRoot, 'data', 'skills', 'cloudhub', 'raw');
  const detailStatus = await readDetailStatus(rawRoot, rawIndex.items ?? []);
  const items = (rawIndex.items ?? []).map((item) => {
    const slug = normalizeSlug(item?.slug);
    return {
      ...item,
      rawDetailPath: slug ? `data/skills/cloudhub/raw/details/${mirrorFileName(slug)}` : item.rawDetailPath ?? null,
      rawErrorPath: slug ? `data/skills/cloudhub/raw/errors/${mirrorFileName(slug)}` : item.rawErrorPath ?? null,
    };
  });
  return {
    ...rawIndex,
...
function selectCloudHubSeedDetails(details, seedLimit) {
...
function buildCommunityProjection(details, rawIndex) {
  ...
  details.forEach((detail, index) => {
    const slug = detailSlug(detail);
    const id = CLOUDHUB_SKILL_ID_START + index;
    const version = normalizeVersion(detail?.latestVersion?.version || detail?.skill?.tags?.latest);
    const skillKey = `cloudhub-${slug}`;
    const name = truncateText(detail?.skill?.displayName || slug, 128);
    const summary = truncateText(detail?.skill?.summary || detail?.latestVersion?.changelog || `CloudHub community skill ${name}.`, 240);
    const description = truncateText(detail?.latestVersion?.changelog || detail?.skill?.summary || summary, 2000);
    const sourceUrl = `https://cloudhub.ai/skills/${slug}`;
    const artifactRef = `cloudhub://skills/${slug}@${version}`;
    const artifactPath = `data/skills/artifacts/cloudhub-${slug}-${version}.json`;
    const manifestUrl = `data/skills/manifests/cloudhub-${slug}.json`;
    const frameworks = deriveFrameworks(detail);
    const features = deriveFeatureLines(detail);
    const owner = detail?.owner ?? {};
    const ownerName = normalizeText(owner.displayName || owner.handle, 'CloudHub Community');
    const ownerImage = normalizeText(owner.image);
    const image = ownerImage.startsWith('https://')
      ? ownerImage
      : `https://cdn.sdkwork.example/skills/cloudhub/${slug}/cover.png`;
    ...
    const skill = {
      ...
      categoryId: CLOUDHUB_CATEGORY_ID,
      packageId: CLOUDHUB_PACKAGE_ID,
      provider: 'CloudHub',
      version,
      versionName: version,
      runtime: 'metadata',
      entrypoint: `cloudhub.skills.${slug.replaceAll('-', '_')}`,
      manifestUrl,
      repositoryUrl: sourceUrl,
      homepageUrl: sourceUrl,
      documentationUrl: sourceUrl,
      licenseName: normalizeText(detail?.latestVersion?.license, 'CloudHub Community'),
      sourceType: 'COMMUNITY',
      marketStatus: 'PUBLISHED',
      visibility: 'PUBLIC',
      reviewStatus: 'APPROVED',
      reviewComment: 'Community metadata mirrored from CloudHub raw local catalog.',
      reviewedBy: 0,
      ...
      ratingCount,
      tags: dedupeBy(['cloudhub', 'community', slug, ...frameworks.map((item) => item.toLowerCase())], (item) => item).slice(0, 16),
      capabilities,
      configSchema,
      defaultConfig,
      source: {
        ...
        url: sourceUrl,
        fetchedAt,
        rawDetailPath: `data/skills/cloudhub/raw/details/${mirrorFileName(slug)}`,
      },
    ...
    const artifactPayload = {
      ...
      source: skill.source,
      instructions: [
        'Display this bundled CloudHub metadata entry from the local SDKWork Skills seed.',
    ...
    const artifact = {
      uuid: shortCloudHubSeedUuid('artifact', slug),
      targetType: 35,
      targetId: id,
      artifactType: 1,
      version,
    ...
    assets.push({
      uuid: shortCloudHubSeedUuid('asset', slug, 'cover'),
      targetType: 35,
      targetId: id,
      artifactId: null,
      assetType: 1,
      ...
      title: `${name} cover`,
      altText: `${name} CloudHub community skill preview`,
      mimeType: 'image/png',
      width: CLOUDHUB_ASSET_WIDTH,
      height: CLOUDHUB_ASSET_HEIGHT,
    ...
    manifests.push({
      path: `data/skills/manifests/cloudhub-${slug}.json`,
      value: manifest,
    });
...
export async function runCloudHubMirror(settings) {
  const plan = buildCloudHubMirrorCommandPlan(settings);
  let rawIndex = null;
  if (settings.fetch) {
    rawIndex = await fetchCloudHubRawMirror(settings);
...
async function main() {
  const settings = parseCloudHubMirrorArgs(process.argv.slice(2));
  if (settings.help) {
    printHelp();
    return;
  }
  const summary = await runCloudHubMirror(settings);
  if (settings.json) {
