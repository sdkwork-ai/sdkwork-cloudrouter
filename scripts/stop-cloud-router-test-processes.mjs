...
function printHelp() {
  console.log(`Usage: node scripts/stop-cloud-router-test-processes.mjs [options]
...
async function stopWindowsProcesses(processes, { dryRun = false } = {}) {
  for (const processInfo of processes) {
    ...
    console.error(`[stop-cloud-router-test-processes] stop ${line}`);
...
async function main() {
  ...
  if (process.platform !== 'win32') {
    console.log('[stop-cloud-router-test-processes] no-op on non-Windows platforms');
    return;
  }
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replaceAll('\\', '/'))) {
  main().catch((error) => {
    console.error(`[stop-cloud-router-test-processes] ${error.message}`);
    process.exit(1);
  });
}
