...
export function parseCloudRouterDevEnvFileContent(content) {
  const values = {};
...
export function loadCloudRouterDevEnvFile(envFile, {
  ...
  if (!existsSync(envFilePath)) {
    throw new Error(`Cloud Router dev env file does not exist: ${envFilePath}`);
  }
  return parseCloudRouterDevEnvFileContent(readFileSync(envFilePath, 'utf8'));
}
...
export function defaultCloudRouterDevPostgresDatabaseUrl() {
  return buildPostgresDatabaseUrl(DEFAULT_DEV_POSTGRES_DATABASE);
}

export function defaultCloudRouterDevPostgresMaxConnections() {
...
export function resolveWorkspaceDevDatabaseEnv({
  ...
  const fileEnv = resolvedDevEnvFile
    ? loadCloudRouterDevEnvFile(resolvedDevEnvFile, { workspaceRoot })
    : {};
...
export function resolveCloudRouterDevDatabaseEnv({
  env = process.env,
  defaultDatabase = 'postgresql',
} = {}) {
  ...
  if (!databaseUrl) {
    if (defaultDatabase === 'postgresql') {
      const defaultDatabaseUrl = defaultCloudRouterDevPostgresDatabaseUrl();
      return {
