...
const KEY_RING_ENV = 'SDKWORK_CLOUDROUTER_UPSTREAM_CREDENTIAL_KEY_RING';
const KEY_RING_FILE_ENV = 'SDKWORK_CLOUDROUTER_UPSTREAM_CREDENTIAL_KEY_RING_FILE';
const INTERNAL_GATEWAY_SECRET_ENV = 'SDKWORK_CLOUDROUTER_INTERNAL_GATEWAY_SIGNING_SECRET';
...
test('development startup creates and reuses private security files', () => {
  const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cloudrouter-dev-key-ring-'));
  const deterministicKeys = [
    Buffer.alloc(32, 0x11),
    Buffer.alloc(32, 0x22),
    Buffer.alloc(32, 0x33),
  ];
  const first = ensureCloudRouterDevSecurityFiles({
    workspaceRoot,
    env: {},
  ...
  const second = ensureCloudRouterDevSecurityFiles({
    workspaceRoot,
    env: {},
    randomBytesFn: () => {
...
test('development dry-run resolves the secret path without writing it', () => {
  const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'cloudrouter-dev-key-ring-'));
  const result = ensureCloudRouterDevSecurityFiles({
    workspaceRoot,
    env: {},
    dryRun: true,
  });
