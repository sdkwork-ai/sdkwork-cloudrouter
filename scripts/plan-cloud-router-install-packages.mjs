...
import { DEFAULT_RELEASE_VERSION } from './cloud-router-release-version.mjs';
...
const INTERNAL_PROJECT_NAME = 'sdkwork-cloudrouter';
const PACKAGE_NAME = 'cloudrouter';
const RUNTIME_DISPLAY_NAME = 'SdkWork CloudRouter';
const EDGE_BINARY_BASENAME = 'cloudrouter';
const INSTALLER_BINARY_BASENAME = 'cloudrouterctl';
...
function printHelp() {
  console.log(`Usage: node scripts/plan-cloud-router-install-packages.mjs [options]
...
function redisPolicyFor({ platform, runtimeProfile, deploymentMode = 'archive' }) {
  ...
  return {
    ...
    envOverrides: [
      ...
      'SDKWORK_CLOUDROUTER_REDIS_USERNAME',
      'SDKWORK_CLOUDROUTER_REDIS_URL',
      'SDKWORK_CLOUDROUTER_REDIS_PASSWORD_FILE',
      'SDKWORK_CLOUDROUTER_REDIS_PASSWORD',
      'SDKWORK_CLOUDROUTER_REDIS_KEY_PREFIX',
      'SDKWORK_CLOUDROUTER_REDIS_TLS',
      'SDKWORK_CLOUDROUTER_REDIS_MAX_CONNECTIONS',
...
function serviceIntegrationFor(platform, deploymentMode) {
  ...
  if (platform === 'linux') {
    return {
      kind: 'systemd',
      manifest: 'service/linux/cloudrouter.service',
    };
  }
  return {
    kind: 'launchd',
    manifest: 'service/macos/com.sdkwork.cloudrouter.plist',
  };
}
