...
const pcRoot = path.join(root, 'apps', 'sdkwork-cloudrouter-pc');
...
function updatePcPackageNames() {
  ...
  for (const dirName of readdirSync(packagesDir)) {
    if (!dirName.startsWith('sdkwork-cloudrouter-pc-')) {
      continue;
    }
    const suffix = dirName.slice('sdkwork-cloudrouter-pc-'.length);
    const scopedName = `@sdkwork/cloudrouter-pc-${suffix}`;
    ...
    if (packageJson.sdkwork?.workspace === 'sdkwork-cloudrouter') {
      packageJson.sdkwork.workspace = 'sdkwork-cloudrouter';
    }
    if (packageJson.dependencies) {
      for (const [dep, value] of Object.entries(packageJson.dependencies)) {
        if (dep.startsWith('sdkwork-cloudrouter-pc-')) {
          const depSuffix = dep.slice('sdkwork-cloudrouter-pc-'.length);
          delete packageJson.dependencies[dep];
          packageJson.dependencies[`@sdkwork/cloudrouter-pc-${depSuffix}`] = value;
        }
      }
    }
...
function updateTsconfigPaths(configPath) {
  ...
  text = replaceAll(text, [
    ['"sdkwork-cloudrouter-pc-', '"@sdkwork/cloudrouter-pc-'],
  ]);
...
function updateNamingManifest() {
  ...
  manifest.application = 'sdkwork-cloudrouter';
  manifest.canonicalApplicationCode = 'cloudrouter';
  manifest.legacyRepositoryStems = [
    {
      path: 'sdkwork-cloudrouter',
      target: 'sdkwork-cloudrouter',
      reason: 'Retired hyphenated repository stem; canonical application code and repository stem is cloudrouter.',
    },
  ];
...
console.log(`Updated ${packageReplacements.length} PC package names to @sdkwork/cloudrouter-pc-*`);
console.log('Updated repository identity manifests to sdkwork-cloudrouter');
