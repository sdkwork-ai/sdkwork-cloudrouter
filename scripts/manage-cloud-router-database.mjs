...
import { IAM_APPLICATION_BOOTSTRAP_ENV } from './lib/cloud-router-topology.mjs';
...
export function createDatabaseManagementPlan({
  ...
  const stepEnv = {
    ...
    SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE:
      settings.deploymentMode ?? env.SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE ?? 'server',
    ...(configFile ? { SDKWORK_CLOUDROUTER_CONFIG_FILE: configFile } : {}),
...
Manage the Cloud Router database through the Rust installer.
