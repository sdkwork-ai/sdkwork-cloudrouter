...
import { createRuntimeConfigTemplate } from './build-cloud-router-install-package.mjs';
import { ensureCloudRouterEnvForLifecycle } from './dev/cloud-router-application-env.mjs';
import { productionGatewayBinaryPath } from './cloud-router-production-artifacts.mjs';
import {
...
function buildStartProductionHelpText() {
  return `Usage: node scripts/start-cloud-router-production.mjs [options]
...
function prepareStartProductionRuntimeConfig({
  ...
  const deploymentMode = normalizeDeploymentMode(
    settings.deploymentMode ?? baseEnv.SDKWORK_CLOUDROUTER_DEPLOYMENT_MODE ?? 'server',
  );
  const defaults = runtimeConfigLocationForPlatform(platform, deploymentMode, baseEnv);
  const requestedConfigFile = String(
    settings.configFile ?? baseEnv.SDKWORK_CLOUDROUTER_CONFIG_FILE ?? defaults.configFile,
  ).trim();
  ...
  const result = {
    ...
    env: {
      SDKWORK_CLOUDROUTER_CONFIG_FILE: configFile,
...
function runtimeConfigPostgresUrlUsesPlaceholder(value) {
  ...
  const legacyDefault = 'postgresql://sdkwork_cloudrouter_router:change-me@localhost:5432/sdkwork_cloudrouter_router';
...
function resolveStartProductionEnv(
  ...
  return {
    ...
    SDKWORK_CLOUDROUTER_EDGE_GATEWAY_BASE_URL:
      ...
      ?? baseEnv.SDKWORK_CLOUDROUTER_EDGE_GATEWAY_BASE_URL
      ?? (allInOne ? edgeBaseUrl : 'http://127.0.0.1:18080'),
    SDKWORK_CLOUDROUTER_EDGE_BACKEND_API_BASE_URL:
      settings.backendApiForwardUrl
      ?? baseEnv.SDKWORK_CLOUDROUTER_EDGE_BACKEND_API_BASE_URL
      ?? (allInOne ? edgeBaseUrl : 'http://127.0.0.1:18081'),
    SDKWORK_CLOUDROUTER_EDGE_APP_API_BASE_URL:
      settings.appApiForwardUrl
      ?? baseEnv.SDKWORK_CLOUDROUTER_EDGE_APP_API_BASE_URL
      ?? (allInOne ? edgeBaseUrl : 'http://127.0.0.1:18082'),
    ...(
      baseEnv.SDKWORK_CLOUDROUTER_APP_RUNTIME_GATEWAY_BASE_URL || allInOne
        ? {
          SDKWORK_CLOUDROUTER_APP_RUNTIME_GATEWAY_BASE_URL:
            baseEnv.SDKWORK_CLOUDROUTER_APP_RUNTIME_GATEWAY_BASE_URL ?? edgeBaseUrl,
        }
        : {}
    ),
    SDKWORK_CLOUDROUTER_EDGE_EXTERNAL_SCHEME:
      settings.externalScheme ?? baseEnv.SDKWORK_CLOUDROUTER_EDGE_EXTERNAL_SCHEME ?? 'http',
    SDKWORK_CLOUDROUTER_EDGE_TRUST_FORWARDED_HEADERS:
      settings.trustForwardedHeaders
        ? '1'
        : baseEnv.SDKWORK_CLOUDROUTER_EDGE_TRUST_FORWARDED_HEADERS ?? '0',
...
function edgeAccessBaseUrl(env) {
  const bind = env.SDKWORK_CLOUDROUTER_SERVER_BIND ?? '0.0.0.0:3900';
...
function buildStartProductionAccessLines(env) {
  ...
  const gatewayTarget = env.SDKWORK_CLOUDROUTER_EDGE_GATEWAY_BASE_URL ?? 'http://127.0.0.1:18080';
  const backendTarget = env.SDKWORK_CLOUDROUTER_EDGE_BACKEND_API_BASE_URL ?? 'http://127.0.0.1:18081';
  const appTarget = env.SDKWORK_CLOUDROUTER_EDGE_APP_API_BASE_URL ?? 'http://127.0.0.1:18082';
  const allInOne = env.SDKWORK_CLOUDROUTER_ALL_IN_ONE_RUNTIME !== '0';
  const lines = [
  ...
  lines.push(
    ...
    `[start-production]   ${CLOUD_ROUTER_EDGE_ENV_KEYS.toolApiRateLimitRequests}=${env[CLOUD_ROUTER_EDGE_ENV_KEYS.toolApiRateLimitRequests]}`,
    `[start-production]   ${CLOUD_ROUTER_EDGE_ENV_KEYS.toolApiRateLimitWindowSeconds}=${env[CLOUD_ROUTER_EDGE_ENV_KEYS.toolApiRateLimitWindowSeconds]}`,
    `[start-production]   ${CLOUD_ROUTER_EDGE_ENV_KEYS.toolApiSdkArchiveRoot}=${env[CLOUD_ROUTER_EDGE_ENV_KEYS.toolApiSdkArchiveRoot] || '(not configured)'}`,
    `[start-production]   SDKWORK_CLOUDROUTER_EDGE_EXTERNAL_SCHEME=${env.SDKWORK_CLOUDROUTER_EDGE_EXTERNAL_SCHEME}`,
    `[start-production]   SDKWORK_CLOUDROUTER_EDGE_TRUST_FORWARDED_HEADERS=${env.SDKWORK_CLOUDROUTER_EDGE_TRUST_FORWARDED_HEADERS}`,
  );
  return lines;
}
...
function resolveStartProductionCommand(
  ...
  const configuredGatewayBinary = env.SDKWORK_CLOUDROUTER_GATEWAY_BIN?.trim();
  ...
  return {
    ...
    args: ['run', '-p', 'sdkwork-api-cloudrouter-standalone-gateway'],
    source: 'cargo',
  };
}
