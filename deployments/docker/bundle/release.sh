#!/usr/bin/env bash
# ============================================================================
# release.sh — release lifecycle for the sdkwork-cloudrouter standalone Docker
...
#                       bundle image.env SDKWORK_CLOUDROUTER_IMAGE_TAG)
...
#                       SDKWORK_CLOUDROUTER_REPLICAS / 1)
...
LOG_PREFIX="[sdkwork-cloudrouter-release]"
...
resolve_image_base() {
  ...
  # SDKWORK_CLOUDROUTER_IMAGE_TAG repository when it carries one (same rule as
  # deploy.sh).
  if [ "${BUNDLE_ROOT}" != "" ]; then
    printf '%s' "registry.sdkwork.com/apps/sdkwork-cloudrouter"
    return
  fi
  local env_image base
  env_image="$(env_key SDKWORK_CLOUDROUTER_IMAGE_TAG)"
  case "${env_image}" in
    *:*) base="${env_image%:*}" ;;
    *)   base="${env_image}" ;;
  esac
  printf '%s' "${base:-registry.sdkwork.com/apps/sdkwork-cloudrouter}"
}

resolve_image_tag() {
  # Priority: --image-tag > env-file SDKWORK_CLOUDROUTER_IMAGE_TAG tag >
  # bundle image.env (same precedence order deploy.sh uses).
  if [ -n "${IMAGE_TAG}" ]; then printf '%s' "${IMAGE_TAG}"; return; fi
  local tag
  tag="$(env_key SDKWORK_CLOUDROUTER_IMAGE_TAG | sed 's/.*://')"
  if [ -n "${tag}" ] && [ "${tag}" != "$(env_key SDKWORK_CLOUDROUTER_IMAGE_TAG)" ]; then
    printf '%s' "${tag}"; return
  fi
  if [ -n "${BUNDLE_IMAGE_ENV}" ] && [ -f "${BUNDLE_IMAGE_ENV}" ]; then
    tag="$(sed -n 's/^SDKWORK_CLOUDROUTER_IMAGE_TAG=//p' "${BUNDLE_IMAGE_ENV}" | tail -1 | tr -d '\r')"
...
per_env_port_key() {
  case "${ENVIRONMENT}" in
    development) printf 'SDKWORK_CLOUDROUTER_DEV_HOST_PORT' ;;
    test)        printf 'SDKWORK_CLOUDROUTER_TEST_HOST_PORT' ;;
    staging)     printf 'SDKWORK_CLOUDROUTER_STAGING_HOST_PORT' ;;
    demo)        printf 'SDKWORK_CLOUDROUTER_DEMO_HOST_PORT' ;;
    production)  printf 'SDKWORK_CLOUDROUTER_PROD_HOST_PORT' ;;
  esac
}

resolve_topology() {
  if [ -z "${REPLICAS}" ]; then REPLICAS="$(env_key SDKWORK_CLOUDROUTER_REPLICAS)"; fi
  REPLICAS="${REPLICAS:-1}"
  case "${REPLICAS}" in ''|*[!0-9]*) die "replicas must be a positive integer" ;; esac
  HOST_PORT_BASE="$(env_key "$(per_env_port_key)")"
  case "${ENVIRONMENT}" in
    development) HOST_PORT_BASE="${HOST_PORT_BASE:-3950}" ;;
    test)        HOST_PORT_BASE="${HOST_PORT_BASE:-3951}" ;;
    staging)     HOST_PORT_BASE="${HOST_PORT_BASE:-3952}" ;;
    demo)        HOST_PORT_BASE="${HOST_PORT_BASE:-3954}" ;;
    production)  HOST_PORT_BASE="${HOST_PORT_BASE:-3953}" ;;
  esac
  case "${HOST_PORT_BASE}" in ''|*[!0-9]*) die "per-environment host port must be a positive integer" ;; esac
  DEPS_MODE="${DEPS_MODE_FLAG:-$(env_key SDKWORK_CLOUDROUTER_DEPS_MODE)}"
  DEPS_MODE="${DEPS_MODE:-external}"
  CLOUDROUTER_CONTAINER_PORT="3900"
}
...
running_images() {
  ...
  projects="$(docker ps --filter "label=com.docker.compose.project" --format '{{.Label "com.docker.compose.project"}}' 2>/dev/null \
    | grep -E "^sdkwork-cloudrouter-${ENVIRONMENT}-i[0-9]+$" | sort -u || true)"
  local p cid img iid
  for p in ${projects}; do
    img="$(docker ps --filter "label=com.docker.compose.project=${p}" --filter "label=com.docker.compose.service=cloudrouter" --format '{{.Image}}' 2>/dev/null | head -1 || true)"
    cid="$(docker ps -q --filter "label=com.docker.compose.project=${p}" --filter "label=com.docker.compose.service=cloudrouter" 2>/dev/null | head -1 || true)"
    iid=""
    if [ -n "${cid}" ]; then
  ...
  if ! image_exists "${target_ref}"; then
    if [ -n "${BUNDLE_ROOT}" ] && [ -f "${BUNDLE_ROOT}/image.tar.gz" ]; then
      ...
      if [ -f "${BUNDLE_IMAGE_ENV}" ]; then
        bundle_tag="$(sed -n 's/^SDKWORK_CLOUDROUTER_IMAGE_TAG=//p' "${BUNDLE_IMAGE_ENV}" | tail -1 | tr -d '\r')"
      fi
...
action_status() {
  ...
  resolve_topology
  info "  topology         : ${REPLICAS} instance(s), management ports ${HOST_PORT_BASE} (+1 per instance) -> container ${CLOUDROUTER_CONTAINER_PORT}, deps=${DEPS_MODE}"
  for index in $(seq 1 "${REPLICAS}"); do
    port="$(instance_port "${index}")"
