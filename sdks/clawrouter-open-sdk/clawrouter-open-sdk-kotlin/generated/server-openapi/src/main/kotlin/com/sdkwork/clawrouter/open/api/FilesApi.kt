...
class FilesApi(private val client: HttpClient) {
    ...
    /** List files */
    suspend fun list(limit: Int? = null, order: String? = null, after: String? = null, before: String? = null): OpenAiFileList? {
        ...
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.aiPath("/files"), query))
        return client.convertValue(raw, object : TypeReference<OpenAiFileList>() {})
    }

    /** Upload file */
    suspend fun create(body: OpenAiFileUploadRequest): OpenAiFile? {
        val raw = client.post(ApiPaths.aiPath("/files"), body, null, null, "multipart/form-data")
        return client.convertValue(raw, object : TypeReference<OpenAiFile>() {})
    }

    /** Delete file */
    suspend fun delete(fileId: String): DeleteResult? {
        val raw = client.delete(ApiPaths.aiPath("/files/${serializePathParameter(fileId, PathParameterSpec("file_id", "simple", false))}"))
        return client.convertValue(raw, object : TypeReference<DeleteResult>() {})
    }

    /** Retrieve file */
    suspend fun retrieve(fileId: String): OpenAiFile? {
        val raw = client.get(ApiPaths.aiPath("/files/${serializePathParameter(fileId, PathParameterSpec("file_id", "simple", false))}"))
        return client.convertValue(raw, object : TypeReference<OpenAiFile>() {})
    }

    /** Retrieve file content */
    suspend fun content(fileId: String): String? {
        val raw = client.get(ApiPaths.aiPath("/files/${serializePathParameter(fileId, PathParameterSpec("file_id", "simple", false))}/content"))
