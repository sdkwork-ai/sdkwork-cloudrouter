...
class ChatApi(private val client: HttpClient) {
    ...
    suspend fun list(limit: Int? = null, order: String? = null, after: String? = null, before: String? = null, model: String? = null, metadata: String? = null): OpenAiChatCompletionList? {
        ...
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.aiPath("/chat/completions"), query))
        return client.convertValue(raw, object : TypeReference<OpenAiChatCompletionList>() {})
    }

    /** Create chat completion */
    suspend fun create(body: OpenAiChatCompletionRequest): OpenAiChatCompletion? {
        val raw = client.post(ApiPaths.aiPath("/chat/completions"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<OpenAiChatCompletion>() {})
    }

    /** Delete stored chat completion */
    suspend fun delete(completionId: String): DeleteResult? {
        val raw = client.delete(ApiPaths.aiPath("/chat/completions/${serializePathParameter(completionId, PathParameterSpec("completion_id", "simple", false))}"))
        return client.convertValue(raw, object : TypeReference<DeleteResult>() {})
    }

    /** Retrieve stored chat completion */
    suspend fun retrieve(completionId: String): OpenAiChatCompletion? {
        val raw = client.get(ApiPaths.aiPath("/chat/completions/${serializePathParameter(completionId, PathParameterSpec("completion_id", "simple", false))}"))
        return client.convertValue(raw, object : TypeReference<OpenAiChatCompletion>() {})
    }

    /** Modify stored chat completion */
    suspend fun update(completionId: String, body: OpenAiChatCompletionUpdateRequest): OpenAiChatCompletion? {
        val raw = client.post(ApiPaths.aiPath("/chat/completions/${serializePathParameter(completionId, PathParameterSpec("completion_id", "simple", false))}"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<OpenAiChatCompletion>() {})
    }
    ...
    suspend fun listMessages(completionId: String, limit: Int? = null, order: String? = null, after: String? = null, before: String? = null): OpenAiChatCompletionMessageList? {
        ...
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.aiPath("/chat/completions/${serializePathParameter(completionId, PathParameterSpec("completion_id", "simple", false))}/messages"), query))
        return client.convertValue(raw, object : TypeReference<OpenAiChatCompletionMessageList>() {})
    }
