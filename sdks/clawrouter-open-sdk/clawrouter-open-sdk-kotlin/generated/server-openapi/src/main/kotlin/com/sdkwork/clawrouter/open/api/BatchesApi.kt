...
class BatchesApi(private val client: HttpClient) {
    ...
    /** List batches */
    suspend fun list(limit: Int? = null, order: String? = null, after: String? = null, before: String? = null): OpenAiBatchList? {
        ...
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.aiPath("/batches"), query))
        return client.convertValue(raw, object : TypeReference<OpenAiBatchList>() {})
    }

    /** Create batch */
    suspend fun create(body: OpenAiBatchCreateRequest): OpenAiBatch? {
        val raw = client.post(ApiPaths.aiPath("/batches"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<OpenAiBatch>() {})
    }

    /** Retrieve batch */
    suspend fun retrieve(batchId: String): OpenAiBatch? {
        val raw = client.get(ApiPaths.aiPath("/batches/${serializePathParameter(batchId, PathParameterSpec("batch_id", "simple", false))}"))
        return client.convertValue(raw, object : TypeReference<OpenAiBatch>() {})
    }

    /** Cancel batch */
    suspend fun cancel(batchId: String): OpenAiBatch? {
        val raw = client.post(ApiPaths.aiPath("/batches/${serializePathParameter(batchId, PathParameterSpec("batch_id", "simple", false))}/cancel"), null)
        return client.convertValue(raw, object : TypeReference<OpenAiBatch>() {})
    }
