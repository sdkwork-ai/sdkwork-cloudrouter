...
class ResponsesApi(private val client: HttpClient) {
    ...
    /** Create response */
    suspend fun create(body: OpenAiResponsesRequest): OpenAiResponse? {
        val raw = client.post(ApiPaths.aiPath("/responses"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<OpenAiResponse>() {})
    }

    /** Compact response */
    suspend fun compact(body: OpenAiResponseCompactRequest): OpenAiResponse? {
        val raw = client.post(ApiPaths.aiPath("/responses/compact"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<OpenAiResponse>() {})
    }

    /** Count response input tokens */
    suspend fun createInputToken(body: OpenAiResponseInputTokenCountRequest): OpenAiResponseInputTokenCount? {
        val raw = client.post(ApiPaths.aiPath("/responses/input_tokens"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<OpenAiResponseInputTokenCount>() {})
    }

    /** Delete response */
    suspend fun delete(responseId: String): DeleteResult? {
        val raw = client.delete(ApiPaths.aiPath("/responses/${serializePathParameter(responseId, PathParameterSpec("response_id", "simple", false))}"))
        return client.convertValue(raw, object : TypeReference<DeleteResult>() {})
    }

    /** Retrieve response */
    suspend fun retrieve(responseId: String, include: List<String>? = null): OpenAiResponse? {
        val query = buildQueryString(listOf(
            QueryParameterSpec("include[]", include, "form", true, false, null)
        ))
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.aiPath("/responses/${serializePathParameter(responseId, PathParameterSpec("response_id", "simple", false))}"), query))
        return client.convertValue(raw, object : TypeReference<OpenAiResponse>() {})
    }

    /** Cancel response */
    suspend fun cancel(responseId: String): OpenAiResponse? {
        val raw = client.post(ApiPaths.aiPath("/responses/${serializePathParameter(responseId, PathParameterSpec("response_id", "simple", false))}/cancel"), null)
        return client.convertValue(raw, object : TypeReference<OpenAiResponse>() {})
    }

    /** List response input items */
    suspend fun listInputItems(responseId: String, limit: Int? = null, order: String? = null, after: String? = null, before: String? = null, include: List<String>? = null): OpenAiResponseInputItemList? {
        val query = buildQueryString(listOf(
        ...
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.aiPath("/responses/${serializePathParameter(responseId, PathParameterSpec("response_id", "simple", false))}/input_items"), query))
        return client.convertValue(raw, object : TypeReference<OpenAiResponseInputItemList>() {})
    }
