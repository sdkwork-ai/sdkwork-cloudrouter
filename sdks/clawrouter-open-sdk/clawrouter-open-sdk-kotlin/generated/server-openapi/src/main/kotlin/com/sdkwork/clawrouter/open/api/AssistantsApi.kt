...
class AssistantsApi(private val client: HttpClient) {
    ...
    suspend fun list(limit: Int? = null, order: String? = null, after: String? = null, before: String? = null): OpenAiAssistantList? {
        ...
        val raw = client.get(ApiPaths.appendQueryString(ApiPaths.aiPath("/assistants"), query))
        return client.convertValue(raw, object : TypeReference<OpenAiAssistantList>() {})
    }

    /** Create assistant */
    suspend fun create(body: OpenAiAssistantCreateRequest): OpenAiAssistant? {
        val raw = client.post(ApiPaths.aiPath("/assistants"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<OpenAiAssistant>() {})
    }

    /** Delete assistant */
    suspend fun delete(assistantId: String): DeleteResult? {
        val raw = client.delete(ApiPaths.aiPath("/assistants/${serializePathParameter(assistantId, PathParameterSpec("assistant_id", "simple", false))}"))
        return client.convertValue(raw, object : TypeReference<DeleteResult>() {})
    }

    /** Retrieve assistant */
    suspend fun retrieve(assistantId: String): OpenAiAssistant? {
        val raw = client.get(ApiPaths.aiPath("/assistants/${serializePathParameter(assistantId, PathParameterSpec("assistant_id", "simple", false))}"))
        return client.convertValue(raw, object : TypeReference<OpenAiAssistant>() {})
    }

    /** Modify assistant */
    suspend fun update(assistantId: String, body: OpenAiAssistantUpdateRequest): OpenAiAssistant? {
        val raw = client.post(ApiPaths.aiPath("/assistants/${serializePathParameter(assistantId, PathParameterSpec("assistant_id", "simple", false))}"), body, null, null, "application/json")
        return client.convertValue(raw, object : TypeReference<OpenAiAssistant>() {})
    }
