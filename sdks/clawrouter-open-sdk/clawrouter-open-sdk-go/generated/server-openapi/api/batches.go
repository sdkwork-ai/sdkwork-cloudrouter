...
// List batches
func (a *BatchesApi) List(limit *int, order *string, after *string, before *string) (sdktypes.OpenAiBatchList, error) {
    ...
    raw, err := a.client.Get(AppendQueryString(AiApiPath("/batches"), query), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiBatchList
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiBatchList](raw)
}

// Create batch
func (a *BatchesApi) Create(body sdktypes.OpenAiBatchCreateRequest) (sdktypes.OpenAiBatch, error) {
    raw, err := a.client.Post(AiApiPath("/batches"), body, nil, nil, "application/json")
    if err != nil {
        var zero sdktypes.OpenAiBatch
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiBatch](raw)
}

// Retrieve batch
func (a *BatchesApi) Retrieve(batchId string) (sdktypes.OpenAiBatch, error) {
    raw, err := a.client.Get(AiApiPath(fmt.Sprintf("/batches/%s", SerializePathParameter(batchId, PathParameterSpec{Name: "batch_id", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiBatch
        return zero, err
    }
...
// Cancel batch
func (a *BatchesApi) Cancel(batchId string) (sdktypes.OpenAiBatch, error) {
    raw, err := a.client.Post(AiApiPath(fmt.Sprintf("/batches/%s/cancel", SerializePathParameter(batchId, PathParameterSpec{Name: "batch_id", Style: "simple", Explode: false}))), nil, nil, nil, "")
    if err != nil {
        var zero sdktypes.OpenAiBatch
        return zero, err
    }
