...
// List files
func (a *FilesApi) List(limit *int, order *string, after *string, before *string) (sdktypes.OpenAiFileList, error) {
    ...
    raw, err := a.client.Get(AppendQueryString(AiApiPath("/files"), query), nil, nil)
    if err != nil {
...
// Upload file
func (a *FilesApi) Create(body sdktypes.OpenAiFileUploadRequest) (sdktypes.OpenAiFile, error) {
    raw, err := a.client.Post(AiApiPath("/files"), body, nil, nil, "multipart/form-data")
    if err != nil {
        var zero sdktypes.OpenAiFile
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiFile](raw)
}

// Delete file
func (a *FilesApi) Delete(fileId string) (sdktypes.DeleteResult, error) {
    raw, err := a.client.Delete(AiApiPath(fmt.Sprintf("/files/%s", SerializePathParameter(fileId, PathParameterSpec{Name: "file_id", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
...
// Retrieve file
func (a *FilesApi) Retrieve(fileId string) (sdktypes.OpenAiFile, error) {
    raw, err := a.client.Get(AiApiPath(fmt.Sprintf("/files/%s", SerializePathParameter(fileId, PathParameterSpec{Name: "file_id", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiFile
        return zero, err
    }
