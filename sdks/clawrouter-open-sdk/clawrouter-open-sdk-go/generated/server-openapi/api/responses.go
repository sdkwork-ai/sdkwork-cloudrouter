...
// Create response
func (a *ResponsesApi) Create(body sdktypes.OpenAiResponsesRequest) (sdktypes.OpenAiResponse, error) {
    raw, err := a.client.Post(AiApiPath("/responses"), body, nil, nil, "application/json")
    if err != nil {
        var zero sdktypes.OpenAiResponse
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiResponse](raw)
}

// Compact response
func (a *ResponsesApi) Compact(body sdktypes.OpenAiResponseCompactRequest) (sdktypes.OpenAiResponse, error) {
    raw, err := a.client.Post(AiApiPath("/responses/compact"), body, nil, nil, "application/json")
    if err != nil {
        var zero sdktypes.OpenAiResponse
        return zero, err
    }
...
func (a *ResponsesApi) CreateInputToken(body sdktypes.OpenAiResponseInputTokenCountRequest) (sdktypes.OpenAiResponseInputTokenCount, error) {
    raw, err := a.client.Post(AiApiPath("/responses/input_tokens"), body, nil, nil, "application/json")
    if err != nil {
        var zero sdktypes.OpenAiResponseInputTokenCount
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiResponseInputTokenCount](raw)
}

// Delete response
func (a *ResponsesApi) Delete(responseId string) (sdktypes.DeleteResult, error) {
    raw, err := a.client.Delete(AiApiPath(fmt.Sprintf("/responses/%s", SerializePathParameter(responseId, PathParameterSpec{Name: "response_id", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
...
// Retrieve response
func (a *ResponsesApi) Retrieve(responseId string, include []string) (sdktypes.OpenAiResponse, error) {
    query := BuildQueryString([]QueryParameterSpec{
        {Name: "include[]", Value: func() interface{} { if include == nil { return nil }; return *include }(), Style: "form", Explode: true, AllowReserved: false},
    })
    raw, err := a.client.Get(AppendQueryString(AiApiPath(fmt.Sprintf("/responses/%s", SerializePathParameter(responseId, PathParameterSpec{Name: "response_id", Style: "simple", Explode: false}))), query), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiResponse
        return zero, err
    }
...
// Cancel response
func (a *ResponsesApi) Cancel(responseId string) (sdktypes.OpenAiResponse, error) {
    raw, err := a.client.Post(AiApiPath(fmt.Sprintf("/responses/%s/cancel", SerializePathParameter(responseId, PathParameterSpec{Name: "response_id", Style: "simple", Explode: false}))), nil, nil, nil, "")
    if err != nil {
        var zero sdktypes.OpenAiResponse
        return zero, err
    }
...
func (a *ResponsesApi) ListInputItems(responseId string, limit *int, order *string, after *string, before *string, include []string) (sdktypes.OpenAiResponseInputItemList, error) {
    query := BuildQueryString([]QueryParameterSpec{
    ...
    raw, err := a.client.Get(AppendQueryString(AiApiPath(fmt.Sprintf("/responses/%s/input_items", SerializePathParameter(responseId, PathParameterSpec{Name: "response_id", Style: "simple", Explode: false}))), query), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiResponseInputItemList
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiResponseInputItemList](raw)
}
