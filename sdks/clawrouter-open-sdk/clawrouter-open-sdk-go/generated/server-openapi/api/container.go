...
func NewContainerApi(client *sdkhttp.Client) *ContainerApi {
    return &ContainerApi{client: client}
}

// List containers
func (a *ContainerApi) List(limit *int, order *string, after *string, before *string) (sdktypes.OpenAiContainerList, error) {
    ...
    raw, err := a.client.Get(AppendQueryString(AiApiPath("/containers"), query), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiContainerList
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiContainerList](raw)
}

// Create container
func (a *ContainerApi) Create(body sdktypes.OpenAiContainerCreateRequest) (sdktypes.OpenAiContainer, error) {
    raw, err := a.client.Post(AiApiPath("/containers"), body, nil, nil, "application/json")
    if err != nil {
        var zero sdktypes.OpenAiContainer
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiContainer](raw)
}

// Delete container
func (a *ContainerApi) Delete(containerId string) (sdktypes.DeleteResult, error) {
    raw, err := a.client.Delete(AiApiPath(fmt.Sprintf("/containers/%s", SerializePathParameter(containerId, PathParameterSpec{Name: "container_id", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
...
// Retrieve container
func (a *ContainerApi) Retrieve(containerId string) (sdktypes.OpenAiContainer, error) {
    raw, err := a.client.Get(AiApiPath(fmt.Sprintf("/containers/%s", SerializePathParameter(containerId, PathParameterSpec{Name: "container_id", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiContainer
        return zero, err
    }
...
func (a *ContainerApi) ListFiles(containerId string, limit *int, order *string, after *string, before *string) (sdktypes.OpenAiContainerFileList, error) {
    query := BuildQueryString([]QueryParameterSpec{
        {Name: "page_size", Value: func() interface{} { if limit == nil { return nil }; return *limit }(), Style: "form", Explode: true, AllowReserved: false},
        {Name: "order", Value: func() interface{} { if order == nil { return nil }; return *order }(), Style: "form", Explode: true, AllowReserved: false},
        {Name: "after", Value: func() interface{} { if after == nil { return nil }; return *after }(), Style: "form", Explode: true, AllowReserved: false},
    ...
    raw, err := a.client.Get(AppendQueryString(AiApiPath(fmt.Sprintf("/containers/%s/files", SerializePathParameter(containerId, PathParameterSpec{Name: "container_id", Style: "simple", Explode: false}))), query), nil, nil)
    if err != nil {
        var zero sdktypes.OpenAiContainerFileList
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiContainerFileList](raw)
}

// Create container file
func (a *ContainerApi) CreateFile(containerId string, body sdktypes.OpenAiContainerFileCreateMultipartRequest) (sdktypes.OpenAiContainerFile, error) {
    raw, err := a.client.Post(AiApiPath(fmt.Sprintf("/containers/%s/files", SerializePathParameter(containerId, PathParameterSpec{Name: "container_id", Style: "simple", Explode: false}))), body, nil, nil, "multipart/form-data")
    if err != nil {
        var zero sdktypes.OpenAiContainerFile
        return zero, err
    }
    return decodeResult[sdktypes.OpenAiContainerFile](raw)
}

// Delete container file
func (a *ContainerApi) DeleteFiles(containerId string, fileId string) (sdktypes.DeleteResult, error) {
    raw, err := a.client.Delete(AiApiPath(fmt.Sprintf("/containers/%s/files/%s", SerializePathParameter(containerId, PathParameterSpec{Name: "container_id", Style: "simple", Explode: false}), SerializePathParameter(fileId, PathParameterSpec{Name: "file_id", Style: "simple", Explode: false}))), nil, nil)
    if err != nil {
        var zero sdktypes.DeleteResult
        return zero, err
    }
