...
const required = [
  'sdk-manifest.json',
  'openapi/cloudrouter-backend-sdk.openapi.json',
  'openapi/cloudrouter-backend-sdk.sdkgen.json',
  'cloudrouter-backend-sdk-typescript/generated/server-openapi/package.json',
  'cloudrouter-backend-sdk-typescript/generated/server-openapi/sdkwork-sdk.json',
  'cloudrouter-backend-sdk-typescript/generated/server-openapi/src/index.ts',
];
const missing = required.filter((entry) => !existsSync(path.join(workspaceRoot, entry)));
if (missing.length > 0) {
  throw new Error('cloudrouter-backend-sdk SDK family is incomplete: ' + missing.join(', '));
}
const assembly = JSON.parse(readFileSync(path.join(workspaceRoot, 'sdk-manifest.json'), 'utf8'));
if (assembly.workspace !== 'cloudrouter-backend-sdk') {
  throw new Error('SDK assembly workspace drifted');
}
const expectedGenerationInputSpec = 'openapi/cloudrouter-backend-sdk.openapi.json';
...
console.log('Verified cloudrouter-backend-sdk SDK family.');
