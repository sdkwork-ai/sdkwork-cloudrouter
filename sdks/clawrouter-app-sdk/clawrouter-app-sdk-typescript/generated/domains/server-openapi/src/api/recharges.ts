...
export class RechargesOrdersCancellationsApi {
  private client: HttpClient;
...
/** Create */
  async create(orderId: string): Promise<Record<string, never>> {
    return this.client.post<Record<string, never>>(appApiPath(`/recharges/orders/${serializePathParameter(orderId, { name: 'orderId', style: 'simple', explode: false })}/cancellations`));
  }
}
...
export class RechargesOrdersApi {
  private client: HttpClient;
  public readonly cancellations: RechargesOrdersCancellationsApi;

  constructor(client: HttpClient) {
    this.client = client;
    this.cancellations = new RechargesOrdersCancellationsApi(client);
  }
