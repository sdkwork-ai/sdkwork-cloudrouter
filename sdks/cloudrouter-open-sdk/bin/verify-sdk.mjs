...
const required = [
  'sdk-manifest.json',
  'openapi/cloudrouter-open-sdk.openapi.json',
  'openapi/cloudrouter-open-sdk.sdkgen.json',
  'cloudrouter-open-sdk-typescript/generated/server-openapi/package.json',
  'cloudrouter-open-sdk-typescript/generated/server-openapi/sdkwork-sdk.json',
  'cloudrouter-open-sdk-typescript/generated/server-openapi/src/index.ts',
];
const missing = required.filter((entry) => !existsSync(path.join(workspaceRoot, entry)));
if (missing.length > 0) {
  throw new Error('cloudrouter-open-sdk SDK family is incomplete: ' + missing.join(', '));
}
const assembly = JSON.parse(readFileSync(path.join(workspaceRoot, 'sdk-manifest.json'), 'utf8'));
if (assembly.workspace !== 'cloudrouter-open-sdk') {
  throw new Error('SDK assembly workspace drifted');
}
const expectedGenerationInputSpec = 'openapi/cloudrouter-open-sdk.sdkgen.json';
const expectedDerivedSpecs = {"sdk-generator":"openapi/cloudrouter-open-sdk.sdkgen.json"};
...
console.log('Verified cloudrouter-open-sdk SDK family.');
