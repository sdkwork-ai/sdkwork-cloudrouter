...
use crate::models::{
    GoogleCachedContent, GoogleCachedContentCreateRequest, GoogleCachedContentListResponse,
    GoogleEmptyResponse,
};

#[derive(Clone)]
pub struct ResponsesGoogleApi {
    client: Arc<SdkworkHttpClient>,
}

impl ResponsesGoogleApi {
    pub fn new(client: Arc<SdkworkHttpClient>) -> Self {
        Self { client }
    }

    /// Google Gemini list cached contents
    pub async fn list_v1beta_cached_contents(
        &self,
        ...
        let path = append_query_string(
            ai_path(&"/google/v1beta/cachedContents".to_string()),
            &query,
        );
    ...
    pub async fn create_v1beta_cached_content(
        &self,
        body: &GoogleCachedContentCreateRequest,
    ) -> Result<GoogleCachedContent, SdkworkError> {
    ...
    pub async fn delete_v1beta_cached_contents(
        &self,
        ...
        let path = ai_path(&format!(
            ...
            serialize_path_parameter(
                cached_content_id,
                PathParameterSpec::new("cached_content_id", "simple", false)
            )
        ));
